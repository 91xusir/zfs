...
void DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	else
	{
		...
		if (bSelected)
		{
			...
			dc.MoveTo(rcCaption.right, rcValue.top); dc.LineTo(rcValue.right - 2, rcValue.top);
			dc.MoveTo(rcCaption.right, rcValue.bottom - 1); dc.LineTo(rcValue.right - 2, rcValue.bottom - 1);
			dc.MoveTo(rcCaption.right, rcValue.top); dc.LineTo(rcCaption.right, rcValue.bottom - 1);
			dc.MoveTo(rcValue.right - 2, rcValue.top); dc.LineTo(rcValue.right - 2, rcValue.bottom - 1);
		}
	}

	CXTPPropertyGridItem* pNext = (CXTPPropertyGridItem*)pGrid->GetItemDataPtr(lpDrawItemStruct->itemID + 1);
...
void CMainFrame::RebuildPoseList()
{
	...
	for (it = g_activeActor->m_data->m_poseMap.begin(); it != g_activeActor->m_data->m_poseMap.end(); it++)
	{
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_paneManager.SetIcons(IDR_TOOLBAR_TOOLS, nIDIcons, sizeof(nIDIcons) / sizeof(nIDIcons[0]), RGB(255, 0, 255));
	XTPPaintManager()->RefreshMetrics();

	// grid
	CXTPDockingPane* paneEnv = m_paneManager.CreatePane(ID_GRID_ENV, CRect(0, 0, 200, 120), xtpPaneDockRight);
	CXTPDockingPane* paneActor = m_paneManager.CreatePane(ID_GRID_ACTOR, CRect(0, 0, 200, 120), xtpPaneDockRight);
	CXTPDockingPane* paneMtlLib = m_paneManager.CreatePane(ID_GRID_MTLLIB, CRect(0, 0, 200, 120), xtpPaneDockRight);
	CXTPDockingPane* panePose = m_paneManager.CreatePane(ID_GRID_POSE, CRect(0, 0, 200, 120), xtpPaneDockRight);
	CXTPDockingPane* paneLinkBox = m_paneManager.CreatePane(ID_GRID_LINKBOX, CRect(0, 0, 200, 120), xtpPaneDockRight);
	...
	// view
	CXTPDockingPane* paneKey = m_paneManager.CreatePane(ID_VIEW_KEY, CRect(0, 0, 200, 160), xtpPaneDockBottom);
	paneKey->SetOptions(xtpPaneNoCloseable | xtpPaneNoHideable);
	paneKey->Attach(CreateViewKey());
	// paneSlider->Attach(CreateViewSlider()); 

	CXTPDockingPane* paneProp = m_paneManager.CreatePane(ID_GRID_CHANNEL_PROP, CRect(0, 0, 240, 120), xtpPaneDockRight);
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	{
		CXTPControlComboBox* m_pPoseList = (CXTPControlComboBox*)CXTPControlComboBox::CreateObject();
...
CWnd* CMainFrame::CreateGridEnv()
{
	if (m_gridEnv.GetSafeHwnd() == 0)
	{
		...
		CXTPPropertyGridItemConstraints* pList;
		{
			...
			pItem = pRoot->AddChildItem(new CCustomItemColor3(_T("背景颜色"), &pEditor->m_clearColor));
			...
			pItem = pRoot->AddChildItem(new CCustomItemBool(_T("渲染碰撞盒"), &pEditor->m_bShowBoundBox));
			...
			*/
			...
			pItem = pRoot->AddChildItem(new CCustomItemColor3(_T("雾的颜色"), &pEditor->m_fogColor));
			pItem = pRoot->AddChildItem(new CXTPPropertyGridItemNumber(_T("最近距离"), pEditor->m_fogNear, &pEditor->m_fogNear));
			pItem = pRoot->AddChildItem(new CXTPPropertyGridItemNumber(_T("最远距离"), pEditor->m_fogFar, &pEditor->m_fogFar));
			pRoot->Expand();
			...
			pItem = pRoot->AddChildItem(new CCustomItemBool(_T("镜头灯光"), &pEditor->m_bCameraLight));
...
CWnd* CMainFrame::CreateGridActor(bool force)
{
	...
	if (force)
	{
		...
		if (!g_activeActor) return &m_gridActor;
		CXTPPropertyGridItem* pRoot;
...
CWnd* CMainFrame::CreateGridPose(bool force)
{
	...
	if (force)
	{
		...
		if (!g_activeActor) return &m_gridPose;
...
CWnd* CMainFrame::CreateGridLinkBox(bool force)
{
	...
	if (force)
	{
		...
		if (!g_activeActor) return &m_gridLinkBox;
...
LRESULT CMainFrame::OnGridNotify(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == XTP_PGN_ITEMVALUE_CHANGED)
	{
		...
		else if (pItem->GetCaption() == "Material type:")
		{
			...
			{
				g_activeActor->m_inst->GetSelectedSkin(pActorItem->GetParentItem())->m_mtlLib->ConvertMaterial(strMatName, pItem->GetValue().GetBuffer());
...
void CMainFrame::OnTimer(UINT nIDEvent)
{
	if (nIDEvent == 0)
	{
		...
		if (g_bRelatedMode)
			sprintf(title, "ZFS Actor Editor [ver.%s][build.%s][%s] %s [关联模式]", VER, build, render, g_openFile.c_str());
		else
			sprintf(title, "ZFS Actor Editor [ver.%s][build.%s][%s] %s", VER, build, render, g_openFile.c_str());
		SetWindowText(title);
}
	else if (nIDEvent == 1)
	{
		...
		if (GetSelectedEffect() && (m_bRebuildGridProp || (DWORD)GetSelectedEffect() != m_selectedChannel))
		{
		...
		else if (GetSelectedMtl() && (m_bRebuildGridProp || (DWORD)GetSelectedMtl() != m_selectedChannel))
		{
			CXTPDockingPane* paneProp;
			paneProp = m_paneManager.FindPane(ID_GRID_CHANNEL_PROP);
			// if(!paneProp)
...
CRT_Material* CMainFrame::GetSelectedMtl()
{
	...
	if (pItem->GetCaption() == "Material")
	{
		...
		if (idx < 0 || idx >= g_activeActor->m_inst->m_skinList.size())
			return NULL;
		return g_activeActor->m_inst->m_skinList[idx]->GetMaterialLib()->GetMaterial(pItem->GetValue());
	}
...
//	CRT_Actor *cha = g_activeActor->m_data;
	...
	if (file[file.GetLength() - 1] != 't' ||
		file[file.GetLength() - 2] != 'c' ||
		file[file.GetLength() - 3] != 'a' ||
...
// TODO: Add your command handler code here
	...
	CString file = dlg.GetPathName();
	if (file[file.GetLength() - 1] != 'i' ||
		file[file.GetLength() - 2] != 'k' ||
		file[file.GetLength() - 3] != 's' ||
		file[file.GetLength() - 4] != '.')
	{
		file += ".ski";
	}
