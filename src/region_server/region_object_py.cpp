...
#define _CHECK_OBJECT_TYPE(ret)											REGION_OBJECT_PYTHON_CHECK_TYPE(self, CRegionObject*, selfObject, ret)
...
PyObject* CPythonRegionObject::SetTimer(region_object_python* self, PyObject* args, PyObject* kw)
{
	...
	if(!PyArg_ParseTupleAndKeywords(args, kw, "l|lllll:SetTimer", (char**)keywords, &id, &interval, &lParam[0], &lParam[1], &lParam[2], &lParam[3]))
...
bool CPythonRegionObject::Initialize(PyObject* module)
{
	...
	if(PyModule_AddIntConstant(module, "OBJECT_TYPE_NPC_COMBATIVE", OBJECT_TYPE_NPC_COMBATIVE) == -1)
		return false;
	...
	if(PyModule_AddIntConstant(module, "OBJECT_TYPE_SCENE_WARFIELD", OBJECT_TYPE_SCENE_WARFIELD) == -1)
		return false;
