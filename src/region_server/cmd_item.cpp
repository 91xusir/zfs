...
#define SAFE_READ(p)		{ if(!(p)) return CMD_ERROR_NONE; }
#define SAFE_ASSERT(p)		{ if ( !(p) ) return CMD_ERROR_NONE; }
...
ITEM_CMD_FUNCTION(cmd_c2r_player_pickitem)
{
	...
	else
	{
		...
		if(rgitem) g_factory->DestroyObject(rgitem);
		...
		if (CmdGiver->m_pTeam)
		{
			for(int index=CmdGiver->m_pTeam->mData.Begin(); index!=CmdGiver->m_pTeam->mData.End(); index++)
			{
				...
				{
					CItemCmdBuilder_Svr::Build_r2c_other_additem(CmdGiver->m_core.Name.c_str(), item, ITEM_REASON_PICK);
...
ITEM_CMD_FUNCTION(cmd_c2r_player_moveitem)
{
    ...
    CItemCmdBuilder_Svr::Build_r2c_player_move_item((EItemContainer)container, pageFrom, iFrom, jFrom, pageTo, iTo, jTo);
    CmdGiver->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_player_repair) 
{
	...
	if (oneOrall)
	{
		...
		else
		{
			...
			CItemCmdBuilder_Svr::Build_r2c_bag_addmoney(-Needendure, ITEM_CONTAINER_PLAYER_COMMON_BAG, ITEM_REASON_PICK);
	...
	if (ItemID_IsValid(origItemLuck))
	{
        ...
        else 
        {
            if (origItemUpdate.level>=0 && origItemUpdate.level<=2)
                ...
                SAFE_ASSERT(ItemIsGemLuck2(origItemLuck.type) || ItemIsGemAntiBroken(origItemLuck.type))
            else if (origItemUpdate.level>=6 && origItemUpdate.level<=8)
                SAFE_ASSERT(ItemIsGemLuck3(origItemLuck.type) || ItemIsGemAntiBroken(origItemLuck.type))
            else if (origItemUpdate.level==10)
                SAFE_ASSERT(false)
            else 
                return CMD_ERROR_UNKNOWN;
		...
		*/
		...
		if(nNum < nR[nCurLev][0])
		{
			...
			if (newItemUpdate.level >= 5)
			{
				...
				cre->Systembroadcast(playername,Name,BossName,SceneName,STRENGTHENEQUIP,AllStr.c_str());
...
ITEM_CMD_FUNCTION(cmd_c2r_player_itemlock)
{
    ...
    CItemCmdBuilder_Svr::Build_r2c_refresh_single_item(ITEM_CONTAINER_PLAYER_COMMON_BAG, page, gridI, gridJ, item);
    CmdGiver->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_trade_invite_response)
{
	...
	if (!pPlayerTo || !pPlayerTo->m_pItemContainer)	return CMD_ERROR_NONE;
	if (pPlayerTo->m_pTrade || cre->m_pTrade)	return CMD_ERROR_NONE;
	if (pPlayerTo->m_pPShop || cre->m_pPShop)	return CMD_ERROR_NONE;
	/**Tianh  09.12.26  玩家在PK状态*/
	if (pPlayerTo->m_lIsCombat || cre->m_lIsCombat) return CMD_ERROR_NONE;
	...
	if (bAgree)
	{
		CTrade *pTrade = RT_NEW CTrade(CmdGiver->m_pItemContainer, pPlayerTo->m_pItemContainer, CmdGiver->m_oId, pPlayerTo->m_oId, g_region->m_pItemManager);
		pTrade->GetPetbag1().Resize(ITEM_PET_BAG_PAGE,ITEM_PET_BAG_LINE,3 + CmdGiver->m_core.GradeLev / 20);
		pTrade->GetPetbag2().Resize(ITEM_PET_BAG_PAGE,ITEM_PET_BAG_LINE,3 + pPlayerTo->m_core.GradeLev / 20);
...
ITEM_CMD_FUNCTION(cmd_c2r_trade_lock)
{
	...
	// 把交易栏的物品传过去
	CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_TRADE_OTHER_ITEMS, pTrade->GetTradeBag(cre->m_pItemContainer) );
	pTradeWith->RecvCmd(g_sendCmd);
	CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_TRADE_OTHER_PET, pTrade->GetTradePetBag(cre->m_pItemContainer) );
	pTradeWith->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_trade_deal)
{
	...
	if (pTradeWith->m_pItemContainer->IsOperateLocked() || cre->m_pItemContainer->IsOperateLocked())
	{
		return CMD_ERROR_NONE;
	}
	...
	if ( pTrade->IsAgreed(pTradeWith->m_pItemContainer) )	// 对方已经同意交易了,双方交易
	{
		if ( pTrade->Deal() )
		{
			...
			cre->RecvCmd(g_sendCmd);
			// 更新道具栏
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_COMMON_BAG, &(pTradeWith->m_pItemContainer->m_Bag) );
			pTradeWith->RecvCmd(g_sendCmd);
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_MISSION_BAG, &(pTradeWith->m_pItemContainer->m_MissionBag) );
			pTradeWith->RecvCmd(g_sendCmd);
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_PET_BAG, &(pTradeWith->m_pItemContainer->m_PetBag) );
			pTradeWith->RecvCmd(g_sendCmd);
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_COMMON_BAG, &(cre->m_pItemContainer->m_Bag) );
			cre->RecvCmd(g_sendCmd);
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_MISSION_BAG, &(cre->m_pItemContainer->m_MissionBag) );
			cre->RecvCmd(g_sendCmd);
			CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_PET_BAG, &(cre->m_pItemContainer->m_PetBag) );
			cre->RecvCmd(g_sendCmd);
			...
			rt2_snprintf(cTmp128, 127, "%s %s", cre->m_core.Name.c_str(), pTradeWith->m_core.Name.c_str());
            cTmp128[127] = 0;
...
ITEM_CMD_FUNCTION(cmd_c2r_shop_query)
{
	...
	if (pNpc->m_pSellBag || !pNpc->m_NpcSellRareItem.empty())
	{
		...
		CItemCmdBuilder_Svr::Build_r2c_shop_buy_query_response(NpcID, pNpc->m_pSellBag, pNpc->m_NpcSellRareItem, nPriceRate);
		cre->RecvCmd(g_sendCmd);
	}
	else
	{
...
ITEM_CMD_FUNCTION(cmd_c2r_shop_buy)
{
	...
	for (int i=0; i<(int)idArray.array.size(); i++)
	{
		...
		if ( ItemIsMissionItem(item.type) )		nMissionItemCount ++;
		else									nCommonItemCount ++;
		// 生成一个新的item
	...
	if (abs(moneyClientCalc-moneyTotal) > 50 && point == 0) // 允许50块钱的误差（可能由于浮点数造成的）
	{
	...
	// 交易
 	...
 	{
		cre->m_userInfo->Deduct(rtRound(point),CDeduct::GetDeductInstance()->GetDeductId(),DEDUCT_TYPE_BUYITEM,point_arrItems);
	}
...
ITEM_CMD_FUNCTION(cmd_c2r_pshop_buy)
{
	...
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(0, 0, ITEM_REASON_PSHOP_NOPLAYER,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	if (!pPlayer->m_pPShop)
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(pPlayer->m_oId, 0, ITEM_REASON_PSHOP_NOPSHOP,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	...
	{
		// 可能是有物品已经被别人买走了,通知买家
		CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(pPlayer->m_oId, 0, ITEM_REASON_PSHOP_NOITEM,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		CItemCmdBuilder_Svr::Build_r2c_pshop_content(pPlayer->m_pPShop);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	...
	if ( ItemIsMissionItem(rItem) )			nMissionItemCount ++;
	else									nCommonItemCount ++;
	...
	if (cre->m_pItemContainer->m_Bag.FreeSpace()<nCommonItemCount || cre->m_pItemContainer->m_MissionBag.FreeSpace()<nMissionItemCount)
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(0, 0, ITEM_REASON_NULL,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	...
	if (amount != 0)
	{
		...
		CItemCmdBuilder_Svr::Build_r2c_bag_refresh(ITEM_CONTAINER_PLAYER_COMMON_BAG, &(pPlayer->m_pItemContainer->m_Bag));
		pPlayer->RecvCmd(g_sendCmd);
	...
	else if(pPShopBag->RemoveItem(rItem))
	{
		if(pPlayer->m_pItemContainer->RemoveItem(rItem))
		{
			...
			CItemCmdBuilder_Svr::Build_r2c_bag_additem(rItem,ITEM_CONTAINER_PLAYER_COMMON_BAG,tmpPage,tmpI,tmpJ,ITEM_REASON_NULL);
			cre->RecvCmd(g_sendCmd);
		}
	}
	...
	CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(pPlayer->m_oId,1,ITEM_REASON_NULL,ItemID,amount);
	cre->RecvCmd(g_sendCmd);
...
{	
	...
	if (!pPlayer || !pPlayer->IsUser())
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(0, 0, ITEM_REASON_PSHOP_NOPLAYER,FakeItemID,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	...
	if (!pPlayer->m_pPShop)
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(pPlayer->m_oId, 0, ITEM_REASON_PSHOP_NOPSHOP,FakeItemID,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	//卖家和买家如果都在交易的状态
	...
	{
		//可能是有物品已经被别人买走了,通知买家
		CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(pPlayer->m_oId, 0, ITEM_REASON_PSHOP_NOITEM,FakeItemID,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		CItemCmdBuilder_Svr::Build_r2c_pshop_content(pPlayer->m_pPShop);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	...
	if (vMoney!= price)
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(0, 0, ITEM_REASON_NULL,FakeItemID,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	if (amount != 0) //买到的如果是叠加物品
	...
	if (pPlayer->m_pItemContainer->m_Bag.FreeSpace()<1 || pPlayer->m_pItemContainer->m_MissionBag.FreeSpace()<1)
	{
		CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(0, 0, ITEM_REASON_NULL,FakeItemID,ItemID,amount);
		cre->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	...
	//反馈消息
	CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(pPlayer->m_oId,1,ITEM_REASON_NULL,FakeItemID,ItemID,amount);
	cre->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_bank_store_pet)
{
	...
	{
		CItemCmdBuilder_Svr::Build_r2c_bag_removeitem(ITEM_CONTAINER_PLAYER_PET_BAG, page, gridI, gridJ, ITEM_REASON_THROW);
		cre->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_bank_withdraw_pet)
{
	...
	if (cre->AddPetByItem(item) && 
		cre->m_pBankPet->RemoveItem(page, gridI, gridJ) &&
		pCon->m_PetBag.Find(item.id, &page, &gridI, &gridJ))
	{
		CItemCmdBuilder_Svr::Build_r2c_refresh_single_item(ITEM_CONTAINER_PLAYER_PET_BAG, page, gridI, gridJ, item);
		cre->RecvCmd(g_sendCmd);
...
ITEM_CMD_FUNCTION(cmd_c2r_decompose_item)
{
	...
	SItemDecompose* pDecomposeClass = g_region->m_pItemManager->m_pDecomposeMgr->FindDecomposing(pItemClass->decomposeID);
	...
	CBag bagTmp(ITEM_PLAYER_INVENTORY_LINE, ITEM_PLAYER_INVENTORY_COLUMN, ITEM_PLAYER_INVENTORY_PAGE, g_region->m_pItemManager);
	...
	CItemCmdBuilder_Svr::Build_r2c_bag_removeitem(ITEM_CONTAINER_PLAYER_COMMON_BAG, page, gridI, gridJ, 1);
	CmdGiver->RecvCmd(g_sendCmd);
	...
	g_region->SendUserLogEvent(PLT_DecomposeItem, CmdGiver->m_userInfo->m_userId, "", 0, strLogItem.c_str(), 0, "", 0, "");
...
ITEM_CMD_FUNCTION(cmd_c2r_enchant_item)
{
	...
	SAFE_ASSERT(CmdGiver->m_pItemContainer->m_Bag.Find(equipItemID, &EquipmentPosPage, &EquipmentPosI, &EquipmentPosJ));
	SAFE_ASSERT(CmdGiver->m_pItemContainer->m_Bag.Find(scrollItemID, &ScrollPosPage, &ScrollPosI, &ScrollPosJ));
	SItemID EquipmentItem = CmdGiver->m_pItemContainer->m_Bag.GetItem(EquipmentPosPage, EquipmentPosI, EquipmentPosJ);
	SItemID OrigEquipmentItem = EquipmentItem;
	SItemID ScrollItem = CmdGiver->m_pItemContainer->m_Bag.GetItem(ScrollPosPage, ScrollPosI, ScrollPosJ);
	...
	SAFE_ASSERT(ItemIsWeapon(EquipmentItem) || ItemIsArmor(EquipmentItem) || ItemIsShipin(EquipmentItem));
	SAFE_ASSERT(ItemIsScrollEnchant(ScrollItem.type));
	SAFE_ASSERT(pItemClassEquipment->requirement[ItemRequirement_Level] >= pItemClassScroll->requirement[ItemRequirement_Level]);
		...
		CItemManager::GetIndexFromType(ScrollItem.type) == REBUILD_ITEM_SET_BIND)
	{
		...
		{
			EquipmentItem.attribRate1 = g_region->m_pItemManager->m_pAttribMgr->CreateAttribRate(CAttribMgr::csGreen);
		}
		else if(EquipmentItem.color == ItemColor_Blue)
		{
			EquipmentItem.attribRate1 = g_region->m_pItemManager->m_pAttribMgr->CreateAttribRate(CAttribMgr::csBlue1);
			EquipmentItem.attribRate2 = g_region->m_pItemManager->m_pAttribMgr->CreateAttribRate(CAttribMgr::csBlue2);
		}
		else if(EquipmentItem.color == ItemColor_Golden)
		{
			EquipmentItem.attribRate1 = g_region->m_pItemManager->m_pAttribMgr->CreateAttribRate(CAttribMgr::csGolden1);
			EquipmentItem.attribRate2 = g_region->m_pItemManager->m_pAttribMgr->CreateAttribRate(CAttribMgr::csGolden2);
		}
		else {return CMD_ERROR_NONE;}
	}
	else if(CItemManager::GetIndexFromType(ScrollItem.type) == DOBIND_ITEM)
	{
		if(EquipmentItem.cBind != IB_NotBind) return CMD_ERROR_NONE;
		EquipmentItem.cBind = IB_Binded;
	}
	else if(CItemManager::GetIndexFromType(ScrollItem.type) == DISBINDE_ITEM)
	{
		if(EquipmentItem.cBind != IB_Binded) return CMD_ERROR_NONE;
		EquipmentItem.cBind = IB_BindWhenEquip;
	}
	else if(CItemManager::GetIndexFromType(ScrollItem.type) == SUPERPLUS10_ITEM)
	{
		if(EquipmentItem.level != 9) return CMD_ERROR_NONE;
		if(GetProb(0.05))
	...
	else if(CItemManager::GetIndexFromType(ScrollItem.type) <= ELE_REFINE_ITEM_MAX && CItemManager::GetIndexFromType(ScrollItem.type) >= ELE_REFINE_ITEM_MIN)
	{
		if(!ItemIsWeapon_MagicWeapon(EquipmentItem.type)) return CMD_ERROR_NONE;
		float rate = 1;
		int	limit = EquipmentItem.params[MW_PARAM_LEV] * EquipmentItem.params[MW_PARAM_LEV] / 25 * 10;
		int eletype;
	...
	else if(CItemManager::GetIndexFromType(ScrollItem.type) <= EQUIP_LEVELUP_ITEM_MAX && CItemManager::GetIndexFromType(ScrollItem.type) >= EQUIP_LEVELUP_ITEM_MIN)
	{
		...
		if(CItemManager::GetIndexFromType(EquipmentItem.type) != originId) {return CMD_ERROR_NONE;}
		if(EquipmentItem.level < 5) {return CMD_ERROR_NONE;};
...
// 		}
		...
		if(GetProb(rate))
		{ //成功，装备升级成DestId，Attrib继承，rate浮动范围(-15~5)，附魔取消
...
// 		}
	...
	CmdGiver->m_pItemContainer->m_Bag.SetItem(EquipmentPosPage, EquipmentPosI, EquipmentPosJ, EquipmentItem);
	...
	if (ItemCanPile(ScrollItem.type))
	{
		if(!CmdGiver->m_pItemContainer->UsePileItem(ScrollPosPage, ScrollPosI, ScrollPosJ))	
			return CMD_ERROR_NONE;
		CItemCmdBuilder_Svr::Build_r2c_player_usepileitem(ScrollItem.id, ScrollPosPage, ScrollPosI, ScrollPosJ);
		CmdGiver->RecvCmd(g_sendCmd);
	}
	else
	{
		CmdGiver->m_pItemContainer->m_Bag.RemoveItem(ScrollPosPage, ScrollPosI, ScrollPosJ);
		CItemCmdBuilder_Svr::Build_r2c_bag_removeitem(ITEM_CONTAINER_PLAYER_COMMON_BAG, ScrollPosPage, ScrollPosI, ScrollPosJ, ITEM_REASON_NULL);
		CmdGiver->RecvCmd(g_sendCmd);
	}
	// 通知Client
	CItemCmdBuilder_Svr::Build_r2c_refresh_single_item(ITEM_CONTAINER_PLAYER_COMMON_BAG, EquipmentPosPage, EquipmentPosI, EquipmentPosJ, EquipmentItem);
	CmdGiver->RecvCmd(g_sendCmd);
	
	if(bSuccess1)
	{
		CItemCmdBuilder_Svr::Build_r2c_enchant_item_result(EquipmentItem.id, ScrollItem.id, itemmsg_success);
		CmdGiver->RecvCmd(g_sendCmd);
	}
	else
	{
		CItemCmdBuilder_Svr::Build_r2c_enchant_item_result(EquipmentItem.id, ScrollItem.id, itemmsg_unknown_error);
		CmdGiver->RecvCmd(g_sendCmd);
	}
	...
	g_region->SendUserLogEvent(PLT_FuseItem, CmdGiver->m_userInfo->m_userId, "", 0, strLogItemOrig.c_str(), 0, strLogItemScroll.c_str(), 0, strLogItemNew.c_str());
...
ITEM_CMD_FUNCTION(cmd_c2r_fuse_gem)
{
	...
	SItemID NewItem = g_region->m_pItemManager->CreateItem(pCharm->lFuse, (EItemColor)item1.color, 0/*, BindType*/);
	...
	//修改日期：2010年6月26日
	//Begin
	if ( !pBag->RemoveItem(ItemIdx, ItemColor_All, ItemNeedNum)  || !GetProb(fRate) /*|| !CmdGiver->AddItem(NewItem)*/ )
	{
	...
	g_region->SendUserLogEvent(PLT_FuseGem, CmdGiver->m_userInfo->m_userId, "", 0, strLogItem1.c_str(), 0, strLogItem2.c_str(), 0, strLogItem3.c_str());
...
ITEM_CMD_FUNCTION(cmd_c2r_fuse_item)
{
	...
	SItemID item1 = pBag->GetItem(page1, gridI1, gridJ1);
	SItemID item2 = pBag->GetItem(page2, gridI2, gridJ2);
	SAFE_ASSERT( (item1.id != item2.id) && (item1.color != ItemColor_White) && (item1.color == item2.color) && (item1.type == item2.type) );
	SItemBase *pItemClass = g_region->m_pItemManager->GetItem(item1.type);
	SAFE_ASSERT(pItemClass);
	...
	if ( !CmdGiver->RemoveGold( int((float)pItemClass->requirement[ItemRequirement_Level]/10.f * (float)item1.color * 200.f) ))
	{
		CmdGiver->SendSystemMessage("钱不够，操作失败");
		return CMD_ERROR_NONE;
	}
	...
	if (item1.cBind==IB_Binded || item2.cBind==IB_Binded)
		...
		BindType = IB_NotBind;
	SItemID NewItem = g_region->m_pItemManager->CreateItem(g_region->m_pItemManager->GetIndexFromType(item1.type), (EItemColor)item1.color, 0, BindType);
	if (!ItemID_IsValid(NewItem))
	{
		CItemCmdBuilder_Svr::Build_r2c_fuse_item_result(itemmsg_unknown_error, origItem1, origItem2);
		CmdGiver->RecvCmd(g_sendCmd);
		return CMD_ERROR_NONE;
	}
	// 人物背包操作
	...
	strLogItem3 += ",";
	strLogItem3 += NewItem.SaveToString(g_region->m_pItemManager);
	g_region->SendUserLogEvent(PLT_FuseItem, CmdGiver->m_userInfo->m_userId, "", 0, strLogItem1.c_str(), 0, strLogItem2.c_str(), 0, strLogItem3.c_str());
...
void CItemCmdBuilder_Svr::Build_r2c_bag_additem(SItemID& item, EItemContainer container, int page, int gridI, int gridJ, int reason)
{
...
void CItemCmdBuilder_Svr::Build_r2c_bag_removeitem(EItemContainer container, int page, int gridI, int gridJ, int reason)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_move_item(EItemContainer container, int pageFrom, int iFrom, int jFrom, int pageTo, int iTo, int jTo)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_dispart_item(EItemContainer container, int pageFrom, int iFrom, int jFrom, int pageTo, int iTo, int jTo,int num)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_usepileitem(DWORD itemID, int page, int gridI, int gridJ)
{
	CG_CmdPacket *pPacket = g_sendCmd;
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_refresh_single_item(EItemContainer container, int page, int gridI, int gridJ, SItemID &item)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_refresh_equip_item(CItemContainerBase::EEquipParts part, SItemID &item)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_equip_item_state(CItemContainerBase::EEquipParts part, char bValid)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_use_magicitem_result(DWORD magicItemID, EMagicItemResult magicType, int money, SItemID& item, int exp)
{
    g_sendCmd->BeginWrite();
    g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_other_additem(const char* actorName, SItemID &item, char reason)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_use_pet(DWORD actorID, SItemID& item, EItemContainer container, int page, int gridI, int gridJ)
{
	CG_CmdPacket *pPacket = g_sendCmd;
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_itemupdate_result(SItemID &itemOrig, SItemID &scrollOrig, SItemID &luckOrig, SItemID &itemNew)
{
	CG_CmdPacket *pPacket = g_sendCmd;
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_player_equipitem(SItemID& item, CItemContainerBase::EEquipParts part)
{
	CG_CmdPacket *pPacket = g_sendCmd;
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_shop_buy_query_response(DWORD npcID, CBag *pBag, std::vector<SNpcSellRareItem>& arrRare, short nPriceRate)
{
...
void CItemCmdBuilder_Svr::Build_r2c_pshop_sell_result(DWORD from, BYTE bSuccess, BYTE bReason,long FakeItemID,long ItemID,short amount)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_pshop_buy_result(DWORD from, BYTE bSuccess, BYTE bReason,long ItemID,short amount)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_pshop_sell_buy(DWORD customer,long FakeItemID ,long itemID,long money,short amount)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_pshop_other_buy(DWORD customer, long itemID,long money,short amount)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_bank_store_response_pet(BYTE bSuccess, BYTE reason/* = ITEM_REASON_NULL*/)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_bank_withdraw_response_pet(BYTE bSuccess, BYTE reason/* = ITEM_REASON_NULL*/)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_bank_store_response(BYTE bSuccess, BYTE reason/* = ITEM_REASON_NULL*/)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_bank_withdraw_response(BYTE bSuccess, BYTE reason/* = ITEM_REASON_NULL*/)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
...
void CItemCmdBuilder_Svr::Build_r2c_enchant_item_result(DWORD equipItemID, DWORD scrollItemID, short ErrorCode)
{
	g_sendCmd->BeginWrite();
	g_sendCmd->WriteShort(r2c_subsystem_item);
