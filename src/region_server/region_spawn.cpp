...
: m_respawnIntervalFactor(1)
, m_respawnCountFactor(1)
, m_nCheatIndex(0)
, m_nLineSysSceneID(1)
{
}
...
{
	stlforeach(std::list<CRegionSpawn>, spawn, m_spawns)
		if (spawn->m_id == id)
			return &*spawn;
...
{
    if( v1 < rect[0] ) return FALSE;
    if( v1 > rect[2] ) return FALSE;
    if( v2 < rect[1] ) return FALSE;
    if( v2 > rect[3] ) return FALSE;
    return TRUE;
}
...
{
    if (VertexIntersectRectangle(r1[0], r1[1], r2)) return TRUE;
    if (VertexIntersectRectangle(r1[0], r1[3], r2)) return TRUE;
    if (VertexIntersectRectangle(r1[2], r1[1], r2)) return TRUE;
    if (VertexIntersectRectangle(r1[2], r1[3], r2)) return TRUE;

    if (VertexIntersectRectangle(r2[0], r2[1], r1)) return TRUE;
    if (VertexIntersectRectangle(r2[0], r2[3], r1)) return TRUE;
    if (VertexIntersectRectangle(r2[2], r2[1], r1)) return TRUE;
    if (VertexIntersectRectangle(r2[2], r2[3], r1)) return TRUE;

    if (LineIntersectLine(r1, r1+2, r2, r2+2)) return TRUE;
    return FALSE;
}
...
bool CRegionSpawnMgr::LoadSpawnTable(const char* filename)
{
	...
	for (int lineId = titleLine + 1; lineId < numLines; lineId++)
	{
		...
		spawn->Init(id, npc, count, (int)interval, rate, &area->lMinX, &area->lMaxX, timeBegin, timeEnd);
		...
		if(cheatCount > 0)
			cheatSpawn->Init(id+10000,npc,cheatCount, (int)interval, rate, &area->lMinX, &area->lMaxX, timeBegin, timeEnd);
...
void CRegionSpawnMgr::SpawnCheat()
{
	...
	while(cnt++ < 100)
	{
		...
		if(spawn->m_curNpcCount >= spawn->m_maxNpcCount) continue;
...
void CRegionSpawn::Init(short	id,
						short	npcID,
						int		maxNpcCount, 
						int		spawnInterval, 
						float	rate,
	...
	if (m_npcID < 0 || m_npcID > 2000)
	{
		...
		if (theSpawn.m_npcID > 2000 || theSpawn.m_npcID < 0)
			ERR("[Check spawn] cheat IT spawn error npcID %d spawnID %d\n",
					theSpawn.m_npcID, theSpawn.m_id);
		...
		for (;it != g_spawnMgr.m_cheatSpawns.end(); ++it)
		{
			...
			if (spawn.m_npcID > 2000 || spawn.m_npcID < 0)
				ERR("[Check spawn] list spawn error at %d npcID %d spawnID %d\n",
						counter, spawn.m_npcID, spawn.m_id);
