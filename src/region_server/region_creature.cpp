...
void CRegionCreature::WgChecked(int iBaseTime, int iRandTime, char *szCheckInfo)
{
    ...
    if (m_wgChecked==0)
    {
        ...
        m_wgCheckedTime = (long)time(NULL)+iBaseTime+rand()%iRandTime; // 检查到外挂，加上120到600秒的延时退出
#ifdef WIN32
...
bool CRegionCreature::CanAttack(CRegionCreature *enemy, SSkill* pSkill)
{
    ...
    if (enemy)
	{
		...
		if(enemy->m_npcInfo != NULL && m_mc && !m_mc->m_bstartTask)//tim.yang  MC  在没有进行怪物攻城的时候才允许城战
		{
        ...
        if (bSelfIsUserOrPet != bEnemyIsUserOrPet)
        {
            if (!bEnemyIsUserOrPet && enemy->m_npcInfo) // 如果敌人是怪
            {
                if (enemy->m_npcInfo->Type==4 || enemy->m_npcInfo->Type==5) // 药材或者矿物
                {
                    ...
                    {
                        if ((pSkill->dwRTarget&SKILL_TARGET_HERB) && (enemy->m_npcInfo->Type==4)) // 药材
                        {
                            if (pSkill->iLevel<enemy->m_npcInfo->Level)
                            {
                                return false; // 等级不够
                            }
                        }else if ((pSkill->dwRTarget&SKILL_TARGET_MINE) && (enemy->m_npcInfo->Type==5)) // 矿物
                        {
                            if (pSkill->iLevel<enemy->m_npcInfo->Level)
                            {
                                return false; // 等级不够
	...
	if (m_ScopeoDinaryInjury) //如果是普攻群体伤害
	{
		...
		for (int i=0; i<iCnt; i++)
		{
			if(pFindCreature[i]->m_type==OBJECT_TYPE_CREATURE || pFindCreature[i]->m_type==OBJECT_TYPE_NPC_COMBATIVE)
			{
				...
				if (pTargetCreature->GetPlayerLogic()->GetLiveType() == CPlayerLogic::eDead) //死亡的
				{
				}
...
void  CRegionCreature::RealAttackcalculating(CRegionCreature *enemy,int phyDmg,int eleDmg[MAX_ELEMENT_ATTR],int ret,long seed)
{
	...
	if((m_core.CriticalHit.GetValue()-enemy->m_core.ReCriticalHit.GetValue())>0)//增加抗必杀属性差数运算
	{
...
{	
	...
	if(m_npcInfo != NULL)
	{
		if(g_RegionCastleWarMgr.IsGuardAnimal(m_npcInfo->Id) == true && m_mc && !m_mc->m_bstartTask)//tim.yang  MC  在没有进行怪物攻城的时候才允许城战
		{
			if(enemy != NULL)
			{
...
// 			return;
	...
	else if (other->m_core.Lev > 30 && m_core.Faction != other->m_core.Faction && g_pkConfig->exploitManager.iOn_Off != 0)
	{
		...
		{
			rt2_sprintf(g_strStaticBuffer, R(MSG_NOT_GET_ROLE_APPARATUS), other->m_core.Name.c_str());
			SendSystemMessage(g_strStaticBuffer);
		}
	}
	...
	if (SetPosition(x, y, z))
	{
		...
		if (bBroadcast)
		{
			if(m_master)
			{
				if (m_master->m_scene)
				{
					m_master->m_scene->BroadcastCmd(m_master->m_pos, DEFAULT_RANGE, g_sendCmd, NULL);
				}
				else
				{
					m_master->RecvCmd(g_sendCmd);
...
// 				}
	...
	GodLightCreateMonster_DelAllMonster();//tim.yang 神灯   玩家死亡清除掉神灯召唤出来的monster
	if(IsUser() && m_IsAddMonster)//玩家神灯召唤出来的monster被打死，将该玩家身上的所有相关属性清除
	{
	...
	if (m_dwPkGetItemTime && IsUser() && m_vPkGetItemList.size() && m_dwPkGetItemTime < rtGetMilliseconds())
	{
		m_dwPkGetItemTime = 0;
		m_vPkGetItemList.clear();
	}
	...
	*/
	...
	if(enemy)
	{
		...
		if ((IsUserPet() && m_ai) || (m_pet && m_pet->m_ai))
		{
			...
			if (petAI && (petAI->m_eAS == CRegionPetAI::AS_SYNERGY || petAI->m_eAS == CRegionPetAI::AS_PASSIVE))
			{
				petAI->OnAttack(enemy);
			}
		}
...
long CRegionCreature::OnDead(CRegionCreature *killer)
{
	...
	if((IsGuarder() || IsMonster()) /*&& OB_TYPE(m_scene) != OBJECT_TYPE_SCENE_FB */&& !m_cheatDummy)
	{
		...
		float totalExp = g_cfg.game.exp_rate * m_killReward.Exp;//g_cfg.game.exp_rate * GetRand( rtFloor(m_killReward.Exp*1.1 + 1.0f), m_killReward.Exp*0.9 );
					...
					{					
						...
						{						
							if(info->m_currMonsterNum < atol(info->Desc->taskReq[0].param[1].c_str()))
							{
								...
								std::string monster = "已杀死" + pMonster->Name + ":" + std::string(rtFormatNumber(info->m_currMonsterNum));
	...
	if (m_pItemContainer)//当玩家装备经验瓶时，只给经验瓶加经验，其他不加
	{
		...
		if(pShi && (neckId == 4306 || neckId == 4515))
		{
			...
			else
			{
				...
				rt2_sprintf(g_strStaticBuffer, R(MSG_BOTTLE_ADD_EXP), Exp, maxExp - neckItem.params[MW_PARAM_EXP]);
				SendBattleInfoMessage(g_strStaticBuffer);
				return;
			}
		}
...
void CRegionCreature::Levelup(int iUpCount)
{
	...
	if (m_userInfo && m_unionID!=0)
	{
		...
		packet.WriteShort(r2g_log_event);
		packet.WriteByte(PLT_LevelChanges);
		packet.WriteLong(m_userInfo->m_userId);
		packet.WriteString(m_userInfo->m_userIpAddress.c_str()); // userIP
		packet.WriteLong(OriginalLevel);
		packet.WriteLong(NowLevel);
		packet.WriteLong(iUpCount);
		g_region->m_gws.EndSend();
	}

	GetPlayerLogic()->OnLevelUp();
}
...
void CRegionCreature::AddGold(int Gold)
{
	...
	if (Gold > 0)
	{
		...
		{
			CItemCmdBuilder_Svr::Build_r2c_bag_addmoney(Gold, ITEM_CONTAINER_PLAYER_COMMON_BAG, ITEM_REASON_PICK);
			RecvCmd(g_sendCmd);
		}
	}
	else 
...
bool CRegionCreature::RemoveGold(int remove)
{
	...
	{
		CItemCmdBuilder_Svr::Build_r2c_bag_addmoney(-remove, ITEM_CONTAINER_PLAYER_COMMON_BAG, ITEM_REASON_PICK);
		RecvCmd(g_sendCmd);
		return true;
	}
	else 
...
//			LOG1("Equip use time: %d\n", rtGetMilliseconds()-dwTime);
		...
		else // UnEquip
		{
			...
			if (m_pItemContainer->UnEquip(part))
			{
				...
				CItemCmdBuilder_Svr::Build_r2c_bag_additem(item, ITEM_CONTAINER_PLAYER_COMMON_BAG, p, i, j, ITEM_REASON_UNEQUIP);
				RecvCmd(g_sendCmd);
			}
		} // UnEquip
	...
	else if(ItemIsPet(item))
	{
		...
		CItemCmdBuilder_Svr::Build_r2c_player_use_pet(m_oId, item, ITEM_CONTAINER_PLAYER_COMMON_BAG, page, gridI, gridJ);
		RecvCmd(g_sendCmd);
	}
	// 神仙(不可重叠摆放道具)
	...
	else if (ItemIsSkill(item))
	{
		...
		{
			if ((&(*itEffect))->pAttr->wID == pSkill->effectID)//如果还有此技能的持续效果  则不能再使用
			{
				...
				if (pSceneInfo)
				{
					...
					if (pSceneInfo->sceneFaction==m_core.Faction || (pSceneInfo->sceneFaction!=FACTION_ZHOU && pSceneInfo->sceneFaction!=FACTION_SHANG))
					{
						...
						else
						{
							...
							if (g_RegionCastleWarMgr.IsCastleWarTime("XiQi") && (m_core.Faction == FACTION_ZHOU))
								return false;
							if (g_RegionCastleWarMgr.IsCastleWarTime("ZhaoGe") && (m_core.Faction == FACTION_SHANG))
								return false;
	...
	else if(ItemIsOther(item.type))
	{
        ...
        if (ItemIsOther_Magic(item.type))
        {
			...
			if (pReward->RewardType[nSelectIndex] == MI_ALL)
			{
				...
				for (nSelectIndex = 0; nSelectIndex < (int)pReward->HitNum.size(); nSelectIndex++)
				{
					if (pReward->HitNum[nSelectIndex] < 0 || !GetProb(pReward->HitNum[nSelectIndex]))
					{
			...
			else if (nSelectIndex>=0 && nSelectIndex<(int)pReward->HitNum.size())
			{
				if (pReward->RewardType[nSelectIndex] == MI_Money)
				{
					...
					CItemCmdBuilder_Svr::Build_r2c_bag_addmoney(nMoney, ITEM_CONTAINER_PLAYER_COMMON_BAG, ITEM_REASON_PICK);
					RecvCmd(g_sendCmd);
				}
...
//--------------------add start by tony 05.06.07--------------------------------------------//
	...
	else if (ItemIsScrollCompose(item.type))
	{
		...
		// 取得配方资料
		SItemCompose* pComposeClass = g_region->m_pItemManager->m_pComposeMgr->FindComposing(pItemClass->composeID);
		if (!pComposeClass)		return false;
		// 判断是否满足学习配方的条件
...
int CRegionCreature::GetDungeonMissionItemNum(DWORD typeId, int difficulty)
{
	...
	{
		ERR1("[CRegionCreature::GetDungeonMissionItemNum] Item is NOT a dungeon mission item(index=%d)\n", typeId);
		return 0;
	}
	...
	if (ItemIsMissionItem(pItemClass->type))
	{
		if (m_pItemContainer->m_MissionBag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	else if (ItemIsPet(pItemClass->type))
	{
		if (m_pItemContainer->m_PetBag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	else
	{
		if (m_pItemContainer->m_Bag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	return 0;
}
...
bool CRegionCreature::RemoveDungeonMissionItem(DWORD typeId, int num, int difficulty)
{
	...
	if (!ItemIsOther_Dungeon(pItemClass->type))
	{
		ERR1("[CRegionCreature::RemoveDungeonMissionItem] Item is NOT a dungeon mission item(index=%d)\n", typeId);
		return false;
	}
...
void CRegionCreature::SavePetDataToItem(CRegionCreature* pMaster)
{
	...
	if (m_ai) ((CRegionPetAI*)m_ai)->m_pItemPet = item;
...
void CRegionCreature::CommonChat(char channel, char const* msg, char const* target, char const* title)
{
...
// 	const char* Cmag = ResolvingLinkChat(msg);
	...
	else if (channel == CHAT_CHANNEL_COUNTRY)
	{//国家频道
		CountryChat(Cmag);
	}
	else if (channel == CHAT_CHANNEL_GM)
	{//GM频道
		if (title && *title)
			GmChat(Cmag, title);
		else 
...
void CRegionCreature::GmChat(const char *msg, const char *title/*=""*/)
{
	...
	g_region->m_gws.SendChat(m_userInfo->m_userId, m_core.Name.c_str(), CHAT_CHANNEL_GM, msg, title);
	return;
...
ETaskOverHead CRegionCreature::GetTaskOverHead(DWORD npcOID)
{
	...
	case CPlayerLogic::eList:
		nT = pReNPC->GetCNpcTaskLogicExt()->GetTaskList(os, this, true, nFinishMain, nDoingMain, nAcceptMain, false);
		nT = pReNPC->GetCNpcTaskLogicExt()->GetTaskList(os, this, false, nFinish, nDoing, nAccept, false);
		break;
...
DWORD CRegionCreature::GetTargetParty(CRegionCreature* pTarget) // 取得敌人的类型，可能是友军，敌军，宏在skill_base.h中
{
    /*
	if(pTarget == NULL)
	{
...
int CRegionUser::Prededuct(short sPoint,long event,long lAuctionId /*= 0*/) // 预扣点数, 用来做用点数向NPC买道具, 不能为负数
{
...
void CRegionUser::Deduct(short sPoint,long orderid,DeductType type,void* v) // 扣点数, 用来做用点数向NPC买道具, 不能为负数
{
    //int lDeductOrderID = 0;
...
bool CRegionUser::wgCheckServer(unsigned char* buf, int& iDataSize, int iBufSize)
{
    ...
    if (cmdID==c2r_attack || cmdID==c2r_update_position)
    {
        ...
        if (((dwNow-m_lClientLastCheckedTime)>5*60)||(cmdID==c2r_attack)) // 每5分钟检查一次
#endif
        {
            ...
            if (!g_region->JhWgCheck(iCheckData, m_lClientLUASeed, cmdID)) // 返回false表示找到外挂
            {
                ...
                }else
                {
                    ...
                    {
                        LOG1("玩家 [%s] 使用外挂\n", m_username.c_str());
                        m_dummy->SendSystemMessage("为了创造公平的游戏环境，请勿使用外挂。使用外挂者每五分钟被禁止移动一分钟");
	...
	if(g_bUnWg)
	{
		...
		if(cmdID != r2c_init)
		{
			if( cmdID == r2c_update_show_info ||
				...
				cmdID == r2c_you_are_dead ||
				cmdID == r2c_subsystem_castlewar ||
...
void CRegionCreature::OnAddAccountTime(long lUserID, char* pAccountTime,char cSucceed)
{
	...
	{	
		...
		{
			rt2_sprintf(g_strStaticBuffer, "充值成功；目前您的会员到期时间为 %s,重新登陆后会员时间会被刷新！！", pAccountTime); 
			SendSystemMessage(g_strStaticBuffer);
		}
}
