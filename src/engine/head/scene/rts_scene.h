...
class RtScene : public RtsSceneDoc, public RtsSceneUtil {
   public:
    static constexpr int MAX_MAP_COUNT = 512;
    static constexpr int MAX_WATER_COUNT = 10;
    static constexpr int MIN_INDEX_COST = -100;
   ...
   public:
    ...
    void DrawTerrain(RtgCamera& inCamera, BYTE cAttrMask, DWORD dwNormalColor = 0xFF88FF88,
                     DWORD dwHitColor = 0xFFFF0000, float fOffset = 0.5f);
    void DrawMiniMap(int iScreenX, int iScreenY, int iScreenWidth, int iScreenHeight,
   ...
   public:
    ...
    RtsSVertexValue* GetVertexValue(int x, int y,
                                    bool bEdit);  // 取得顶点属性数据, x, y 为顶点的索引
    BYTE             GetVertexHeightLevel(int x, int y, bool bTile);  // x, y 为顶点的索引
    void             SetVertexHeightLevel(int x, int y, bool bTerrain, bool bTile,
                                          BYTE cHeightLevel);  // x, y 为顶点的索引
    ...
    RtsObject* PickLine(const RtgVertex3& v0, const RtgVertex3& v1, float& fOutDist, int& fOutX,
                        int& fOutY, DWORD dwType);
    RtsObject* PickRay(const RtgVertex3& v, const RtgVertex3& dir, float& fOutDist, int& fOutX,
                       int& fOutY, DWORD dwType);
    ...
    void ComputeVertexNormal(
        int iX,
        int iY);  // x, y 为顶点的索引，一个块的顶点数为(g_iSceneTileCntX+1)*(g_iSceneTileCntY+1)
    ...
    bool TakeGeometry(const RtgAABB& inAABB, RtgVertex3* pVertices, int& iVertexNum,
    ...
    RtsSGrid* GetGrid(int _girdx, int _gridy, bool bedit) {
        ...
        return ((RtSceneBlock*)_block->m_pBlock)->m_pGrid +
               ((_gridy - _blocky * g_iSceneGridCntY) * g_iSceneGridCntX +
                (_girdx - _blockx * g_iSceneGridCntX));
    }
    ...
    void AddRenderObject(RtsObject* _obj) { m_ObjectRender.AddObject(_obj); }
    ...
    void SetMinimapMaskTexture(const char* szTextureFileName, float fL = 0.f, float fT = 0.f,
                               float fR = 1.f, float fB = 1.f);
   ...
   public:
    ...
    void CreateGrass(RtsSTileMap* tile, RtSceneBlock* block, RtsSVertex* v, WORD tid, WORD density,
                     float minH, float maxH, float aspect);
   ...
   public:
    ...
    inline bool SupportTerrainRender_PP() const {
        return !!m_pDevice->GetEffect(Rtg_Effect_Terrain);
    }
   ...
   public:
    ...
    struct _TransActorHolder {
        ...
        _TransActorHolder(CRT_ActorInstance* _act, float fv) : act(_act), fVisible(fv) {}
    };
