...
bool RtsSceneDoc::NewMap(const char* szMapFileName) {
    ...
    m_iCenterPosY = -g_iSceneBlockOffsetY;
#ifndef REGION_SERVER
    m_SceneBlockManager.InitSceneBlock(&m_blockMapArray[0], m_szFilePath, g_iSceneBlockOffsetX,
                                       g_iSceneBlockOffsetY);
#else
...
bool RtsSceneDoc::AddMapBlock(int iPosX, int iPosY, RtsSceneBlockRtb* pMap) {
    ...
    if (iPosX < -1 || iPosY < -1 || iPosX > m_iBlockXCnt || iPosY > m_iBlockYCnt)
        return false;
    if (iPosX == -1 || iPosY == -1 || iPosX == m_iBlockXCnt ||
        iPosY == m_iBlockYCnt) {  // 要修改大小，地图被扩大了
        int iNewSizeX = m_iBlockXCnt, iNewSizeY = m_iBlockYCnt;
        if (iPosX == -1 || iPosX == m_iBlockXCnt)
            iNewSizeX++;
        if (iPosY == -1 || iPosY == m_iBlockYCnt)
            iNewSizeY++;
        if (iPosX == -1)
            xOffset = 1;
        else
        ...
        for (y = 0; y < iNewSizeY; y++) {
            if (y < yOffset)
                continue;
            if ((y - yOffset) >= m_iBlockYCnt)
                continue;
            for (x = 0; x < iNewSizeX; x++) {
                if (x < xOffset)
                    continue;
                if ((x - xOffset) >= m_iBlockXCnt)
                    continue;
                _blockArray[y * iNewSizeX + x] =
                    m_blockMapArray[(y - yOffset) * m_iBlockXCnt + (x - xOffset)];
...
#ifndef REGION_SERVER
    m_SceneBlockManager.InitSceneBlock(&m_blockMapArray[iPosY * m_iBlockXCnt + iPosX], m_szFilePath,
                                       iPosX - m_iCenterPosX, iPosY - m_iCenterPosY);
#else
    m_blockMapArray[iPosY * m_iBlockXCnt + iPosX].New(m_szFilePath, iPosX - m_iCenterPosX,
                                                      iPosY - m_iCenterPosY);
#endif
...
#ifndef REGION_SERVER
                m_SceneBlockManager.CreateSceneBlock(_block, m_szFilePath, x - GetCenterX(),
                                                     y - GetCenterY(), true);
#else
    ...
    for (int y = 0; y < m_iBlockYCnt; ++y) {
        for (int x = 0; x < m_iBlockXCnt; ++x) {
            ...
            if (_blockMap->Check_CanLoad()) {
                m_SceneBlockManager.CreateSceneBlock(_blockMap, m_szFilePath, x - m_iCenterPosX,
                                                     y - m_iCenterPosY, true);
                bNeedRelease = true;
            }
...
RtSceneBlockTerrain* RtsSceneDoc::GetTerrainBlock(int iTerrainX, int iTerrainY, int& iBx,
                                                  int& iBy) {
    OffsetTerrainToMap(iTerrainX, iTerrainY);
    if (iTerrainX < 0 || iTerrainY < 0)
        return NULL;
...
bool RtsSceneDoc::SetTerrainAttr(int iTerrainX, int iTerrainY, BYTE cAttrMask) {
    ...
    if (pBlock == NULL)
        return false;
    return pBlock->SetTerrainAttr(iTerrainX - (iBx - m_iCenterPosX) * g_iSceneTerrainCntX,
                                  iTerrainY - (iBy - m_iCenterPosY) * g_iSceneTerrainCntY,
                                  cAttrMask);
}
...
bool RtsSceneDoc::GetCanWalk(int iTerrainX, int iTerrainY) {
    ...
    if (pBlock == NULL)
        return false;
    return pBlock->CanWalk(iTerrainX - (iBx - m_iCenterPosX) * g_iSceneTerrainCntX,
                           iTerrainY - (iBy - m_iCenterPosY) * g_iSceneTerrainCntY);
}
...
bool RtsSceneDoc::RemoveTerrainAttr(int iTerrainX, int iTerrainY, BYTE cAttrMask) {
    ...
    if (pBlock == NULL)
        return false;
    return pBlock->RemoveTerrainAttr(iTerrainX - (iBx - m_iCenterPosX) * g_iSceneTerrainCntX,
                                     iTerrainY - (iBy - m_iCenterPosY) * g_iSceneTerrainCntY,
                                     cAttrMask);
}
...
RtSceneBlockTerrain::STileAttr* RtsSceneDoc::GetTerrainPathAttr(int iTerrainX, int iTerrainY) {
    ...
    return pBlockMap->m_pBlock->m_pTileAttr +
           ((iTerrainY - iBy * g_iSceneTerrainCntY) * g_iSceneTerrainCntY +
            (iTerrainX - iBx * g_iSceneTerrainCntX));
}
...
void RtsSceneDoc::GetTerrainByPosFast(float fPos[], float fDir[], short sRad[], int& iMinX,
                                      int& iMinY, int& iMaxX, int& iMaxY) {
...
void RtsSceneDoc::GetTerrainByPosFast(long lPos[], long lDir[], short sRad[], int& iMinX,
                                      int& iMinY, int& iMaxX, int& iMaxY) {
    float fRtX1, fRtY1, fRbX1, fRbY1, fLtX1, fLtY1, fLbX1, fLbY1;
    float fRtX, fRtY, fRbX, fRbY, fLtX, fLtY, fLbX, fLbY, x, y;
    x = lDir[0] - lPos[0];
...
bool RtsSceneDoc::AddAreaInfo(const char* szAreaFileName) {
    ...
    *pFile << dwRead;
    if (dwRead != dwFileVersion) {
        RtCoreLog().Error(
            "RtsSceneDoc::AddAreaInfo: Opened area information [%s] Version [%d] unknown.\n",
    ...
    *pFile << dwRead;
    if (dwRead != dwFileFlag) {
        RtCoreLog().Error(
            "RtsSceneDoc::AddAreaInfo: Opened area information file [%s] format error.\n",
            szAreaFileName);
        RtCoreFile().CloseFile(pFile);
        return false;
    }
            ...
            for (x = 0; x < m_iBlockXCnt; x++) {
                ...
                if (pMap->m_szFileName[0]) {
                    ...
                    for (i = 0; i < m_iAreaCount; i++) {
                        pArea = m_pAreas + i;
                        if (!(fMinX >= pArea->lMaxX || fMinY >= pArea->lMaxY ||
                              fMaxX < pArea->lMinX || fMaxY < pArea->lMinY)) {
                            if (z) {
...
RtsSceneBlockRtb::RtsSceneBlockRtb()
    : m_pBlock(NULL), m_region_id(0), m_dwEnvironmentLighting(0), m_dwSunLighting(0),
      ...
      m_dwPostProcessObjectHLColorEmi(0), m_dwPostProcessObjectHLColorDif(0),
      m_dwPostProcessObjectHLColorSpe(0), m_fPostProcessObjectHLColorSpePower(0), m_dwFogColor(0),
      m_fFogNear(0.f), m_fFogFar(0.f), m_fBackMusicStartTime(0.f), m_fBackMusicDifferenceTime(0.f),
      m_ppAreas(NULL), m_iMapAreaCount(0), m_lLastUseTime(0), m_blockPosx(0), m_blockPosy(0),
...
bool RtsSceneBlockRtb::New(const char* szPath, int iOffsetX, int iOffsetY) {
    ...
    return ((RtSceneBlock*)m_pBlock)
        ->NewBlock(szPathName, m_pBlock->m_iBlockPosX * g_fSceneTileWidth * g_iSceneTileCntX,
                   m_pBlock->m_iBlockPosY * g_fSceneTileWidth * g_iSceneTileCntY);
#endif
}
