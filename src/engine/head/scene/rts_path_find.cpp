...
bool RtsPathPath::Next(int& x, int& y, RtsPathFind* pPath, bool bIgnoreActor, bool bIgnoreTarget,
                       int actorType) {
    ...
    if (m_iFirst != m_iLast) {
        ...
        if (1 == actorType && !m_Path[m_iFirst].isWalk) {
            ...
            curNode =
                pPath->FindPathByTwoPos(cBlock, start, end, bIgnoreActor, bIgnoreTarget, actorType);
            if (!curNode) {
                return false;
            }
...
RtsPathPath::PPos* RtsPathPath::GetStep(int iStep) {
    if (m_iFirst == m_iLast)
        return NULL;
    if (iStep == 0)
        return m_Path + m_iFirst;
    int iCnt = PathCount();
    if ((iStep + 1) > iCnt)
        return NULL;
    iCnt = m_iFirst + iStep;
    if (iCnt >= (int)PF_MAX_DEPTH)
        iCnt %= PF_MAX_DEPTH;
    return m_Path + iCnt;
}
...
bool RtsPathContainer::RemoveNode(RtsPathPos* pNode) {
    ...
    if (pNodes->dwActorId == pNode->dwActorId && pNodes->sActorType == pNode->sActorType) {
        ...
        RtsPathPos* p = pNodes;
        while (p->pNext) {
            if (p->pNext->dwActorId == pNode->dwActorId &&
                p->pNext->sActorType == pNode->sActorType) {
                pDelNode = p->pNext;
...
bool RtsPathFind::CanAdd(DWORD dwActorId, short sActorType, int iTerrainX, int iTerrainY,
                         char cBlock, bool bOnlyOne) {
    if (cBlock == PB_NONE)
        return true;
    ...
    if (cBlock) {
        if (pAttr->pContainer) {
            ...
            if (pAttr->pContainer->iBlock) {
                ...
                while (p->pNext) {
                    if (p->pNext->dwActorId !=
                        dwActorId /* && p->pNext->sActorType != sActorType*/) {
                        m_lastResult = PR_CPLAYER;
                        return false;
                    }
                    ++i;
                ...
                if (p) {
                    if (p->dwActorId != dwActorId && p->sActorType != sActorType) {
                        if (!(p->m_pChkFunc &&
                              p->m_pChkFunc(sActorType, dwActorId, p->sActorType, p->dwActorId))) {
                            m_lastResult = PR_CPLAYER;
                            return false;
                        }
                    }
...
bool RtsPathFind::CanAdd(DWORD dwActorId, short sActorType, int iMinTerrainX, int iMinTerrainY,
                         int iMaxTerrainX, int iMaxTerrainY, char cBlock, bool bOnlyOne) {
    if (cBlock == PB_NONE)
        return true;
            ...
            if (cBlock) {
                if (pAttr->pContainer) {
                    ...
                    if (pAttr->pContainer->iBlock) {
                        ...
                        while (p->pNext) {
                            if (p->pNext->dwActorId != dwActorId &&
                                p->pNext->sActorType != sActorType) {
                                m_lastResult = PR_CPLAYER;
                                return false;
                            }
                            ++i;
                        ...
                        if (p) {
                            if (p->dwActorId != dwActorId && p->sActorType != sActorType) {
                                if (!(p->m_pChkFunc &&
                                      p->m_pChkFunc(sActorType, dwActorId, p->sActorType,
                                                    p->dwActorId))) {
                                    m_lastResult = PR_CPLAYER;
                                    return false;
                                }
...
RtsPathPos* RtsPathFind::AddActor(DWORD dwActorId, short sActorType, int iTerrainX, int iTerrainY,
                                  bool bPath, char cBlock, void* pChkFunc) {
    if (cBlock == PB_NONE)
        return NULL;
...
RtsPathPos* RtsPathFind::AddActor(DWORD dwActorId, short sActorType, int iMinTerrainX,
                                  int iMinTerrainY, int iMaxTerrainX, int iMaxTerrainY, bool bPath,
                                  char cBlock, void* pChkFunc) {
    if (cBlock == PB_NONE)
        return NULL;
    ...
    for (int i = iMinTerrainX; i <= iMaxTerrainX; ++i) {
        for (int j = iMinTerrainY; j <= iMaxTerrainY; ++j) {
            ...
            RtSceneBlockTerrain::STileAttr* pAttr =
                m_pTerrain->GetTerrainPathAttr(iTerrainX, iTerrainY);
            if (pAttr == NULL) {
                m_lastResult = PR_CANCEL;
...
bool RtsPathFind::RemoveActor(RtsPathPath*& pActor) {
    ...
    for (int i = pActor->CurMinX(); i <= pActor->CurMaxX(); ++i) {
        for (int j = pActor->CurMinY(); j <= pActor->CurMaxY(); ++j) {
            ...
            if (pAttr == NULL) {
                m_lastResult = PR_CANCEL;
                if (pActor->CurMinX() == pActor->CurMaxX() &&
                    pActor->CurMinY() == pActor->CurMaxY())
                    return true;
                else
                    break;
            }
...
bool RtsPathFind::RemoveActor(RtsPathPos*& pActor) {
    ...
    for (int i = pActor->CurMinX(); i <= pActor->CurMaxX(); ++i) {
        for (int j = pActor->CurMinY(); j <= pActor->CurMaxY(); ++j) {
            RtSceneBlockTerrain::STileAttr* pAttr =
                m_pTerrain->GetTerrainPathAttr(pActor->CurMinX(), pActor->CurMinY());
            if (pAttr == NULL) {
                m_lastResult = PR_CANCEL;
...
bool RtsPathFind::SmoothPath(RtsPathPath* pNode, bool vIgnoreActor)  //平滑路径并抛弃出发点
{
    ...
    if (First == Last || First + 1 == Last || First + 2 == Last) {  //在两个点以内，不需要做平滑
        //去掉出发点
    ...
    //    for(;Current+1<Last;)
    for (; Current + 1 != Last;) {
        if (!HaveBlockoffAround(NodePath[Check].MinX, NodePath[Check].MinY) &&
            WalkAble(pNode->dwActorId, pNode->sActorType, NodePath[Check].MinX,
                     NodePath[Check].MinY, NodePath[Current + 1].MinX, NodePath[Current + 1].MinY,
                     vIgnoreActor)) {
            Current++;
        } else {
            Check = Current;
    ...
    //tim.yang  automove
    if (pNode->m_iFirst < 0 || pNode->m_iLast < 0 || pNode->m_iFirst > PF_MAX_DEPTH ||
        pNode->m_iLast > PF_MAX_DEPTH)
        pNode->ClearPathEx();
#endif
    return _check_path(pNode);
}

bool RtsPathFind::WalkAble(DWORD dwActorId, short sActorType, int vStartX, int vStartY, int vEndX,
                           int vEndY, bool vIgnoreActor)  //不含出发点
{
    //将该直线光栅化//略有些不同
    ...
    } else  //不忽略人
    {
        ...
        {
            for (i = MinY; i <= MaxY; i++) {
                if (!CanAdd(dwActorId, sActorType, x1, i)) {  //不行
                    if (!(i == vStartY && x1 == vStartX))     //忽略起始点
                    {
                        return false;
                    }
...
bool RtsPathFind::MoveTo(RtsPathPos*& pActor, int iMinTerrainX, int iMinTerrainY, int iMaxTerrainX,
                         int iMaxTerrainY, char cBlock) {
    bool  bPath;
    DWORD dwActorId  = pActor->dwActorId;
    short sActorType = pActor->sActorType;
    bPath            = (pActor->cType & PT_PATH) != 0;
    if (CanAdd(dwActorId, sActorType, iMinTerrainX, iMinTerrainY, iMaxTerrainX, iMaxTerrainY,
               cBlock)) {
        if (!RemoveActor(pActor)) {
            return false;
        }
        pActor = AddActor(dwActorId, sActorType, iMinTerrainX, iMinTerrainY, iMaxTerrainX,
                          iMaxTerrainY, bPath, cBlock);
        return (pActor != NULL);
    }
    return false;
}

bool RtsPathFind::MoveTo(RtsPathPath*& pActor, int iMinTerrainX, int iMinTerrainY, int iMaxTerrainX,
                         int iMaxTerrainY, char cBlock) {
    bool  bPath;
    DWORD dwActorId  = pActor->dwActorId;
    short sActorType = pActor->sActorType;
    ...
    if (CanAdd(dwActorId, sActorType, iMinTerrainX, iMinTerrainY, iMaxTerrainX, iMaxTerrainY,
        ...
        pActor = (RtsPathPath*)AddActor(dwActorId, sActorType, iMinTerrainX, iMinTerrainY,
                                        iMaxTerrainX, iMaxTerrainY, bPath, cBlock);
        return (pActor != NULL);
    }
    return false;
}
...
int RtsPathFind::NextFindNode(char cBlock, int x, int y, SFindNode* pNode, int actorType) {
    ...
    while (pCur) {
        if (x == pCur->x && y == pCur->y)
            return 1;  //如果 (x,y) 在当前的路径中,直接返回
        pCur = pCur->pParent;
    }

    WORD wDepth = pNode->wDepth + 1;
    if (wDepth >= m_wMaxDepth)
        return 1;  // 如果当前的深度已经超过预先设置的深度
    ...
    pCur->wDepth  = wDepth;
    pCur->pParent = pNode;
    if (m_wCurMaxDepth < wDepth)
        m_wCurMaxDepth = wDepth;
    ...
    if (pParent)
        pParent->pNextBest = pCur;
    else
        m_pWeightRoot = pCur;
    return 0;
}

SFindNode* RtsPathFind::FindPathByTwoPos(char cBlock, SPointScene spSrc, SPointScene spTarget,
                                         bool bIgnoreActor, bool bIgnoreTarget, int actorType) {
    ...
    while (1) {
        ...
        if (m_bIgnoreTarget) {
            if (m_cSearchType == 0) {
                if (x == m_iCurTargetX) {
                    if (abs(y - m_iCurTargetY) == 1)
                        break;
                } else if (y == m_iCurTargetY) {
                    if (abs(x - m_iCurTargetX) == 1)
                        break;
                }
        ...
        if (x == m_iCurTargetX && y == m_iCurTargetY)
            break;  // 已经到达目的地
...
bool RtsPathFind::FindPathEx(RtsPathPath* pActor, int iTargetX, int iTargetY, bool bIgnoreActor,
                             bool bIgnoreTarget, int actorType) {
    if (!pActor)
        return false;

    const int startX = pActor->curPos.MinX;
    const int startY = pActor->curPos.MinY;
    ...
    if (1 == actorType) {
        ...
        if (Distance2(startX, startY, iTargetX, iTargetY)) {
            if (FindPathNormal(pActor, startX, startY, iTargetX, iTargetY, bIgnoreActor,
                               bIgnoreTarget, actorType))
                return true;
        } else {
            if (FindPathFromWP(pActor, startX, startY, iTargetX, iTargetY, bIgnoreActor,
                               bIgnoreTarget, actorType))
                return true;
        }
    } else {
        if (FindPathNormal(pActor, startX, startY, iTargetX, iTargetY, bIgnoreActor, bIgnoreTarget,
                           actorType))
            return true;
    }
    return false;
...
bool RtsPathFind::FindPathNormal(RtsPathPath* pActor, int startX, int startY, int endX, int endY,
                                 bool bIgnoreActor, bool bIgnoreTarget, int actorType,
                                 bool bAppend) {
    SPointScene start, end;
    //start.x = pActor->curPos.MinX;
    ...
    SFindNode* pCurNode =
        FindPathByTwoPos(pActor->cBlock, start, end, bIgnoreActor, bIgnoreTarget, actorType);
    if (!pCurNode) {
        return false;
    }
...
bool RtsPathFind::_check_path(RtsPathPath* pNode) {
    if (pNode->m_iLast >= PF_MAX_DEPTH || (pNode->m_iLast - pNode->m_iFirst) >= PF_MAX_DEPTH ||
...
bool RtsPathFind::FindSpace(DWORD dwActorId, short sActorType, int iTerrainX, int iTerrainY,
                            int& iSpaceX, int& iSpaceY, bool bOnlyOne) {
    int x, y, j;
    for (j = 0; j < CARRY_WIDTH; j++) {
        y = -j;
        for (x = -j; x <= j; x++) {
            if (CanAdd(dwActorId, sActorType, iTerrainX + x, iTerrainY + y, (char)PB_BLOCK,
                       bOnlyOne)) {
                iSpaceX = iTerrainX + x;
                iSpaceY = iTerrainY + y;
                return true;
...
void RtsPathFind::LoadWayPointInfo(const char* pstrfilename) {
    ...
    for (; list_it != wp_list.end(); ++list_it, ++i) {
        ...
        for (; vector_it != list_it->end(); ++vector_it) {
            const long distance = Distance(wp->x, wp->y, vector_it->x, vector_it->y);
...
bool RtsPathFind::DijkstraFindPath(long startwp, long endwp, std::vector<rtPOINT>& v) {
    ...
    while (1) {
        ...
        while (tempEdge != NULL) {
            ...
            if (point == -1)
                return false;
            if (m_wayPointInfo[point]->visited == false &&
...
long RtsPathFind::GetNearPathPoint(long sx, long sy) {
    ...
    long                                 temp;
    long                                 point = -1;
    for (auto it = m_wayPointInfo.begin(); it != m_wayPointInfo.end(); ++it) {
...
bool RtsPathFind::FindPathFromWP(RtsPathPath* pActor, int startX, int startY, int endX, int endY,
                                 bool bIgnoreActor, bool bIgnoreTarget, int actorType) {
    const long start = GetBestStartPathPoint(pActor, startX, startY, endX, endY);
    const long end   = GetNearPathPoint(endX, endY);
    ...
    if (DijkstraFindPath(start, end, pointlist)) {
		...
		*/
        ...
        //进行初始点到一个路点A*寻路
        FindPathNormal(pActor, startX, startY, m_wayPointInfo[start]->x, m_wayPointInfo[start]->y,
                       bIgnoreActor, bIgnoreTarget, actorType);
        ...
        //进行最后一个路点到目的A*寻路
        FindPathNormal(pActor, m_wayPointInfo[end]->x, m_wayPointInfo[end]->y, endX, endY,
                       bIgnoreActor, bIgnoreTarget, actorType, true);
...
bool RtsPathFind::HaveBlockoffAround(int iX, int iY) {
    ...
    if (pAttr == NULL || pAttr->bWalk)
        return true;  // 如果此处是障碍就直接返回
...
int RtsPathFind::GetNeartwoPathPoint(RtsPathPath* pActor, long sx, long sy, deque<long>& WPqueue) {
    ...
    for (; it != m_wayPointInfo.end(); ++it) {
        ...
        if (temp < dist) {
            ...
            if (dist <= MAX_WP_DISTANCE && WalkAble(pActor->dwActorId, pActor->cType, sx, sy,
                                                    (it->second)->x, (it->second)->y, true)) {
