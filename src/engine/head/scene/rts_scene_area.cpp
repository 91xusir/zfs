...
RtsAreaHelper* RtsSceneArea::AddArea(RtString& szName) {
    ...
    pNew->lMaxX         = 0;
    pNew->lMaxY         = 0;
    m_mapAreas.insert(std::unordered_multimap<std::string, RtsAreaHelper*>::value_type(
        std::string(szName), pNew));
    return pNew;
}
...
std::string RtsSceneArea::FindArea(int x, int y) {
    ...
    std::unordered_multimap<std::string, RtsAreaHelper*>::iterator it;
    for (it = m_mapAreas.begin(); it != m_mapAreas.end(); it++) {
        AreaHelper = it->second;
        if (x > AreaHelper->lMinX && x < AreaHelper->lMaxX && y > AreaHelper->lMinY &&
            y < AreaHelper->lMaxY) {
            return it->first;
        }
    }
