...
#define SHOWTICK()  {RtCoreLog().Info("line:%d tick:%d\n", __LINE__, GetTickCount()-dwCurrentTime);}
...
namespace ui 
{
	...
	#ifndef REAL_EQUAL
	#define REAL_EQUAL(a,b)     ( ((a) - (b) > -UI_REAL_PRECISION) && ((a) - (b) < UI_REAL_PRECISION) )/*是否相等*/
	#endif
	...
	{
		wfVisible			= 1 << 0,   // 是否可见
		wfGrabMouse			= 1 << 1,   // 是否相应鼠标事件(MouseDown,MouseUp,MouseClick,MouseEnter,MouseLeave,MouseWheel)
	...
	*/
	...
	//丢弃对象(对象使用完毕)
	#define DROP_RTUI_OBJECT(ptr)	{ if (ptr) { (ptr)->drop(); (ptr) = NULL; } }

	#define UI_ENSURE(clause)		{ if (!(clause))    return; }
	#define UI_ENSURE_B(clause)		{ if (!(clause))    return false; }
	#define UI_ENSURE_P(clause)		{ if (!(clause))    return NULL; }
	...
	struct RtwPixel
	{
		...
		{
			r = _r; g = _g; b = _b; a = _a;
			rBack = r; gBack = g; bBack = b; aBack = a;
		}
	...
	struct RtwRect	
	{
		...
		{
			return SPoint((left+right)>>1, (top+bottom)>>1);
		}
		...
		//某点是否在矩阵内
		bool IsContained(const SPoint& point) const
		{
		...
		bool IsContain(const SPoint& point) const
		{
		...
		void Inflate(const SSize& size)
		{
			Inflate(size.width, size.height, size.width, size.height);
		}
		...
		void Offset(const SSize& size)
		{
			Offset(size.width, size.height);
		}

		void Offset(const SPoint& point)
		{
	...
	{
		return (rc1.left == rc2.left && rc1.right == rc2.right && rc1.top == rc2.top && rc1.bottom == rc2.bottom);
	}
