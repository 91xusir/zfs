...
namespace ui 
{
	...
	public:
		...
		public:
			...
			//设置图片(前景图?)
			void SetImage(RtwImage* pImage=NULL, RtwImage *HightImage=NULL, RtwImage *PushedImage=NULL)
			{
		...
		public:
			SItem():
						  ...
						  SelectEnd(0), SelectBegin(0), strFontName(""), nFontSize(12),imgSize(SSize(0, 0)),pHoverImage(NULL), pPushedImage(NULL)
			{
	...
	public:
		...
		//纵向滚动条
		void SetScrollBarV(bool ScrollBarV)
		{
			m_bScrollBarV=ScrollBarV;
		}
	...
	public:
		...
		bool PickItem(SPoint& point, int& outLine, int& outCol, int& outItemIndex, bool& bDoubleByte, bool& outNear); // 从一个点来选取行、列、Item位置、是否双字节字符、靠近左右
