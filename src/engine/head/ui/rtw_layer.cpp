...
void CUiLayer::Render(CUiWidgetContainer* pExcludeWidgets/* = NULL*/)
{
	...
	if (m_pChildren)
	{
		for (UiWidgetList::reverse_iterator iterWidget = m_pChildren->m_Widgets.rbegin(); iterWidget != m_pChildren->m_Widgets.rend(); ++iterWidget)
		{
			RtwWidget* pWidget = *iterWidget;
...
bool CUiLayer::PickWidget(const SPoint& Position, RtwWidget** ppWidget/* = NULL*/, DWORD FlagMask/* = 0x00000000*/, CUiWidgetContainer* pExcludeWidgets/* = NULL*/)
{
	if (ppWidget)
	{
...
bool CUiLayer::FindWidget(const std::string& WidgetName, RtwWidget** ppWidget/* = NULL*/, bool bRecursive/* = false*/)
{
