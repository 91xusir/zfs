...
RtwTextBox::RtwTextBox()
					   :m_pScrollBarV(NULL),
						...
						m_CursorPosLine(0),
						m_CursorPosCol(0),
						m_CursorColor(RtwPixel(0xffccaaff)),
						m_CursorPosColBak(0),
						m_SelStartLine(0),
						m_SelStartCol(0),
						m_SelEndLine(0),
...
bool RtwTextBox::Create(DWORD Id)
{
	...
	m_pScrollBarV = (RtwVScrollBar*)g_workspace.getWidgetFactory()->createWidget(wtScrollBarV, "$Sbv");
		...
		)
		{
			...
			if (item.type == eit_Text || item.type == eit_Name||item.type==eit_Image)
			{
				...
				else if (item.hyperLink.childType == hlinkItem)
				{
					...
					{
						rt2_sprintf(szBuf, "item[%ld, %ld, %ld]meti", item.hyperLink.vecParam[0], item.hyperLink.vecParam[1], item.hyperLink.vecParam[2]);
						str += szBuf;
					}
				}
				else
			...
			case eit_Text:
				{
					...
					{
						pFont = g_workspace.getFontManager()->CreateFont(item.strFontName, item.nFontSize, RtwFont::FONT_BOLD);
					}
					else
					{
						pFont = g_workspace.getFontManager()->CreateFont("", 12, RtwFont::FONT_BOLD);
					}
...
// 					}
					...
					if(pCurrentImage)
					{
						...
						g_workspace.getRenderDevice()->DrawImage(pCurrentImage->getRenderImage(), rcRender, align, pClipRect);
					} 
				}
				break;
			case eit_ImageSequence:
				...
				if (item.getImageSequence() && item.getImageSequence()->getCurrentImage())
				{
					...
					item.getImageSequence()->getCurrentImage()->SetCtrlTransparent(m_Transparency);
					g_workspace.getRenderDevice()->DrawImage(item.getImageSequence()->getCurrentImage(), rcRender, align , &rcAfterClip);
				}
			default:
				break;
			}
	...
	if (g_workspace.getFocusWidget() == this && getEnableInput())
	{
		if ( (rtMilliseconds() / 500) & 1 )
		{
			if (m_CursorRect.top >= rcAfterClip.top && m_CursorRect.bottom <= rcAfterClip.bottom)
				g_workspace.getRenderDevice()->DrawRectangle(m_CursorRect, SETCOLORALPHA(m_CursorColor, m_Transparency));
		}
	}
}
...
//捡取item
bool RtwTextBox::PickItem(SPoint& point, int& outLine, int& outCol, int& outItemIndex, bool& bDoubleByte, bool& outNear)
{
	outLine = 0;
	std::list<SLine>::iterator iterLine = m_Lines.begin();
					...
					case eit_Text:
						{
							...
							RtwFont*  pkFont = g_workspace.getFontManager()->CreateFont(item.strFontName, item.nFontSize);

							if ( pkFont->PickCharacter(point, iIndex, bDoubleByte, bNear, item.text, item.rect))
							{
								outItemIndex	=  j;
								outCol			+= iIndex;
								outNear			=  bNear;
...
void RtwTextBox::OnMouseLDown_This(RtwWidget* pWidget, RtwEventDelegate* pEvent)
{
	...
	{
		//捡取item
		if (PickItem(SPoint(pEvent->mouse.x, pEvent->mouse.y), line, col, itemIndex, bDoubleByte, bNear))
		{
			// 光标
			m_CursorPosLine	= line;
			m_CursorPosCol	= col;
	...
	case 'V':
		if (g_workspace.isKeyPressed(vkCtrl))
		{
			if (g_workspace.getClipboard())
			{
				...
				{
					std::string  sText = getText( );
					if( m_Capacity>0 && m_Capacity <= (int)sText.size()+(int)strClipboard.size() )
					{
						return;
					}
	...
	case eit_Text:
		{
			...
			{
				SLine NewLine;
				NewLine.height = pLine->height; //g_workspace.getFontManager()->getDefaultFont()->getFontSize();
...
void RtwTextBox::_CalcItemSelectionRect(SItem* pItem)
{
	...
	{
		pItem->rcSelect.left = pItem->rect.left + g_workspace.getFontManager()->CreateFont(pItem->strFontName, pItem->nFontSize)->GetStringWidth(pItem->text, _SelBegin);
		if (_SelEnd >= 0)
			pItem->rcSelect.right = pItem->rect.left + g_workspace.getFontManager()->CreateFont(pItem->strFontName, pItem->nFontSize)->GetStringWidth(pItem->text, _SelEnd);
		else
			pItem->rcSelect.right = pItem->rect.left + g_workspace.getFontManager()->CreateFont(pItem->strFontName, pItem->nFontSize)->GetStringWidth(pItem->text);
...
void RtwTextBox::SetRichText(const std::string& richtext)
{
	...
	for (
				std::list<SLine>::iterator iterLine = m_pRichTextLoader->m_Lines.begin();
...
// 				}
		...
		case eit_ImageSequence:
			{
				int width = item.getImageSequence() ? item.getImageSequence()->getImageSize().width : 0;
				int height = item.getImageSequence() ? item.getImageSequence()->getImageSize().height : 0;
		...
		case eit_Text:
			{
				int width  = (int)item.text.length() * (m_FontSize>>1) ; //g_workspace.getFontManager()->GetStringWidth(item.text);
				int height = 0;
				if (imageheight == 0)
				{
				...
				if (item.rect.right >= m_ViewRect.right)
				{
					if (m_bWordWrap)
					{
						...
						if ( pkFont->PickCharacter(m_ViewRect.right - item.rect.left, PickIndex, bDoubleByte, bNear, item.text) )
						{
							...
							if (!pNextLine->lineitems.empty())
							{
								...
								if (_ItemFirst.type == eit_Text && NewItem.color == _ItemFirst.color)
								{
	...
	if (m_pScrollBarV && m_bScrollBarV)
	{
		...
		else 
		{
			...
			else 
			{
				...
				int _Value = _OrigValue; //(_OrigValue >= 0 && _OrigValue <= Delta) ? _OrigValue : 0;
				if (_Value > Delta)
					_Value = Delta;
				else if (_Value < 0)
					_Value = 0;
	...
	if (pNextLine)
	{
		...
		for (int iCut = ItemIndexFrom; iCut < ItemCount; ++iCut)
		{
			...
			if (!pNextLine->lineitems.empty())
			{
				// 与下一行的第一个Item合并
				SItem& _ItemFirst = pNextLine->lineitems.front();
				if (_ItemLast.type == eit_Text && _ItemFirst.type == eit_Text && _ItemLast.color == _ItemFirst.color)
				{
...
void RtwTextBox::RefreshSelectionRect()
{
	...
	foreach (std::list<SLine>, iterLine, m_Lines)
	{
		...
		{
			int _width1 = pItem->SelectBegin * (m_FontSize>>1);//g_workspace.getFontManager()->GetStringWidth(pItem->text, pItem->SelectBegin);
			int _width2 = pItem->SelectEnd   * (m_FontSize>>1);//g_workspace.getFontManager()->GetStringWidth(pItem->text, pItem->SelectEnd);
