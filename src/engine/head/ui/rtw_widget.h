...
namespace ui 
{
	...
	*/
	...
	public:
		...
		bool PickWidget(
							const SPoint& Position, RtwWidget** ppWidget = NULL, 
		...
		SSize getOffsetParentSize() { return m_pParent ? SSize(m_rcFrame.left - m_pParent->GetFrameRect().left, m_rcFrame.top - m_pParent->GetFrameRect().top) : SSize(0, 0); }
		bool getEnable();
...
public:
	...
	virtual void SetWidgetEffect(
										int nShowEffect = esNormal,
										int nHideEffect = esNormal,
										int nDelay = 300
								);
