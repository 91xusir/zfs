...
RtwForm::RtwForm()
    : m_pTitleBarImage(NULL), m_pBtn_Close(NULL), m_bMode(false), m_bEnableEscapeHide(false) {
    m_Flag |= wfDClick;
}
...
bool RtwForm::Create(DWORD Id) {
    ...
    m_pBtn_Close =
        dynamic_cast<RtwButton*>(g_workspace.getWidgetFactory()->createWidget(wtButton, "$Close"));
...
void RtwForm::DrawBackground(const RtwRect* pClipRect /* = NULL*/) {
    ...
    //else
    {
        ...
        if (pClipRect)
            bgRect.ClipBy(*pClipRect);
    }
    if (m_BkColor != 0) {
        g_workspace.getRenderDevice()->DrawRectangle(
            RtwRect(bgRect.left, bgRect.top + m_padding.top, bgRect.right, bgRect.bottom),
        ...
        if (m_pTitleBarImage) {
            ...
            if (pImage) {
                ...
                g_workspace.getRenderDevice()->DrawImage(
                    pImage, m_rcTitle, SUiTextAlignment(alignNear, alignNear), &bgRect);
            }
        }
