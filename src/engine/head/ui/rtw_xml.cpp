...
bool RtwXmlLoader::LoadFromFile(const std::string& FileName, RtwWidget* pParent,
    ...
    if (!_ProcessNode((RtsXmlDoc::NodePtr*)m_pXmlDoc->GetRoot(), pParent,
                      pWidgetsCreated)) {  //解析出错
...
//从字符串解析xml
bool RtwXmlLoader::LoadFromString(const std::string& strXml, RtwWidget* pParent /* = NULL*/,
...
bool RtwXmlLoader::_ProcessNode(RtsXmlDoc::NodePtr* pNode, RtwWidget* pParent /* = NULL*/,
                                std::list<DWORD>* pWidgetsCreated /* = NULL*/
) {
    if (!pNode)
        return false;
...
bool RtwXmlLoader::_ProcessNode_AddWidget(RtwWidget* pParent, CUiLayer* pLayer,
                                          RtwWidget* pWidget) {
    UI_ENSURE_B(pWidget);
...
bool RtwXmlLoader::_ProcessAttrib_Widget(RtwWidget* pWidget, RtsXmlDoc::NodePtr* pNode) {
    ...
    if (!strTemplate.empty()) {
        CUiTemplate* pTempl =
            g_workspace.getThemeManager()->getCurrentTheme()->FindTemplate(strTemplate);
        if (pTempl)
    ...
    {
        if (strActive == "true")
            pWidget->ModifyFlags(wfFocus, 0);
        else if (strActive == "false")
            pWidget->ModifyFlags(0, wfFocus);
    }

    if (!strMouseEvent.empty())  //鼠标事件可用否
    {
        if (strMouseEvent == "true")
            pWidget->ModifyFlags(wfGrabMouse, 0);
        else if (strMouseEvent == "false")
            pWidget->ModifyFlags(0, wfGrabMouse);
    }

    if (!strMouseMove.empty())  //鼠标移动
    {
        if (strMouseMove == "true")
            pWidget->ModifyFlags(wfMouseMove, 0);
        else if (strMouseMove == "false")
            pWidget->ModifyFlags(0, wfMouseMove);
    }
    ...
    if (!strName.empty())
        pWidget->SetName(strName);  //控件名

    if (!strText.empty())
        pWidget->SetText(strText);  //显示文字

    RtwRect rcLocation;
    rcLocation.FromString(strLocation);  //控件显示矩形
    if (!strLocation.empty()) {          //控件位置
        pWidget->Move(rcLocation);
        pWidget->SetWidgetSize(
            SSize(rcLocation.right - rcLocation.left, rcLocation.bottom - rcLocation.top));
    }
    ...
    if (!strDragOut.empty()) {
        if (strDragOut == "true")
            pWidget->ModifyFlags(wfDragOut, 0);
        else if (strDragOut == "false")
            pWidget->ModifyFlags(0, wfDragOut);
    }

    //可拉入(拖拽用)
    if (!strDragIn.empty()) {
        if (strDragIn == "true")
            pWidget->ModifyFlags(wfDragIn, 0);
        else if (strDragIn == "false")
            pWidget->ModifyFlags(0, wfDragIn);
    }
...
bool RtwXmlLoader::_ProcessAttrib_Form(RtwForm* pWidget, RtsXmlDoc::NodePtr* pNode) {
    ...
    if (!strAutoScrollH.empty()) {
        if (strAutoScrollH == "true")
            pWidget->SetAutoScrollH(true);
        else if (strAutoScrollH == "false")
            pWidget->SetAutoScrollH(false);
    }

    //esc控制隐藏窗体
    std::string strEscapeHide = pNode->GetProp_s("EscapeHide");
    if (!strEscapeHide.empty()) {
        if (strEscapeHide == "true")
            pWidget->SetEscapeHide(true);
        else if (strEscapeHide == "false")
            pWidget->SetEscapeHide(false);
    }

    //边框收缩?
    std::string strBorderpadding = pNode->GetProp_s("Borderpadding");
    if (!strBorderpadding.empty()) {
        if (strBorderpadding == "true")
            pWidget->SetBorderpadding(true);
        else if (strBorderpadding == "false")
            pWidget->SetBorderpadding(false);
    }
    return true;
}
...
//聊天输入框
bool RtwXmlLoader::_ProcessAttrib_ChatInputBox(RtwChatInputBox*    pWidget,
...
bool RtwXmlLoader::_ProcessChild_Widget(RtwWidget* pWidget, CUiLayer* pLayer,
                                        RtsXmlDoc::NodePtr* pNode,
                                        std::list<DWORD>*   pWidgetsCreated) {
...
bool RtwXmlLoader::_ProcessChild_Layer(CUiLayer* pWidget, CUiLayer* pLayer,
                                       RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_Form(RtwForm* pWidget, CUiLayer* pLayer,
                                      RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_Button(RtwButton* pWidget, CUiLayer* pLayer,
                                        RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_Label(RtwLabel* pWidget, CUiLayer* pLayer,
                                       RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_EditBox(RtwTextBox* pWidget, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_ChatInputBox(RtwChatInputBox* pWidget, CUiLayer* pLayer,
                                              RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_HtmlView(RtwHtmlView* pWidget, CUiLayer* pLayer,
                                          RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_ScrollBarV(RtwVScrollBar* pWidget, CUiLayer* pLayer,
                                            RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_ScrollBarH(RtwHScrollBar* pWidget, CUiLayer* pLayer,
                                            RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_ChatBox(RtwChatBox* pWidget, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
    return true;
}

bool RtwXmlLoader::_ProcessChild_3dView(Rtw3DView* pWidget, CUiLayer* pLayer,
                                        RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
    return true;
}

bool RtwXmlLoader::_ProcessChild_ListBox(RtwListBox* pWidget, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
    RtsXmlDoc::NodePtr* pChildNode = pNode->pChildren;
    while (pChildNode) {
...
bool RtwXmlLoader::_ProcessChild_ProgressBar(RtwProgressBar* pWidget, CUiLayer* pLayer,
                                             RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_ComboBox(RtwComboBox* pWidget, CUiLayer* pLayer,
                                          RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode))
...
bool RtwXmlLoader::_ProcessChild_TabCtrl(RtwTab* pWidget, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_CheckButton(CUiCheckButton* pWidget, CUiLayer* pLayer,
                                             RtsXmlDoc::NodePtr* pNode) {
...
bool RtwXmlLoader::_ProcessChild_Tree(RtwTree* pWidget, CUiLayer* pLayer,
                                      RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
    RtsXmlDoc::NodePtr* pChildNode = pNode->pChildren;
    while (pChildNode) {
...
bool RtwXmlLoader::_ProcessChild_PopupMenu(RtwPopupMenu* pWidget, CUiLayer* pLayer,
                                           RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
bool RtwXmlLoader::_ProcessChild_MediaFrame(CUiMediaFrame* pWidget, CUiLayer* pLayer,
                                            RtsXmlDoc::NodePtr* pNode) {
    UI_ENSURE_B(_ProcessChild_Widget(pWidget, pLayer, pNode));
...
//从模板中处理继承等元素
bool RtwXmlLoader::_Process_Template(RtsXmlDoc::NodePtr* pNode, RtwWidget* pWidget,
                                     CUiTemplate* pTempl) {
    if (pTempl) {
...
RtwWidget* RtwXmlLoader::_Process_Widget(RtwWidget* pParent, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
...
RtwForm* RtwXmlLoader::_Process_Form(RtwWidget* pParent, CUiLayer* pLayer,
                                     RtsXmlDoc::NodePtr* pNode) {
...
RtwButton* RtwXmlLoader::_Process_Button(RtwWidget* pParent, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
...
RtwAliasButton* RtwXmlLoader::_Process_AliasButton(RtwWidget* pParent, CUiLayer* pLayer,
                                                   RtsXmlDoc::NodePtr* pNode) {
    RtwAliasButton* pWidget =
        (RtwAliasButton*)g_workspace.getWidgetFactory()->createWidget(wtAliasButton);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
RtwLabel* RtwXmlLoader::_Process_Label(RtwWidget* pParent, CUiLayer* pLayer,
                                       RtsXmlDoc::NodePtr* pNode) {
...
RtwTextBox* RtwXmlLoader::_Process_EditBox(RtwWidget* pParent, CUiLayer* pLayer,
                                           RtsXmlDoc::NodePtr* pNode) {
...
RtwChatInputBox* RtwXmlLoader::_Process_ChatInputBox(RtwWidget* pParent, CUiLayer* pLayer,
                                                     RtsXmlDoc::NodePtr* pNode) {
    RtwChatInputBox* pWidget =
        (RtwChatInputBox*)g_workspace.getWidgetFactory()->createWidget(wtChatInputBox);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
    // 属性
...
RtwHtmlView* RtwXmlLoader::_Process_HtmlView(RtwWidget* pParent, CUiLayer* pLayer,
                                             RtsXmlDoc::NodePtr* pNode) {
...
RtwVScrollBar* RtwXmlLoader::_Process_ScrollBarV(RtwWidget* pParent, CUiLayer* pLayer,
                                                 RtsXmlDoc::NodePtr* pNode) {
    RtwVScrollBar* pWidget =
        (RtwVScrollBar*)g_workspace.getWidgetFactory()->createWidget(wtScrollBarV);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
RtwHScrollBar* RtwXmlLoader::_Process_ScrollBarH(RtwWidget* pParent, CUiLayer* pLayer,
                                                 RtsXmlDoc::NodePtr* pNode) {
    RtwHScrollBar* pWidget =
        (RtwHScrollBar*)g_workspace.getWidgetFactory()->createWidget(wtScrollBarH);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
RtwListBox* RtwXmlLoader::_Process_ListBox(RtwWidget* pParent, CUiLayer* pLayer,
                                           RtsXmlDoc::NodePtr* pNode) {
...
Rtw3DView* RtwXmlLoader::_Process_3dView(RtwWidget* pParent, CUiLayer* pLayer,
                                         RtsXmlDoc::NodePtr* pNode) {
...
RtwChatBox* RtwXmlLoader::_Process_ChatBox(RtwWidget* pParent, CUiLayer* pLayer,
                                           RtsXmlDoc::NodePtr* pNode) {
...
RtwProgressBar* RtwXmlLoader::_Process_ProgressBar(RtwWidget* pParent, CUiLayer* pLayer,
                                                   RtsXmlDoc::NodePtr* pNode) {
    RtwProgressBar* pWidget =
        (RtwProgressBar*)g_workspace.getWidgetFactory()->createWidget(wtProgressBar);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
RtwComboBox* RtwXmlLoader::_Process_ComboBox(RtwWidget* pParent, CUiLayer* pLayer,
                                             RtsXmlDoc::NodePtr* pNode) {
...
RtwTab* RtwXmlLoader::_Process_TabCtrl(RtwWidget* pParent, CUiLayer* pLayer,
                                       RtsXmlDoc::NodePtr* pNode) {
...
CUiCheckButton* RtwXmlLoader::_Process_CheckButton(RtwWidget* pParent, CUiLayer* pLayer,
                                                   RtsXmlDoc::NodePtr* pNode) {
    CUiCheckButton* pWidget =
        (CUiCheckButton*)g_workspace.getWidgetFactory()->createWidget(wtCheckButton);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
RtwTree* RtwXmlLoader::_Process_Tree(RtwWidget* pParent, CUiLayer* pLayer,
                                     RtsXmlDoc::NodePtr* pNode) {
...
RtwPopupMenu* RtwXmlLoader::_Process_PopupMenu(RtwWidget* pParent, CUiLayer* pLayer,
                                               RtsXmlDoc::NodePtr* pNode) {
    RtwPopupMenu* pWidget =
        (RtwPopupMenu*)g_workspace.getWidgetFactory()->createWidget(wtPopupMenu);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
CUiMediaFrame* RtwXmlLoader::_Process_MediaFrame(RtwWidget* pParent, CUiLayer* pLayer,
                                                 RtsXmlDoc::NodePtr* pNode) {
    CUiMediaFrame* pWidget =
        (CUiMediaFrame*)g_workspace.getWidgetFactory()->createWidget(wtMediaFrame);
    UI_ENSURE_P(_ProcessNode_AddWidget(pParent, pLayer, pWidget));
...
bool RtwXmlLoader::_Process_CheckHighlightImage(CUiCheckButton*     pWidget,
                                                RtsXmlDoc::NodePtr* pNode) {
    RtwComplexImage* pImage = _Process_ComplexImage(pNode);
...
bool RtwXmlLoader::_MergeRtXmlNode(RtsXmlDoc::NodePtr* pNodeTo, RtsXmlDoc::NodePtr* pNodeFrom) {
    ...
    for (; iterAttrib != pNodeFrom->mapAttribute.end(); ++iterAttrib) {
        ...
        std::map<std::string, std::string>::iterator iterAttFind =
            pNodeTo->mapAttribute.find(AttName);
