...
RtwImage::RtwImage()
    : m_bBlend(false), m_Transparency(1.0), m_bHighLight(false), m_FillMode(ImageFillMode_Stretch),
      border(NULL) {
    //顺便将后备透明度设置一下
    SetTransparency(1.0);
}
...
void RtwImage::SetTransparency(float transparen) {
    ...
    if (m_Transparency == 0.0) {
        ...
        m_Color.a = static_cast<int>(255 * transparen);
    }
...
void CUiImageRegister::registerImage(RtwImage* vpImage) {
    if (vpImage) {
        EXT_SPACE::unordered_map<RtwImage*, DWORD>::iterator iterFinder =
            m_LiveImages.find(vpImage);
        UI_CHECK(iterFinder == m_LiveImages.end());
