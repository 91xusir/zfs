...
RtwWidget::RtwWidget()
						:mnRenderTargetIndex(INVALIDE_RENDERTARGET),
						mbClearRenderTaget(false),
						mlBackBufferDiffuse(0x000000),
						m_bIsUseSystemFont(false),
						mbNoClipRect(true),
						mbOnlyRenderToShow(false),
						m_FontSize(12),
						...
						m_MouseLeave(false),
						m_pBackGround(NULL),
						m_pImageSequence(NULL),
						m_Borderpadding(true),
...
// 		if(m_pBackGround->border) m_pBackGround->border->drop();
		if(m_pBackGround ) DROP_TRUI_IMAGE(m_pBackGround);
	}
...
bool RtwWidget::PickWidget(const SPoint& Position, RtwWidget** ppWidget, DWORD FlagMask, CUiWidgetContainer* pExcludeWidgets)
{
	...
	if (getVisible() && ((GetFlags() & FlagMask) == FlagMask) && m_rcFrame.IsContained(Position) )
	{
		...
		if ( 
				(
					*ppWidget)->getWidgetType() != wtLayer 
...
//根据id查找子组件
bool RtwWidget::FindChild(DWORD WidgetID, RtwWidget** ppWidget/* = NULL*/, bool bRecursive/* = false*/)
{
	if (m_pChildren)
...
//根据组件名查找子组件
bool RtwWidget::FindChild(const std::string& WidgetName, RtwWidget** ppWidget/* = NULL*/, bool bRecursive/* = false*/)
{
	if (m_pChildren)
...
//移动函数(根据给出的大小移动位置)
//being
void RtwWidget::Offset(const SSize& size, bool bTriggerEvent/* = true*/, bool bAutoScrollParent/* = true*/)
{
	//当成x,y处理
...
void RtwWidget::Move(const SPoint& pos, bool bTriggerEvent/* = true*/, bool bAutoScrollParent/* = true*/)
{
	SSize OffsetSize(pos.x-m_rcFrame.left, pos.y-m_rcFrame.top);
	Offset(OffsetSize);
}

void RtwWidget::Move(const RtwRect& rect, bool bAbs/* = false*/, bool bTriggerEvent/* = true*/, bool bAutoScrollParent/* = true*/)
{
	RtwRect rcOrig = m_rcFrame;
	// 如果大小未发生改变，就调用Offset
	if (
			rect.getWidth() == m_rcFrame.getWidth()
		&& rect.getHeight() == m_rcFrame.getHeight()
	)
	{
...
void RtwWidget::AutoHintSize()
{
	...
	if (m_pHint)
	{
		...
		rc.bottom	= nLineCount * g_workspace.getFontManager()->getFontSize() + RtwTextBox::m_LineSpace * (nLineCount + 1) - 1;
		m_pHint->Move(rc);
	}
}
...
void RtwWidget::Render(const RtwRect* pClipRect/* = NULL*/, bool bAlwaysRender/* = false*/)
{
	...
	{
		g_workspace.getRenderDevice()->SetTextureRenderTarget( mnRenderTargetIndex , mbClearRenderTaget , mlBackBufferDiffuse );
...
{
	RtCoreLog().Info("%s[%s] %s(0x%x)\n", prefix.c_str(), g_WidgetTypeShortName[m_WidgetType].c_str(), getName().c_str(), GetFlags());
...
void RtwWidget::DrawForeground(const RtwRect* pClipRect/* = NULL*/)
{
	...
	{	
		if(m_UltraText1.FontSize>0)
			...
			g_workspace.getFontManager()->DrawString(
															m_UltraText1.Text, UltraRect, SETCOLORALPHA(m_UltraText1.TextColor, m_Transparency), 
															g_workspace.getFontManager()->getDefaultFont(),true, m_UltraText1.Align, false, pClipRect
													);
	}
	...
	if (!m_UltraText2.Text.empty())
	{
		if(m_UltraText2.FontSize>0)
			...
			g_workspace.getFontManager()->DrawString(
															m_UltraText2.Text, UltraRect, SETCOLORALPHA(m_UltraText2.TextColor, m_Transparency),
															g_workspace.getFontManager()->getDefaultFont(),true, m_UltraText2.Align, false, pClipRect
													);
	}
	//end
	...
	if (m_pImageSequence)
	{
		...
		if (pImage)
			g_workspace.getRenderDevice()->DrawImage(pImage->getRenderImage(), m_rcFrame, SUiTextAlignment(alignNear, alignNear), pClipRect);
	}
	...
	if (!REAL_EQUAL(m_MaskValue, 0) && m_MaskValue > 0)
	{
		...
		if (m_MaskValue > 0 && m_MaskValue <= _Factor)
		{
			...
			g_workspace.getRenderDevice()->DrawTriangle(p1, p3, p2, SETCOLORALPHA(m_MaskColor, m_Transparency));
		}
		else 
		{
			p2.x = p1.x;
			...
			if (m_MaskValue <= 0.5 - _Factor)
			{
				...
				if (m_MaskValue <= 0.25)
				{
					p3.y = p1.y - (Real)tan((Real)(2 * UI_PI * (0.25 - m_MaskValue))) * (Width >>1);
				}
				else 
				{
					p3.y = p1.y + (Real)tan((Real)(2 * UI_PI * (m_MaskValue - 0.25))) * (Width >>1);
				}
			...
			else 
			{
				...
				if (m_MaskValue <= 0.5 + _Factor)
				{
					...
					if (m_MaskValue <= 0.5)
					{
						p3.x = p1.x + (Real)tan((Real)(2 * UI_PI * (0.5 - m_MaskValue))) * (Height >>1);
					}
					else 
					{
						p3.x = p1.x - (Real)tan((Real)(2 * UI_PI * (m_MaskValue - 0.5))) * (Height >>1);
					}
				...
				else 
				{
					...
					if (m_MaskValue <= 1 - _Factor)
					{
						...
						if (m_MaskValue <= 0.75)
						{
							p3.y = p1.y + (Real)tan((Real)(2 * UI_PI * (0.75 - m_MaskValue))) * (Width >>1);
						}
						else 
						{
							p3.y = p1.y - (Real)tan((Real)(2 * UI_PI * (m_MaskValue - 0.75))) * (Width >>1);
						}
					...
					else 
					{
						...
						if (m_MaskValue <= 1)
						{
							...
							p2.y = m_rcFrame.top;
							p3.y = m_rcFrame.top;
							p3.x = p1.x - (Real)tan((Real)(2 * UI_PI * (1 - m_MaskValue))) * (Height >>1);
...
void RtwWidget::DrawBackground(const RtwRect* pClipRect/* = NULL*/)
{
	...
	{
		g_workspace.getRenderDevice()->DrawRectangle(rcRender, SETCOLORALPHA(m_BkColor, m_Transparency));
	}

	if (g_workspace.getDebugMode())
	{
		g_workspace.getRenderDevice()->DrawRectangleBorder(m_rcFrame, SETCOLORALPHA(RtwPixel(0xffef4fff), m_Transparency));
	}
	...
	if (m_pBackGround)
	{
		...
		if (pBackgroundImg)
		{
			...
			g_workspace.getRenderDevice()->DrawImage(pBackgroundImg, rect, SUiTextAlignment(alignNear, alignNear), &rcRender);
			if( mbHighLight)
			{
		...
		if(m_pBackGround->border && m_pBackGround->border->getRenderImage())
		{
			...
			if (pClipRect)
			{
				if (m_Borderpadding)
				{
					g_workspace.getRenderDevice()->DrawBorderImage(m_pBackGround->border,RtwRect(
																										m_rcFrame.left,m_rcFrame.top+m_padding.top,m_rcFrame.right,
																										m_rcFrame.bottom),m_pBackGround->borderSize,
...
//鼠标滑过时的图片
void RtwWidget::SetHoverBoderImage(const string &strBoderFile, RtwRect srcRc, SMargin BorderMargin)
{
...
void RtwWidget::OnHint()
{
	...
	{
		//默认hint
		if(m_pHint->getWidgetType() == wtEditBox && !strcmp(m_HintText.substr(0,6).c_str(),"<html>"))
		{
			//html
		...
		else if(m_pHint->getWidgetType() == wtEditBox && !strcmp(m_HintText.substr(0,5).c_str(),"<xml>"))
		{
			//xml
			((RtwTextBox*)m_pHint)->SetAutoSizeH(false);
			((RtwTextBox*)m_pHint)->SetAutoSizeV(true);
...
void RtwWidget::OnMove_AnchorWidget(RtwWidget* pWidget, RtwEventDelegate* pEvent)
{
	...
	foreach (std::list<SUiAnchor>, iterAnchor, m_Anchors)
	{
		...
		RtwRect rcAnchor = pRelativeWidget ? pRelativeWidget->GetFrameRect() : g_workspace.getViewportRect();
		if (bNeedDrop)
...
void RtwWidget::AddAnchor(SUiAnchor& Anchor, RtwWidget* pRelativeWidget)
{
	...
	if (pRelativeWidget)
	{
		...
		_RegisterEvent(pRelativeWidget, &(pRelativeWidget->EvMove), RTW_CALLBACK(this, RtwWidget, OnMove_AnchorWidget));
		_RegisterEvent(pRelativeWidget, &(pRelativeWidget->EvOffset), RTW_CALLBACK(this, RtwWidget, OnMove_AnchorWidget));
	}
	else 
	{
...
//事件注册
void RtwWidget::_RegisterEvent(RtwWidget* pWidget, RtwEventDispatcher* _pDispatcher, CUiDelegate& _Delegate)
{
	...
	for (; iterRE != m_RegisteredEvents.end(); ++iterRE)
	{
		...
		if (re.WidgetId == WidgetId && re.pDispatcher == _pDispatcher && re.Delegate_.IsEqual(_Delegate))
			return;
	}
...
RtwRect RtwWidget::CalcChildrenRect()
{
	...
	for (UiWidgetList::iterator iter = m_pChildren->m_Widgets.begin(); iter != m_pChildren->m_Widgets.end(); ++iter)
	{
		RtwWidget* pWidget = *iter;
...
void RtwWidget::SetAutoScrollV(bool bAutoScrollV)
{
	...
	if (m_bAutoScrollV)
	{
		if (!m_pAutoScrollV)
		{
			...
			m_pAutoScrollV = (RtwVScrollBar*)g_workspace.getWidgetFactory()->createWidget(wtScrollBarV, "$Sbv");
...
void RtwWidget::SetAutoScrollH(bool bAutoScrollH)
{
	...
	if (m_bAutoScrollH)
	{
		if (!m_pAutoScrollH)
		{
			...
			m_pAutoScrollH = (RtwHScrollBar*)g_workspace.getWidgetFactory()->createWidget(wtScrollBarH, "$Sbh");
...
void RtwWidget::SetUltraText1(
								const std::string& text/* = ""*/, SUiTextAlignment& align/* = SUiTextAlignment(alignFar, alignFar)*/,
...
void RtwWidget::SetUltraText2(
								const std::string& text/* = ""*/, SUiTextAlignment& align/* = SUiTextAlignment(alignNear, alignNear)*/,
								int fontSize/* = 0*/, RtwPixel textColor/* = RtwPixel(0xffffffff)*/
							 )
{
...
CUiWidgetFactory::~CUiWidgetFactory()
{
	...
	if (!m_LiveWidgets.empty())
	{
		...
		rt2_sprintf(strOutput, "[CUiWidgetFactory::~CUiWidgetFactory] 一些窗口部件没有被销毁(共%d个): \n", (int)m_LiveWidgets.size());
		int counter = 10;
		for (EXT_SPACE::unordered_map<DWORD, RtwWidget*>::iterator iterWidget = m_LiveWidgets.begin(); iterWidget != m_LiveWidgets.end(); ++iterWidget)
		{
...
RtwWidget* CUiWidgetFactory::createWidget(EWidgetType type, const std::string& name/* = ""*/)
{
	...
	if (pWidget)
	{
		EXT_SPACE::unordered_map<DWORD, RtwWidget*>::iterator iterFinder = m_LiveWidgets.find(pWidget->getId());
...
{
	for (UiWidgetList::iterator iter = pWidgetContainer->m_Widgets.begin(); iter != pWidgetContainer->m_Widgets.end(); ++iter)
	{
		RtwWidget* pWidget = *iter;
		if (pWidget)
		{
...
//在容器里根据id 查找组件
bool CUiWidgetContainer::FindWidget(DWORD WidgetID, RtwWidget** ppWidget/* = NULL*/, bool bRecursive/* = false*/)
{
	if (ppWidget)
		*ppWidget = NULL;

	for (UiWidgetList::iterator iterFinder = m_Widgets.begin(); iterFinder != m_Widgets.end(); ++iterFinder)
	{
...
//查找组件,根据名字
bool CUiWidgetContainer::FindWidget(const std::string& WidgetName, RtwWidget** ppWidget/* = NULL*/, bool bRecursive/* = false*/)
{
	if (ppWidget)
		*ppWidget = NULL;
...
void CUiWidgetContainer::Run(DWORD dwDelta)
{
	...
	{
		for (UiWidgetList::iterator iterWidget = m_Widgets.begin(); iterWidget != m_Widgets.end(); ++iterWidget)
		{
			RtwWidget* pWidget = *iterWidget;
			if (pWidget)
			{
...
void CUiWidgetContainer::Render(CUiWidgetContainer* pExcludeWidgets/* = NULL*/)
{
	if ((int)m_Widgets.size() > 0)
	{
		for (UiWidgetList::reverse_iterator iterWidget = m_Widgets.rbegin(); iterWidget != m_Widgets.rend(); ++iterWidget)
		{
			RtwWidget* pWidget = *iterWidget;
			if (pExcludeWidgets && pExcludeWidgets->FindWidget(pWidget->getId()))
				continue;
...
//打印组件结构
void CUiWidgetContainer::PrintToConsole(const std::string& name, const std::string& prefix/* = ""*/)
{
...
//捡取
bool CUiWidgetContainer::PickWidget(
											const SPoint& Position, RtwWidget** ppWidget/* = NULL*/, DWORD FlagMask/* = 0x00000000*/,
