...
RtwChatBox::RtwChatBox()
    : m_curItem(NULL), m_curPiece(NULL), m_position(0), m_range(0), m_ClearSystime(0),
      m_currentTime(0), m_Scrollnum(0) {
    ...
    if (((int)m_Lines.size() - 1) < 3) {
        for (int i = 0; i < (int)m_Lines.size(); ++i) {
            ...
            foreach (std::list<SItem>, iterItem, plastLine->lineitems) {
                ...
                iterItem->rect.left--;
                iterItem->rect.right--;
                if (plastLine->lineitems.size() == count &&
                    iterItem->rect.right <= m_rcFrame.left) {
...
bool RtwChatBox::InsertChatMessage(const std::string& channel, const std::string& sendname,
                                   const std::string& text, const RtwPixel color, int indexBefore,
                                   bool bRefresh, void* param) {
    return true;
}

//lmk 聊天窗口消息
void RtwChatBox::Add(int mask, DWORD color, const char* description, char* fromname, char* message,
                     long vFromRegionID, bool ischange) {
    ...
    while (nPos < strMessage.size()) {
        ...
        if (strMessage[nPos] == '^' && strMessage[nPos + 1] == '-' && strMessage[nPos + 2] == 'f' &&
            ...
            if (pLastLine->rect.getWidth() + (int)strTempMsg.size() * 6 + 12 > nLineWidth &&
                ischange) {
                SItem itemText;
