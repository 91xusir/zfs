...
#define UI_DELEGATE_M(receiver, clazz, handle)                                                     \
    (CUiDelegate((IUiEventReceiver*)static_cast<void*>(receiver),                                  \
                 (CUiDelegate::Thiscall)(int(clazz::*)(void*, void*)) & clazz::handle))
...
#define RTW_CALLBACK_1(receiver, clazz, handle, param1)                                            \
    (CUiDelegate((IUiEventReceiver*)static_cast<void*>(receiver),                                  \
                 (CUiDelegate::Thiscall)(int(clazz::*)(void*, void*)) & clazz::handle, param1))
...
class CUiDelegateF {
   ...
   public:
    int Signal(const IUiEventReceiver* receiver, const void* sender, const void* arg) const {
        return (const_cast<CUiDelegateF*>(this)->*m_caller)((IUiEventReceiver*)receiver,
    ...
    bool IsNull() const { return m_caller == NULL; }

    bool IsEqual(CUiDelegateF& Other) const { return m_caller == Other.m_caller; }
   ...
   protected:
    CUiDelegateF(Thiscall handle, Caller caller) : m_handle(handle), m_caller(caller) {}

   public:
    CUiDelegateF() : m_handle(NULL), m_caller(NULL) {}

    CUiDelegateF(const CUiDelegateF& copy) : m_handle(copy.m_handle), m_caller(copy.m_caller) {}

    CUiDelegateF(Thiscall handle)
        : m_thiscall(handle), m_caller(&ui::CUiDelegateF::DelegateThiscall) {}

    CUiDelegateF(Fastcall handle)
        : m_fastcall(handle), m_caller(&ui::CUiDelegateF::DelegateFastcall) {}

    CUiDelegateF(Cdecl handle) : m_cdecl(handle), m_caller(&ui::CUiDelegateF::DelegateCdecl) {}

    CUiDelegateF(CdeclF handle) : m_cdeclf(handle), m_caller(&ui::CUiDelegateF::DelegateCdeclF) {}
};
...
class CUiDelegateM : public CUiDelegateF {
   ...
   public:
    CUiDelegateM() : m_receiver(NULL), m_param(NULL) {}

    template <class Q>
    CUiDelegateM(IUiEventReceiver* receiver, Q handle, void* param = NULL)
        : CUiDelegateF(handle), m_receiver(receiver), m_param(param) {}

    CUiDelegateM(CdeclF handle) : CUiDelegateF(handle), m_receiver(NULL), m_param(NULL) {}

    CUiDelegateM(const CUiDelegateF& copy) : CUiDelegateF(copy), m_receiver(NULL), m_param(NULL) {}

    CUiDelegateM(const CUiDelegateM& copy)
        : CUiDelegateF((const CUiDelegateF&)copy), m_receiver(copy.m_receiver),
