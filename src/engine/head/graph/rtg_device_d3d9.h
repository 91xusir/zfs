...
enum { Max_Stream_Num = 8, Bloom_Kernel_Size = 13 };
...
class RtgDeviceD3D9 : public RtgDevice, public CD3DApplication {
   ...
   public:
    ...
    virtual unsigned long GetElapsedMS() const { return m_dwElapsedMS; }

    virtual float GetElapsedTime() { return m_fElapsedTime; }

    virtual float GetAppTime() { return m_fTime; }
   ...
   public:
    ...
    virtual BOOL SetStream_vb(UINT _StreamIndex, IDirect3DVertexBuffer9* _vb, UINT _Stride,
                              UINT _Offset = 0, UINT _vNum = 0, DWORD _FVF = 0) {
        ...
        if (FAILED(m_pd3dDevice->SetStreamSource(_StreamIndex, _vb, _Offset, _Stride)))
            return FALSE;
        return TRUE;
    }

    virtual BOOL SetStream_ib(IDirect3DIndexBuffer9* ib, UINT _Offset = 0) {
    ...
    virtual BOOL DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinIndex,
        ...
        ++RtGetPref()->Draw_Call;
        if (FAILED(m_pd3dDevice->DrawIndexedPrimitive(Type, BaseVertexIndex, MinIndex, NumVertices,
    ...
    virtual BOOL DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex,
        ...
        RtGetPref()->Proc_FaceNum += PrimitiveCount;
        ++RtGetPref()->Draw_Call;
        if (FAILED(m_pd3dDevice->DrawPrimitive(PrimitiveType, StartVertex, PrimitiveCount)))
            return FALSE;
        return TRUE;
    }
    ...
    virtual BOOL DrawIndexedPrimitiveUP(D3DPRIMITIVETYPE Type, UINT MinVertexIndex,
                                        ...
                                        UINT _vStride, UINT _PrimitiveCount) {
        RtGetPref()->Proc_FaceNum += _PrimitiveCount;
        ++RtGetPref()->Draw_Call;
        if (FAILED(m_pd3dDevice->DrawIndexedPrimitiveUP(Type, MinVertexIndex, NumVertices,
                                                        _PrimitiveCount, _Indices, D3DFMT_INDEX16,
                                                        _Vertex, _vStride)))
            return FALSE;
        return TRUE;
    }
    ...
    virtual BOOL DrawPrimitiveUP(D3DPRIMITIVETYPE Type, void* _Vertex, UINT _vStride,
        ...
        RtGetPref()->Proc_FaceNum += _PrimitiveCount;
        ++RtGetPref()->Draw_Call;
        if (FAILED(m_pd3dDevice->DrawPrimitiveUP(Type, _PrimitiveCount, _Vertex, _vStride)))
            return FALSE;
        return TRUE;
    }
    ...
    virtual BOOL SetStream_vb(UINT _StreamIndex, RtgBufferItem* _vbItem, UINT _Stride, UINT _Offset,
                              UINT _vNum, DWORD _FVF = 0) {
        return SetStream_vb(_StreamIndex, (IDirect3DVertexBuffer9*)_vbItem->pBuffer, _Stride,
                            _Offset, _vNum, _FVF);
    }

    virtual BOOL SetStream_vb(UINT _StreamIndex, RtgBufferItem* _vbItem, UINT _Stride,
                              DWORD _FVF = 0) {
        return SetStream_vb(_StreamIndex, (IDirect3DVertexBuffer9*)_vbItem->pBuffer, _Stride, 0,
                            _vbItem->dwSize / _Stride, _FVF);
    }
    ...
    virtual BOOL SetStream_vb(UINT _StreamIndex, RtgShareBufferItem* _vbItem, UINT _Stride,
                              DWORD _FVF = 0) {
        return SetStream_vb(_StreamIndex,
                            (IDirect3DVertexBuffer9*)_vbItem->pShareBuf->bufItem->pBuffer, _Stride,
                            _vbItem->Offset, _vbItem->Size / _Stride, _FVF);
    }

    virtual BOOL SetStream_ib(RtgShareBufferItem* _ibItem) {
        m_ibDesc._Offset = _ibItem->Offset / sizeof(WORD);
        if (FAILED(m_pd3dDevice->SetIndices(
                (IDirect3DIndexBuffer9*)_ibItem->pShareBuf->bufItem->pBuffer)))
            return FALSE;
        return TRUE;
    }

    virtual BOOL DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, UINT PrimitiveCount,
                                      UINT NumVertices = 0, UINT _ibOffset = 0) {
        return DrawIndexedPrimitive(Type, 0, 0,
                                    (NumVertices == 0 ? m_vbDesc[0]._vNum : NumVertices),
                                    m_ibDesc._Offset + _ibOffset, PrimitiveCount);
    }

    virtual BOOL DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, RtgBufferItem* vb, UINT _vbStride,
                                      RtgBufferItem* ib, UINT _PrimitiveCount) {
    ...
    virtual BOOL DrawPrimitiveVB(RTGPrimitive eType, UINT dwVertexStart, UINT dwVerticesCount,
                                 UINT dwPrimitiveCount);  // for vb mgr
    ...
    virtual BOOL LockVertexBuffer(void* pVertexBuffer, void** ppData, DWORD dwOffset = 0,
                                  DWORD dwSize = 0, RTGLock eLockFlag = RTGLK_DISCARD);
