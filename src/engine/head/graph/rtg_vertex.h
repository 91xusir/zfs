...
struct Int2 {
    ...
    bool operator==(const Int2& _int2) const { return (x == _int2.x) && (y == _int2.y); }
...
class RtgVertex2 {
   ...
   public:
    ...
    inline void Set(const RtgVertex2& v) {
        x = v.x;
        y = v.y;
    }

    inline void Get(float* ptr) {
        ptr[0] = x;
        ptr[1] = y;
    }
    ...
    friend RtArchive& operator<<(RtArchive& Ar, RtgVertex2& v) { return Ar << v.x << v.y; }
};
...
class RtgVertex3 {
   ...
   public:
    ...
    RtgVertex3(float v0, float v1, float v2) {
        m_v[0] = v0;
        m_v[1] = v1;
        m_v[2] = v2;
    }

    RtgVertex3(const float v[3]) {
        m_v[0] = v[0];
        m_v[1] = v[1];
        m_v[2] = v[2];
    }

    RtgVertex3(const RtgVertex3& rhs) {
        m_v[0] = rhs[0];
        m_v[1] = rhs[1];
        m_v[2] = rhs[2];
    }

    RtgVertex3(const float3& _f3) : x(_f3.x), y(_f3.y), z(_f3.z) {}

    ~RtgVertex3() {}

    inline BOOL IfZero(void) {
        if (m_v[0] != 0.0f || m_v[1] != 0.0f || m_v[2] != 0.0f)
            return FALSE;
        else
            return TRUE;
    }
    ...
    inline RtgVertex3 operator-() { return RtgVertex3(-x, -y, -z); }

    void Output();

    friend RtArchive& operator<<(RtArchive& Ar, RtgVertex3& v) { return Ar << v.x << v.y << v.z; }
...
class RtgVertex4 {
   ...
   public:
    ...
    RtgVertex4(float a, float b, float c, float d) {
        x = a;
        y = b;
        z = c;
        w = d;
...
class RtgOrientation {
   public:
    ...
    RtgOrientation(float x, float y, float z, float ang) {
        this->x = x;
        this->y = y;
        this->z = z;
        this->ang = ang;
    ...
    void Set(float x, float y, float z, float ang) {
        this->x = x;
        this->y = y;
        this->z = z;
        this->ang = ang;
...
RtgVertex3 RtgVertex3::Cross(const RtgVertex3& rhs) const {
    return RtgVertex3(m_v[1] * rhs[2] - m_v[2] * rhs[1], m_v[2] * rhs[0] - m_v[0] * rhs[2],
                      m_v[0] * rhs[1] - m_v[1] * rhs[0]);
}
...
void RtgVertex4::Normalize3(void) {
    float poweradd = x * x + y * y + z * z;
    if (poweradd == 1.0f || poweradd == 0.0f)
        return;
    float len = (float)fsqrt(poweradd);
    x /= len;
    y /= len;
