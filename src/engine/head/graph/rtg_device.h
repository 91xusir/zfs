...
class RtgDevice {
   ...
   public:
    ...
    struct SConfig {
        ...
        SConfig()
            : iDefaultStartX(-1), iDefaultStartY(-1), fCameraFOV(45.0f), fCameraAspect(1.333f),
              fCameraNear(3.0f), fCameraFar(5000.0f), bEnableFog(true), hCustomWndHandle(nullptr),
              szWindowTitle("gc") {}
   ...
   public:
    bool Init(RTHINSTANCE hInst, RtRuntimeClass* pEventClass, RtRuntimeClass* pCameraClass,
              SConfig& gameIni, SUserConfig& m_userConfig);
...
----------------------------------------------------------------------*/
   ...
   protected:
    void ReadGraphIni(SConfig& gameIni);
    //void ReadUserIni(RtIni* pUserIni);
    void ReadUserIni(SUserConfig& m_userConfig);
    void ApplyConfig();
};
