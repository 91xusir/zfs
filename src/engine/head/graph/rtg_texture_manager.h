...
/*
      utility description
     */
enum { D3DX_UI_MIPLEVEL = 1, Text_File_Path_Size = 200 };

/* texture lock flag */
enum { Text_Lock_Read = 1 << 0, Text_Lock_Write = 1 << 1, Text_Lock_All = 0xffffffff };
...
typedef struct _RtgTextItem {
    ...
    _RtgTextItem(){CM_MEMPROTECTOR(fileName, MAX_PATH)} _RtgTextItem(const _RtgTextItem& object) {
        memcpy(this, &object, sizeof(_RtgTextItem));
        CM_MEMPROTECTOR(fileName, MAX_PATH)
    }

    ~_RtgTextItem() { CM_MEMUNPROTECTOR(fileName) }
} RtgTextItem;
...
typedef struct _RtgSeqTextItem {
    ...
    _RtgSeqTextItem(){
        CM_MEMPROTECTOR(fileName, MAX_PATH)} _RtgSeqTextItem(const _RtgSeqTextItem& object) {
        memcpy(this, &object, sizeof(_RtgSeqTextItem));
        CM_MEMPROTECTOR(fileName, MAX_PATH)
    }

    ~_RtgSeqTextItem() { CM_MEMUNPROTECTOR(fileName) }
} RtgSeqTextItem;
...
class RtgTextureManager {
   public:
    enum { Text_Catch_Size = 32, Text_Load_PreFrame = 1 };
    ...
    RtgTextItem* CreateTextureFromFile_Direct(const char filePath[], UINT width = D3DX_DEFAULT,
    ...
    RtgTextItem* CreateTextureFromFile(const char fileName[], UINT width = D3DX_DEFAULT,
                                       UINT height = D3DX_DEFAULT, UINT mipLevels = D3DX_DEFAULT,
                                       D3DFORMAT format = D3DFMT_FROM_FILE,
                                       D3DPOOL poolType = D3DPOOL_MANAGED, DWORD Usage = 0,
                                       BOOL bForceLoad = false);

    RtgTextItem* CreateTexture(UINT width, UINT height, UINT mipLevels = 1,
                               D3DFORMAT format = D3DFMT_A8R8G8B8,
                               D3DPOOL poolType = D3DPOOL_MANAGED, DWORD Usage = 0);

    RtgTextItem* CreateTextureFromFileMemery(PVOID pData, UINT dataSize, UINT width = D3DX_DEFAULT,
                                             UINT      height = D3DX_DEFAULT,
                                             UINT      mipLevels = D3DX_DEFAULT,
                                             D3DFORMAT format = D3DFMT_FROM_FILE,
                                             D3DPOOL poolType = D3DPOOL_MANAGED, DWORD Usage = 0);

    RtgTextItem* CreateTextureFromArchive(RtArchive* pArchive, UINT width = D3DX_DEFAULT,
                                          UINT height = D3DX_DEFAULT, UINT mipLevels = D3DX_DEFAULT,
                                          D3DFORMAT format = D3DFMT_FROM_FILE,
                                          D3DPOOL poolType = D3DPOOL_MANAGED, DWORD Usage = 0);

    RtgTextItem* CreateTextureFromARGBData(PVOID pData, UINT dataSize, UINT width, UINT height,
                                           UINT mipLevels = 1, D3DFORMAT format = D3DFMT_A8R8G8B8,
                                           D3DPOOL poolType = D3DPOOL_MANAGED, DWORD Usage = 0);

    RtgTextItem* CreateDynamicTexture(UINT width, UINT height, D3DFORMAT format = D3DFMT_A8R8G8B8);

    RtgTextItem* CreateFrameBufferTexture(UINT width, UINT height,
                                          D3DFORMAT format = D3DFMT_A8R8G8B8);

    RtgTextItem* CreateDepthBufferTexture(UINT width, UINT height, D3DFORMAT format = D3DFMT_D24S8);

    RtgTextItem* CreateUiTexture(const char fileName[], UINT width = D3DX_DEFAULT,
                                 UINT height = D3DX_DEFAULT);
    ...
    void EnableMutiThreadLoad(bool bEnable = true) { m_bMutiThreadLoad = bEnable; }
   ...
   protected:
    ...
    std::map<std::string, RtgTextItem*> m_mapText;
    std::set<RtgTextItem*>              m_setText;
    std::vector<RtgTextItem*>           m_vecResetText;
    std::list<RtgTextItem*>             m_listCatch;
    std::vector<std::string>            m_vecPath;
    RtgTextItem*                        m_texChessboard;
    bool                                m_bExit;
    bool                                m_bExited;
    std::list<RtgTextItem*>             m_loadQueue;
    std::list<RtgTextItem*>             m_cratQueue;
