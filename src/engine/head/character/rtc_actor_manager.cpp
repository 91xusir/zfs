...
RtcAcotrManager::RtcAcotrManager() : m_bExit(true), m_bExited(true), m_bMutiThreadLoad(true) {}
...
void RtcAcotrManager::Exit() {
    ...
    for (actorcache_t::iterator i = m_actorCache.begin(); i != m_actorCache.end(); ++i) {
        (*i)->StopPose();
    }
    ...
    for (objmap_t::iterator it = m_objMap.begin(); it != m_objMap.end(); ++it) {
...
CRT_ActorInstance* RtcAcotrManager::CreateActor(const char         _fileName[],
    ...
    struct _FindPred {
        _FindPred(const char _name[]) : _findName(_name) {}
    ...
    actorcache_t::iterator iiac =
        find_if(m_actorCache.begin(), m_actorCache.end(), _FindPred(_name));
...
CRT_SkinInstance* RtcAcotrManager::CreateSkin(const char _fileName[], CRT_ActorInstance* _act,
                                              bool bForceLoad /* = false */
) {
...
CRT_Effect* RtcAcotrManager::CreateEffect(const char _fileName[], CRT_ActorInstance* _act,
                                          bool bForceLoad /* = false */
) {
...
CRT_MaterialLib* RtcAcotrManager::CreateMtl(const char _fileName[], CRT_SkinInstance* _skin,
                                            bool bForceLoad /* = false */) {
...
void RtcAcotrManager::Load_Thread(void* _ptMgr) {
    ...
    while (!ptMgr->m_bExit) {
        ...
        P_LOGINFO("now Loading:" + _ao._filename);
...
PoolFindRes RtcAcotrManager::FindObject(CRT_PoolObject** _obj, const char _name[],
                                        char _fullName[]) {
    RTASSERT(_obj);
    RTASSERT(_name);
    RTASSERT(_fullName);
...
bool RtcAcotrManager::LoadDirect(CRT_PoolObject* _obj, const char _name[], const char _fullName[],
                                 RtRuntimeClass* _Request) {
    RTASSERT(_obj);
    RTASSERT(_name);
    RTASSERT(_fullName);
    RTASSERT(_Request);
    ...
    if (!_obj->IsKindOf(RT_RUNTIME_CLASS(CRT_PoolObject)) || !_obj->IsKindOf(_Request)) {
...
CRT_PoolObject* RtcAcotrManager::LoadDirect(const char _name[], const char _fullName[],
                                            RtRuntimeClass* _Request) {
    ...
    if (!tmp->IsKindOf(RT_RUNTIME_CLASS(CRT_PoolObject)) || !tmp->IsKindOf(_Request)) {
        RtCoreLog().Error("load object fiald, type unmatch\n");
        DEL_ONE(tmp);
        return NULL;
    }
