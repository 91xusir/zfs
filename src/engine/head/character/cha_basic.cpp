...
bool RtcSkinSubset::Create() {
    ...
    m_pib = RtGetRender()->GetBuffMgr()->CreateBuffer(faceList.buffer_size(), D3DFMT_INDEX16, 0,
                                                      D3DPOOL_MANAGED, faceList.pointer());
...
bool CRT_VaSkin::Create(CRT_Actor* act) {
    ...
    if (m_bStatic) {
        ...
        RtgSetCreateBufferDebugInfo();
        m_pvb =
            RtGetRender()->GetBuffMgr()->CreateBuffer(m_aryVertex.buffer_size(), SRT_StaticVtx::fvf,
                                                      0, D3DPOOL_MANAGED, m_aryVertex.pointer());
    ...
    try {
        if (Ar.IsLoading()) {
            ...
            if (strcmp(m_szVersion, szVersion) == 0) {
                Ar << m_animSpeed << m_animType << m_bDynamicShadow << m_bones << m_boundBoxList
                   << nouse0 << m_bUseVC << m_bUseLight << m_bZTest << m_eftList << m_frameNum
                   << m_poseMap << nouse1 << m_skinList;
            } else {
                Ar.Seek(pos, RtArchive::SeekBegin);
                Ar << m_animType << m_frameNum << m_bUseLight << m_bUseVC << m_bZTest
                   << m_bDynamicShadow << m_animSpeed << m_poseMap << m_bones << m_skinList
                   << m_eftList << m_boundBoxList;
            }
        } else {
            Ar.Serialize(szVersion, 9);
            Ar << m_animSpeed << m_animType << m_bDynamicShadow << m_bones << m_boundBoxList
    ...
    for (size_t i = 0; i < m_bones.size(); ++i) {
        ...
        if (_parent)
            ...
            RtCoreLog().Error("%s: [%s] can't get parent bone [%s]\n", __FUNCTION__,
                              m_poTag.c_str(), _bone->ParentName.c_str());
    }
}
    ...
    for (it1 = m_poseMap.begin(); it1 != m_poseMap.end(); it1++) {
        ...
        if (it2 != act->m_poseMap.end()) {
            ...
            for (int i = 0; i < eventVec.size(); i++) {
                ...
                eventVec[i].KeyFrame =
                    (it1->second).StartFrm + per * ((it1->second).EndFrm - (it1->second).StartFrm);
            }
...
bool CRT_Actor::Merge2(CRT_Actor* act) {
    ...
    for (TPoseMap::iterator it = act->m_poseMap.begin(); it != act->m_poseMap.end(); ++it) {
...
CRT_PoolObject* CRT_MaterialLib::NewObject() {
    CRT_MaterialLib* lib =
        (CRT_MaterialLib*)RtcGetActorManager()->NewObject(RT_RUNTIME_CLASS(CRT_MaterialLib));
