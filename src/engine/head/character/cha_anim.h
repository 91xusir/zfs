...
class CRT_SkinInstance : public RtObject {
    ...
    bool ComputeVC(RtgMatrix16& world, vcArray_t& aryVc, int offset, RtgVertex3 vSkyLight,
    ...
    bool ResourceReady() const { return GetState() == Object_State_Ready; }

    void AttachActor(CRT_ActorInstance* _actor) { m_actor = _actor; }

    CRT_MaterialLib* GetMaterialLib() { return m_mtlLib; }
    ...
    CRT_Skin* GetCore() { return m_core; }
    ...
    void EnableMip(bool bEnable = true) { m_bEnableMip = bEnable; }
...
typedef void (*CreateFinishNotify_t)(CRT_ActorInstance* _actor, ActorNotifType nttype, void* param0,
                                     void* param1);

struct ActorNotify {
    ActorNotify() : param0(NULL), param1(NULL), notify(NULL) {}

    ActorNotify(CreateFinishNotify_t _notify, void* _param0, void* _param1)
        : param0(_param0), param1(_param1), notify(_notify) {}

    ActorNotify(const ActorNotify& _n) : param0(_n.param0), param1(_n.param1), notify(_n.notify) {}
...
enum { AIF_Light_Enable = (1 << 0), AIF_ZTest_Enable = (1 << 1), AIF_Vc_Enable = (1 << 2) };
...
class CRT_ActorInstance : public RtObject {
   ...
   public:
    ...
    bool ComputeVC(RtgMatrix16& world, vcArray_t& aryVc, RtgVertex3 vSkyLight,
                   RtgLightItem* pLights);
    ...
    CRT_Actor* GetCore() { return m_core; }
    ...
    //lyymark act对象帧
    void UseFrame(float frame) { m_curFrame = frame; }

    RtgAABB* GetBoundingBox() { return &m_boundingBox; }

    RtgAABB* GetStaticBoundingBox() { return &m_staticBoundingBox; }

    void SetCoreObject(CRT_Actor* core) { m_core = core; }

    void SetTagPoseNotify() { RegisterNotify(&m_tagPoseNotify); }
    ...
    void SetOffset(RtgVertex3 offset) { m_offset = offset; }
    ...
    bool GetBoneMat(RtgMatrix12* _Mat, const char _Name[]) {
        ...
        if (GetState() != Object_State_Ready ||
            (boneIndex = m_core->GetBoneIndex(_Name)) == Error_BoneIndex) {
    ...
    void UpdateMatrix() {
        ...
        // 检查是否有父对象
        if (m_parent) {  // 如果有父对象，尝试获取父对象插槽信息m_parentSlot矩阵
    ...
    const RtgMatrix12* GetMatrix() { return &m_localMat; }
    ...
    const RtgMatrix12* GetWorldMatrix() { return &m_matrix; }

    void ShowBone(bool bShow) {
        if (!m_core)
            return;
        for (vector<SRT_Bone>::iterator i = m_bones.begin(); i != m_bones.end(); ++i)
            i->isPaint = bShow;
    }
    ...
    void RegisterNotify(CRT_PoseNotify* notify) { m_notify = notify; }
    ...
    const SRT_Pose& GetCurrentPose() { return m_curPose; }

    bool IsPlayingPose() const { return m_curPose.IsVaild(); }
    ...
    bool HasAttrib(DWORD _attrib) const { return !!(m_Flags & _attrib); }
    ...
    void Unregister_ActorNotify() { m_Notify.notify = NULL; }
    ...
    void AddEffect(CRT_Effect* _effect) { m_effectList.push_back(_effect); }
    ...
    void Update_WorldBoundBox() {
        ...
        RtgVertex3 _box[] = {
            RtgVertex3(_min.x, _min.y, _min.z), RtgVertex3(_max.x, _min.y, _min.z),
            RtgVertex3(_max.x, _max.y, _min.z), RtgVertex3(_min.x, _max.y, _min.z),
            RtgVertex3(_min.x, _min.y, _max.z), RtgVertex3(_max.x, _min.y, _max.z),
            RtgVertex3(_max.x, _max.y, _max.z), RtgVertex3(_min.x, _max.y, _max.z),
        };
        ...
        m_aabboxWorld.vPos.Set((_min.x + _max.x) * 0.5f, (_min.y + _max.y) * 0.5f,
                               (_min.z + _max.z) * 0.5f);
        m_aabboxWorld.vExt.Set((_max.x - _min.x) * 0.5f, (_max.y - _min.y) * 0.5f,
                               (_max.z - _min.z) * 0.5f);
    }

    const RtgAABB* Get_WorldBoundBox() const { return &m_aabboxWorld; }

    void AddChild(CRT_ActorInstance* _act) { m_childs.push_back(_act); }
   ...
   public:
    SRT_Pose           m_curPose;            // 当前姿态
    SRT_Pose           m_prePose;            // 先前姿态
    CRT_PoseNotify*    m_notify;             // 指向通知对象的指针
    float              m_curFrame;           // 当前动画帧
    float              m_oldFrame;           // 上一帧动画
    bool               m_bLoop;              // 是否循环播放
    long               m_lastFrame;          // 上一帧索引
    RtgAABB            m_aabboxWorld;        // 世界空间中的包围盒
    RtgAABB            m_boundingBox;        // 当前对象的包围盒
    RtgAABB            m_staticBoundingBox;  // 静态对象的包围盒
    std::string        m_strHighLightBone;   // 高亮骨骼名称
    bool               m_bRenderByParent;    // 是否由父对象渲染
    TagPoseNotify      m_tagPoseNotify;      // 姿态通知标记
    bool               m_bDisplay;           // 是否显示
    bool               m_bUpdate;            // 是否需要更新
    float              m_animSpeed;          // 动画速度
    std::string        m_parentSlot;         // 父槽名称
    RtgMatrix12        m_matrix;             // 对象变换矩阵
    RtgMatrix12        m_localMat;           // 局部变换矩阵
    bool               m_bValid;             // 是否有效
    bool               m_bEnableShadow;      // 是否启用阴影
    float              m_visible;            // 可见度
    bool               m_bDrawScale;         // 是否绘制缩放
    RtgVertex3         m_drawScale;          // 绘制的缩放值
    RtgVertex3         m_offset;             // 偏移量
    bool               m_bRenderEffect;      // 是否渲染效果
    ActorNotify        m_Notify;             // 通知处理对象
    bool               m_bUseLight;          // 是否使用光照
    bool               m_bUseVC;             // 是否使用视锥体剔除
    bool               m_bZTest;             // 是否启用深度测试
    DWORD              m_Flags;              // 标志位
    std::string        m_Name;               // 对象名称
    CRT_ActorInstance* m_parent;             // 父演员实例指针
    childArray_t       m_childs;             // 子对象数组
    boneArray_t        m_bones;              // 骨骼数组
    UINT               m_lastTickFrame;      // 上次计时的帧
