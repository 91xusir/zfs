...
#define BEGIN_VERSION_CONVERT(v)                                                                   \
    char __end = v;                                                                                \
    Ar << __end;                                                                                   \
    if (!__end) {
...
class RtcSkinSubset {
   public:
    RtcSkinSubset() : m_pib(NULL), _Id(0) {}
    ...
    RtgBufferItem* RequestIB() { return m_pib; }
...
struct SRT_PoseEvent {
    ...
    SRT_PoseEvent() : KeyFrame(0), nTagActIndex(-1) {}
...
struct SRT_Pose {
    ...
    bool IsVaild() const { return StartFrm <= EndFrm; }
...
struct SRT_VaVtxNor {
    ...
    friend RtArchive& operator<<(RtArchive& Ar, SRT_VaVtxNor& v) { return Ar << v.xyz; }
};
...
struct SRT_VaVtxUV {
    ...
    friend RtArchive& operator<<(RtArchive& Ar, SRT_VaVtxUV& v) { return Ar << v.UV[0] << v.UV[1]; }
};
...
struct SRibbonFrame {
    ...
    friend RtArchive& operator<<(RtArchive& Ar, SRibbonFrame& v) { return Ar << v.segNum << v.pos; }
};
...
struct SRT_BoneRef {
    ...
    bool operator<(const SRT_BoneRef& ref) const { return Weight < ref.Weight ? true : false; }
    ...
    void Output() { RtCoreLog().Info("idx = %d,weight = %.3f\n", bondIndex, Weight); }
};
...
class CRT_PoolObject : public RtObject {
   ...
   public:
    std::string m_diskFile;  // 存储磁盘文件路径的字符串
    std::string m_poTag;     // 存储对象标签的字符串
    int         m_poRef;     // 存储对象引用计数的整数
    char        m_poType;    // 存储对象类型的字符
};
...
#endif
{
   public:
    ...
    virtual void SetName2(const char _name[]) { m_Name = _name; }
...
class CRT_EffectBase : public CRT_Effect {
    ...
    CRT_EffectBase() : m_actor(NULL) {}

    virtual ~CRT_EffectBase() { Destroy(); }
    ...
    virtual void LinkActor(CRT_ActorInstance* _actor) { m_actor = _actor; }
    ...
    virtual const char* GetName() { return m_eftName.c_str(); }

    virtual void SetName(const char* name) { m_eftName = name; }
...
class CRT_MaterialLib : public CRT_PoolObject {
   public:
    ...
    size_t GetMtlNum() const { return m_mtlList.size(); }

    bool RequestTick() { return true; }
    ...
    bool RequestUseFrame() { return true; }
    ...
    bool RemoveMaterial(const std::string strName) {
        for (vector<CRT_Material*>::iterator it = m_mtlList.begin(); it != m_mtlList.end(); ++it) {
...
class CRT_Skin : public CRT_PoolObject {
    ...
    CRT_Skin() : m_bStatic(false), m_bBoard(false), m_mtlLib(NULL), m_bDynShadow(true) {
    ...
    virtual bool OnCreateInstance(CRT_SkinInstance* skin) { return false; }

    virtual RtgBufferItem* RequestVB(CRT_SkinInstance* skin, DWORD* _Offset, DWORD* _Size) {
        return NULL;
    }

    virtual UINT vertexNum() const { return 0; }

    virtual UINT vertexSize() const { return 0; }

    virtual DWORD vertexFvf() const { return 0; }

    virtual verArray_t* Get_VertexArray() { return NULL; }
...
class CRT_VaSkin : public CRT_Skin {
    ...
    CRT_VaSkin() : m_pvb(NULL) {
        SetState(Object_State_New);
        m_boardPos.Zero();
    }

    virtual ~CRT_VaSkin() { Destroy(); }
    ...
    virtual long Serialize(RtArchive& Ar) {
        ...
        Ar << m_bNor << m_bUV << m_aryVertex << m_frmList;
    ...
    void SetVertex(const vector<SRT_StaticVtx>& verVertex) { m_aryVertex = verVertex; }

    virtual UINT vertexNum() const { return m_aryVertex.size(); }

    virtual UINT vertexSize() const { return sizeof(SRT_StaticVtx); }

    virtual DWORD vertexFvf() const { return SRT_StaticVtx::fvf; }

    virtual verArray_t* Get_VertexArray() { return &m_aryVertex; }
...
class CRT_SkelSkin : public CRT_Skin {
    ...
    virtual UINT vertexNum() const { return m_verVertex.size(); }
...
class keyframe {
   public:
    ...
    bool GetKeyFrame(unsigned long frame, T& key) {  // 遍历关键帧列表，寻找包含指定帧的区间
    ...
    unsigned long Size() { return (unsigned long)m_keyList.size(); }

    T* GetKeyFrame(int idx, unsigned long& frame) {
        if (idx < 0 || idx >= (int)m_keyList.size())
            return NULL;
    ...
    T* GetKeyFrame(unsigned long frame) {
        for (unsigned long i = 0; i < m_keyList.size(); i++) {
            if (m_keyList[i].frame == frame)
                return &m_keyList[i].data;
        }
        return NULL;
    }
