...
class CUnpackFileReader : public RtArchive {
   public:
    CUnpackFileReader(/*RtOutputDevice* vpError,*/ CUnpackFile* vpUnpackFile, bool vTextMode)
        : mTextMode(vTextMode), mpcDiskBuffer(NULL),
    ...
    virtual int Tell() { return (int)mpUnpackFile->Tell(); };

    virtual int TotalSize() { return (int)mpUnpackFile->OriginSize(); };
    ...
    virtual long Serialize(void* V, int iLength) {
        ...
        if (mTextMode) {
            ...
            if (mpcDiskBuffer) {
                if ((mnCurrentPosition < mpUnpackFile->OriginSize()) &&
                    ((mnCurrentPosition + iLength) <= mpUnpackFile->OriginSize()) && (V != NULL)) {
    ...
    void CacheDiskStore() {
        DEL_ARRAY(mpcDiskBuffer);
        mnCurrentPosition = 0;
        if (mpUnpackFile && mpUnpackFile->OriginSize() > 0 &&
...
class MemoryPackFileWriter : public RtArchive {
   public:
    MemoryPackFileWriter(CPack* vpPack, CCompress* vpCompress,
                         const string& vrFileName /*,RtOutputDevice* vpError*/)
        : mName(vrFileName), mpPack(vpPack), mpCompress(vpCompress),
          /*mpError(vpError),*/
          mCurrent(0), mpBuffer(NULL), mBufferSize(0), mDataSize(0) {
        m_nMode |= RtArchive::store;
    }

    virtual ~MemoryPackFileWriter() { Close(); }
    ...
    virtual int Tell() { return mCurrent; }
    ...
    virtual long Serialize(void* V, int iLength) {
        ...
        if (mCurrent - mDataSize > 0) {  //当前指针已经再数据的后面，中间填充0
...
*/
    ...
    BOOL Copy(const char* pPackageName, const char* pDestFile, const char* pSrcFile,
              BOOL bReplaceExisting, BOOL bEvenIfReadOnly, BOOL bAttributes,
              void (*fnProgress)(FLOAT fFraction)) {
        return FALSE;
    }

    BOOL MakeDirectory(const char* pPath, BOOL bTree = 0) { return FALSE; }

    BOOL DeleteDirectory(const char* pPath, BOOL bRequireExists = 0, BOOL bTree = 0) {
        return FALSE;
    }

    BOOL Move(const char* pPackageName, const char* Dest, const char* Src, BOOL ReplaceExisting = 1,
              BOOL EvenIfReadOnly = 0, BOOL Attributes = 0) {
        return FALSE;
    }

    BOOL SetFilePermission(const char* pFilename, DWORD dwPermission) { return FALSE; }

    DWORD GetFilePermission(const char* pFilename) { return 0; }
    ...
    CUnpackFileReader* CreateUnpackFileReader(const char* vpFilename,
                                              DWORD       vFlags /*,RtOutputDevice* vpError*/) {
    ...
    MemoryPackFileWriter* CreateFileWriter(const char* pPackName, const char* pFilename,
                                           DWORD dwFlags /*, RtOutputDevice* pError*/) {
        if (pPackName == NULL) {
            return NULL;
        }
        ...
        CCompress* pCompress =
            PackManager()->GetCompress(CPack::NULL_COMPRESS, CNULLCompress::LAST_VERSION);
        if (pCompress == NULL) {
        ...
        if (dwFlags & FW_Append) {
            ...
            DEL_ONE(pFile);
            MemoryPackFileWriter* pWrite =
                RT_NEW            MemoryPackFileWriter(pPack, pCompress, FileName /*,pError*/);
    ...
    BOOL Delete(const char* pFilename, BOOL bRequireExists = 0, BOOL bEvenReadOnly = 0) {
        ...
        bool Ok = false;
        if (GetFileInfoFromAllPackEx(pPack, (const CPack::SFileInfo*&)pFileInfo,
                                     FileName.c_str())) {
    ...
    SQWORD GetCreateTime(const char* pFilename) { return 0; }

    BOOL SetCreateTime(const char* pFilename) { return 0; }

    SQWORD GetAccessTime(const char* pFilename) { return 0; }

    BOOL SetAccessTime(const char* pFilename) { return 0; }

    SQWORD GetModifyTime(const char* pFilename) { return 0; }

    BOOL SetModifyTime(const char* pFilename) { return 0; }
    ...
    BOOL SetDefaultDirectory(const char* pFilename) { return FALSE; }

    RtString GetDefaultDirectory() { return ""; }
   ...
   private:
    ...
    bool GetFileInfoFromAllPackEx(CPack*& vprGetPack, const CPack::SFileInfo*& vprGetFileInfo,
...
class CRtPackAndAnsiManager : public RtFileManager {
   public:
    ...
    BOOL Copy(const char* pPackageName, const char* pDestFile, const char* pSrcFile,
              BOOL bReplaceExisting, BOOL bEvenIfReadOnly, BOOL bAttributes,
              void (*fnProgress)(FLOAT fFraction)) {
        return mpAnsiManager->Copy(pPackageName, pDestFile, pSrcFile, bReplaceExisting,
                                   bEvenIfReadOnly, bAttributes, fnProgress);
    }
    ...
    BOOL Move(const char* pPackageName, const char* Dest, const char* Src, BOOL ReplaceExisting = 1,
              BOOL EvenIfReadOnly = 0, BOOL Attributes = 0) {
        return mpAnsiManager->Move(pPackageName, Dest, Src, ReplaceExisting, EvenIfReadOnly,
                                   Attributes);
    }
    ...
    RtArchive* CreateFileWriter(const char* pPackageName, const char* pFilename,
                                DWORD dwFlags /*, RtOutputDevice* pError*/) {
        RtArchive* pArchive;
        pArchive = mpAnsiManager->CreateFileWriter(pPackageName, pFilename, dwFlags /*,pError*/);
        if (pArchive == NULL) {
            pArchive =
                mpPackManager->CreateFileWriter(pPackageName, pFilename, dwFlags /*,pError*/);
        }
        return pArchive;
    }
    ...
    RtString GetDefaultDirectory() { return mpAnsiManager->GetDefaultDirectory(); }
