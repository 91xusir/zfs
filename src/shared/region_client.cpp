...
void SCreature::SerializeToDB(long reason, CG_CmdPacket& cmd) {
    ...
    cmd << Contribute << Point << Record << UnionRecord << JobRecord << BornTag << Sex << Exploit
        << actorID;
}
...
void SCreature::Serialize(char type, CG_CmdPacket* cmd) {
    int i;
    switch (type) {
        case NET_UPDATE_ALL:  // 序列化所有属性数据
        case NET_CRE_UPDATE_ATTR_ALL:
            // model info
            // *cmd <<
            *cmd << Lev << Exp << Name << Metier << MetierLev << Faction << AttrPoint << SkillPoint
                 << VigourPoint << Cul << GradeLev;
            // 基本属性
            *cmd << Con << Str << Dex << Int << Hit;
            // 生命值和魔法值
            *cmd << Hp << MaxHp << Mp << MaxMp << EnP << MaxEnP;
            // 攻击、防御、躲避等属性
            *cmd << Attack << DamageMin << DamageMax << Dodge << Armor << ReCriticalHit
                 << ReSkillCritical << ReFaint << ReParry;
            // 攻击相关的速度和范围
        ...
        case NET_CRE_UPDATE_MODEL:  // 序列化模型相关数据
            *cmd << ModelId;
            if (ComplexModel) {
                *cmd << HeadModelId;
        ...
        case NET_CRE_UPDATE_ATTR_SIMPLE:  // 序列化简化的属性数据
            *cmd << Hp << MaxHp << Mp << MaxMp;
            *cmd << Lev << Exp << AttrPoint << SkillPoint << VigourPoint << MovSpeed << Cul
                 << GradeLev << Pk;
            *cmd << AbiMove << AbiNormalAttack;
            break;

        case NET_CRE_UPDATE_ATTR_ELEMENT_ARMOR:  // 序列化元素护甲数据
            for (i = 0; i < MAX_ELEMENT_ATTR; i++) {
                *cmd << ElementAbsorb[i];
            }
            break;
        case NET_CRE_UPDATE_PET_INFO:  // 序列化宠物相关数据
...
void SCreature::RecomputeAttr(bool ChangeBody) {
    ...
    if (info) {
        ...
        //+回血*回血分值
        WarScores = ((1 + Lev * info->PlayerLev) *
                     ((DamageMin.GetValue() + DamageMax.GetValue()) / 2 * (info->PhysicalDamage) *
                          Armor.GetValue() * (info->PhysicalDefense) +
                      ElementDamage[ELEMENT_FIRE].GetValue() * (info->FireDamage) *
                          ElementArmor[MAX_ELEMENT_ATTR].GetValue() * (info->FireDefense) +
                      ElementDamage[ELEMENT_WATER].GetValue() * (info->WaterDamage) *
                          ElementArmor[ELEMENT_WATER].GetValue() * (info->WaterDefense) +
                      ElementDamage[ELEMENT_POISON].GetValue() * (info->PoisonDamage) *
                          ElementArmor[ELEMENT_POISON].GetValue() * (info->PoisonDefense)) *
                     (5 / 12 + Attack.GetValue() * (info->Attack))) *
                        (1 + CriticalHit.GetValue() * (info->SlayR)) +
                    SkillCriRate * (info->SkillCriRate) *
                        (AttSpeed.GetValue() * (info->DamagesSpeed)) *
                        (1 + AbsorbHp.GetValue() * (info->AbsorbHp)) +
                    GetMaxHp() / (24 / 0.7) * (info->PlayerHP) *
                        (1 + ReParry.GetValue() * (info->ReParry)) *
                        (1 + ReCriticalHit.GetValue() * (info->ReCriticalHit) +
                         ReSkillCritical.GetValue() * (info->ReSkillCritical)) +
...
float SCreature::GetSkillCriRate(int enemyLev, float ReSkillCr /*=0.0*/) {
    ...
    if (((m1 / (m1 + enemyLev * 2.0 + 30)) * 0.1 + (m2 / (m2 + enemyLev * 10.0)) * 0.2 + m1 / 2000 +
         SkillCriRate / 100.0 - ReSkillCr / 100.0) <= 0)
        return 0.0;
    else
        return (m1 / (m1 + enemyLev * 2.0 + 30)) * 0.1 + (m2 / (m2 + enemyLev * 10.0)) * 0.2 +
               m1 / 2000 + SkillCriRate / 100.0 - ReSkillCr / 100.0;
}
...
bool CRS_Pet::Load(const char* File) {
    ...
    for (j++; j < Total; j++) {
        ...
        sscanf(csv[j]["第零阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[0], &tmp.ModelChange[0],
               &tmp.AttackRangeChange[0]);
        sscanf(csv[j]["第一阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[1], &tmp.ModelChange[1],
               &tmp.AttackRangeChange[1]);
        sscanf(csv[j]["第二阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[2], &tmp.ModelChange[2],
               &tmp.AttackRangeChange[2]);
        sscanf(csv[j]["第三阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[3], &tmp.ModelChange[3],
               &tmp.AttackRangeChange[3]);
        sscanf(csv[j]["第四阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[4], &tmp.ModelChange[4],
               &tmp.AttackRangeChange[4]);
        sscanf(csv[j]["第五阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[5], &tmp.ModelChange[5],
               &tmp.AttackRangeChange[5]);
        sscanf(csv[j]["第六阶段"].Str(), "%d,%d,%f", &tmp.ModelLevel[6], &tmp.ModelChange[6],
               &tmp.AttackRangeChange[6]);
...
bool CRS_Exp::Load(const char* file) {
    ...
    // 加载 CSV 文件，如果加载失败则返回 false
    if (!csv.LoadTableFile(file))
        return false;

    // 查找标题行中包含 "ID" 的行，返回行号
    j = csv.FindLineByString("ID");
    // 如果没有找到包含 "ID" 的行，则返回 false
    if (j < 0)
        return false;
    // 设置 CSV 文件的标题行为找到的那一行
    csv.SetTitleLine(j);

    int Total = csv.GetLineCount();  // 获取 CSV 文件中的总行数
    // 从标题行的下一行开始，逐行读取数据
    for (j++; j < Total; j++) {
        // 将每一行的 "升级所需经验值" 列数据存储到 m_expTable 中
        m_expTable.push_back(csv[j]["升级所需经验值"].DWord());
        // 将每一行的 "修为等级提升所需经验值" 列数据存储到 m_CulexpTable 中
        m_CulexpTable.push_back(csv[j]["修为等级提升所需经验值"].DWord());
        // 将每一行的 "升级获得修为" 列数据存储到 m_CulTable 中
...
bool CRS_UserActor::Load(const char* file) {
    ...
    for (j++; j < Total; j++) {
        ...
        for (int i = 0; i < MAX_CREATE_ITEM_NUM; i++)
            tmp.Item[i] = 0;
        if (sscanf(csv[j]["携带道具"].Str(), "%d,%d,%d,%d,%d,%d,%d,%d", &tmp.Item[0], &tmp.Item[1],
                   &tmp.Item[2], &tmp.Item[3], &tmp.Item[4], &tmp.Item[5], &tmp.Item[6],
                   &tmp.Item[7]) != 8) {
        ...
        for (int i = 0; i < MAX_CREATE_BODY_NUM; i++)
            tmp.BodyItem[i] = 0;
        if (sscanf(csv[j]["装备道具"].Str(), "%d,%d,%d,%d,%d,%d,%d,%d", &tmp.BodyItem[0],
                   &tmp.BodyItem[1], &tmp.BodyItem[2], &tmp.BodyItem[3], &tmp.BodyItem[4],
                   &tmp.BodyItem[5], &tmp.BodyItem[6], &tmp.BodyItem[7]) != 8) {
        ...
        for (int i = 0; i < MAX_CREATE_SKILL_NUM; i++)
            tmp.Skill[i] = 0;
        if (sscanf(csv[j]["携带技能"].Str(), "%d,%d,%d,%d,%d,%d,%d,%d", &tmp.Skill[0],
                   &tmp.Skill[1], &tmp.Skill[2], &tmp.Skill[3], &tmp.Skill[4], &tmp.Skill[5],
                   &tmp.Skill[6], &tmp.Skill[7]) != 8) {
...
bool SPeriodicity::NowIsTime() {
    ...
    if (iStartYear != 0) {
        if ((t->tm_year + 1970) < iStartYear)
            return false;
        if ((t->tm_mon + 1) < iStartMonth)
            return false;
        if ((t->tm_mday) < iStartDay)
            return false;
    }

    if (iEndYear != 0) {
        if ((t->tm_year + 1970) > iEndYear)
            return false;
        if ((t->tm_mon + 1) > iEndMonth)
            return false;
        if ((t->tm_mday) > iEndDay)
            return false;
    }
    ...
    switch (cycMode) {
        ...
        case 1:  // 每天
            break;
        case 2:  // 每周
            if (cycWeek[t->tm_wday] == 0)
                return false;
            break;
        case 3:  // 每月
            if (cycMonth[0] < (t->tm_mon + 1) || cycMonth[1] > (t->tm_mon + 1))
                return false;
            break;
    }
    ...
    if (iStartSecond) {
        if (iStartSecond < iSecond)
            return false;
    }

    if (iEndSecond) {
        if (iEndSecond > iSecond)
            return false;
    }
...
bool CRS_FuBen::GetTriggerKey(const char* str, vector<STaskKey>& key) {
    ...
    if (k && strlen(k) > 0) {
        while (k) {
            char buf[10][128];
            memset(buf, 0, 10 * 128);
            sscanf(k, "%s %s %s %s %s %s %s %s %s %s", buf[0], buf[1], buf[2], buf[3], buf[4],
    ...
    for (j++; j < Total; j++) {
        ...
        if (pStrTmp) {
            ...
            if ((k != NULL) && (strlen(k) > 0)) {
                while (k != NULL) {
                    if (sscanf(k, "%d,%d,%d", &tmp.blockPos[counter].x, &tmp.blockPos[counter].y,
                               &tmp.blockDir[counter]) != 3) {
        ...
        if (pStrTmp) {
            ...
            if ((k != NULL) && (strlen(k) > 0)) {
                while (k != NULL) {
                    if (sscanf(k, "%d,%d,%d", &tmp.triggerPos[counter].x,
                               &tmp.triggerPos[counter].y, &tmp.triggerDir[counter]) != 3) {
        ...
        if (pStrTmp) {
            ...
            if ((k != NULL) && (strlen(k) > 0)) {
                while (k != NULL) {
                    if (sscanf(k, "%d,%d,%d", &tmp.opendoorPos[counter].x,
                               &tmp.opendoorPos[counter].y, &tmp.opendoorDir[counter]) != 3) {
                        RtCoreLog().Info("表现物件的位置坐标格式出错。\n");
                        return false;
                    }
                    k = strtok(NULL, delim);
...
bool CRS_FuBen::Load(const char* file) {
    ...
    for (j++; j < Total; j++) {
        ...
        if (sscanf(csv[j]["等级要求"].Str(), "%d,%d,%d,%d,%d", &tmp.reqLev[0], &tmp.reqLev[1],
                   &tmp.reqLev[2], &tmp.reqLev[3], &tmp.reqLev[4]) != 5) {
            RtCoreLog().Info("CRS_FuBen::Load: parse 等级要求[%s] error.\n",
                             csv[j]["等级要求"].Str());
            return false;
        }
        if (sscanf(csv[j]["出生点"].Str(), "%d,%d,%d", &tmp.bornPos[0], &tmp.bornPos[1],
                   &tmp.bornPos[2]) != 3) {
            RtCoreLog().Info("CRS_FuBen::Load: parse 出生点 error.\n");
            return false;
        }
        if (sscanf(csv[j]["退出点"].Str(), "%d,%d,%d", &tmp.exitPos[0], &tmp.exitPos[1],
                   &tmp.exitPos[2]) != 3) {
        ...
        if (tmp.enterNum > 1) {
            if (sscanf(csv[j]["等待点"].Str(), "%d,%d,%d", &tmp.waitPos[0], &tmp.waitPos[1],
                       &tmp.waitPos[2]) != 3) {
        ...
        if (tmp.cyc.cycMode == 2) {
            if (sscanf(csv[j]["周期模式"].Str(), "%d,%d,%d,%d,%d,%d,%d", &tmp.cyc.cycWeek[0],
                       &tmp.cyc.cycWeek[1], &tmp.cyc.cycWeek[2], &tmp.cyc.cycWeek[3],
                       &tmp.cyc.cycWeek[4], &tmp.cyc.cycWeek[5], &tmp.cyc.cycWeek[6]) != 7) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 周期模式 error.\n");
                return false;
            }
        } else if (tmp.cyc.cycMode == 3) {
            if (sscanf(csv[j]["周期模式"].Str(), "%d,%d", &tmp.cyc.cycMonth[0],
                       &tmp.cyc.cycMonth[1]) != 2) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 周期模式 error.\n");
                return false;
            }
        } else {
        ...
        if (strlen(csv[j]["开始日期"].Str())) {
            if (sscanf(csv[j]["开始日期"].Str(), "%d-%d-%d", &tmp.cyc.iStartYear,
                       &tmp.cyc.iStartMonth, &tmp.cyc.iStartDay) != 3) {
        ...
        if (strlen(csv[j]["结束日期"].Str())) {
            if (sscanf(csv[j]["结束日期"].Str(), "%d-%d-%d", &tmp.cyc.iEndYear, &tmp.cyc.iEndMonth,
                       &tmp.cyc.iEndDay) != 3) {
        ...
        if (strlen(csv[j]["点1"].Str())) {
            if (sscanf(csv[j]["点1"].Str(), "%d,%d,%d", &tmp.pos1[0], &tmp.pos1[1], &tmp.pos1[2]) !=
                3) {
        ...
        if (strlen(csv[j]["点2"].Str())) {
            if (sscanf(csv[j]["点2"].Str(), "%d,%d,%d", &tmp.pos2[0], &tmp.pos2[1], &tmp.pos2[2]) !=
                3) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 点1 error.\n");
                return false;
            }
        ...
        if (strlen(csv[j]["点3"].Str())) {
            if (sscanf(csv[j]["点3"].Str(), "%d,%d,%d", &tmp.pos3[0], &tmp.pos3[1], &tmp.pos3[2]) !=
                3) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 点1 error.\n");
                return false;
            }
        ...
        if (strlen(csv[j]["点4"].Str())) {
            if (sscanf(csv[j]["点4"].Str(), "%d,%d,%d", &tmp.pos4[0], &tmp.pos4[1], &tmp.pos4[2]) !=
                3) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 点1 error.\n");
                return false;
            }
        ...
        if (strlen(csv[j]["点5"].Str())) {
            if (sscanf(csv[j]["点5"].Str(), "%d,%d,%d", &tmp.pos5[0], &tmp.pos5[1], &tmp.pos5[2]) !=
                3) {
                RtCoreLog().Info("CRS_FuBen::Load: parse 点1 error.\n");
                return false;
            }
...
bool CRS_CreModel::Load(const char* file) {
    ...
    for (j++; j < Total; j++) {
        ...
        if (sscanf(csv[j]["模型缩放"].Str(), "%f,%f,%f", &tmp.ModelScale[0], &tmp.ModelScale[1],
                   &tmp.ModelScale[2]) != 3) {
...
bool CRS_Npc::Load(const char* File) {
    ...
    for (j++; j < Total; j++) {
        ...
        if (sscanf(csv[j]["抗性"].Str(), "%d,%d,%d", &tmp.ElementArmor[0], &tmp.ElementArmor[1],
                   &tmp.ElementArmor[2]) != 3) {
...
bool CRS_Task::GetTaskKey(const char* str, vector<STaskKey>& key) {
    ...
    if (k && strlen(k) > 0) {
        while (k) {
            char buf[20][128];
            memset(buf, 0, 20 * 128);
            sscanf(k, "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s", buf[0], buf[1],
...
bool CRS_Task::Load(const char* File) {
    ...
    for (j++; j < Total; j++) {
        ...
        // gao
        if (sscanf(csv[j]["接受NPC坐标"].Str(), "%d,%d",
                   &tmp.m_posTaskBegin[0],  // NPC所在中地图坐标
...
void CRS_Task::GetTaskByFb(int fbId, int dif, vector<long>& taskId) {
    for (unsigned int i = 0; i < m_fbTable.size(); i++) {
        if (m_fbTable[i].fbId == fbId && m_fbTable[i].visDif <= dif)
            taskId.push_back(m_fbTable[i].id);
    }
}

void CRS_Task::GetTaskDescForMiddleMap(vector<vector<STaskDesc*>*>& vectorTaskDesc,
                                       map<string, int>&            stringToInt) {
    guard;
    map<string, int>::iterator it;
    for (unsigned int i = 0; i < m_table.size(); i++) {
...
bool CRS_SceneInfo::Load(const char* File) {
    ...
    for (j++; j < Total; j++) {
        ...
        pStr                   = csv[j]["本方重生点1"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirth1X), &(tmp.fRebirth1Y));
        pStr = csv[j]["本方重生点2"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirth2X), &(tmp.fRebirth2Y));
        pStr = csv[j]["本方重生点3"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirth3X), &(tmp.fRebirth3Y));
        pStr = csv[j]["对方重生点1"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fX_Oppose1), &(tmp.fY_Oppose1));
        pStr = csv[j]["对方重生点2"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fX_Oppose2), &(tmp.fY_Oppose2));
        pStr = csv[j]["对方重生点3"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fX_Oppose3), &(tmp.fY_Oppose3));
        pStr = csv[j]["红名重生点"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirthRedX1), &(tmp.fRebirthRedY1));
        pStr = csv[j]["红名重生点2"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirthRedX2), &(tmp.fRebirthRedY2));
        pStr = csv[j]["红名重生点3"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirthRedX3), &(tmp.fRebirthRedY3));
        pStr = csv[j]["红名重生点4"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirthRedX4), &(tmp.fRebirthRedY4));
        pStr = csv[j]["红名重生点5"].Str();
        sscanf(pStr, "%f;%f", &(tmp.fRebirthRedX5), &(tmp.fRebirthRedY5));
        tmp.iPKAttr  = csv[j]["PK属性"].Int();
        int iFaction = csv[j]["阵营"].Int();
        if (iFaction == 0)
            tmp.sceneFaction = FACTION_ZHOU;
        else if (iFaction == 1)
            tmp.sceneFaction = FACTION_SHANG;
        else if (iFaction == 3)
            tmp.sceneFaction = FACTION_NEUTRAL;
...
bool CTaskInfo::IsTaskGroupAccept(DWORD group) {
    if (group == 0 || group == -1)
        return false;  //tim.yang  日常任务

    list<STaskInfo>::iterator it;
    for (it = m_taskList.begin(); it != m_taskList.end(); it++) {
        if (/*PZH*/ /*it->IsDoing != 1*/ !it->FIsDoing())
            continue;
...
void CTaskInfo::CleanTask() {
    ...
    for (it = m_taskList.begin(); it != m_taskList.end();) {
        ...
        if (desc->repeat && (time(NULL) - (DWORD)it->LastFinishTime > (DWORD)desc->getTimeLimit) &&
            desc->group != -1) {
            it = m_taskList.erase(it);
            continue;
        }
...
bool CTaskInfo::SaveToString(string& str) {
    ...
    for (it = m_taskList.begin(); it != m_taskList.end(); it++) {
        ...
        if (desc->group == -1) {
			...
			@		str += g_strStaticBuffer;
#else
            rt2_sprintf(g_strStaticBuffer, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", (*it).Id,
                        (*it).FinishNum, (*it).IsDoing, (*it).BeginTime,
                        (*it).DayTimeTaskFinishTime, (*it).m_currMonsterNum, (*it).m_finditemID,
                        (*it).m_findnpcID, (*it).m_srcNpcID, (*it).m_killmonsterID,
                        (*it).dwUseItem);
            str += g_strStaticBuffer;
#endif
            //--------chg end   by tony 06.05.11--------------------//
			...
			@		str += g_strStaticBuffer;
#else
            rt2_sprintf(g_strStaticBuffer, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", (*it).Id,
                        (*it).FinishNum, (*it).IsDoing, (*it).BeginTime, (*it).m_currMonsterNum,
                        (*it).m_finditemID, (*it).m_findnpcID, (*it).m_srcNpcID,
...
int TRUMP_ActorSoulSec(SItemID& itemID, int rifine, STrump* pTrump) {
    ...
    if (ItemID_IsValid(itemID) && ItemIsTrump(itemID)) {
        ...
        //}
        if (rifine < 6) {
            return 100 * 1 * (int(itemID.params[TRUMP_LEV] / 20) + 1) * itemID.params[TRUMP_LEV] /
                   (4 + itemID.params[TRUMP_LEV]) / 10.f;
        }
        return 100 * 1 * (int(itemID.params[TRUMP_LEV] / 20) + 1) * itemID.params[TRUMP_LEV] /
               (4 + itemID.params[TRUMP_LEV]);
    } else {
        return 0;
    }
}
...
float TRUMP_CentiSoul(SItemID& itemID, int rifine) {
    ...
    if (ItemID_IsValid(itemID) && ItemIsTrump(itemID)) {
        switch (rifine) {
            ...
            case 4:
            case 5:
                return 0.1f * (float(itemID.params[TRUMP_LEV] / 20) + 1) *
                       float(itemID.params[TRUMP_LEV]);
                break;
            case 6:
                return 1.f * (float(itemID.params[TRUMP_LEV] / 20) + 1) *
                       float(itemID.params[TRUMP_LEV]);
                break;
            case 7:
                return 60.f;
