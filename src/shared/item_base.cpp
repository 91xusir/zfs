...
{
	guard;
	static RtString str[ITEM_ATTRIB_COUNT] = { "", "Con", "Str", "Dex", "Int", "Hit", \
		"G_CRE_ATTACK", "G_CRE_DODGE", "G_UNKNOWN", "G_CRE_DAMAGE", "G_CRE_ARMOR", \
		"G_CRE_ATTACK_RATE", "G_CRE_DODGE_RATE", "G_CRE_DAMAGE_RATE", "G_CRE_ARMOR_RATE", \
		"G_CRE_RATE_FIRE_ARMOR", "G_CRE_RATE_WATOR_ARMOR", "G_CRE_RATE_POISON_ARMOR", "G_CRE_RATE_FIRE_DAMAGE", "G_CRE_RATE_WATOR_DAMAGE", "G_CRE_RATE_POISON_DAMAGE", \
		"G_CRE_REQUIRE_STR", "G_CRE_REQUIRE_DEX", "G_CRE_REQUIRE_INT", "G_CRE_REQUIRE_HIT", \
		"AbsorbMp", \
		"AttSpeed", "AbsorbHp", "AbsorbMp", "G_CRE_MAX_HP", "G_CRE_MAX_MP", \
		"ConPower", "StrPower", "DexPower", "IntPower", "HitPower", "",\
		"MoveSpeed", "CriticalHit", \
		"G_CRE_DAMAGE_MIN", "G_CRE_DAMAGE_MAX", "G_CRE_DAMAGE_ABSORB", "G_CRE_LUCK", "Skill(unsupported yet)", \
		"AttRange", "G_MAXHP_RATE", "G_MAXMP_RATE", \
		"", "", \
		"G_SKILL_CRITICAL_RATE", "G_SKILL_CRITICAL_DAM",\
		"G_CRE_EXP_RATE","G_CRE_GOLD_RATE",\
		"G_CRE_REPARRY","G_CRE_REFAINT","G_CRE_RESKILLCRITICAL","G_CRE_RECRITICALHIT","",""};
...
{
	guard;
	if ( att1.attrib != att2.attrib )			return false;
	if ( att1.count != att2.count )				return false;
	if ( att1.reqStr != att2.reqStr )			return false;
	if ( att1.reqDex != att2.reqDex )			return false;
	if ( att1.reqInt != att2.reqInt )			return false;
	if ( att1.reqHit != att2.reqHit )			return false;
...
bool operator==(const SItemID &item1, const SItemID &item2)
{
	...
	if ( item1.cBind != item2.cBind )						return false;
	if ( item1.attribID1 != item2.attribID1 )				return false;
	if ( item1.attribID2 != item2.attribID2)				return false;
	if ( item1.attribRate1 != item2.attribRate1)			return false;
	if ( item1.attribRate2 != item2.attribRate2)			return false;
	if ( item1.enchantAttrib != item2.enchantAttrib)		return false;
    if ( item1.cLock != item2.cLock)                        return false;
    if ( item1.TimeLimit != item2.TimeLimit)                return false;
	if ( item1.paramCount != item2.paramCount)				return false;
	for (int i=0; i<item1.paramCount; i++)
		if (item1.params[i] != item2.params[i])				return false;
	if ( item1.curWear != item2.curWear)					return false;	
	if ( item1.Valid != item2.Valid)						return false;	
...
bool ItemID_HaveParams(DWORD type)
{
	...
	if (ItemIsPet(type) || ItemIsWeapon_Sword(type) || ItemIsWeapon_Ball(type) || ItemIsWeapon_Wand(type) || ItemIsWeapon_Wheel(type))
		return true;
	else 
		return false;
...
void SItemID::SaveToReadableString(CItemManager* pItemMgr, std::string &str)
{
	...
	rt2_sprintf(g_strItemBuffer10240, "id=%d;name=%s;itemID=%d;type=0x%x;color=%d;level=%d;count=%d;attr_blue=%d;attr_blueRate=%d;attr_golden=%d;attr_goldenRate=%d;attr_anchant=%d;cBind=%d;cLock=%d",
        (int)id,
        pItemClass->name,
        (int)itemID,
        (int)type,
...
const char* SItemID::SaveToString(CItemManager *pItemMgr)
{
    ...
    if (count > 1)      cCountable = 1;
    else                cCountable = 0;
    if (enchantAttrib)  cEnchant = 1;
    else                cEnchant = 0;
    if (TimeLimit)      cTimeLimit = 1;
    else                cTimeLimit = 0;
	...
	if (version==11)
    {
        ...
        if (color==ItemColor_Green)
        {
            ...
            else 
            {
                ...
                {
                    color = ItemColor_White;
                    RtCoreLog().Info("A green item's ATTRIB IS INVALID, and now change it white item!(item ID=%d, attrib=%d)\n", id, attribID1);
                }
            }
        ...
        else if (color==ItemColor_Blue)
        {
            ...
            else 
            {
                ...
                {
                    color = ItemColor_White;
                    RtCoreLog().Info("A blue item's ATTRIB(1) IS INVALID, and now change it white item!(item ID=%d, attrib=%d)\n", id, attribID1);
                }
                else if (attribID2!=0 && !pItemMgr->m_pAttribMgr->IsAttribExist(attribID2))
                {
                    RtCoreLog().Info("A blue item's ATTRIB(2) IS INVALID, and now change it white item!(item ID=%d, attrib=%d)\n", id, attribID2);
                    color = ItemColor_White;
                }
            }
        ...
        else if (color==ItemColor_Golden)
        {
            ...
            else 
            {
                ...
                {
                    color = ItemColor_White;
                    RtCoreLog().Info("A golden item's ATTRIB(1) IS INVALID, and now change it white item!(item ID=%d, attrib=%d)\n", id, attribID1);
                }
                else if (attribID2!=0 && !pItemMgr->m_pAttribMgr->IsAttribExist(attribID2))
                {
                    RtCoreLog().Info("A golden item's ATTRIB(2) IS INVALID, and now change it white item!(item ID=%d, attrib=%d)\n", id, attribID2);
                    color = ItemColor_White;
                }
            }
...
bool CItemComposeMgr::CheckCompose(DWORD ComposeId, CItemContainerBase* pItemContainer, int* pCanMakeCount/* = NULL*/)
{
...
bool CItemComposeMgr::LoadResource(const char* FileName)
{
	...
	for (i++; i<LineCount; i++)
	{
		...
		{
			RtCoreLog().Info("[CItemComposeMgr::LoadResource] Material ID and Count NOT pair! (Line %d)\n", i);
			return false;
		}
...
bool CItemDecomposeMgr::LoadResource(const char* FileName)
{
	...
	for (i++; i<LineCount; i++)
	{
		...
		if (!MaterialIDs || MaterialIDs[0]=='\0')
			return false;
		if ( (!MaterialNums_Green || MaterialNums_Green[0]=='\0') || (!MaterialNums_Blue || MaterialNums_Blue[0]=='\0') || (!MaterialNums_Golden || MaterialNums_Golden[0]=='\0') )
			return false;
		...
		if ( (counterID != counterNum_Green) || (counterID != counterNum_Blue) || (counterID != counterNum_Golden) )
		{
			RtCoreLog().Info("[CItemDecomposeMgr::LoadResource] Material ID and Count NOT pair! (Line %d)\n", i);
			return false;
		}
...
bool CSuitItemManager::LoadResource()
{
	...
	for (int i=1; i<nLineCount; i++)
	{
		...
		if (iter!=m_suits.end())
		{
			RtCoreLog().Info("[CSuitItemManager::LoadResource] Same ID found in SuitItem table!(line %d)", i);
			CHECK(0);
			return false;
		}
...
BYTE CAttribMgr::CreateAttribRate(ColorSelector color)
{
	...
	if (color==csGreen)
	{
		iRate = rand() % (ATTRIB_GREEN_MAX_RATE-ATTRIB_GREEN_MIN_RATE+1) + ATTRIB_GREEN_MIN_RATE;
	}
	...
	{
		iRate = rand() % (ATTRIB_GOLDEN_MAX_RATE-ATTRIB_GOLDEN_MIN_RATE+1) + ATTRIB_GOLDEN_MIN_RATE;
	}
...
bool CAttribMgr::LoadResource()
{
	...
	for (int i=titleLine+1; i<nLineCount; i++)
	{
		...
		if (iter!=m_attribs.end())
		{
			RtCoreLog().Info("[CAttribMgr::LoadResource] Same ID found in ItemAttrib table!(line %d)", i);
			CHECK(0);
			return false;
		}
...
bool SCreateItem::SaveToString(CItemManager* pItemMgr, std::string& outStr)
{
	...
	if (!IsValid(pItemMgr))
	{
		RtCoreLog().Info("[SCreateItem::SaveToString] Struct is Not invalid!(id=%d,color=%d,level=%d,cBind=%d)\n", index, color, level, cBind);
		return false;
	}
...
bool CItemManager::Init()
{
    ...
    if (m_BankInitLineCount <= 0 || m_BankMaxLineCount <= 0 || m_BankInitLineCount > MAX_BAG_LINE_COUNT || m_BankMaxLineCount > MAX_BAG_LINE_COUNT)
    {
        RtCoreLog().Info("[CItemManager::Init] Invalid BankInitLine or BankMaxLine\n");
        return false;
    }

	if ( !m_pIDManager->Init() )			return false;
	if ( !m_pAttribMgr->Init() )			return false;
	if ( !LoadResource() )					return false;
	if ( !m_pSuitManager->Init() )			return false;
	if ( !m_pSelectionTable->Init(this))	return false;
	if ( !m_pComposeMgr->Init())			return false;
	if ( !m_pDecomposeMgr->Init())			return false;
    if ( !m_pMagicEggRewardMgr->Init())     return false;
...
SItemID CItemManager::CreateItem(RtString name, EItemColor color/* = ItemColor_White*/, BYTE level/* = 0*/, EItemBind cBind/* = IB_NotBind*/, WORD ColorAttribID1/* = 0*/, BYTE ColorRate1/* = 0*/, WORD ColorAttribID2/* = 0*/, BYTE ColorRate2/* = 0*/, DWORD TimeLimit/* = 0*/)
{
	guard;
	...
	if ( !pItemBase )
		...
		return _CreateItem(pItemBase->type, pItemBase, color, level, cBind, ColorAttribID1, ColorRate1, ColorAttribID2, ColorRate2, TimeLimit);
	unguard;
}

SItemID CItemManager::CreateItem(DWORD index, EItemColor color/* = ItemColor_White*/, BYTE level/* = 0*/, EItemBind cBind/* = IB_NotBind*/, WORD ColorAttribID1/* = 0*/, BYTE ColorRate1/* = 0*/, WORD ColorAttribID2/* = 0*/, BYTE ColorRate2/* = 0*/, DWORD TimeLimit/* = 0*/)
{
	guard;
...
SItemID CItemManager::_CreateItem(DWORD type, SItemBase *pBase, EItemColor color /* = ItemColor_White */, BYTE level/* = 0*/, EItemBind cBind/* = IB_NotBind*/, WORD ColorAttribID1/* = 0*/, BYTE ColorRate1/* = 0*/, WORD ColorAttribID2/* = 0*/, BYTE ColorRate2/* = 0*/, DWORD TimeLimit/* = 0*/)
{
	guard;
...
bool CItemManager::GenerateCreateString(std::vector<SCreateItem>& arrCreateItem, std::string& outCreateString)
{
	guard;
...
bool CItemManager::AnalyseCreateString(std::string& CreateString, std::vector<SCreateItem>& outArrayCreateItem)
{
	guard;
	...
	for (i=0; i<(int)arrHeaderTokens.size(); i++)
	{
		...
		{
			RtCoreLog().Info("[CItemManager::AnalyseCreateString] Invalid token(ignored)!(token=%s)\n", token.c_str());
		}
	}
...
SItemID CItemManager::CreateItemByStruct(SCreateItem& ci)
{
	...
	SItemID item = CreateItem(ci.index, (EItemColor)ci.color, ci.level, ci.cBind, ci.colorAttrib1, ci.colorRate, ci.colorAttrib2, ci.colorRate, TimeLimit);
	return item;

	unguard;
}

bool CItemManager::InfuseColor(SItemID& item, EItemColor color, WORD attribID1/* = 0*/, BYTE ColorRate1/* = 0*/, WORD attribID2/* = 0*/, BYTE ColorRate2/* = 0*/)
{
	guard;

	if (!ItemID_IsValid(item))
		return false;
	if ( !(ItemIsWeapon(item.type) || ItemIsArmor(item.type) || ItemIsShipin(item.type) || ItemIsTrump(item.type)) )
		return false;
	...
	case ItemColor_Green:
		{
			...
			else
			{
				...
				{
					RtCoreLog().Info("[CItemManager::InfuseColor] Invalid ColorAttrib(%d)", attribID1);
					attribID1 = 0;
					return false;
				}
			}
			...
			if ( (attribID1 == 0) || (ColorRate1<ATTRIB_GREEN_MIN_RATE || ColorRate1>ATTRIB_GREEN_MAX_RATE) )
			{
				return false;
			}
			else
	...
	case ItemColor_Blue:
		{
			...
			else 
			{
				...
				{
					possIDs.clear();
					m_pAttribMgr->GetPossibleAttribs(pItemClass->grade, CAttribMgr::csBlue1, possIDs);
					for (i=0; i<possIDs.size(); i++)
					{
						if (attribID1 == possIDs[i])
							break;
				...
				{
					possIDs.clear();
					m_pAttribMgr->GetPossibleAttribs(pItemClass->grade, CAttribMgr::csBlue2, possIDs);
					for (i=0; i<possIDs.size(); i++)
					{
					...
					if (i==possIDs.size()) // 传进来的属性id没有找到，不合法
					{
						RtCoreLog().Info("[CItemManager::InfuseColor] Invalid ColorAttrib(%d)", attribID2);
						attribID2 = 0;
						return false;
					}
				}
			...
			if ( (attribID1==0 && attribID2==0) || ((ColorRate1<ATTRIB_BLUE_MIN_RATE || ColorRate1>ATTRIB_BLUE_MAX_RATE) && (ColorRate2<ATTRIB_BLUE_MIN_RATE || ColorRate2>ATTRIB_BLUE_MAX_RATE)) )
			{
				return false;
			}
			else 
	...
	case ItemColor_Golden:
		{
			...
			else 
			{
				...
				{
					possIDs.clear();
					m_pAttribMgr->GetPossibleAttribs(pItemClass->grade, CAttribMgr::csGolden1, possIDs);
					for (i=0; i<possIDs.size(); i++)
					{
						if (attribID1 == possIDs[i])
							break;
				...
				{
					possIDs.clear();
					m_pAttribMgr->GetPossibleAttribs(pItemClass->grade, CAttribMgr::csGolden2, possIDs);
					for (i=0; i<possIDs.size(); i++)
					{
						if (attribID2 == possIDs[i])
							break;
			...
			if ( (attribID1==0 && attribID2==0) || ((ColorRate1<ATTRIB_GOLDEN_MIN_RATE || ColorRate1>ATTRIB_GOLDEN_MAX_RATE) && (ColorRate2<ATTRIB_GOLDEN_MIN_RATE || ColorRate2>ATTRIB_GOLDEN_MAX_RATE)) )
			{
				return false;
			}
			else 
...
//	csv.SetTitleLine(1);
	...
	int bCanUpdate, bCanTrade, bCanDelete, bCanDrop, bMission, bCanPile, bCanNpcTrade, bCanNotFuse  = 0;
	int value, grade  = 0; 
	int itemCredit = 0;                             // 仙誉
	...
	for (int i=1; i<nLineCount; i++)
	{
		...
		if (iter!=m_mapIndexToItem.end())
		{
			RtCoreLog().Info("[CItemManager::LoadResource] Same item ID found in item table!(line %d,itemid %d)", i,index);
			CHECK(0);
			return false;
		}
		...
		if ( type==1 )						// 武器
		{
			...
			{
				RtCoreLog().Info("[Item.LoadResource] A weapon's CriticalRate is wrong! line %d! Ignored.\n", i);
     ...
     //               pArmor->UpdateSkins[counter][PATH_STRING_LEN-1];
			...
			if (pChar && pChar[0]!='\0')
			{
				...
				{
					rt2_strncpy(pArmor->UpdateEffects[ITEM_WEAPON_EFFECT_FROM_LEVEL+counter], k, PATH_STRING_LEN-1);
                    pArmor->UpdateEffects[ITEM_WEAPON_EFFECT_FROM_LEVEL+counter][PATH_STRING_LEN-1] = 0;
					counter ++;
					k = strtok(NULL, sep1);
				}
			}
		...
		else
		{
			...
			{
				int n = atoi(k);
				if (n > CAREER_REQUIREMENT_COUNT)		return false;
		...
		if (ItemCanPile(pItemBase->type))
		{
			...
			{
				RtCoreLog().Info("[CItemManager::LoadResource] Pile Item 初始使用次数/最大使用次数 Error! (Line %d)\n", i);
				return false;
			}
		}
...
:m_nLineCount(nLineCount), m_nColumnCount(nColumnCount), m_nPageCount(nPageCount), m_pIOnItemChange(NULL)
{
	guard;

	if (nLineCount>MAX_BAG_LINE_COUNT || nColumnCount>MAX_BAG_COLUMN_COUNT || nPageCount>MAX_BAG_PAGE_COUNT)
...
const char* CBag::SaveToString(CItemManager *pItemMgr)
{
	...
	rt2_sprintf(cTmp128, ",%d,%d,%d,%d,", nItemCountTemp, m_nPageCount, m_nLineCount, m_nColumnCount);
	rt2_strcat(g_strItemBuffer10240, cTmp128);
	for (int page=0; page < GetPageCount(); page++)
		for (int i=0; i < GetLineCount(); i++)
			for (int j=0; j < GetColCount(); j++)
			{
				if (nItemCounter==0)	goto lbSaveMoney;
...
int CBag::LoadFromString(const char* str, CItemManager *pItemMgr)
{
	...
	if (version==6)
	{
		...
		nColCount = atoi(k);
		k = strtok(NULL, sep);
		if (nPageCount!=m_nPageCount || nLineCount!=m_nLineCount || nColCount!=m_nColumnCount)
		{
...
bool CBag::Find(SItemID itemID, int *pOutPage /*= NULL*/, int *pOutI /*= NULL*/, int *pOutJ /*= NULL*/) const
{
	guard;
...
bool CBag::FindID(DWORD ID, int *pOutPage /*= NULL*/, int *pOutI /*= NULL*/, int *pOutJ /*= NULL*/) const
{
	guard;
...
bool CBag::Find(DWORD ID, int *pOutPage /*= NULL*/, int *pOutI /*= NULL*/, int *pOutJ /*= NULL*/) const
{
	guard;
	for (int p=0; p<m_nPageCount; p++)
		for (int i=0; i<m_nLineCount; i++)
...
bool CBag::FindType(DWORD typeID, EItemColor color/* = ItemColor_All*/, int *pOutCount/* = NULL*/, int *pOutPage/* = NULL*/, int *pOutI/* = NULL*/, int *pOutJ/* = NULL*/) const
{
	guard;
	int count = 0;
	bool bFirst = true;
...
bool CBag::FindTypeWithParams(DWORD typeID, std::vector<DWORD>& paramsIndices, std::vector<DWORD>& paramsCheck, EItemColor color/* = ItemColor_All*/, int *pOutCount/* = NULL*/, int *pOutPage/* = NULL*/, int *pOutI/* = NULL*/, int *pOutJ/* = NULL*/) const
{
	guard;
	...
	{
		RtCoreLog().Info("[CBag::FindTypeWithParams] Params Indices Count != Params Check Count!\n");
		return false;
	}
...
bool CBag::IsContain(vector<DWORD> &arrItemID) const
{
	...
	if ( arrItemID.empty() )			return true;
...
bool CBag::RemoveItem(DWORD typeID, EItemColor color/* = ItemColor_All*/, int count /*= -1*/)
{
	...
	// 删除数量不大于存在数量
	int itemCount;
	if ( !FindType(typeID, color, &itemCount) )		return false;
...
bool CBag::RemoveItemWithParams(DWORD typeID, std::vector<DWORD>& paramsIndices, std::vector<DWORD>& paramsCheck, EItemColor color/* = ItemColor_All*/, int count/* = -1*/)
{
	guard;
	...
	if (paramsIndices.size() != paramsCheck.size())
	{
		RtCoreLog().Info("[CBag::RemoveItemWithParams] Params Indices Count != Params Check Count!\n");
		return false;
	}
	int nCheckParamsCount = (int)paramsIndices.size();
	for (cc=0; cc<nCheckParamsCount; cc++)
	...
	// 删除数量不大于存在数量
	int itemCount;
	if ( !FindTypeWithParams(typeID, paramsIndices, paramsCheck, color, &itemCount))	return false;
	if ( itemCount < count )						return false;
...
bool CBag::MoveItem(CBag* fromContainer, int fromI, int fromJ, CBag* toContainer, int toI, int toJ)
{
	guard;
	return MoveItem(fromContainer, m_nCurrentPage, fromI, fromJ, toContainer, m_nCurrentPage, toI, toJ);
	unguard;
}
...
bool CBag::AddItem(SItemID& id, int *pOutPage/* = NULL*/, int *pOutI/* = NULL*/, int *pOutJ/* = NULL*/, bool bPileIfPossible/* = true*/)
{
	guard;
...
bool CBag::AddPileItem(SItemID& item, int *pOutPage/* = NULL*/, int *pOutI/* = NULL*/, int *pOutJ/* = NULL*/)
{
	guard;
	...
	for (int p=0; p<m_nPageCount; p++)
		for (int i=0; i<m_nLineCount; i++)
			...
			{
				SItemID id = _GetItem(p, i, j);
				if ( ItemCanPile(id)  &&  id.type==item.type  &&  id.count+item.count <= MAX_PILE_COUNT)
				{
					// 如果超过一个格子的数量就不添加
					SItemBase* pItemClass = m_pItemMgr->GetItem(id.type);
					if (!pItemClass)		return false;
					if ( (pItemClass->maxCount!=0) && (id.count+item.count > pItemClass->maxCount) )
						continue;
					// 检查参数相同才添加
					bool bRes1 = false;
...
/////////////////////////////////////////////////////
bool CBag::ResolveItem(CBag* fromContainer, int fromPage, int fromI, int fromJ, CBag* toContainer,  int toPage, int toI, int toJ,int Num /*分离数量*/)
{
	guard;
	...
	if (itemTo.id!=0 && (itemTo.type == itemFrom.type) && ItemCanPile(itemTo) && ItemCanPile(itemFrom) && (itemFrom.cBind == itemTo.cBind))
	{
...
/////////////////////////////////////////////
bool CBag::MoveItem(CBag* fromContainer, int fromPage, int fromI, int fromJ, CBag* toContainer, int toPage, int toI, int toJ)
{
	guard;
...
{
	if (!pBag || pBag->GetPageCount()!=m_nPageCount || pBag->GetLineCount()!=m_nLineCount || pBag->GetColCount()!=m_nColumnCount)
		return false;
...
bool operator==(const CBag &bag1, const CBag &bag2)
{
	...
	if ( pageCount!=bag2.GetPageCount() || lineCount!=bag2.GetLineCount() || columnCount!=bag2.GetColCount() )
		return false;
...
CItemContainerBase::CItemContainerBase(CItemManager *pManager, SCreature *pCre)
	:m_Bag(ITEM_PLAYER_INVENTORY_LINE, ITEM_PLAYER_INVENTORY_COLUMN, ITEM_PLAYER_INVENTORY_PAGE, pManager),
	m_MissionBag(ITEM_PLAYER_INVENTORY_LINE, ITEM_PLAYER_INVENTORY_COLUMN, ITEM_PLAYER_MISSION_PAGE, pManager),
...
bool CItemContainerBase::CheckEquip(EEquipParts vPart, SItemID& item, SCreature &cre) const
{
	...
	int iReqStr=0, iReqDex=0, iReqInt=0, iReqHit=0, iReqCon=0, iReqLevel=0, iReqMaxLevel=150;
	GetItemAttribRequirement(m_pItemManager, item, iReqStr, iReqDex, iReqInt, iReqHit, iReqCon, iReqLevel, iReqMaxLevel);
...
bool CItemContainerBase::Equip(SItemID& id, EEquipParts vPart/* = EQUIP_DEFAULT_PART*/, bool bRemoveFromBag/* = false*/, SItemID *pItemUnEquip1/* = NULL*/, EEquipParts *pPartUnEquip1/* = NULL*/, SItemID *pItemUnEquip2/* = NULL*/, EEquipParts *pPartUnEquip2/* = NULL*/)
{
	guard;
	...
	if (pItemUnEquip1)		pItemUnEquip1->Clear();
	if (pPartUnEquip1)		*pPartUnEquip1 = CItemContainerBase::EQUIP_DEFAULT_PART;
	if (pItemUnEquip2)		pItemUnEquip2->Clear();
	if (pPartUnEquip2)		*pPartUnEquip2 = CItemContainerBase::EQUIP_DEFAULT_PART;
...
int CItemContainerBase::GetUnEquipItemCount(SItemID &item, EEquipParts vPart/* = EQUIP_DEFAULT_PART*/)
{
	guard;
	...
	if (vPart==NECKLACE)
	{
		if (ItemID_IsValid(m_nItemID[NECKLACE]))			return 1;
		else												return 0;
	}
	else if (vPart==RING)
	{
		if (ItemID_IsValid(m_nItemID[RING]))				return 1;
		else												return 0;
	}
	else if (vPart==CHARM)
	{
		if (ItemID_IsValid(m_nItemID[CHARM]))				return 1;
		else												return 0;
	}
	else if (vPart==BODY)
	{
		if (ItemID_IsValid(m_nItemID[BODY]))				return 1;
		else												return 0;
	}
	else if (vPart==FOOT)
	{
		if (ItemID_IsValid(m_nItemID[FOOT]))				return 1;
		else												return 0;
	}
	else if (vPart==GLOVE)
	{
		if (ItemID_IsValid(m_nItemID[GLOVE]))				return 1;
		else												return 0;
	}
	else if (vPart==WEAPON_3)
	{
		if (ItemID_IsValid(m_nItemID[WEAPON_3]))			return 1;
		else												return 0;
	}
	else if (vPart==CAESTUS)
	{
		if (ItemID_IsValid(m_nItemID[CAESTUS]))				return 1;
		else												return 0;
	}
	else if (vPart==RING2)
	{
		if (ItemID_IsValid(m_nItemID[RING2]))				return 1;
		else												return 0;
	}
	else if (vPart==KITS)
	{
		if (ItemID_IsValid(m_nItemID[KITS]))				return 1;
		else												return 0;
	}
	else if (vPart==TRUMP)
	{
		if (ItemID_IsValid(m_nItemID[TRUMP]))				return 1;
		else												return 0;
	}
	else if (vPart==FASHION)
	{
		if (ItemID_IsValid(m_nItemID[FASHION]))				return 1;
		else												return 0;
	}
	else if (vPart==SEALEDBOOK)
	{
		if (ItemID_IsValid(m_nItemID[SEALEDBOOK]))				return 1;
		else												return 0;
	}
	else if (vPart==AEROCRAFT)
	{
		if (ItemID_IsValid(m_nItemID[AEROCRAFT]))				return 1;
		else												return 0;
	}
...
bool CItemContainerBase::GetEquipItem(EEquipParts vPart, SItemBase*& vrpItem, bool bIgnoreValid/* = false*/) const
{
	guard;
...
bool CItemContainerBase::IsContain(vector<DWORD> &arrItemID) const
{
	...
	{
		DWORD id = (*iter);
		if ( !m_Bag.Find(id) && !m_MissionBag.Find(id) && !m_PetBag.Find(id) )			return false;
	}
	return true;
...
bool CItemContainerBase::IsContain(vector<SItemID> &arrItems) const
{
	...
	{
		SItemID item = (*iter);
		if ( !m_Bag.Find(item) && !m_MissionBag.Find(item) && !m_PetBag.Find(item) )			return false;
	}
	return true;
...
bool CItemContainerBase::ExchangeItems(CBag* pBagFrom, CBag* pBagTo, vector<DWORD>& arrItems)
{
	...
	CBag bagFrom1(pBagFrom->GetLineCount(), pBagFrom->GetColCount(), pBagFrom->GetPageCount(), pBagFrom->m_pItemMgr);
	CBag bagTo1(pBagTo->GetLineCount(), pBagTo->GetColCount(), pBagTo->GetPageCount(), pBagFrom->m_pItemMgr);
...
bool CItemContainerBase::ExchangeItems(CBag* pBagFrom, CItemContainerBase* pConTo, vector<DWORD>& arrItems)
{
	guard;
	...
	conTo1.m_PetBag.Resize(pConTo->m_PetBag.GetPageCount(),pConTo->m_PetBag.GetLineCount(),pConTo->m_PetBag.GetColCount());
	BOOL_ENSURE(bagFrom1.CloneFrom(pBagFrom))
...
bool CItemContainerBase::ExchangeItems(CItemContainerBase* pCon1, vector<SItemID>& arrItems1, CItemContainerBase* pCon2, vector<SItemID>& arrItems2)
{
	guard;
	...
	conClone1.m_PetBag.Resize(pCon1->m_PetBag.GetPageCount(),pCon1->m_PetBag.GetLineCount(),pCon1->m_PetBag.GetColCount());
	conClone2.m_PetBag.Resize(pCon2->m_PetBag.GetPageCount(),pCon2->m_PetBag.GetLineCount(),pCon2->m_PetBag.GetColCount());
...
bool CItemContainerBase::LoseEquip(EEquipParts vPart, int nNum)
{
	...
	do
	{
		...
		if(!(ItemIsWeapon(m_nItemID[vPart]) || ItemIsArmor(m_nItemID[vPart]) || ItemIsShipin(m_nItemID[vPart])))
		{
			break;
		}
...
bool operator==(const CItemContainerBase &con1, const CItemContainerBase &con2)
{
	...
	if (con1.m_Bag != con2.m_Bag)		return false;
	if (con1.m_Bag != con2.m_Bag)		return false;
...
void ItemUpdate_GetExtraRequirement(SItemID& item, CItemManager *pItemMgr, float& fStr, float& fHit, float& fInt, float& fDex)
{
	...
	if (itemLevel<0)
	{
		...
		if ( (cTmp & 0x01) != 0x0)			    { metier = METIER_WARRIOR; } //"战士";
		else if ( (cTmp & 0x02) != 0x0)			{ metier = METIER_HUNTER; } //"女猎手";
		else if ( (cTmp & 0x04) != 0x0)			{ metier = METIER_TAOIST; } //"道士";
		else if ( (cTmp & 0x08) != 0x0)			{ metier = METIER_WIZARD; } //"术士";
...
void GetItemAttribRequirement(CItemManager* pItemMgr, SItemID& item, int& iStr, int& iDex, int& iInt, int& iHit, int& iCon, int& iLevel, int& iMaxLevel)
{
	guard;
	...
	for (; iter!=pItemMgr->m_mapIndexToItem.end(); iter++)
	{
		...
		for (i=0; i<ISL_COUNT; i++)
		{
			...
			if (pItemBase->selectLevel>=levelFrom && pItemBase->selectLevel<=levelTo)
			{
				...
				{
					Explain_ItemSelectionType((EItemSelectionType)j, type2, subtype2, metier2, str2);
...
void CItemSelectionTable::Explain_ItemSelectionType(EItemSelectionType ist, int& outType, int& outSubType, int& metier, std::string& str)
{
	guard;
	switch (ist)
	{
...
void CItemSelectionTable::Explain_ItemSelectionLevel(EItemSelectionLevel isl, int& outLevelFrom, int& outLevelTo, std::string& str)
{
	guard;
	...
	default:
		...
		outLevelTo = 0;
		str = "Invalid";
		RtCoreLog().Info("[CItemSelectionTable::Explain_ItemSelectionLevel] INVALID param\n");
		break;
	}
...
bool CItemSelectionTable::FindSelection(SItemSelector& sel, std::vector<DWORD>& outItemIDs)
{
	...
	{
		RtCoreLog().Info("[CItemSelectionTable::FindSelection] Invalid struct SItemSelector.level\n");
		return false;
	}
	if (sel.type<0 || sel.type>IST_COUNT)
	{
		RtCoreLog().Info("[CItemSelectionTable::FindSelection] Invalid struct SItemSelector.type\n");
		return false;
	}
...
bool CMagicEggRewardMgr::LoadResource(const char* FileName)
{
    ...
    for (iLine++; iLine<LineCount; iLine++)
    {
        ...
        {
            RtCoreLog().Info("[CMagicEggRewardMgr::LoadResource] Param count != rate count (line %d)\n", iLine);
            return false;
        }
        ...
        case MI_Item:
            {
                ...
                {
                    pReward->AddReward(MI_Item, arrRates[nTmp], arrParams[nTmp], GreenRate, BlueRate, GoldenRate,MaxCount);
                }
            }
            break;
        case MI_Exp:
		...
		case MI_ALL:
			{
				...
				for (nTmp = 0; nTmp < (int)arrParams.size(); nTmp++)
				{
					pReward->AddReward(MI_ALL, arrRates[nTmp], arrParams[nTmp],GreenRate, BlueRate, GoldenRate,MaxCount);
				}
			}
			break;
        default:
            RtCoreLog().Info("[CMagicEggRewardMgr::LoadResource] Invalid magicEgg type (line=%d)\n", iLine);
            return false;
        }
    }
    ...
    for (; iterReward != m_Rewards.end(); iterReward++)
    {
        ...
        {
            RtCoreLog().Info("[CMagicEggRewardMgr::LoadResource] 出现几率之和不等于100 (GroupID=%d,Rate=%d)\n", reward.GroupID, reward.CurrHitNum);
            return false;
        }
    }
					...
					for ( int pp = p; pp < m_nPageCount; ++pp )
					{
						for ( int ii = i; ii < m_nLineCount; ++ii )
						{
							...
							{   
								++iii;
								//找到A之后所有可跟A堆叠的物品
								if ( ItemCanPile( GetItem( pp, ii, jj ).type ) && (GetItem( pp, ii, jj ).type == GetItem( p, i, j ).type) && (GetItem( pp, ii, jj ).cBind == GetItem( p, i, j ).cBind) )
								{   //判断叠加后是否超出堆叠上限
...
void CBag::ItemUpdateQuickSort( std::vector< SItemID > & item, int left, int right )//背包整理快速排序 add by 李泽华 2010.05.10
{
	guard;
	int i( left ),j( right );
	SItemID middle;
