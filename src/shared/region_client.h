...
// -------------------------------------------------------------------
/* 对象类型 */
const unsigned char OBJECT_TYPE_OBJECT   = 1;  // 普通对象
const unsigned char OBJECT_TYPE_CREATURE = 2;  // 生物体（如怪物、玩家等）
const unsigned char OBJECT_TYPE_ITEM     = 3;  // 物品
const unsigned char OBJECT_TYPE_NPC_SERVICE = 4;  // 服务性质的NPC（提供任务、交易、仓库等）
const unsigned char OBJECT_TYPE_NPC_COMBATIVE  = 5;   // 战斗NPC（可以攻击和被攻击）
const unsigned char OBJECT_TYPE_SCENE          = 6;   // 场景
const unsigned char OBJECT_TYPE_SCENE_FB       = 7;   // 场景副本
const unsigned char OBJECT_TYPE_SCENE_WARFIELD = 8;   // 场景战场
const unsigned char OBJECT_TYPE_SPIRIT         = 9;   // 精灵
const unsigned char OBJECT_TYPE_ACTIVITY       = 10;  // 活动
const unsigned char OBJECT_TYPE_MAX = 11;  // 最大对象类型数（用于表示对象类型的上限）

/* 聊天频道类型 */
const char CHAT_CHANNEL_NORMAL     = 1;   // 普通聊天频道
const char CHAT_CHANNEL_TEAM       = 2;   // 队伍聊天频道
const char CHAT_CHANNEL_PARTY      = 3;   // 组队聊天频道
const char CHAT_CHANNEL_COUNTRY    = 4;   // 国家聊天频道
const char CHAT_CHANNEL_PUBLIC     = 5;   // 公共聊天频道
const char CHAT_CHANNEL_PRIVATE    = 6;   // 私人聊天频道
const char CHAT_CHANNEL_BATTLEINFO = 7;   // 战斗信息频道
const char CHAT_CHANNEL_SYSTEM     = 8;   // 系统聊天频道
const char CHAT_CHANNEL_GM         = 9;   // GM聊天频道
const char CHAT_CHANNEL_WORLD      = 10;  // 世界聊天频道
const char CHAT_CHANNEL_MAX = 11;  // 最大聊天频道数（不用于具体聊天，表示频道数量的上限）
const char CHAT_CHANNEL_NEWS = 12;  // 新闻频道
...
/* 命令类型 */
const unsigned char DO_COMMAND_GETSKIN     = 1;  // 获取皮肤
const unsigned char DO_COMMAND_GETMINE     = 2;  // 获取矿石
const unsigned char DO_COMMAND_GETMEDICINE = 3;  // 获取药品
const unsigned char DO_COMMAND_RIFINE      = 4;  // 提炼物品
const unsigned char DO_COMMAND_TASKCOLECT  = 5;  // 收集任务物品
const unsigned char DO_COMMAND_CATCHPET    = 6;  // 捕捉宠物

/* 同步标志 */
const long SYNC_FLAG_COMPLEX_MODEL = 0x00000001;  // 复杂模型
const long SYNC_FLAG_POSITION      = 0x00000002;  // 位置
const long SYNC_FLAG_DIRECTION     = 0x00000004;  // 方向
const long SYNC_FLAG_USER          = 0x00000008;  // 用户
const long SYNC_FLAG_PET           = 0x00000010;  // 宠物
const long SYNC_FLAG_SPIRIT        = 0x00000020;  // 精灵
const long SYNC_FLAG_WAR           = 0x00000040;  // 战争
const long SYNC_FLAG_BOSS          = 0x00000080;  // Boss
const long SYNC_FLAG_TRIGGER       = 0x00000100;  // 触发器
const long SYNC_FLAG_CALL          = 0x00000200;  // 调用

const long SYNC_FLAG_METIER_WARRIOR = 0x00000400;  // 战士
const long SYNC_FLAG_METIER_HUNTER  = 0x00000800;  // 猎人
const long SYNC_FLAG_METIER_WIZARD  = 0x00001000;  // 法师
const long SYNC_FLAG_METIER_TAOIST  = 0x00002000;  // 道士

/* 移动状态 */
const unsigned char MOVE_STATE_IDLE      = 0x01;  // 空闲
const unsigned char MOVE_STATE_WALK      = 0x02;  // 行走
const unsigned char MOVE_STATE_RIDE      = 0x04;  // 骑乘
const unsigned char MOVE_STATE_STATIC    = 0x08;  // 静态
const unsigned char PK_STATE_NORMAL      = 0x00;  // 白名 PK 状态
const unsigned char PK_STATE_YELLOW_NAME = 0x10;  // 黄名 PK 状态
const unsigned char PK_STATE_RED_NAME    = 0x20;  // 红名 PK 状态

/* 事件定义 */
const short EVENT_ATTACK        = 1;  // 攻击事件
const short EVENT_SET_POSITION  = 2;  // 设置位置事件
const short EVENT_LEVEL_UP      = 3;  // 升级事件
const short EVENT_BORN          = 4;  // 出生事件
const short EVENT_DEAD          = 5;  // 死亡事件
const short EVENT_STATIC_DIALOG = 6;  // 静态对话事件
const short EVENT_PLAY_POSE     = 7;  // 播放动作事件
const short EVENT_CHECK_PROCESS = 8;  // 检查进程事件
const short EVENT_CULLEVEL_UP   = 9;  // 级别提升事件
...
/* ride operate */
const char RIDE_OFF = 0;  // 不骑乘
const char RIDE_ON  = 1;  // 骑乘中
...
/*势力声望*/
const short XIANYU   = 1;  // 仙域
const short GUANFU   = 2;  // 官府
const short MANZU    = 3;  // 蛮族
const short YISHI    = 4;  // 异士
const short YIJUN    = 5;  // 疑军
const short LIANMENG = 6;  // 联盟

/* dialog define */
const short DIALOG_HTML      = 1;  // 对话框类型：HTML 对话框
const short DIALOG_NPC_TRADE = 2;  // 对话框类型：NPC 交易对话框

//extern char var[100];
/* region send to client */
const short r2c_base               = 1000;
const short r2c_remove_item        = r2c_base + 1;   // 移除物品
const short r2c_force_position     = r2c_base + 2;   // 强制位置
const short r2c_snapshot           = r2c_base + 3;   // 快照
const short r2c_levelup_skill      = r2c_base + 4;   // 技能升级
const short r2c_update_show_info   = r2c_base + 5;   // 更新显示信息
const short r2c_subsystem_team     = r2c_base + 6;   // 团队子系统
const short r2c_login_ret          = r2c_base + 7;   // 登录返回
const short r2c_gm_bulletin        = r2c_base + 8;   // GM公告（跑马灯）
const short r2c_warfield_view_rep  = r2c_base + 9;   // 战场视图报告
const short r2c_mw_add_ele         = r2c_base + 10;  // 增加元素
const short r2c_model_info         = r2c_base + 11;  // 模型信息
const short r2c_warfield_report    = r2c_base + 12;  // 战场报告
const short r2c_show_dialog        = r2c_base + 13;  // 显示对话框
const short r2c_subsystem_union    = r2c_base + 14;  // 联盟子系统
const short r2c_active_pet         = r2c_base + 15;  // 激活宠物
const short r2c_kick               = r2c_base + 16;  // 踢出
const short r2c_update_creature    = r2c_base + 17;  // 更新生物
const short r2c_chat               = r2c_base + 18;  // 聊天
const short r2c_subsystem_item     = r2c_base + 19;  // 物品子系统
const short r2c_name_state         = r2c_base + 20;  // 名称状态
const short r2c_switch_region      = r2c_base + 21;  // 切换区域
const short r2c_subsystem_mail     = r2c_base + 22;  // 邮件子系统
const short r2c_you_are_dead       = r2c_base + 23;  // 你已死亡
const short r2c_task_overhead      = r2c_base + 24;  // 任务悬浮
const short r2c_buy_card_ret       = r2c_base + 25;  // 购买卡片返回
const short r2c_shortcut           = r2c_base + 26;  // 快捷方式
const short r2c_task_event         = r2c_base + 27;  // 任务事件
const short r2c_system_message     = r2c_base + 28;  // 信息显示
const short r2c_load_config        = r2c_base + 29;  // 客户端配置加载
const short r2c_create_char        = r2c_base + 30;  // 创建角色
const short r2c_init               = r2c_base + 31;  // 初始化
const short r2c_learn_skill        = r2c_base + 32;  // 学习技能
const short r2c_ride_ret           = r2c_base + 33;  // 骑乘返回
const short r2c_refresh_skill      = r2c_base + 34;  // 刷新技能
const short r2c_update_object_info = r2c_base + 35;  // 更新对象信息
const short r2c_mw_add_mp          = r2c_base + 36;  // 增加魔法点
const short r2c_gm_response        = r2c_base + 37;  // GM响应
const short r2c_herolist           = r2c_base + 38;  // 英雄列表
...
const short r2c_update_cre_delta   = r2c_base + 42;  // 更新生物增量
const short r2c_adjust_position    = r2c_base + 43;  // 调整位置
const short r2c_change_model       = r2c_base + 44;  // 更改模型
...
const short r2c_refresh_effect     = r2c_base + 48;  // 刷新效果
const short r2c_finish_fb_task     = r2c_base + 49;  // 完成副本任务
const short r2c_warfield_msg       = r2c_base + 50;  // 战场消息
const short r2c_wg_checked         = r2c_base + 51;  // 检查到外挂
const short r2c_recover_pet_ret    = r2c_base + 52;  // 恢复宠物返回
// const short r2c_play_pose           = r2c_base + 45; // 原本的定义被注释掉
const short r2c_subsystem_castlewar = r2c_base + 53;  // 城堡战争子系统
const short r2c_show_clear_skill_ui = r2c_base + 54;  // 显示清除技能UI
const short r2c_play_effect         = r2c_base + 55;  // 播放特效
const short r2c_update_card_list    = r2c_base + 56;  // 更新卡片列表
const short r2c_update_skill_exp    = r2c_base + 57;  // 更新技能经验
const short r2c_subsystem_auction   = r2c_base + 58;  // 拍卖子系统
const short r2c_master_list         = r2c_base + 59;  // 师傅列表
const short r2c_player_info         = r2c_base + 60;  // 玩家信息
const short r2c_subsystem_feod      = r2c_base + 61;  // FEOD子系统
const short r2c_event               = r2c_base + 62;  // 事件
const short r2c_check_question      = r2c_base + 63;  // 发送图片给客户端，询问问题
const short r2c_wg_data             = r2c_base + 64;  // 外挂数据
const short r2c_save_circle_rep     = r2c_base + 65;  // 保存圆形报告
const short r2c_active_circle_rep   = r2c_base + 66;  // 激活圆形报告
//send reborn tell it zhe user kill monster num and rigelevel
const short r2c_killmonster_addrige = r2c_base + 67;  // 杀死怪物并记录击杀次数和等级
...
const short r2c_show_schedule = r2c_base + 103;  //客户端显示1进度条并持续进度指定时间
    //客户端显示1转盘，最多4个旋转选项，旋转结果根据参数值发送消息通知服务器
const short r2c_random_function     = r2c_base + 104;
...
const short c2r_base = 1000;  // 基础值

const short c2r_update_model      = c2r_base + 1;   // 更新模型
const short c2r_attack            = c2r_base + 2;   // 攻击
const short c2r_default_trigger   = c2r_base + 3;   // 默认触发器
const short c2r_subsystem_team    = c2r_base + 4;   // 队伍子系统
const short c2r_login             = c2r_base + 5;   // 登录
const short c2r_update_position   = c2r_base + 6;   // 更新位置
const short c2r_p_change_model    = c2r_base + 7;   // 更改模型
const short c2r_drop_item         = c2r_base + 8;   // 丢弃物品
const short c2r_subsystem_union   = c2r_base + 9;   // 联盟子系统
const short c2r_quit_game         = c2r_base + 10;  // 退出游戏
const short c2r_finish_fb_task    = c2r_base + 11;  // 完成副本任务
const short c2r_leave_fb          = c2r_base + 12;  // 离开副本
const short c2r_trigger           = c2r_base + 13;  // 触发器
const short c2r_ride              = c2r_base + 14;  // 骑乘
const short c2r_select_char       = c2r_base + 15;  // 选择角色
const short c2r_master_list       = c2r_base + 16;  // 师傅列表
const short c2r_system_auction    = c2r_base + 17;  // 系统拍卖
const short c2r_consign_card      = c2r_base + 18;  // 寄售卡
const short c2r_p_change_item     = c2r_base + 19;  // 更改物品
const short c2r_add_attr_point    = c2r_base + 20;  // 添加属性点
const short c2r_subsystem_item    = c2r_base + 21;  // 物品子系统
const short c2r_request_card_list = c2r_base + 22;  // 请求卡片列表
const short c2r_enter             = c2r_base + 23;  // 进入
const short c2r_mw_add_ele        = c2r_base + 24;  // 修炼法宝
const short c2r_print_my_data     = c2r_base + 25;  // 在server端打印玩家数据，用于调试
const short c2r_subsystem_feod    = c2r_base + 26;  // Feod子系统
const short c2r_exp_card          = c2r_base + 27;  // 经验卡
const short c2r_study_skill       = c2r_base + 28;  // 学习技能
const short c2r_report_info       = c2r_base + 29;  // 报告信息
const short c2r_get_item          = c2r_base + 30;  // 获取物品
const short c2r_play_pose         = c2r_base + 31;  // 播放姿势
const short c2r_teleport          = c2r_base + 32;  // 传送
const short c2r_cancel_task       = c2r_base + 33;  // 取消任务
const short c2r_query_player_info = c2r_base + 34;  // 查询玩家信息
const short c2r_get_model_info    = c2r_base + 35;  // 获取模型信息
const short c2r_herolist_query    = c2r_base + 36;  // 查询英雄列表
const short c2r_shortcut          = c2r_base + 37;  // 快捷方式
const short c2r_mw_add_mp         = c2r_base + 38;  // 法宝注入MP
const short c2r_enter_fb          = c2r_base + 39;  // 进入副本
const short c2r_charge_card       = c2r_base + 40;  // 充值卡
const short c2r_master_prentice   = c2r_base + 41;  // 师徒系统
const short c2r_request_clear_skill   = c2r_base + 42;  // 请求清除技能
const short c2r_interactive_npc       = c2r_base + 43;  // 互动NPC
const short c2r_recover_pet           = c2r_base + 44;  // 恢复宠物
const short c2r_query_task_overhead   = c2r_base + 45;  // 询问NPC头上的任务标志
const short c2r_warfield_leave        = c2r_base + 46;  // 战场离开
const short c2r_subsystem_mail        = c2r_base + 47;  // 邮件子系统
const short c2r_save_config           = c2r_base + 48;  // 客户端配置的存盘
const short c2r_set_pet_info          = c2r_base + 49;  // 设置宠物信息
const short c2r_chat                  = c2r_base + 50;  // 聊天
const short c2r_buy_card              = c2r_base + 51;  // 购买卡片
const short c2r_warfield_view         = c2r_base + 52;  // 战场视图
const short c2r_change_equip          = c2r_base + 53;  // 更改装备
const short c2r_warfield_enter        = c2r_base + 54;  // 战场进入
const short c2r_move_item             = c2r_base + 55;  // 移动物品
const short c2r_confirm_clear_skill   = c2r_base + 56;  // 确认清除技能
const short c2r_subsystem_castlewar   = c2r_base + 57;  // 战争子系统
const short c2r_confirm_switch_region = c2r_base + 58;  // 确认切换区域
// 开始行走, 在发送这个命令之前发送c2r_update_position会导致断开连接
const short c2r_start                = c2r_base + 59;
const short c2r_get_next_gift        = c2r_base + 60;  // 获取下一个礼物
const short c2r_check_answer         = c2r_base + 61;  // 回答服务器的问题
const short c2r_wg_data              = c2r_base + 62;  // WG数据
const short c2r_save_circle          = c2r_base + 63;  // 保存圆圈
const short c2r_active_circle        = c2r_base + 64;  // 激活圆圈
const short c2r_rage_burst           = c2r_base + 65;  // 怒气爆发
const short c2r_send_message         = c2r_base + 66;  // 发送消息
...
const char PART_HAND   = 3;  //手
const char PART_FOOT   = 4;  //脚
const char PART_WEAPON = 5;  //武器
const char PART_MAX    = 6;  //最大值
...
struct SCreature {
	...
	*/
    ...
    // 模型信息
    short       ModelId;       // 模型ID
    bool        ComplexModel;  // 是否为复杂模型
    short       HeadModelId;   // 头部模型ID
    std::string huiyuantime;   // 会员时间
    ...
    CAttrValue MaxHp;            // 最大生命值
    CAttrValue MaxMp;            // 最大魔法值
    ...
    bool CheckSkillDelta(SCreature& old) {
        if (Metier != old.Metier)
            return true;
        if (MetierLev != old.MetierLev)
            return true;
        if (Lev != old.Lev)
            return true;
        if (SkillPoint != old.SkillPoint)
            return true;
    ...
    void SetShieldHp(float v) {
        ShieldHp = v;
        CheckShieldHp();
    }

    void SetShieldMpR(int v) {
        ShieldMpR = v;
        CheckShieldMpR();
    }
    ...
    void AddShieldHp(float add) {
        ShieldHp += add;
        CheckShieldHp();
    }

    void AddShieldMpR(int add) {
        ShieldMpR += add;
        CheckShieldMpR();
    }
    ...
    //add by Tianh
    void CheckSoul() {
        if (Soul > GetSoulValueMax())
            Soul = GetSoulValueMax();
        else if (Soul < 0)
            Soul = 0;
    }
    ...
    void AddSoul(int add) {
        Soul += add;
        CheckSoul();
    }

    //4.8 LeiJun 属性脚本操作调用
    void SetPKValue(long v) {
        if (v < 0)
            Pk = 0;
        else
            Pk = v;
    ...
    //GradeLev
    int GetGradeStage() {
        return (GradeLev / 20 + 1);
    }  //Tianh 修为阶段  根据  修为阶段=int(修为等级/20+1) 算出来

    int GetGradeMaxLev() { return (std::min)(99, Lev + 4); }  //角色的最大修为等级
...
struct SNpc {
    ...
    //PZH
    char
        cTaskType;  //任务相关类型，0和任务没任何关系，1从这的任务列表接任务2从这随机接任务2这和跑环任务相关
    ...
    bool AddEventCommand(const char* sEvent, const char* sCommand, const char* sParam1 = 0,
                         const char* sParam2 = 0, const char* sParam3 = 0,
                         const char* sParam4 = 0) {
        if (!sEvent || !sCommand)
            return false;
...
class CRS_Task {
   public:
    ...
    void       GetTaskDescForMiddleMap(vector<vector<STaskDesc*>*>& vectorTaskDesc,
...
struct STaskInfo {
    ...
    DWORD Id;
    DWORD
    IsDoing;  //PZH 原来只有两种状态，1表示正在做，0表示不是正在做，现改成3状态，1表示正在做，3正在做并可完成了, 4别人共享给你的任务
    DWORD FinishNum;
    DWORD BeginTime;
    ...
    bool FIsDoing() { return 1 == IsDoing || 3 == IsDoing; }

    void FSetDoing() { IsDoing = 1; }
    ...
    void FSetCanFinish() { IsDoing = 3; }

    bool FIsCanFinish() { return 3 == IsDoing; }

    bool FIsShared() { return 4 == IsDoing; }

    void FSetShared() { IsDoing = 4; }
...
enum EFbTaskState { FBT_UNVALID, FBT_UNFINISH, FBT_FINISHED };
...
struct SPeriodicity {
    ...
    union {
        ...
        int cycMonth
            [2];  // 表示以月之中的时间范围, 开始结束都包括. 如: 20,25 表示每个月的20号到25号
    };
   ...
   public:
    ...
    SFuBen* FindPos(std::string SenceName) {
        for (int i = 0; i < (int)m_table.size(); i++)
            if (m_table[i].bornSceneBlock == SenceName)
                return &m_table[i];
        return NULL;
    }
...
int   TRUMP_ActorSoulSec(SItemID& itemID, int rifine,
                         STrump* pTrump = NULL);  // 修炼法宝时人物灵力每秒减的值
float TRUMP_CentiSoul(
    SItemID& itemID,
    int rifine);  // 法宝各种灵力值提升百分点所需的人物灵力值	6法宝灵力,7元气值,0-5法宝附加元素
...
// help function
bool TestAttackRange(long attRange, long srcPos[3], SCreModel* srcModel, long tarPos[3],
                     SCreModel* tarModel);
...
struct SMailEx {
   public:
    enum EReceiverType { RT_All, RT_Selected, RT_Count };
                ...
                case RT_Selected:
                    *pPacket << counter;
                    for (i = 0; i < counter && counterForSafe > 0; i++, counterForSafe--) {
                        *pPacket << nTmp;
                        arrReceiverID.push_back(nTmp);
                    }
                    *pPacket << counter;
                    for (i = 0; i < counter && counterForSafe > 0; i++, counterForSafe--) {
                        *pPacket << strTmp;
                        arrReceiverName.push_back(strTmp);
                    }
                    *pPacket << counter;
                    for (i = 0; i < counter && counterForSafe > 0; i++, counterForSafe--) {
                        *pPacket << strTmp;
                        arrReceiverUnionName.push_back(strTmp);
                    }
                    break;
                default:
                    return false;
...
struct SUserAttr {
    ...
    friend CG_CmdPacket& operator<<(CG_CmdPacket& cmd, SUserAttr& attr) {
        ...
        cmd << attr.x << attr.y;
        cmd << attr.Cul << attr.CulExp << attr.Soul << attr.Contribute << attr.Point << attr.Record
            << attr.GradeLev;
...
struct SUserData {
    ...
    void Serialize(CG_CmdPacket& packet, bool download) {
        ...
        packet << extraData << popularity << isFirstLogin << isDead << ChangeModelID
               << WarScores /*<<UpdateSCDTime*/;
        for (int ii = 0; ii < SCRIPT_FUNC_STATE_COUNT; ii++)
            packet << lFunctionState[ii];

        if (download) {
            packet << userID << name << account << huiyuanTime << isVip
                   << bAdult;  //tim.yang  会员时间
            packet << unionID << unionName << unionNickName << unionIcon;
            packet << seed << privileges << cSwitchRegion << bankPass;
            packet << headModelID << lastUpdateTime << onlineTime << bTeamFree << popularity
                   << isFirstLogin << isDead;
...
class CIniVariable {
   public:
    ...
    //字符串
    CIniVariable(RtIni* vpIni, const string& vrSection, const string& vrEntry,
                 const char** vppOut) {
