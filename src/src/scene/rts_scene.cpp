...
void RtScene::SetupWaterShader(BYTE cWaterType)
{
    ...
    if (iIdx<0 || iIdx>=MAX_WATER_COUNT) return;
...
void RtScene::RequestBlockReady(const RtgVertex3& _Pos, bool bForcePreLoad /* = false */)
{
    ...
    if ((blockMap = GetBlockMapbyIndex(_Indexx, _Indexy)) && blockMap->Check_CanLoad())
    {
        ...
        m_SceneBlockManager.CreateSceneBlock(
            blockMap, 
            m_szFilePath,
            _Indexx - m_iCenterPosX,
            _Indexy - m_iCenterPosY,
            true);
    ...
    {
        if ((blockMap = GetBlockMapbyIndex(_plIndex[i].x, _plIndex[i].y)) && blockMap->Check_CanLoad())
        {
            m_SceneBlockManager.CreateSceneBlock(
                blockMap, 
                m_szFilePath, 
                _plIndex[i].x - m_iCenterPosX,
                _plIndex[i].y - m_iCenterPosY,
                bForcePreLoad);
...
void RtScene::RequestBlockObject(const RtgVertex3& _Pos)
{
    ...
    GetGridByPos(_Pos.x - m_fLoadObjectDistance, _Pos.y - m_fLoadObjectDistance, _gridMinx, _gridMiny);
    GetGridByPos(_Pos.x + m_fLoadObjectDistance, _Pos.y + m_fLoadObjectDistance, _gridMaxx, _gridMaxy);
...
void RtScene::RequestEnvUpdate(const RtgVertex3& _Pos)
{
    ...
    if ((_Pos.x - _block->m_vMin.x) < s_fBlockBoundaryWidth)
    {
        if (_horBlock = GetBlockMapbyIndex(_blockx - 1, _blocky))
            r1 = (s_fBlockBoundaryWidth - (_Pos.x - _block->m_vMin.x)) / (s_fBlockBoundaryWidth * 2.f);
        bleft = true;
    }
    else if ((_block->m_vMax.x - _Pos.x) < s_fBlockBoundaryWidth)
    {
        if (_horBlock = GetBlockMapbyIndex(_blockx + 1, _blocky))
            r1 = (s_fBlockBoundaryWidth - (_block->m_vMax.x - _Pos.x)) / (s_fBlockBoundaryWidth * 2.f);
        bleft = false;
    }
    ...
    if ((_Pos.y - _block->m_vMin.y) < s_fBlockBoundaryWidth)
    {
        if (_velBlock = GetBlockMapbyIndex(_blockx, _blocky - 1))
            r2 = (s_fBlockBoundaryWidth - (_Pos.y - _block->m_vMin.y)) / (s_fBlockBoundaryWidth * 2.f);
        btop = true;
    }
    else if ((_block->m_vMax.y - _Pos.y) < s_fBlockBoundaryWidth)
    {
        if (_velBlock = GetBlockMapbyIndex(_blockx, _blocky + 1))
            r2 = (s_fBlockBoundaryWidth - (_block->m_vMax.y - _Pos.y)) / (s_fBlockBoundaryWidth * 2.f);
        btop = false;
    }
                ...
                for (int j = 0; j < tileMap->mapCnt; ++j)
                {
                    ...
                    if (!m_texMaps[rmId].texItem)
                    {
                        m_texMaps[rmId].texItem = 
                            RtGetRender()->GetTextMgr()->CreateTextureFromFile(m_texMaps[rmId].szFile,
                            D3DX_DEFAULT,
                            D3DX_DEFAULT,
                            D3DX_DEFAULT,
                            D3DFMT_FROM_FILE,
                        ...
                        if (!m_texMaps[rmId].texItem)
                        {
                            m_texMaps[rmId].texItem = RtGetRender()->GetTextMgr()->GetChessboardText();
...
void RtScene::DrawTerrain(RtgCamera& inCamera, BYTE cAttrMask, DWORD dwNormalColor, DWORD dwHitColor, float fOffset)
{
    int x, y, i, j;
    ...
    for (gridlist_t::iterator it = m_listRenderGrid.begin(); it != m_listRenderGrid.end(); ++it)
    {
        ...
        for (y=0; y<RTS_BLOCK_GRID_SIZE; y++)
        {
            for (x=0; x<RTS_BLOCK_GRID_SIZE; x++)
            {
                for (i=0; i<24; i++)
                {
                    SGetTerrainVertex(pGrid->pBlock, iTriangleIdx[i], x+pGrid->x, y+pGrid->y, pV[i].v);
                    pV[i].v.z += fOffset;
                }
                for (i=0; i<4; i++)
                {
                    ...
                    BYTE b = pGrid->pBlock->GetTerrainAttr((x+pGrid->x)*2+i%2, (y+pGrid->y)*2+i/2);
                    ...
                    {
                        if (!pGrid->pBlock->CanWalk((x+pGrid->x)*2+i%2, (y+pGrid->y)*2+i/2))
                        {
                            dwColor = 0xff0000ff;
                        }
                    }
        ...
        m_pDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, pVertices, sizeof(RtgVertexVC), dwVertexCount/3);
...
void RtScene::ProcessSM(RtgTextItem* texSrc, int sizSrc, RtgTextItem* texDst, int sizDst)
{
    ...
    if (RtGetRender()->GetTextMgr()->Lock(&lockSrc, texSrc))
    {
        ...
        {
            _PiexlAccess<DWORD> ps = _PiexlAccess<DWORD>(lockSrc.data, lockSrc.width, lockSrc.height, lockSrc.pitch);
            _PiexlAccess<BYTE> pd = _PiexlAccess<BYTE>(lockDst.data, lockDst.width, lockDst.height, lockDst.pitch);
...
namespace {
    ...
    struct SM_TexCoordTrans
    {
        ...
        void Render(RtgDevice* _dev, RtgTextItem* texItem)
        {
            ...
            BYTE* pvb = (BYTE*)dev->GetBuffMgr()->Lock_Shared_vb(xNum * yNum * 4 * sizeof(VertexT), &vbItem);
            WORD* pib = (WORD*)dev->GetBuffMgr()->Lock_Shared_ib(xNum * yNum * 6 *sizeof(WORD), &ibItem);
...
void RtScene::BuildShadowMap(const Int2& _blockIndex)
{
    ...
    RtgTextItem* texColor = 
        RtGetRender()->GetTextMgr()->CreateTexture(
        sizeSM, sizeSM, 1, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, D3DUSAGE_RENDERTARGET);
    RtgTextItem* texDepth = 
        RtGetRender()->GetTextMgr()->CreateTexture(
    ...
    if (!pBlock->m_texItemSM)
        if (!(pBlock->m_texItemSM = RtGetRender()->GetTextMgr()->CreateTexture(sizeSMMap, sizeSMMap, 1, D3DFMT_L8)))
            return;

    float rg = sqrtf((pBlock->m_vMax.x - pBlock->m_vMin.x) * (pBlock->m_vMax.x - pBlock->m_vMin.x) +
        (pBlock->m_vMax.y - pBlock->m_vMin.y) * (pBlock->m_vMax.y - pBlock->m_vMin.y)) * 0.8f; 
    ...
    for (int i = 0; i < g_iSceneTileCntX * g_iSceneTileCntY * 4; ++i)
    {
        ...
        tct.SetUV(i, v4.x / v4.w * 0.5f + 0.5f + uvOft.x, 1.f - (v4.y / v4.w * 0.5f + 0.5f) + uvOft.y);
        v.uvSM.Set(v4.x / v4.w * 0.5f + 0.5f + uvOft.x, 1.f - (v4.y / v4.w * 0.5f + 0.5f) + uvOft.y);
    }
    ...
    if ((eMask & RTGRM_BLENDOBJ_ONLY) && m_bShowBlockFlag && (!m_bRenderMiniMapMode))
    {
        if (m_fShowBlockFlagTime > 0.f)
        {
            if ((s_fBlockFlagShowTime-m_fShowBlockFlagTime)<s_fBlockFlagShowTimeIn)
                m_shadowBlockFlag.dwTextureFactor =  (((int)((s_fBlockFlagShowTime-m_fShowBlockFlagTime)/s_fBlockFlagShowTimeIn*255.f))<<24)|0x00FFFFFF;
            else if (m_fShowBlockFlagTime<s_fBlockFlagShowTimeOut)
                m_shadowBlockFlag.dwTextureFactor =  (((int)(m_fShowBlockFlagTime/s_fBlockFlagShowTimeOut*255.f))<<24)|0x00FFFFFF;
            else
...
void RtScene::RecreateGrass(RtSceneBlock* block)
{
    ...
    int grid_num = (g_iSceneTileCntX/RTS_BLOCK_GRID_SIZE)*(g_iSceneTileCntY/RTS_BLOCK_GRID_SIZE);
        ...
        for (int y = 0; y < RTS_BLOCK_GRID_SIZE; ++y)
        {
            for (int x = 0; x < RTS_BLOCK_GRID_SIZE; ++x)
            {
                ...
                if (tileMap->grassDensity > 0)
                {
                    ...
                    CreateGrass(tileMap, 
                        pGrid->pBlock,
                        pGrid->pBlock->m_pVertices + tileId * 4, 
                        tileMap->grassId, 
                        tileMap->grassDensity, 
                        tileMap->minH,
                        tileMap->maxH,
                        tileMap->aspectW
                        );
...
void RtScene::CreateGrass(RtsSTileMap* tile, RtSceneBlock* block, RtsSVertex* v, WORD tid, WORD density, float minH, float maxH, float aspect)
{
    RTASSERT(block);
    RTASSERT(tile);
    RTASSERT(v);
    ...
    for (int i = 0; i < density; ++i)
    {
        ...
        if (rand() > RAND_MAX / 2)
        {
            pos = v[0].pPos->v + ((v[1].pPos->v + (v[2].pPos->v - v[1].pPos->v) * f0) - v[0].pPos->v) * f1;
            dir = n0;
        }
        else
        {
            pos = v[1].pPos->v + ((v[2].pPos->v + (v[3].pPos->v - v[2].pPos->v) * f0) - v[1].pPos->v) * f1;
            dir = n1;
        }
        ...
        RtgVertex3 rgt[] = { 
            RtgVertex3(cosf(rad), sinf(rad), 0.f),
            RtgVertex3(cosf(rad + PI * 0.5f), sinf(rad + PI * 0.5f), 0.f)
        };

        RtgVertex3 _Color0 = dir.Dot(-m_lightDirect.vDirection) * 
            RtgColorToVector(blockMap->m_dwSunLighting) + RtgColorToVector(blockMap->m_dwEnvironmentLighting);
        RtgVertex3 _Color1;
...
void RtScene::UpdateRenderGridList(RtgCamera& _cam)
{
    ...
    if ((blockMap = GetBlockMapbyIndex(_Indexx, _Indexy)) && blockMap->GetState() == Object_State_Ready)
    ...
    for (size_t i = 0; i < _plIndexNum; ++i)
    {
        if ((blockMap = GetBlockMapbyIndex(_plIndex[i].x, _plIndex[i].y)) && 
            ...
            _cam.CheckVisable(((RtSceneBlock*)blockMap->m_pBlock)->m_vMin,((RtSceneBlock*)blockMap->m_pBlock)->m_vMax))
        {
            ((RtSceneBlock*)blockMap->m_pBlock)->GetRenderGrid(&m_listRenderGrid, _cam);
        }
    }
...
bool RtScene::PreRender(RtgCamera& inCamera, RtSceneBlock* pBlock)
{
    ...
    for (gridlist_t::iterator it = m_listRenderGrid.begin(); it != m_listRenderGrid.end(); ++it)
    {
        ...
        else
        {
            for (int i = 0; i < pGrid->aryObject.size(); ++i)
            {
                ...
                if (obj->IsKindOf(RT_RUNTIME_CLASS(RtsActor)))
                {
                    ...
                    if (bReflect && (GetTerrainMapIdx(obj->GetMatrix()._30, obj->GetMatrix()._31) > 200.f))
                    {
        ...
        {
            for (list<RtsDynamicLight*>::iterator it = pGrid->dLight.begin(); it != pGrid->dLight.end(); ++it)
            {
...
void RtScene::RenderSceneWater(RtgCamera& inCamera)
{
    ...
    if (m_bRenderSceneWater && m_pWaterRenderMapHead)
    {
        ...
        RTGVertexFormat eWaterRenderVertexFormat = m_bOptionWaterCatoptricSky ? RTG_VNCT2 : RTG_VNCT;
        DWORD dwWaterRenderVertexStride          = sizeof(float)*8+sizeof(DWORD);
        if (m_bOptionWaterCatoptricSky)          dwWaterRenderVertexStride += sizeof(float)*2;
        ...
        while (pCurMap)
        {
            ...
            while (iCurTile>=0)
            {
                ...
                for (k=0; k<4; k++)
                {
                    ...
                    {
                        ((DWORD*)pV)[6] = RtgAddVertexColor2(pVertex->pPos->v, vWaterNormal, pVertex->pPos->c, pCurMap->dwColor, pCurMap->fColorRate, m_pFirstDynamicLight);
                        ((DWORD*)pV)[6] = (((DWORD*)pV)[6]&0x00FFFFFF) | (pVertex->pPos->cWaterAlphaMap<<24);
                    }else
                    {
                        ((DWORD*)pV)[6] = RtgAddVertexColor2(pVertex->pPos->v, vWaterNormal, pVertex->pPos->c, pCurMap->dwColor, pCurMap->fColorRate, NULL);
                        ((DWORD*)pV)[6] = (((DWORD*)pV)[6]&0x00FFFFFF) | (pVertex->pPos->cWaterAlphaMap<<24);
                        //((DWORD*)pV)[6] = (pVertex->pPos->c&0x00FFFFFF) | (pVertex->pPos->cWaterAlphaMap<<24);
                    }

                    memcpy(pV+sizeof(RtgVertexVNC), fWaterUV[(sy)%iWaterUVSpan+k/2][(sx)%iWaterUVSpan+k%2], sizeof(float)*2);
                    if (m_bOptionWaterCatoptricSky)
                    {
                        memcpy(pV+sizeof(RtgVertexVNC)+sizeof(float)*2, fWaterUV[(sy)%iWaterUVSpan+k/2][(sx)%iWaterUVSpan+k%2], sizeof(float)*2);
                    }
                    pVertex ++;
    ...
    for (gridlist_t::iterator it = m_listRenderGrid.begin(); it != m_listRenderGrid.end(); ++it)
    {
        ...
        for (y=0; y<RTS_BLOCK_GRID_SIZE; y++)
        {
            for (x=0; x<RTS_BLOCK_GRID_SIZE; x++)
            {
                ...
                if (pTileMap->cWaterType==0) continue;
                ...
                if (m_iWaterRenderTileCur>=m_iRenderTileCount)
                {
                    ...
                    memcpy(pTempRenderTile, m_pRenderTile, sizeof(RtsSRenderTile)*m_iWaterRenderTileCur);
                    DEL_ARRAY(m_pRenderTile);
...
bool RtScene::LoadEnvironmentInfo(const char* szFilename)
{
    ...
    *pArc << m_pDevice->m_pCamera->m_fFarPlane;
    m_pDevice->m_pCamera->SetProjParams(
        m_pDevice->m_pCamera->m_fFOV,
        m_pDevice->m_pCamera->m_fAspect,
        m_pDevice->m_pCamera->m_fNearPlane,
        m_pDevice->m_pCamera->m_fFarPlane);
...
{
    if (m_lightSkyColor==dwLightColor) return;
...
RtsObject* RtScene::PickLine(const RtgVertex3 &v0, const RtgVertex3 &v1, float& fOutDist, int& iOutX, int& iOutY, DWORD dwType)
{
...
RtsObject* RtScene::PickRay(const RtgVertex3 &v, const RtgVertex3 &dir, float& fOutDist, int& iOutX, int& iOutY, DWORD dwType)
{
...
bool RtScene::GetHeight(const RtgVertex3 &v, float& fOutDist, int& iOutX, int& iOutY, DWORD dwType)
{
...
void RtScene::GetSortTileMapIdx(RtsSTileMap& tileMapIdx)
{
    ...
    tileMapIdx.dwSortMapIdx = ((bit[3]&0x0FF)<<24) | ((bit[2]&0x0FF)<<16) | ((bit[1]&0x0FF)<<8) | (bit[0]&0x0FF);

    tileMapIdx.mapCnt = 1;
    if (bit[0]!=bit[1]) tileMapIdx.mapCnt ++;
    if (bit[1]!=bit[2]) tileMapIdx.mapCnt ++;
    if (bit[2]!=bit[3]) tileMapIdx.mapCnt ++;
    ...
    {
        uv[i] = 0;
        if (tileMapIdx.mtl[0]==bit[i]) uv[i] |= 0x01;
        if (tileMapIdx.mtl[1]==bit[i]) uv[i] |= 0x02;
        if (tileMapIdx.mtl[2]==bit[i]) uv[i] |= 0x04;
        if (tileMapIdx.mtl[3]==bit[i]) uv[i] |= 0x08;
    }
...
RtSceneBlock* RtScene::GetBlockByPos(float fPosX, float fPosY)
{
    ...
    if (fPosX<0.f || fPosY<0.f) return NULL;
...
RtsSVertexValue* RtScene::GetVertexValue(int x, int y, bool bEdit)
{
    ...
    if (pBlockMap->GetState() != Object_State_Ready)
        return NULL;
    if (bEdit) ((RtSceneBlock*)pBlockMap->m_pBlock)->SetModifyFlag(true);
    return ((RtSceneBlock*)pBlockMap->m_pBlock)->m_pPositions + ((y-iBy*g_iSceneTileCntY)*(g_iSceneTileCntX+1)+(x-iBx*g_iSceneTileCntX));
}
...
BYTE RtScene::GetVertexHeightLevel(int x, int y, bool bTile)
{
    ...
    if (pV)
    {
        if (bTile)  return pV->cHeightMap;
        else        return pV->cTerrainHMap;
    }
    return 0;
}
...
void RtScene::SetVertexHeightLevel(int x, int y, bool bTerrain, bool bTile, BYTE cHeightLevel)
{
    ...
    if (pV)
    {
        if (bTerrain)   pV->cTerrainHMap = cHeightLevel;
        if (bTile)      pV->cHeightMap   = cHeightLevel;
    }
}
RtsSVertex* RtScene::GetBlockVertex(int x, int y)
{
    ...
    if (pBlockMap->GetState() != Object_State_Ready)
        return NULL;
    return ((RtSceneBlock*)pBlockMap->m_pBlock)->m_pVertices + ((y-iBy*g_iSceneTileCntY)*g_iSceneTileCntX+(x-iBx*g_iSceneTileCntX))*4;
}
...
RtsSTileMap* RtScene::GetTileMap(int iX, int iY)
{
    ...
    return pBlock->m_pTileMapIdx+((iY-(iBy-m_iCenterPosY)*g_iSceneTileCntY)*g_iSceneTileCntX+(iX-(iBx-m_iCenterPosX)*g_iSceneTileCntX));
}


void RtScene::SetTileMapGrass(int iX, int iY, int Density, WORD tid, RtgVertex2 szh, float AspectW)
{
    int iBx = 0;
    int iBy = 0;
    ...
    int tileId = ((iY - (iBy - m_iCenterPosY) * g_iSceneTileCntY) * g_iSceneTileCntX + (iX - (iBx - m_iCenterPosX) * g_iSceneTileCntX));
    RtsSTileMap* pMap = pBlock->m_pTileMapIdx + tileId;
   
    if (pMap)
    {  
        ClearGrass(pMap);
        CreateGrass(pMap, pBlock, pBlock->m_pVertices + tileId * 4, tid, Density, szh.x, szh.y, AspectW);
...
void RtScene::SetTileMapMtl(int iX, int iY, BYTE cMtl, DWORD dwFlag)
{
    ...
    RtsSTileMap* pMap = pBlock->m_pTileMapIdx+((iY-(iBy-m_iCenterPosY)*g_iSceneTileCntY)*g_iSceneTileCntX+(iX-(iBx-m_iCenterPosX)*g_iSceneTileCntX));
    if (pMap)
    {
        pBlock->SetModifyFlag(true);
        if (dwFlag&0x01) pMap->mtl[0] = cMtl;
        if (dwFlag&0x02) pMap->mtl[1] = cMtl;
        if (dwFlag&0x04) pMap->mtl[2] = cMtl;
        if (dwFlag&0x08) pMap->mtl[3] = cMtl;
...
void RtScene::ComputeVertexColor(int iX, int iY)
{
    ...
    if (pBlock)
    {
        pBlock->ComputeVertexColor(iX+(m_iCenterPosX-iBx)*g_iSceneTileCntX, iY+(m_iCenterPosY-iBy)*g_iSceneTileCntY);
...
bool RtScene::AddObject(objectId _objId)
{
    ...
    ma.Translate(pObject->Get_AABBox()->vPos.x, pObject->Get_AABBox()->vPos.y, pObject->Get_AABBox()->vPos.z);
    mb.Unit();
...
bool RtScene::TakeGeometry(const RtgAABB& inAABB, RtgVertex3* pVertices, int& iVertexNum, short* pIndices, int& iIndexNum)
{
    ...
    if (iBMinX>=m_iBlockXCnt) iBMinX = m_iBlockXCnt-1;
    if (iBMinY>=m_iBlockYCnt) iBMinY = m_iBlockYCnt-1;
    ...
    if (iBMaxX>=m_iBlockXCnt) iBMaxX = m_iBlockXCnt-1;
    if (iBMaxY>=m_iBlockYCnt) iBMaxY = m_iBlockYCnt-1;
    ...
    for (by=iBMinY; by<=iBMaxY; by++)
    {
        for (bx=iBMinX; bx<=iBMaxX; bx++)
        {
            ...
            if (iMinX<iTMinX) iMinX = iTMinX;
            if (iMinY<iTMinY) iMinY = iTMinY;
            if (iMaxX>=iTMaxX) iMaxX = iTMaxX;
            if (iMaxY>=iTMaxY) iMaxY = iTMaxY;
...
{
    if (m_listPickRayLine.size()==0) return;
...
RtsModel* RtScene::FindFirstModel(const char* szFilename)
{
    ...
    for (iy=0; iy<m_iBlockYCnt; iy++)
    {
        for (ix=0; ix<m_iBlockXCnt; ix++)
        {
            ...
            {
                pModel = ((RtSceneBlock*)m_blockMapArray[iy*m_iBlockXCnt+ix].m_pBlock)->FindFirstModel(szFilename);
...
{
    if (m_bRenderMiniMapMode==bMiniMap) return;
...
void RtScene::DrawMiniMap(int iScreenX,
                          int iScreenY, 
                          int iScreenWidth, 
                          int iScreenHeight, 
                          float fSceneWidth, 
                          ...
                          )
{
    ...
    while (fY<fMaxY)
    {
        ...
        tt = 1-(fTexTy - fTexBy)/m_fMinimapHeight; tb = 1-(fY - fTexBy)/m_fMinimapHeight;
        t2t = (fTexTy-fMinYO)/fSceneHeight; t2t = Clamp(t2t, 0.f, 1.f); t2t = t2t*(m_fMinimapMaskBottom-m_fMinimapMaskTop)+(1.f-m_fMinimapMaskBottom);
        t2b = (fTexBy-fMinYO)/fSceneHeight; t2b = Clamp(t2b, 0.f, 1.f); t2b = t2b*(m_fMinimapMaskBottom-m_fMinimapMaskTop)+(1.f-m_fMinimapMaskBottom);
        ...
        while (fX<fMaxX)
        {
            ...
            {
                tl = (fX-fTexTx)/m_fMinimapWidth;  tr = (fTexBx-fTexTx)/m_fMinimapWidth;
                t2l = (fTexTx-fMinXO)/fSceneWidth; t2l = Clamp(t2l, 0.f, 1.f); t2l = t2l*(m_fMinimapMaskRight-m_fMinimapMaskLeft)+m_fMinimapMaskLeft;
                t2r = (fTexBx-fMinXO)/fSceneWidth; t2r = Clamp(t2r, 0.f, 1.f); t2r = t2r*(m_fMinimapMaskRight-m_fMinimapMaskLeft)+m_fMinimapMaskLeft;
                tl = Clamp(tl, fEMin, fEMax);
                tr = Clamp(tr, fEMin, fEMax);
                t2l = Clamp(t2l, fEMin, fEMax);
                t2r = Clamp(t2r, fEMin, fEMax);

                SceneMiniMapRotate2(iCenterX, iCenterY, iX,  iEy, iDltx, iDlty, fMapRotateCos, fMapRotateSin);
                SceneMiniMapRotate2(iCenterX, iCenterY, iX,  iY,  iDlbx, iDlby, fMapRotateCos, fMapRotateSin);
                SceneMiniMapRotate2(iCenterX, iCenterY, iEx, iEy,  iDrtx, iDrty, fMapRotateCos, fMapRotateSin);
                SceneMiniMapRotate2(iCenterX, iCenterY, iEx, iY, iDrbx, iDrby, fMapRotateCos, fMapRotateSin);
...
void RtScene::SetMinimapMaskTexture(const char* szTextureFileName, float fL, float fT, float fR, float fB) // "minimap_mask.tga"
{
