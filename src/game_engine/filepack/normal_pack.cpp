...
void* ThreadFunc(void* vpParameter)
{
	...
	CCompress* pCompress=p->mpPack->mpCompressFactory->GetReference(p->mpFileInfo->mCompressMode,p->mpFileInfo->mCompressVersion);
...
void CNormalPack::GetContentVersion(string& vrMainVersion,string& vrSubVersion)//读出包内容的主次版本信息
{
...
void CNormalPack::SetContentVersion(string& vrMainVersion,string& vrSubVersion)//设置包内容的主次版本信息
{
	Dirty();//有变动，设置为脏
...
bool CNormalPack::CheckOverlap()
{
	...
	for(It=mFileTable.begin();It!=mFileTable.end();++It)
	{
		...
		if (pFileInfo->mSizeInPack > 0)
			...
			TEMP_BLOCK_INFO(pFileInfo->mOffset, pFileInfo->mSizeInPack, (void*)pFileInfo, pFileInfo->mName));
	}
...
CUnpackFile* CNormalPack::GetUnpackFile(CPack::SFileInfo* vpFileInPack)				//得到一个不打包文件//要删除
{
	//检查缓冲池中是否已经有该文件
	...
	{//没有找到
		...
		CCompress* pCompress=mpCompressFactory->GetReference(vpFileInPack->mCompressMode,vpFileInPack->mCompressVersion);
