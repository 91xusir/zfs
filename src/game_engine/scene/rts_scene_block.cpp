...
RtSceneBlock::RtSceneBlock()
    : m_cLastOpFlag(0), m_fOffsetX(0.f), m_fOffsetY(0.f),
      m_iGridSize(g_iSceneGridCntX * g_iSceneGridCntY), m_texItemSM(0)
...
void RtSceneBlock::UpdateGrid(int iMinX, int iMinY, int iMaxX, int iMaxY) {
    ...
    if (iMinX < 0)
        iMinX = 0;
    if (iMaxX > g_iSceneTileCntX)
        iMaxX = g_iSceneTileCntX;
    if (iMinX < 0)
        iMaxX = 0;
    if (iMaxY > g_iSceneTileCntY)
        iMaxY = g_iSceneTileCntY;
        ...
        for (gx = iGMinX; gx < iGMaxX; gx++) {
            ...
            for (y = pGrid->y; y <= pGrid->y + RTS_BLOCK_GRID_SIZE; y++) {
                for (x = pGrid->x; x <= pGrid->x + RTS_BLOCK_GRID_SIZE; x++) {
                    ...
                    if (pVertex->pPos->v.z < pGrid->vMin.z)
                        pGrid->vMin.z = pVertex->pPos->v.z;
                    if (pVertex->pPos->v.z > pGrid->vMax.z)
                        pGrid->vMax.z = pVertex->pPos->v.z;
                    if (pVertex->pPos->fWaterHeight > pGrid->vMax.z)
                        pGrid->vMax.z = pVertex->pPos->fWaterHeight;
                }
            }
        }
    }
    ...
    for (x = 0; x < m_iGridSize; x++) {
        if (m_pGrid[x].vMin.z < m_vMin.z)
            m_vMin.z = m_pGrid[x].vMin.z;
        if (m_pGrid[x].vMax.z > m_vMax.z)
            m_vMax.z = m_pGrid[x].vMax.z;
    }
}

RtsObject* RtSceneBlock::PickRay(const RtgVertex3& v, const RtgVertex3& dir, float& fOutDist,
    ...
    // 开始
    if (x == g_iSceneTileCntX)
        x = g_iSceneTileCntX - 1;
    if (y == g_iSceneTileCntY)
        y = g_iSceneTileCntY - 1;
    while (x >= 0 && y >= 0 && x < g_iSceneTileCntX && y < g_iSceneTileCntY) {
        ...
        {  // 碰撞
            pG = m_pGrid + ((y / RTS_BLOCK_GRID_SIZE) * g_iSceneGridCntX +
                            (x / RTS_BLOCK_GRID_SIZE));  // 取得格子

            if (pGrid != pG)  // 如果和上一次不在同一个格子里面
            {
                if (rtgIsLineIntersectAABB(v, vd, pG->vMin, pG->vMax)) {  // 比较格子内的模型
                    pGrid = pG;
                    if (dwType & (PICK_OBJECT | PICK_HELPER))  // 物件
                    {
                        for (int iObj = 0; iObj < pGrid->aryObject.size(); iObj++) {
                            ...
                            if (pObject) {
                                if (pObject->GetLastRenderFrame() >=
                                    RtGetRender()->GetRenderFrame())
                                    continue;
                                ...
                                if (rtgIsLineIntersectAABB(vLine0, vLine1, vMin, vMax, &fDist,
                                                           NULL)) {
                                    fDist = vLine0.Distance(vLine1) * fDist;
                                    if (pObject->IsKindOf(RT_RUNTIME_CLASS(RtsModel)))

#if DO_CONSOLE
                                        if (g_pScene->m_bEnablePickRayRecord)
                                            g_pScene->AddPickRayPoint(fDist);
#endif
            ...
            // 和地表做碰撞
            {
                ...
                if (dwType & PICK_VTERRAIN) {
                    if (rtgIsRayHitTriangle(v, dir, m_pVertices[pFace[0].wIdx].pPos->v,
                                            m_pVertices[pFace[1].wIdx].pPos->v,
                                            m_pVertices[pFace[2].wIdx].pPos->v, &fDist)) {
                        if (fDist >= 0.f) {
                            iOutX        = x;
                            iOutY        = y;
                            fTerrainDist = fDist;
                    ...
                    } else if (rtgIsRayHitTriangle(v, dir, m_pVertices[pFace[3].wIdx].pPos->v,
                                                   m_pVertices[pFace[4].wIdx].pPos->v,
                                                   m_pVertices[pFace[5].wIdx].pPos->v, &fDist)) {
                        if (fDist >= 0.f) {
                            iOutX        = x;
                            iOutY        = y;
                            fTerrainDist = fDist;
    ...
    if (fTerrainDist < 1e10) {
        if (g_pScene->m_bEnablePickRayRecord)
            g_pScene->AddPickRayPoint(fTerrainDist);
    }
#endif
...
// 返回的X和Y都是行走网格的XY
RtsObject* RtSceneBlock::PickLine(const RtgVertex3& v0, const RtgVertex3& v1, float& fOutDist,
                                  int& iOutX, int& iOutY, DWORD dwType) {
...
bool RtSceneBlock::GetHeight(const RtgVertex3& v, float& fOutDist, int& iOutX, int& iOutY,
                             DWORD dwType) {
    RtsSIndex* pFace;
    RtgVertex3 v0, v1, v2;
    ...
    if (dwType & PICK_VTERRAIN) {
        if (rtgIsRayHitTriangle(v, dir, m_pVertices[pFace[0].wIdx].pPos->v,
                                m_pVertices[pFace[1].wIdx].pPos->v,
                                m_pVertices[pFace[2].wIdx].pPos->v, &fOutDist)) {
            return true;  // 找到地表
        } else if (rtgIsRayHitTriangle(v, dir, m_pVertices[pFace[3].wIdx].pPos->v,
                                       m_pVertices[pFace[4].wIdx].pPos->v,
                                       m_pVertices[pFace[5].wIdx].pPos->v, &fOutDist)) {
            return true;  // 找到地表
        }
    } else if (dwType & PICK_TERRAIN) {
        v0   = m_pVertices[pFace[0].wIdx].pPos->v;
    ...
    for (i = 0; i < m_iGridSize; i++) {
        ...
        for (j = 0; j < pGrid->aryObject.size(); j++) {
            ...
            if (pObj && pObj->IsKindOf(RT_RUNTIME_CLASS(RtsModel))) {
                if (stricmp(((RtsActor*)pObj)->GetActor()->GetCore()->m_poTag.c_str(),
                            szFilename) == 0) {
...
void RtSceneBlock::ComputeVertexColor(int iX, int iY, RtsSVertex* pVertex) {
    ...
    */
    ...
    if (pLights == NULL) {
        ...
        pVertex->pPos->c = RtgComputeVertexColor(pVertex->pPos->v, pVertex->pPos->n, vAmb, vDif,
                                                 vEmi, g_pScene->m_lightTerrainAmbient, pLights);
    }
    ...
    if (pLights == NULL) {
        ...
        pVertex->pPos->dwTerrainColor = RtgComputeVertexColor(
            vTP, pVertex->pPos->n, vAmb, vDif, vEmi, g_pScene->m_lightTerrainAmbient, pLights);
    }
}
    ...
    for (int i = 0; i < m_iGridSize; ++i) {
        ...
        for (int j = 0; j < _grid->aryObject.size(); ++j) {
            ...
            if (_obj && _obj->IsKindOf(RT_RUNTIME_CLASS(RtsModel))) {
                ...
                _model->GetActor()->ComputeVC(_model->GetMatrix(), _model->m_aryVc, _lightOffset,
                                              _light);
...
void RtSceneBlock::DrawNormal(RtgCamera& inCamera, RtgDevice& inDevice, DWORD dwColor,
                              float fLength) {
    ...
    for (iMapLevel = 0; iMapLevel < 4; iMapLevel++) {
        for (pVertex = m_pRenderStartVertex[iMapLevel].pHead; pVertex != 0;
             pVertex = pVertex->pNext) {
            inDevice.DrawLine(pVertex->pPos->v, pVertex->pPos->v + pVertex->pPos->n * fLength,
                              &dwColor);
        }
    }
...
void RtSceneBlock::DrawTerrain(RtgCamera& inCamera, RtgDevice& inDevice, BYTE cAttrMask,
                               DWORD dwNormalColor, DWORD dwHitColor, float fOffset) {
    int          x, y, i, j, g, iIdx;
    RtsSGrid*    pGrid;
    DWORD        dwVertexCount;
        ...
        if (inCamera.CheckVisable(pGrid->vMin, pGrid->vMax)) {
            ...
            for (y = 0; y < RTS_BLOCK_GRID_SIZE; y++) {
                for (x = 0; x < RTS_BLOCK_GRID_SIZE; x++) {
                    if ((x + pGrid->x) >= g_iSceneTileCntX)
                        continue;
                    if ((y + pGrid->y) >= g_iSceneTileCntY)
                        continue;
                    iIdx = (y + pGrid->y) * g_iSceneTileCntX * g_iSceneTileRate * g_iSceneTileRate +
                           (x + pGrid->x) * g_iSceneTileRate;

                    for (i = 0; i < 24; i++) {
                        GettTerrainVertex(this, iTriangleIdx[i], x + pGrid->x, y + pGrid->y,
                                          pV[i].v);
                        pV[i].v.z += fOffset;
                    }
                    for (i = 0; i < 4; i++) {
                        ...
                        switch (i) {
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                if (m_pTileAttr[iIdx + 0 + g_iSceneTileCntX * g_iSceneTileRate]
                                        .cAttr &
                                    cAttrMask)
                                    dwColor = dwHitColor;
                                break;
                            case 3:
                                if (m_pTileAttr[iIdx + 1 + g_iSceneTileCntX * g_iSceneTileRate]
                                        .cAttr &
                                    cAttrMask)
                                    dwColor = dwHitColor;
                                break;
            ...
            inDevice.DrawPrimitiveUP(D3DPT_TRIANGLELIST, pVertices, sizeof(RtgVertexVC),
                                     dwVertexCount / 3);
...
bool RtSceneBlock::TakeGeometry(const RtgAABB& inAABB, RtgVertex3* pVertices, int& iVertexNum,
    ...
    if (iMinY < 0)
        iMinY = 0;
    if (iMaxX >= g_iSceneTileCntX)
        iMaxX = g_iSceneTileCntX - 1;
    if (iMaxY >= g_iSceneTileCntY)
        iMaxY = g_iSceneTileCntY - 1;
...
bool RtSceneBlock::OnResourceAllocate() {
    ...
    memset(m_pPositions, 0,
           sizeof(RtsSVertexValue) * (g_iSceneTileCntX + 1) * (g_iSceneTileCntY + 1));
...
bool RtSceneBlock::OnSerializeBlockObject(RtArchive* pfile) {
    ...
    for (RtsSGrid* _grid = m_pGrid; _grid < _gridEnd; ++_grid) {
        ...
        for (int i = 0; i < objNum; ++i) {
            ...
            if (RtObjectManager()->GetObject(_objId) &&
                RtObjectManager()->GetObject(_objId)->DataVaild())
...
bool RtSceneBlock::LoadBlock(const char* szBlockFileName) {
    ...
    m_texItemSM =
        RtGetRender()->GetTextMgr()->CreateTextureFromFile(szPathSM, D3DX_DEFAULT, D3DX_DEFAULT, 1);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        m_texMinmap[i] = RtGetRender()->GetTextMgr()->CreateTextureFromFile(
            minmapName, D3DX_DEFAULT, D3DX_DEFAULT, 1);
    }
...
bool RtSceneBlock::OnSerializeBlockTerrain(RtArchive* pFile, DWORD dwType) {
    ...
    switch (dwType) {
        case 0x0001:
            ...
            for (y = 0; y <= g_iSceneTileCntY; y++) {
                for (x = 0; x <= g_iSceneTileCntX; x++) {
                    pFile->Serialize(&(m_pPositions[y * (g_iSceneTileCntX + 1) + x].cHeightMap),
                                     sizeof(BYTE) * 4);
                }
            }
            break;
        ...
        case 0x0002:
            ...
            for (y = 0; y < g_iSceneTileCntY; y++) {
                for (x = 0; x < g_iSceneTileCntX; x++) {
                    pFile->Serialize(&(m_pPositions[y * (g_iSceneTileCntX + 1) + x].c),
                                     sizeof(DWORD));
                }
            }
            break;
        ...
        case 0x0003:
            ...
            for (y = 0; y < g_iSceneTileCntY; y++) {
                for (x = 0; x < g_iSceneTileCntX; x++) {
                    pFile->Serialize(&(m_pPositions[y * (g_iSceneTileCntX + 1) + x].dwTerrainColor),
                                     sizeof(DWORD));
                }
            }
            break;
        ...
        case 0x0005:
            ...
            for (y = 0; y < g_iSceneTileCntY; y++) {
                for (x = 0; x < g_iSceneTileCntX; x++) {
                    pFile->Serialize(&(m_pTileMapIdx[y * g_iSceneTileCntX + x].cWaterType),
                                     sizeof(BYTE));
                }
            }
            break;
            ...
            for (y = 0; y < g_iSceneTileCntY; y++) {
                for (x = 0; x < g_iSceneTileCntX; x++) {
                    ...
                    if (tile->grassDensity > 0) {
                        ...
                        if (tile->grassVertex)
                            pFile->Serialize(tile->grassVertex,
                                             tile->grassDensity * sizeof(RtgVertexGrass) * 8);
                    }
                }
            }
    ...
    if (bAdd) {
        ...
        for (y = -1; y <= g_iSceneTileCntY; y++) {
            for (x = -1; x <= g_iSceneTileCntX; x++) {
                g_pScene->ComputeVertexNormal(m_iBlockPosX * g_iSceneTileCntX + x,
                                              m_iBlockPosY * g_iSceneTileCntY + y);
            }
        }
    } else {
        ...
        if (pBlock) {
            for (i = 0; i < g_iSceneTileCntY; i++) {
                pBlock->m_pVertices[(i * g_iSceneTileCntX + g_iSceneTileCntX - 1) * 4 + 1].pPos =
                    pBlock->m_pPositions + (i * (g_iSceneTileCntX + 1)) + g_iSceneTileCntY;
                pBlock->m_pVertices[(i * g_iSceneTileCntX + g_iSceneTileCntX - 1) * 4 + 3].pPos =
                    pBlock->m_pPositions + ((i + 1) * (g_iSceneTileCntX + 1)) + g_iSceneTileCntY;
            }
        }
        ...
        if (pBlock) {
            for (i = 0; i < g_iSceneTileCntX; i++) {
                pBlock->m_pVertices[((g_iSceneTileCntY - 1) * g_iSceneTileCntX + i) * 4 + 2].pPos =
                    pBlock->m_pPositions + (g_iSceneTileCntY * (g_iSceneTileCntX + 1)) + i;
                pBlock->m_pVertices[((g_iSceneTileCntY - 1) * g_iSceneTileCntX + i) * 4 + 3].pPos =
                    pBlock->m_pPositions + (g_iSceneTileCntY * (g_iSceneTileCntX + 1)) + i + 1;
            }
        }
        ...
        pBlock = g_pScene->GetBlock(m_iBlockPosX - 1, m_iBlockPosY - 1);
        if (pBlock) {
            pBlock->m_pVertices[(g_iSceneTileCntY * g_iSceneTileCntX - 1) * 4 + 3].pPos =
                pBlock->m_pPositions + ((g_iSceneTileCntY + 1) * (g_iSceneTileCntX + 1)) - 1;
        }
    }
}
