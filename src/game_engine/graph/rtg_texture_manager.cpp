...
RtgTextureManager::RtgTextureManager(IDirect3DDevice9* pd3dDevice)
    : m_pDevice(pd3dDevice), m_texChessboard(NULL), m_bExit(true), m_bExited(true),
...
RtgTextureManager::~RtgTextureManager() {
    ...
    for (list<RtgTextItem*>::iterator i = m_listCatch.begin(); i != m_listCatch.end(); ++i) {
        DeleteTexture(*i);
    }
...
#else
    for (map<string, RtgTextItem*>::iterator i = m_mapText.begin(); i != m_mapText.end(); ++i) {
        RtgTextItem* texItem = i->second;
        RtCoreLog().Error("texture unrelease(ref = %d) : %s\n", texItem->Ref, texItem->fileName);
    }

    for (set<RtgTextItem*>::iterator i = m_setText.begin(); i != m_setText.end(); ++i) {
...
bool RtgTextureManager::FrameMove() {
    ...
    if (texItem) {
        ...
        if (SUCCEEDED(D3DXCreateTextureFromFileInMemoryEx(
                m_pDevice, texItem->pData, texItem->dataSize, texItem->width, texItem->height,
                texItem->mipLevels, texItem->Usage, texItem->format, texItem->poolType,
                D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &texItem->pText))) {
...
IDirect3DSurface9* RtgTextureManager::GetFrameTargt(RtgTextItem* texItem) {
    ...
    if (texItem && (texItem->Usage & D3DUSAGE_RENDERTARGET) &&
...
IDirect3DSurface9* RtgTextureManager::GetDepthTargt(RtgTextItem* texItem) {
    ...
    if (texItem &&
        (texItem->format == D3DFMT_D24S8 || texItem->format == D3DFMT_D24X8 ||
         texItem->format == D3DFMT_D16 || D3DFMT_D32) &&
        (texItem->Usage & D3DUSAGE_DEPTHSTENCIL) && (texItem->poolType == D3DPOOL_DEFAULT))
        text = GetD3dTexture(texItem);
...
bool RtgTextureManager::GetTextFilePath(char textPath[], const char textName[]) {
    ...
    const char* textExt[]    = {".dds", ".tga", ".ifl"};
    const int   extNum       = sizeof(textExt) / sizeof(const char*);
    bool        hasExtension = (strrchr(textName, '.') != nullptr);

    for (const auto& path : m_vecPath) {
        if (hasExtension) {
            rt2_sprintf(textPath, "%s\\%s", path.c_str(), textName);
            if (RtCoreFile().FileExist(textPath))
                return true;
        } else {
            for (int j = 0; j < extNum; ++j) {
                rt2_sprintf(textPath, "%s\\%s%s", path.c_str(), textName, textExt[j]);
                if (RtCoreFile().FileExist(textPath))
                    return true;
            }
        }
...
bool RtgTextureManager::IsDynamicTexture(RtgTextItem* texItem) {
    ...
    return ((texItem->poolType == D3DPOOL_DEFAULT) && (texItem->Usage & D3DUSAGE_DYNAMIC));
}
...
bool RtgTextureManager::Save(RtgTextItem* texItem, const char* fileName) {
    ...
    if (FAILED(D3DXSaveTextureToFile(fileName, GetTextFileFmt(fileName), texItem->pText, NULL)))
        return false;
...
bool RtgTextureManager::Lock(RtgTextLock* texLock, RtgTextItem* texItem) {
    ...
    if (NativeLockable(texItem)) {
        ...
        D3DXCreateTexture(m_pDevice, texItem->width, texItem->height, 1, 0, texItem->format,
                          D3DPOOL_SYSTEMMEM, &texLock->texLock);
        bCreateSM = true;
...
RtgTextItem* RtgTextureManager::CreateTextureFromFile_Direct(
    const char filePath[], UINT width /* = D3DX_DEFAULT */, UINT height /* = D3DX_DEFAULT */,
    UINT mipLevels /* = 0 */, D3DFORMAT format /* = D3DFMT_FROM_FILE */,
    D3DPOOL poolType /* = D3DPOOL_MANAGED */, DWORD Usage /* = 0 */, BOOL bForceLoad) {
    RTASSERT(m_pDevice);
    ...
    if (IsSequenceTextFile(filePath)) {
        ...
        while (str && seqItem->frameNum < RtgSeqTextItem::SetText_Max_Text_Num) {
            ...
            seqItem->apText[seqItem->frameNum] = CreateTextureFromFile(
                path, width, height, mipLevels, format, poolType, Usage, bForceLoad);
...
RtgTextItem* RtgTextureManager::CreateTextureFromFile(
    const char fileName[], UINT width /* = D3DX_DEFAULT */, UINT height /* = D3DX_DEFAULT */,
    UINT mipLevels /* = 0 */, D3DFORMAT format /* = D3DFMT_FROM_FILE */,
    D3DPOOL poolType /* = D3DPOOL_MANAGED */, DWORD Usage /* = 0 */, BOOL bForceLoad) {
    RTASSERT(m_pDevice);
    ...
    return CreateTextureFromFile_Direct(filePath, width, height, mipLevels, format, poolType, Usage,
                                        bForceLoad);
}

RtgTextItem* RtgTextureManager::CreateTexture(UINT width, UINT height, UINT mipLevels /* = 1 */,
...
RtgTextItem* RtgTextureManager::CreateTextureFromFileMemery(
    PVOID pData, UINT dataSize, UINT width /* = D3DX_DEFAULT */, UINT height /* = D3DX_DEFAULT */,
    UINT mipLevels /* = D3DX_DEFAULT */, D3DFORMAT format /* = D3DFMT_FROM_FILE */,
    D3DPOOL poolType /* = D3DPOOL_MANAGED */, DWORD Usage /* = 0 */
) {
    RTASSERT(m_pDevice);
...
RtgTextItem* RtgTextureManager::CreateTextureFromArchive(
    RtArchive* pArchive, UINT width /* = D3DX_DEFAULT */, UINT height /* = D3DX_DEFAULT */,
    UINT mipLevels /* = D3DX_DEFAULT */, D3DFORMAT format /* = D3DFMT_FROM_FILE */,
    D3DPOOL poolType /* = D3DPOOL_MANAGED */, DWORD Usage /* = 0 */
) {
    RTASSERT(pArchive);
    ...
    return CreateTextureFromFileMemery(pData, pArchive->TotalSize(), width, height, mipLevels,
                                       format, poolType, Usage);
}

RtgTextItem* RtgTextureManager::CreateTextureFromARGBData(PVOID pData, UINT dataSize, UINT width,
                                                          UINT height, UINT mipLevels /* = 1 */,
                                                          D3DFORMAT format /* = D3DFMT_A8R8G8B8 */,
                                                          D3DPOOL poolType /* = D3DPOOL_MANAGED */,
                                                          DWORD   Usage /* = 0 */
) {
...
RtgTextItem* RtgTextureManager::CreateDynamicTexture(UINT width, UINT height,
                                                     D3DFORMAT format /* = D3DFMT_A8R8G8B8*/
) {
    return CreateTexture(width, height, 1, format, D3DPOOL_DEFAULT, D3DUSAGE_DYNAMIC);
}

RtgTextItem* RtgTextureManager::CreateFrameBufferTexture(UINT width, UINT height,
                                                         D3DFORMAT format /* = D3DFMT_A8R8G8B8 */
) {
    return CreateTexture(width, height, 1, format, D3DPOOL_DEFAULT, D3DUSAGE_RENDERTARGET);
}

RtgTextItem* RtgTextureManager::CreateDepthBufferTexture(UINT width, UINT height,
                                                         D3DFORMAT format /* = D3DFMT_D24S8 */
) {
    return CreateTexture(width, height, 1, format, D3DPOOL_DEFAULT, D3DUSAGE_DEPTHSTENCIL);
}
...
RtgTextItem* RtgTextureManager::CreateUiTexture(const char fileName[],
    ...
    return CreateTextureFromFile(fileName, width, height, D3DX_UI_MIPLEVEL, D3DFMT_FROM_FILE,
                                 D3DPOOL_MANAGED, 0, true);
}
    ...
    switch (textItem->textSrc) {
        case Text_Src_File: {
            ...
            if (FAILED(D3DXCreateTextureFromFileInMemoryEx(
                    m_pDevice, textItem->pData, textItem->dataSize, textItem->width,
                    textItem->height, textItem->mipLevels, textItem->Usage, textItem->format,
                    textItem->poolType, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL,
                    &textItem->pText))) {
        ...
        case Text_Src_ARGB_Data: {
            ...
            if (FAILED(D3DXCreateTexture(m_pDevice, textItem->width, textItem->height,
                                         textItem->mipLevels, textItem->Usage, textItem->format,
                                         textItem->poolType, &textItem->pText))) {
                return false;
            }
            ...
            if (textItem->poolType == D3DPOOL_DEFAULT) {
                if (FAILED(D3DXCreateTexture(m_pDevice, textItem->width, textItem->height, 1, 0,
                                             textItem->format, D3DPOOL_SYSTEMMEM, &textFill))) {
    ...
    if (--texItem->Ref <= 0) {
        if (texItem->textSrc == Text_Src_File) {
            if (m_listCatch.size() >= Text_Catch_Size) {
                ...
                RTASSERT(find(m_listCatch.begin(), m_listCatch.end(), texItem) ==
                         m_listCatch.end());
            }
