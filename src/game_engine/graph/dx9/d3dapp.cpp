...
DWORD CD3DApplication::_m_dwStatTimerRunD3D9;
DWORD CD3DApplication::_m_dwStatTimerRenderD3D9;
DWORD CD3DApplication::_m_dwStatTimerPresentD3D9;
...
CD3DApplication::CD3DApplication()
{
    ...
    m_dwStatTimerRun        = 0;
    m_dwStatTimerRender     = 0;
    m_dwStatTimerPresent    = 0;
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	/*int iSamples = D3DMULTISAMPLE_2_SAMPLES;
	for(; iSamples>=D3DMULTISAMPLE_2_SAMPLES;iSamples--)
	{
		
		if(SUCCEEDED(m_pD3D->CheckDeviceMultiSampleType(m_d3dSettings.AdapterOrdinal(),	pDeviceInfo->DevType, m_d3dpp.BackBufferFormat,       true,(D3DMULTISAMPLE_TYPE)iSamples , NULL)) &&
		   SUCCEEDED(m_pD3D->CheckDeviceMultiSampleType(m_d3dSettings.AdapterOrdinal(),	pDeviceInfo->DevType, m_d3dpp.AutoDepthStencilFormat, true,(D3DMULTISAMPLE_TYPE)iSamples , NULL)))
		{
			m_d3dpp.MultiSampleType = (D3DMULTISAMPLE_TYPE)iSamples;			
		}
	}*/
...
HRESULT CD3DApplication::Render3DEnvironment()
{
    ...
    m_dwStatTimerRun       = _m_dwStatTimerRunD3D9     ;
    m_dwStatTimerRender    = _m_dwStatTimerRenderD3D9  ;
    m_dwStatTimerPresent   = _m_dwStatTimerPresentD3D9 ;
    _m_dwStatTimerRunD3D9      = 0;
    _m_dwStatTimerRenderD3D9   = 0;
    _m_dwStatTimerPresentD3D9  = 0;
    ...
    if( m_bFrameMoving || m_bSingleStep )
    {
        ...
        rtClock(_m_dwStatTimerRunD3D9);
        if( FAILED( hr = FrameMove() ) )
            return hr;
        rtUnclock(_m_dwStatTimerRunD3D9);
    ...
    rtClock(_m_dwStatTimerRenderD3D9);
	if( FAILED( hr = Render() ) )
		return hr;
    rtUnclock(_m_dwStatTimerRenderD3D9);

    UpdateStats();

    // Show the frame on the primary surface.
    rtClock(_m_dwStatTimerPresentD3D9);
    hr = m_pd3dDevice->Present( NULL, NULL, NULL, NULL );
    rtUnclock(_m_dwStatTimerPresentD3D9);
    if( D3DERR_DEVICELOST == hr )
        m_bDeviceLost = true;


    //extern double g_RenderUseTime;
    //g_RenderUseTime = tick_end();
