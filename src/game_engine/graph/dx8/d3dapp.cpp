...
//paul add
HWND	childWnd;	//子窗口HWND	for swap chain ,必须全局
//end 

namespace rt_graph_dx8 {
...
DWORD CD3DApplication::_m_dwStatTimerRun = 0;
DWORD CD3DApplication::_m_dwStatTimerRender = 0;
DWORD CD3DApplication::_m_dwStatTimerPresent = 0;
...
CD3DApplication::CD3DApplication()
{
    ...
    m_strWindowTitle    = _T("rtg3D Application");
    m_iDefaultStartX    = CW_USEDEFAULT;
    m_iDefaultStartY    = CW_USEDEFAULT;
    ...
    m_eMaxMultiSampleTypeWindowed = D3DMULTISAMPLE_16_SAMPLES;
    m_eMaxMultiSampleTypeFullscreen = D3DMULTISAMPLE_16_SAMPLES;
}
...
bool CD3DApplication::SetFullScreenMode(DWORD dwWidth, DWORD dwHeight, DWORD dwDepth)
{
    ...
    for( DWORD m=0; m < pDevice->dwNumModes; m++ )
    {
        ...
        if (pDevice->modes[m].Width!=dwWidth || pDevice->modes[m].Height!=dwHeight)
        {
            continue;
        }
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    if( m_hWnd == NULL)
    {
        ...
        if (hIcon==NULL)
        {
            ...
            hIcon = ExtractIcon( hInstance, szExePath, 0 );  // 如果没有，就用找到的第一个
        }
		...
		wcex.lpszMenuName   = 0;
		wcex.lpszClassName	= _T("rtg3DWndClass");
		wcex.hIconSm		= hIcon;
//         WNDCLASS wndClass = {
//             0, WndProc, 0, 0, hInstance,
//             hIcon,
//             NULL,//LoadCursor( NULL, IDC_ARROW ), // 如果用LoadCursor就会在游戏里面每帧都被重设，如果游戏要控制鼠标，就要设置成NULL
//             (HBRUSH)GetStockObject(BLACK_BRUSH),
//             NULL, _T("rtg3DWndClass")
//         };
        ...
        //AdjustWindowRect( &rc, m_dwWindowStyle, TRUE ); // 有菜单最后一个参数用true，否则用false
        AdjustWindowRect( &rc, m_dwWindowStyle, false );

        // Create the render window
        m_hWnd = CreateWindow( _T("rtg3DWndClass"), m_strWindowTitle, m_dwWindowStyle,
                               m_iDefaultStartX, m_iDefaultStartY,
                               (rc.right-rc.left), (rc.bottom-rc.top), 0L,
                               NULL,//LoadMenu( hInstance, MAKEINTRESOURCE(IDR_MENU) ),
                               hInstance, 0L );
		TCHAR* szChildWinName= _T("ChildWindow");
		childWnd=CreateWindow(_T("rtg3DWndClass"),szChildWinName,WS_BORDER|WS_POPUP,
			67,42,256,256,m_hWnd,NULL,
			hInstance,0L);
		if(!childWnd)
		{
			::MessageBox(0,"创建用于交换链渲染的窗口失败","error",MB_OK);
		}
		::ShowWindow(childWnd,SW_HIDE);
    }
...
{
    RtCoreLog().Info("CD3DApplication::Initialize3DEnvironment\n");
    ...
    // 选择最大的抗锯齿选项. Janhail add
    int isWindowed, iSamples;
    for (isWindowed=0; isWindowed<=1; isWindowed++)
    {
        ...
        {
            D3DFORMAT fmt;
            if (isWindowed) fmt = pAdapterInfo->d3ddmDesktop.Format;
            else            fmt = pModeInfo->Format;

            if(SUCCEEDED(m_pD3D->CheckDeviceMultiSampleType(m_dwAdapter,
                pDeviceInfo->DeviceType, fmt, isWindowed,(D3DMULTISAMPLE_TYPE)iSamples)))
            {
                if (isWindowed) pDeviceInfo->MultiSampleTypeWindowed = (D3DMULTISAMPLE_TYPE)iSamples;
                else            pDeviceInfo->MultiSampleTypeFullscreen = (D3DMULTISAMPLE_TYPE)iSamples;
    ...
    if( m_bWindowed )   m_d3dpp.MultiSampleType = pDeviceInfo->MultiSampleTypeWindowed;
    else                m_d3dpp.MultiSampleType = pDeviceInfo->MultiSampleTypeFullscreen;


    // Create the device
    hr = m_pD3D->CreateDevice( m_dwAdapter, pDeviceInfo->DeviceType, m_hWndFocus,
                pModeInfo->dwBehavior, &m_d3dpp, &m_pd3dDevice );
    if( SUCCEEDED(hr) )
    {
...
{
    RtCoreLog().Info("CD3DApplication::Resize3DEnvironment\n");
...
{
    RtCoreLog().Info("CD3DApplication::ToggleFullscreen\n");
...
{
    RtCoreLog().Info("CD3DApplication::ForceWindowed\n");
...
{
    RtCoreLog().Info("CD3DApplication::AdjustWindowForChange\n");
...
HRESULT CD3DApplication::Render3DEnvironment()
{
    ...
    m_dwStatTimerRun       = _m_dwStatTimerRun     ;
    m_dwStatTimerRender    = _m_dwStatTimerRender  ;
    m_dwStatTimerPresent   = _m_dwStatTimerPresent ;
    _m_dwStatTimerRun      = 0;
    _m_dwStatTimerRender   = 0;
    _m_dwStatTimerPresent  = 0;
    ...
    if( m_bFrameMoving || m_bSingleStep )
    {
        ...
        rtClock(_m_dwStatTimerRun);
        if( FAILED( hr = FrameMove() ) )
            return hr;
        rtUnclock(_m_dwStatTimerRun);
    ...
    rtClock(_m_dwStatTimerRender);
    if( FAILED( hr = Render() ) )
        return hr;
    rtUnclock(_m_dwStatTimerRender);
    ...
    rtClock(_m_dwStatTimerPresent);
    m_pd3dDevice->Present( NULL, NULL, NULL, NULL );
    rtUnclock(_m_dwStatTimerPresent);

    return S_OK;
}


void CD3DApplication::IUpdateAppTime()
{
...
{
    RtCoreLog().Info("CD3DApplication::Cleanup3DEnvironment\n");
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    if( MSGERR_APPMUSTEXIT == dwType )
    {
        ...
        if (!OnD3D8ErrorMessage(strMsg, 1))
        {
    ...
    else
    {
        ...
        if (!OnD3D8ErrorMessage(strMsg, 0))
        {
