...
D3DDeviceInfo::~D3DDeviceInfo( void )
{
    if( pDeviceComboList != NULL )
    {
        for( UINT idc = 0; idc < pDeviceComboList->Count(); idc++ )
		{
			...
			D3DDeviceCombo* pkCombo = (D3DDeviceCombo*)pDeviceComboList->GetPtr(idc);
			DEL_ONE( pkCombo );
		}
        //delete pDeviceComboList;
		DEL_ONE( pDeviceComboList );
    }
}
...
D3DDeviceCombo::~D3DDeviceCombo( void )
{
	...
	DEL_ONE( pDepthStencilFormatList );
	DEL_ONE( pMultiSampleTypeList );
	DEL_ONE( pMultiSampleQualityList );
	DEL_ONE( pDSMSConflictList );
	DEL_ONE( pVertexProcessingTypeList );
	DEL_ONE( pPresentIntervalList );
}
...
CD3DEnumeration::~CD3DEnumeration()
{
    if( m_pAdapterInfoList != NULL )
    {
        for( UINT iai = 0; iai < m_pAdapterInfoList->Count(); iai++ )
		{
			...
			D3DAdapterInfo* pkAdapter = (D3DAdapterInfo*)m_pAdapterInfoList->GetPtr(iai);
			DEL_ONE( pkAdapter );
		}
        //delete m_pAdapterInfoList;
		DEL_ONE( m_pAdapterInfoList );
    }
    DEL_ONE( m_pAllowedAdapterFormatList );
}
...
HRESULT CD3DEnumeration::Enumerate()
{
    ...
    m_pAdapterInfoList = RT_NEW CArrayList( AL_REFERENCE );
    if( m_pAdapterInfoList == NULL )
        return E_OUTOFMEMORY;

    m_pAllowedAdapterFormatList = RT_NEW CArrayList( AL_VALUE, sizeof(D3DFORMAT) );
    ...
    UINT adapterOrdinal;
#if DO_STAT
    UINT nvPerfHUDOrdinal;
    bool bUseNVPerfHUD = false;
    for (adapterOrdinal = 0; adapterOrdinal < numAdapters; adapterOrdinal++)
    {
        D3DADAPTER_IDENTIFIER9 Identifier;
        HRESULT Res=m_pD3D->GetAdapterIdentifier(adapterOrdinal,0,&Identifier);
        if (strcmp(Identifier.Description,"NVIDIA NVPerfHUD")==0)
        {
            nvPerfHUDOrdinal = adapterOrdinal;
            bUseNVPerfHUD = true;
            RtCoreLog().Error("D3D: 找到NVIDIA NVPerfHUD设备, 将使用他进行初始化.\n");
            break;
        }
    }
#endif
    ...
    for (adapterOrdinal = 0; adapterOrdinal < numAdapters; adapterOrdinal++)
    {
        ...
        if (bUseNVPerfHUD)
        {
            if (adapterOrdinal!=nvPerfHUDOrdinal)
            {
                continue;
            }
        }
#endif
        pAdapterInfo = RT_NEW D3DAdapterInfo;
        if( pAdapterInfo == NULL )
            return E_OUTOFMEMORY;
        pAdapterInfo->pDisplayModeList = RT_NEW CArrayList( AL_VALUE, sizeof(D3DDISPLAYMODE)); 
        pAdapterInfo->pDeviceInfoList  = RT_NEW CArrayList( AL_REFERENCE );
        if( pAdapterInfo->pDisplayModeList == NULL ||
            pAdapterInfo->pDeviceInfoList == NULL )
        {
            //delete pAdapterInfo;
			DEL_ONE( pAdapterInfo );
            return E_OUTOFMEMORY;
        }
                                           ...
                                           CArrayList* pAdapterFormatList )
{
    ...
    {
        pDeviceInfo = RT_NEW D3DDeviceInfo;
        if( pDeviceInfo == NULL )
            return E_OUTOFMEMORY;
        pDeviceInfo->pDeviceComboList = RT_NEW CArrayList( AL_REFERENCE ); 
    ...
    for( UINT iaf = 0; iaf < pAdapterFormatList->Count(); iaf++ )
    {
        ...
        for( UINT ibbf = 0; ibbf < backBufferFormatArrayCount; ibbf++ )
        {
            ...
            for( UINT iiw = 0; iiw < 2; iiw++)
            {
                ...
                pDeviceCombo = RT_NEW D3DDeviceCombo;
                if( pDeviceCombo == NULL )
                    return E_OUTOFMEMORY;
                pDeviceCombo->pDepthStencilFormatList = RT_NEW CArrayList( AL_VALUE, sizeof( D3DFORMAT ) );
                pDeviceCombo->pMultiSampleTypeList = RT_NEW CArrayList( AL_VALUE, sizeof( D3DMULTISAMPLE_TYPE ) );
                pDeviceCombo->pMultiSampleQualityList = RT_NEW CArrayList( AL_VALUE, sizeof( DWORD ) );
                pDeviceCombo->pDSMSConflictList = RT_NEW CArrayList( AL_VALUE, sizeof( D3DDSMSConflict ) );
                pDeviceCombo->pVertexProcessingTypeList = RT_NEW CArrayList( AL_VALUE, sizeof( VertexProcessingType ) );
                pDeviceCombo->pPresentIntervalList = RT_NEW CArrayList( AL_VALUE, sizeof( UINT ) );
