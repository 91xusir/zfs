...
bool RtgDeviceD3D9::ResetDevice(BOOL bFullscreen, DWORD dwWidth, DWORD dwHeight, DWORD dwDepth)
{
    ...
    if (pDeviceCombo)
    {
        ...
        iBitDepth = 16;
        if( dm.Format == D3DFMT_X8R8G8B8 || dm.Format == D3DFMT_A8R8G8B8 || dm.Format == D3DFMT_R8G8B8 )
        {
            iBitDepth = 32;
        }
        if ((BOOL)m_d3dSettings.IsWindowed!=bFullscreen && dm.Width==dwWidth && dm.Height==dwHeight && iBitDepth==dwDepth)
        {
            return true; // 如果没有改变就直接返回
        }
        if (m_d3dSettings.IsWindowed && !bFullscreen && dm.Width==dwWidth && dm.Height==dwHeight)
        {
    ...
    for( idc = 0; idc < pDeviceInfo->pDeviceComboList->Count(); idc++ )
    {
        ...
        if ((BOOL)pDeviceCombo->IsWindowed==bFullscreen) continue;
        if (pDeviceCombo->BackBufferFormat!=pDeviceCombo->AdapterFormat) continue;
        ...
        if (bestFormat==bitFormat)
        {
            ...
            break;
        }else if ((bitFormat==D3DFMT_X8R8G8B8 || bitFormat==D3DFMT_A8R8G8B8 || bitFormat==D3DFMT_R8G8B8)&&dwDepth==32)
        {
            iBestDeviceCombo = idc;
        }
    }
    ...
    for( idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
    {
        ...
        if( pdm->Format != pDeviceCombo->AdapterFormat )
            continue;
        if( pdm->Width == dwWidth && pdm->Height == dwHeight && pdm->RefreshRate == iMaxRefreshRate)
        {
            // 找到最佳的显示方式了
            iBestDisplayMode = idm;
            break;
        }else if( pdm->Width == dwWidth && pdm->Height == dwHeight && pdm->RefreshRate>iBestRefreshRate )
        {
    ...
    if (bFullscreen)
    {
        ...
        m_d3dSettings.Fullscreen_DisplayMode = *((D3DDISPLAYMODE*)pAdapterInfo->pDisplayModeList->GetPtr(iBestDisplayMode));
    }else
    {
...
void RtgDeviceD3D9::OnGetDeviceCaps()
{
    ...
    if (!(m_d3dCaps.TextureAddressCaps & D3DPTADDRESSCAPS_BORDER))  RtCoreLog().Info("    不支持贴图Border模式.\n");
    if (!(m_d3dCaps.TextureAddressCaps & D3DPTADDRESSCAPS_CLAMP))   RtCoreLog().Info("    不支持贴图Clamp模式.\n");
    if (!(m_d3dCaps.TextureAddressCaps & D3DPTADDRESSCAPS_WRAP))    RtCoreLog().Info("    不支持贴图Wrap模式.\n");
    ...
    if (!(m_d3dCaps.TextureCaps & D3DPTEXTURECAPS_MIPMAP))          RtCoreLog().Info("    不支持 D3DPTEXTURECAPS_MIPMAP.\n");
...
bool RtgDeviceD3D9::OnAfterCreate3DDevice()
{
	...
	m_iTexNumFormats = 0;
    if (m_d3dSettings.PDeviceInfo()->DevType==D3DDEVTYPE_HAL)
    {   // 硬件
        for ( i=0; i<sizeof(texFormats)/sizeof(TextureFormat); i++ )
        {
            if (SUCCEEDED(m_pD3D->CheckDeviceFormat(m_d3dSettings.AdapterOrdinal(), D3DDEVTYPE_HAL, d3dPixelFormat, 0, D3DRTYPE_TEXTURE, enumRTGTextureFormat[texFormats[i].m_eFormat])))
            {
                texFormats[i].m_bSupported = 1;
                m_TexFormat[m_iTexNumFormats++] = texFormats[i];
            }
        }
		if (SUCCEEDED(m_pD3D->CheckDeviceFormat( m_d3dSettings.AdapterOrdinal(), D3DDEVTYPE_HAL, d3dPixelFormat, 0, D3DRTYPE_TEXTURE, D3DFMT_DXT5)))
		{
    ...
    for ( i=0; i<m_iTexNumFormats; i++ )
    {
        RtCoreLog().Info("    %2d: [%s] %d Bits, %d Alpha", i, pTexFormatsName[i], m_TexFormat[i].m_iBits, m_TexFormat[i].m_iAlphaBits);
...
BOOL RtgDeviceD3D9::OnSetFVF()
{
    ...
    case RTG_VNCT:
        ...
        break;
    case RTG_VNCT2:
        m_dwVertexFVF |= D3DFVF_NORMAL|D3DFVF_DIFFUSE|D3DFVF_TEX2|D3DFVF_TEXCOORDSIZE2(0)|D3DFVF_TEXCOORDSIZE2(1);
        break;
    case RTG_VNCT3:
...
//}
//
BOOL RtgDeviceD3D9::DrawPrimitiveVB(RTGPrimitive eType, void *pVertices, UINT dwVertexStart, UINT dwVerticesCount, UINT dwPrimitiveCount)
{
    LOG_FUNCTION;
    STAT(m_iLastRenderNumFaces+=dwPrimitiveCount);
...
BOOL RtgDeviceD3D9::DrawPrimitiveVB(RTGPrimitive eType, UINT dwVertexStart, UINT dwVerticesCount, UINT dwPrimitiveCount)
{
    LOG_FUNCTION;
    STAT(m_iLastRenderNumFaces+=dwPrimitiveCount);
    STAT(m_iLastRenderDrawPrimitiveVB++);
...
{
    return m_pd3dDevice->SetStreamSource(dwStreamNumber, (IDirect3DVertexBuffer9*)pStreamData, 0, dwStride)==D3D_OK;
}
...
BOOL RtgDeviceD3D9::CreateVertexBuffer(UINT dwLength, void** ppVertexBuffer, DWORD dwUsage)
{
    ...
    return m_pd3dDevice->CreateVertexBuffer(dwLength, dwD3DUsage, 0, D3DPOOL_DEFAULT, (IDirect3DVertexBuffer9**)ppVertexBuffer, NULL)==D3D_OK;
}

BOOL RtgDeviceD3D9::LockVertexBuffer(void* pVertexBuffer, void** ppData, DWORD dwOffset, DWORD dwSize, RTGLock eLockFlag)
{
    LOG_FUNCTION;
    return ((LPDIRECT3DVERTEXBUFFER9)pVertexBuffer)->Lock(dwOffset, dwSize, ppData, enumRTGLock[eLockFlag])==D3D_OK;
}
...
BOOL RtgDeviceD3D9::CreateIndexBuffer(UINT dwLength, void** ppIndexBuffer, DWORD dwUsage)
{
    ...
    return m_pd3dDevice->CreateIndexBuffer(dwLength, dwD3DUsage, D3DFMT_INDEX16, D3DPOOL_DEFAULT, (IDirect3DIndexBuffer9**)ppIndexBuffer, NULL)==D3D_OK;
}

BOOL RtgDeviceD3D9::LockIndexBuffer(void* pIndexBuffer, void** ppData, DWORD dwOffset, DWORD dwSize, RTGLock eLockFlag)
{
    LOG_FUNCTION;
    return ((LPDIRECT3DINDEXBUFFER9)pIndexBuffer)->Lock(dwOffset, dwSize, ppData, enumRTGLock[eLockFlag])==D3D_OK;
}
...
void RtgDeviceD3D9::SetTextureFilterMip(RTGTextureFilter eType)
{
    ...
    {
        m_pd3dDevice->SetSamplerState( i, D3DSAMP_MIPFILTER, enumRTGTextureFilter[m_eTextureFilterMip]);
    }
}
...
void RtgDeviceD3D9::SetTextureFilterMin(RTGTextureFilter eType)
{
    ...
    for (int i=0; i<this->m_Ability.iMaxMultiTextures; i++)
    {
        m_pd3dDevice->SetSamplerState( i, D3DSAMP_MINFILTER, enumRTGTextureFilter[m_eTextureFilterMin]);
    }
}
...
void RtgDeviceD3D9::SetTextureFilterMag(RTGTextureFilter eType)
{
    ...
    for (int i=0; i<this->m_Ability.iMaxMultiTextures; i++)
    {
        m_pd3dDevice->SetSamplerState( i, D3DSAMP_MAGFILTER, enumRTGTextureFilter[m_eTextureFilterMag]);
    }
}
...
void RtgDeviceD3D9::SetTextureAddress(RTGTextureAddress eType)
{
    ...
    for (int i=0; i<this->m_Ability.iMaxMultiTextures; i++)
    {
        m_pd3dDevice->SetSamplerState( i, D3DSAMP_ADDRESSU, enumRTGTextureAddress[m_eTextureAddress]);
        m_pd3dDevice->SetSamplerState( i, D3DSAMP_ADDRESSV, enumRTGTextureAddress[m_eTextureAddress]);
    }
}
...
bool RtgDeviceD3D9::CreatePostProcessResource()
{
    ...
    if (!m_texPostDown0)
        m_texPostDown0 = GetTextMgr()->CreateTexture(
            m_bloomTexW,
            m_bloomTexH,
            1,
            D3DFMT_A8R8G8B8,
    ...
    if (!m_texPostDown1)
        m_texPostDown1 = GetTextMgr()->CreateTexture(
            m_bloomTexW,
            m_bloomTexH,
            1,
            D3DFMT_A8R8G8B8,
...
BOOL RtgDeviceD3D9::CreateTextureByFile(_TexItem* pItem, const char* szTexFileName, int iMipMap)
{
    ...
    if (m_pd3dDevice->CreateTexture( iWidth, iHeight, iMipMap, 0, enumRTGTextureFormat[m_pTexColorFormat->m_eFormat], D3DPOOL_MANAGED, &pTex, NULL)!=D3D_OK)
    {
    ...
    for ( int i=0; i<iMipMap; i++ )
    {
        ...
        m_pTexColorFormat->fill(iWidth, iHeight, desc.pBits, desc.Pitch, img.GetRawData(), iWidth*4);
        pTex->UnlockRect(i);
...
BOOL RtgDeviceD3D9::CreateTextureByImage(_TexItem* pItem, RtgImage& img)
{
	...
	if (img.info.bDDS)
	{
        ...
        case RtgImage::EPF_L8: dwD3dFormat = D3DFMT_L8; break;
        case RtgImage::EPF_ARGB : dwD3dFormat = D3DFMT_A8R8G8B8; break;
		default:
		case RtgImage::EPF_DXT5: dwD3dFormat = D3DFMT_DXT5; break;
		}
		if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, pItem->iMipMapCount, 0, dwD3dFormat, D3DPOOL_MANAGED, &pTex, NULL)!=D3D_OK)
		{
	...
	}else
	{
		...
		{
		case 1:  dwD3dFormat = D3DFMT_A8R8G8B8; break;
		case 2:  dwD3dFormat = D3DFMT_A4R4G4B4; break;
		case 3:  dwD3dFormat = D3DFMT_A1R5G5B5; break;
        case 8:  dwD3dFormat = D3DFMT_L8; break;
		default: dwD3dFormat = enumRTGTextureFormat[m_pTexColorFormat->m_eFormat]; break;
		}
		if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, pItem->iMipMapCount, 0, dwD3dFormat, D3DPOOL_MANAGED, &pTex, NULL)!=D3D_OK)
		{
...
BOOL RtgDeviceD3D9::CreateTexture(_TexItem* pItem, BOOL bDxManaged)
{
    ...
    if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, pItem->iMipMapCount, 0, dwD3dFormat, bDxManaged?D3DPOOL_MANAGED:D3DPOOL_DEFAULT, &pTex, NULL)!=D3D_OK)
    {
...
BOOL RtgDeviceD3D9::CreateTextureDynamic(_TexItem* pItem)
{
    ...
    if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, 1, D3DUSAGE_DYNAMIC, dwD3dFormat, D3DPOOL_DEFAULT, &pTex, NULL)!=D3D_OK)
    {
...
BOOL RtgDeviceD3D9::CreateTextureRenderTarget(_TexItem* pItem)
{
    ...
    if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, 1, D3DUSAGE_RENDERTARGET, kRenderTargetFormat, D3DPOOL_DEFAULT, &pTex, NULL)!=D3D_OK)
    {
    ...
    {
        if (m_pd3dDevice->CreateOffscreenPlainSurface(pItem->iWidth, pItem->iHeight, kRenderTargetFormat, D3DPOOL_SYSTEMMEM, 
            (LPDIRECT3DSURFACE9*)&pItem->pOffScreen, NULL)!=D3D_OK)
        {
            RtCoreLog().Warn("RtgDeviceD3D9错误: CreateTextureRenderTarget调用CreateOffscreenPlainSurface失败.\n");
            return FALSE;
        }
    }
	CHECK(pTex->GetLevelCount()==1);
...
BOOL RtgDeviceD3D9::CreateTextureDepthStencil(_TexItem* pItem)
{
    ...
    LPDIRECT3DTEXTURE9 pTex;
    CHECK(pItem->iMipMapCount==1); // RenderTarget只能为1
    if (m_pd3dDevice->CreateTexture( pItem->iWidth, pItem->iHeight, 1, D3DUSAGE_DEPTHSTENCIL, enumRTGTextureFormat[pItem->cFormat], D3DPOOL_DEFAULT, &pTex, NULL)!=D3D_OK)
    {
...
BOOL RtgDeviceD3D9::UpdateTexture(_TexItem* pItem, void *pBuffer)
{
	...
	m_pTexColorFormat->fill(pItem->iWidth, pItem->iHeight, desc.pBits, desc.Pitch, pBuffer, pItem->iWidth*4);
	pTex->UnlockRect(0);
	return TRUE;
}
...
BOOL RtgDeviceD3D9::ClearRenderTarget(bool bStencil, bool bTarget, bool bZBuffer, DWORD dwColor, float fNewZ, DWORD dwStencil)
{
    HRESULT hr;
    DWORD dwFlag = 0;
    if (bStencil) dwFlag |= D3DCLEAR_STENCIL;
    if (bTarget)  dwFlag |= D3DCLEAR_TARGET;
    if (bZBuffer) dwFlag |= D3DCLEAR_ZBUFFER;
...
void RtgDeviceD3D9::DrawPerfInformation()
{
    ...
    if (curr - prev >= 1000)
    {
        ...
        rt2_sprintf(textPerf, 
            ...
            pref->Time_Render,
            pref->Time_Update,
            pref->Used_BuffSize / 1048576.f,
            RtGetRender()->GetAvailableTextureMem() / 1048576.f,
            pref->Proc_FaceNum,
            pref->Draw_Call,
            ...
            RtcGetActorManager()->GetCratQueueSize(),
            GetTextMgr()->GetLoadQueueSize(),
...
void ImageBlur( BYTE* pBits, int iWidth, int iHeight, DWORD fA, DWORD fR, DWORD fG, DWORD fB )
{
    ...
    {
        if (g_pBlurDestBuffer) RtCoreMem().Free(g_pBlurDestBuffer);
        g_pBlurDestBuffer = (BYTE*)RtCoreMem().Alloc(iWidth*iHeight*sizeof(T), __FILE__, __LINE__);
    ...
    for (i=0;i<w;i++)
    {
        ...
        for (j=1; j<h-1; j++)
        {
            ...
            T cA = ((((T*)b)[b1]&fA) + (((T*)b)[b2]&fA) + (((T*)b)[b3]&fA) + (((T*)b)[b4]&fA)
                + (((T*)b)[b5]&fA) + (((T*)b)[b6]&fA) + (((T*)b)[b7]&fA) + (((T*)b)[b8]&fA) + (((T*)b)[b9]&fA))/9;
            T cR = ((((T*)b)[b1]&fR) + (((T*)b)[b2]&fR) + (((T*)b)[b3]&fR) + (((T*)b)[b4]&fR)
                + (((T*)b)[b5]&fR) + (((T*)b)[b6]&fR) + (((T*)b)[b7]&fR) + (((T*)b)[b8]&fR) + (((T*)b)[b9]&fR))/9;
            T cG = ((((T*)b)[b1]&fG) + (((T*)b)[b2]&fG) + (((T*)b)[b3]&fG) + (((T*)b)[b4]&fG)
                + (((T*)b)[b5]&fG) + (((T*)b)[b6]&fG) + (((T*)b)[b7]&fG) + (((T*)b)[b8]&fG) + (((T*)b)[b9]&fG))/9;
            T cB = ((((T*)b)[b1]&fB) + (((T*)b)[b2]&fB) + (((T*)b)[b3]&fB) + (((T*)b)[b4]&fB)
                + (((T*)b)[b5]&fB) + (((T*)b)[b6]&fB) + (((T*)b)[b7]&fB) + (((T*)b)[b8]&fB) + (((T*)b)[b9]&fB))/9;
...
void RtgDeviceD3D9::DrawString(float fX, float fY, DWORD dwColor, const char* pString, DWORD dwFlags)
{
...
BOOL RtgDeviceD3D9::SetMaterial(RtgVertex4& vDiffuse, RtgVertex4& vAmbient, RtgVertex4& vSpecular, RtgVertex4& vEmissive, float fPower)
{
...
HRESULT RtgDeviceD3D9::ConfirmDevice(D3DCAPS9* pCaps, DWORD dwBehavior, D3DFORMAT eFormat, D3DFORMAT eF2)
{
    do
    {
        if( dwBehavior & D3DCREATE_PUREDEVICE )                 break;
        if(!(pCaps->TextureCaps & (D3DPTEXTURECAPS_ALPHAPALETTE|D3DPTEXTURECAPS_ALPHA))) break;
        //if(!(pCaps->RasterCaps & D3DPRASTERCAPS_ZTEST))         break;
        if(!(pCaps->AlphaCmpCaps & D3DPCMPCAPS_GREATEREQUAL))   break;
        if(!(pCaps->ZCmpCaps & D3DPCMPCAPS_LESSEQUAL))          break;
        if(!(pCaps->SrcBlendCaps & D3DPBLENDCAPS_DESTCOLOR))    break;
        if(!(pCaps->DestBlendCaps & D3DPBLENDCAPS_DESTCOLOR))   break;
        if(!(pCaps->TextureOpCaps & D3DTEXOPCAPS_DISABLE))      break;
        return S_OK;
    } while(0);
...
DWORD RtgDeviceD3D9::GetDisplayMode(RtgDisplayMode* pModeBuf, DWORD dwBufSize)
{
    ...
    for( idc = 0; idc < pDeviceInfo->pDeviceComboList->Count(); idc++ )
    {
        ...
        if (pDeviceCombo)
        {
            ...
            iBitDepth = 16;
            if( adapterFormat == D3DFMT_X8R8G8B8 || adapterFormat == D3DFMT_A8R8G8B8 || adapterFormat == D3DFMT_R8G8B8 )
            {
                iBitDepth = 32;
            }
            ...
            for( idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
            {
                ...
                {
                    dwR = ((iBitDepth&0xFF)<<24) | ((displayMode.Width&0x00000FFF)<<12) | (displayMode.Height&0x00000FFF);
                    dmMap.insert(dwR);
                }
            }
        }
...
bool RtgDeviceD3D9::GetCanDoWindowed()
{
    ...
    for( idc = 0; idc < pDeviceInfo->pDeviceComboList->Count(); idc++ )
    {
        ...
        if (pDeviceCombo)
        {
            if( pDeviceCombo->IsWindowed )  bCanDoWindowed = true;
        }
    }
