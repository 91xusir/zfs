...
#else
    rt2_strncpy( strDestination, tstrSource, cchDestChar );
    strDestination[cchDestChar-1] = '\0';
    return S_OK;
#endif   
}
...
#else
    rt2_strncpy( tstrDestination, strSource, cchDestChar );
...
HRESULT CArrayList::Add( void* pEntry )
{
    ...
    if( m_pData == NULL || m_NumEntries + 1 > m_NumEntriesAllocated )
    {
        ...
        pDataNew = RT_NEW BYTE[NumEntriesAllocatedNew * m_BytesPerEntry];
        if( pDataNew == NULL )
