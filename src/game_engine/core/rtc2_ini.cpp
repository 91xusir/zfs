...
static void AzothIniParserCallback(azoth_value* arg1, azoth_value* arg2, int type, void* arg) {
    ...
    if (arg == 0 || arg1->value.str.val[0] == 0) {
        if (arg1)
            azoth_freestr(arg1->value.str.val);
        if (arg2) {
...
static int AzothIniOutputToFile(int iItemType, const char* szArg1, const char* szArg2,
                                int iValueType, void* arg) {
    if (szArg1[0] == 0)
        return 0;
...
long RtIni::SItem::ConvertToLong() const {
    switch (eType) {
        ...
        case VT_DOUBLE:
            return (long)atof(szValue.c_str());
        case VT_STRING:
            return atol(szValue.c_str());
        default:
            return 0;
...
double RtIni::SItem::ConvertToFloat() const {
    switch (eType) {
        case VT_LONG:
            return (double)atol(szValue.c_str());
        case VT_DOUBLE:
...
bool RtIni::OpenFile(const char* szFilename, bool bWrite)  // 定义函数，用于打开一个 .ini 文件
{
    ClearData();  // 清除已有数据，以便开始新文件的解析

    int iResult =
        AzothParseIniFile(szFilename, AzothIniErrorCallback, AzothIniParserCallback, this);
    // 调用 AzothParseIniFile 函数解析 .ini 文件，返回结果保存在 iResult 中

    if (!(bWrite && iResult == -1))  // 如果不是在写模式下且文件打开失败（返回 -1），则进入以下逻辑
    {
        if (iResult)  // 如果解析返回值不为 0，则表示存在错误
        {
            switch (iResult)  // 根据不同的错误代码进行处理
            {
                case -1:  // 无法打开文件
                    break;
                default:  // 解析错误
            ...
            return false;  // 返回 false 表示文件打开或解析失败
        }
    }

    releaseAllBuffer();  // 释放所有已分配的缓冲区

    m_szFilename = szFilename;  // 保存文件名
    m_bCanWrite = bWrite;       // 保存是否可写的标志
    m_bInit = true;             // 标记初始化成功
    m_pCurrentSection = NULL;   // 重置当前的 section 指针
    m_pCurrentItem = NULL;      // 重置当前的 item 指针

    return true;  // 返回 true 表示文件打开并解析成功
}
...
bool RtIni::AddSection(const char* szSection) {
    if (!FindSection(szSection)) {
        ...
        if (m_pLastSection)
            m_pLastSection->pNext = m_pCurrentSection;
...
bool RtIni::AddEntry(const char* szSection, const char* szEntry, const char* szValue) {
    ...
    if (m_bSameEntry || !FindEntry(0, szEntry)) {
        ...
        if (m_pCurrentSection->pFirstItem == NULL)
            m_pCurrentSection->pFirstItem = m_pCurrentItem;
        if (m_pCurrentSection->pLastItem)
            m_pCurrentSection->pLastItem->pNext = m_pCurrentItem;
...
bool RtIni::DelSection(const char* szSection) {
    if (FindSection(szSection)) {
        if (m_pCurrentSection->pNext)
            m_pCurrentSection->pNext->pPrev = m_pCurrentSection->pPrev;
        if (m_pCurrentSection->pPrev)
            m_pCurrentSection->pPrev->pNext = m_pCurrentSection->pNext;
        if (m_pCurrentSection->pNext == NULL)
            m_pLastSection = m_pCurrentSection->pPrev;
        if (m_pCurrentSection->pPrev == NULL)
            m_pFirstSection = m_pCurrentSection->pNext;
...
bool RtIni::DelEntry(const char* szSection, const char* szEntry, bool bAllSameEntry) {
    if (bAllSameEntry) {
        while (FindEntry(szSection, szEntry)) {
            ...
            if (m_pCurrentItem->pNext == NULL)
                m_pCurrentSection->pLastItem = m_pCurrentItem->pPrev;
            if (m_pCurrentItem->pPrev == NULL)
                m_pCurrentSection->pFirstItem = m_pCurrentItem->pNext;
...
#define AZOTH_INI_VISIT_ITEM(result, a1, a2, a3, a4, a5)                                           \
    if (pCallbackVisit)                                                                            \
        result = pCallbackVisit(a1, a2, a3, a4, a5);                                               \
    else                                                                                           \
        result = this->OnVisit(a1, a2, a3, a4, a5);

    int iResult = 0;
    if (!FirstSection())
        return iResult;
    do {
        AZOTH_INI_VISIT_ITEM(iResult, AZOTH_INI_SECTION, m_pCurrentSection->szName.c_str(), 0, 0,
                             arg);
        if (iResult)
            return iResult;
        if (FirstEntry()) {
            do {
                AZOTH_INI_VISIT_ITEM(iResult, AZOTH_INI_ENTRY, m_pCurrentItem->szName.c_str(),
                                     m_pCurrentItem->szValue.c_str(), m_pCurrentItem->eType, arg);
                if (iResult)
                    return iResult;
            } while (NextEntry());
        }
...
int RtIni::OnVisit(int iItemType, const char* szArg1, const char* szArg2, int iValueType,
                   void* arg) {
    return 0;
}
