...
bool RtMemoryArchive::IsOverflow(int offset)
{
	...
	{
		m_bIsError = TRUE;
		RtCoreLog().Error("RtMemoryArchive: ar = memory overflow,m_size = %d,offset = %d", m_dataSize,offset);
		return true;
	}
    ...
    )
{
    ...
    }else
    {
        ...
        if (pCBState) pCBState(pFilename, pArc?FRS_COMPLETE:FRS_FAIL, iParam1, iParam2);
        ...
        }else
        {
            ...
            if (pf)
            {
				...
				{ 
					...
					rt2_strncpy2(dir, pt, pf-pt+1); dir[pf-pt+1]=0;
				}
                pf ++;
                pd = rtStrRChr(pf, '.');
                if (pd && pd>pf)
                {
					...
					{ 
						...
						rt2_strncpy2(fname, pf, pd-pf);fname[pd-pf]=0; 
					}
                    if (ext)   
					{ 
						//strncpy(ext, pd+1, pe-pd); ext[pe-pd]=0; 
						rt2_strncpy2(ext, pd+1, pe-pd);ext[pe-pd]=0; 
					}
                }else
                {
                    if (fname) strcpy(fname, pf);
                    if (ext)   ext[0] = 0;
                }
            }else
            {
                ...
                */
                ...
                if (pd && pd>path)
                {
					...
					{ 
						...
						rt2_strncpy2(fname, path, pd-path);fname[pd-path]=0; 
					}
                    if (ext)   
					{
						//strncpy(ext, pd+1, pe-pd); ext[pe-pd]=0; 
