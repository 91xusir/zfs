...
//#define MY_DXTRACE_ERR(str,hr) DXTRACE_ERR(str, hr)
#define MY_DXTRACE_ERR(str,hr) (hr)
                                   ...
                                   DWORD dwPrimaryBitRate )
{
    ...
    if( FAILED( hr = DirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
        return MY_DXTRACE_ERR( TEXT("DirectSoundCreate8"), hr );

    // Set DirectSound coop level 
    if( FAILED( hr = m_pDS->SetCooperativeLevel( hWnd, dwCoopLevel ) ) )
        return MY_DXTRACE_ERR( TEXT("SetCooperativeLevel"), hr );

    DSCAPS dsCaps;
    dsCaps.dwSize = sizeof(DSCAPS);
    if( FAILED( hr = m_pDS->GetCaps( &dsCaps ) ) )
        return MY_DXTRACE_ERR( TEXT("GetCaps"), hr );

    // if (dsCaps.dwFlags & DSCAPS_PRIMARYSTEREO) //支持立体声
    dwPrimaryChannels = dwPrimaryChannels<2 ? dwPrimaryChannels : 2;
    dwPrimaryFreq     = dwPrimaryFreq<44100 ? dwPrimaryFreq : 44100; 

    if (dsCaps.dwFlags & DSCAPS_SECONDARY16BIT) //支持 16 位
    {
        dwPrimaryBitRate = dwPrimaryBitRate<16 ? dwPrimaryBitRate : 16;
    }else
    {
        dwPrimaryBitRate = dwPrimaryBitRate<8 ? dwPrimaryBitRate : 8;
    }

    // Set primary buffer format
    if( FAILED( hr = SetPrimaryBufferFormat( dwPrimaryChannels, dwPrimaryFreq, dwPrimaryBitRate ) ) )
        return MY_DXTRACE_ERR( TEXT("SetPrimaryBufferFormat"), hr );

    // 以下到结束都是初始化3D音效的部分
    if( FAILED( hr = Get3DListenerInterface(&m_pDS3DListener) ) )
        return MY_DXTRACE_ERR( TEXT("Get3DListenerInterface"), hr );

    /*
    DS3DLISTENER ds3d;
    ds3d.dwSize = sizeof(DS3DLISTENER);
    if( FAILED( hr = m_pDS3DListener->GetAllParameters(&ds3d) ) )
        return MY_DXTRACE_ERR( TEXT("GetAllParameters"), hr );
    DS3DLISTENER ds3d;
    ds3d.dwSize = sizeof(DS3DLISTENER);
    ds3d.flDistanceFactor   = 1.0f;
    ds3d.flDopplerFactor    = 10.0f;
    ds3d.flRolloffFactor    = 8.0f;
    ds3d.vOrientFront       = D3DXVECTOR3(0.0f,0.0f,1.0f);
    ds3d.vOrientTop         = D3DXVECTOR3(0.0f,1.0f,0.0f);
    ds3d.vPosition          = D3DXVECTOR3(0.0f,0.0f,0.0f);
    ds3d.vVelocity          = D3DXVECTOR3(0.0f,0.0f,0.0f);

    if( FAILED( hr = m_pDS3DListener->SetAllParameters(&ds3d ,DS3D_IMMEDIATE ) ) )
        return MY_DXTRACE_ERR( TEXT("SetAllParameters"), hr );
    */
                                               ...
                                               DWORD dwPrimaryBitRate )
{
    ...
    if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &pDSBPrimary, NULL ) ) )
        return MY_DXTRACE_ERR( TEXT("CreateSoundBuffer"), hr );
    ...
    if( FAILED( hr = pDSBPrimary->SetFormat(&wfx) ) )
        return MY_DXTRACE_ERR( TEXT("SetFormat"), hr );
...
HRESULT CSoundManager::Get3DListenerInterface( LPDIRECTSOUND3DLISTENER* ppDSListener )
{
    ...
    *ppDSListener = NULL;
    ...
    {
        SAFE_RELEASE( pDSBPrimary );
        return MY_DXTRACE_ERR( TEXT("QueryInterface"), hr );
    }
...
void CSoundManager::Set3DListener(DS3DLISTENER& in3DListener)
{
    m_pDS3DListener->SetAllParameters(&in3DListener, DS3D_IMMEDIATE);
}
                               ...
                               DWORD dwNumBuffers )
{
    ...
    apDSBuffer = RT_NEW LPDIRECTSOUNDBUFFER[dwNumBuffers];
    if( apDSBuffer == NULL )
    {
        hr = E_OUTOFMEMORY;
        goto LFail;
    ...
    if( pWaveFile->GetSize() == 0 )
    {
        ...
        RtCoreLog().Error("RtaSound: 找不到Wave文件[%s]，或者该文件为空.\n", strWaveFileName);
        hr = E_FAIL;
        goto LFail;
    }
    ...
    if ( pwfx == NULL )
    {
        RtCoreLog().Error("RtaSound: [%s]不可用的Wave格式.\n", strWaveFileName);
        hr = E_FAIL;
        goto LFail;
    }

    if ( (dwCreationFlags&DSBCAPS_CTRL3D) && (pwfx->nChannels > 1) )
    {
        // Too many channels in wave.  Sound must be mono when using DSBCAPS_CTRL3D
        RtCoreLog().Error("RtaSound: 用于3D音效的Wave文件必须是单声道的.[%s]\n", strWaveFileName);
        hr = E_FAIL;
        goto LFail;
    }

    if ( (dwCreationFlags&DSBCAPS_CTRL3D) && (pwfx->wFormatTag != WAVE_FORMAT_PCM) )
    {
        // Sound must be PCM when using DSBCAPS_CTRL3D
        RtCoreLog().Error("RtaSound: 用于3D音效的Wave文件必须是PCM格式.[%s]\n", strWaveFileName);
        hr = E_FAIL;
        goto LFail;
    }
    ...
    if( FAILED(hr) )
    {
        ...
        if( hr != DSERR_BUFFERTOOSMALL )
            MY_DXTRACE_ERR( TEXT("CreateSoundBuffer"), hr );
    ...
    for( i=1; i<dwNumBuffers; i++ )
    {
        ...
        {
            MY_DXTRACE_ERR( TEXT("DuplicateSoundBuffer"), hr );
            goto LFail;
        }
    }

    // Init the sound
    pSound->Init( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile );
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSoundManager::CreateFromMemory( CSound* pSound, 
                                        BYTE* pbData,
                                        ULONG  ulDataSize,
                                        LPWAVEFORMATEX pwfx,
                                        DWORD dwCreationFlags, 
                                        ...
                                        DWORD dwNumBuffers )
{
    ...
    if( m_pDS == NULL )
        return CO_E_NOTINITIALIZED;
    if( pbData == NULL || pSound == NULL || dwNumBuffers < 1 )
        return E_INVALIDARG;
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSoundManager::CreateStreaming( CStreamingSound* pStreamingSound, 
                                        LPTSTR strWaveFileName, 
                                        DWORD dwCreationFlags, 
                                        GUID guid3DAlgorithm,
                                        DWORD dwNotifyCount, 
                                        ...
                                        HANDLE hNotifyEvent )
{
    ...
    if( m_pDS == NULL )
        return CO_E_NOTINITIALIZED;
    if( strWaveFileName == NULL || pStreamingSound == NULL || hNotifyEvent == NULL )
        return E_INVALIDARG;
    ...
    aPosNotify = RT_NEW DSBPOSITIONNOTIFY[ dwNotifyCount ];
    if( aPosNotify == NULL )
                                                          ...
                                                          aPosNotify ) ) )
    {
        ...
        SAFE_DELETE( aPosNotify );
        return MY_DXTRACE_ERR( TEXT("SetNotificationPositions"), hr );
    }
    ...
    // Init the sound
    pStreamingSound->Init( pDSBuffer, dwDSBufferSize, pWaveFile, dwNotifySize );
...
// Desc: 
//-----------------------------------------------------------------------------
void CSound::Init( LPDIRECTSOUNDBUFFER* apDSBuffer, DWORD dwDSBufferSize, DWORD dwNumBuffers, CWaveFile* pWaveFile )
{
    DWORD i;
    Clear();

    m_apDSBuffer = RT_NEW LPDIRECTSOUNDBUFFER[dwNumBuffers];
...
HRESULT CSound::SetVolume(long lVolume)
{
    ...
    if(m_apDSBuffer && SUCCEEDED( hr = m_apDSBuffer[0]->GetCaps(&dsbCaps)))
    {
        if(dsbCaps.dwFlags & DSBCAPS_CTRLVOLUME )
        {
            if (lVolume>0)          lVolume =0;
            else if (lVolume<-9000) lVolume = -9000;
            hr = m_apDSBuffer[0]->SetVolume(lVolume);
            if (!FAILED(hr))
            {
                m_lCurVolume = lVolume;
            }
        }
    }
...
long CSound::DescendVolume()
{
    const long lDescendVolumeStep = -200;
    if (m_apDSBuffer)
    {
        m_lCurVolume += lDescendVolumeStep;
        if (m_lCurVolume < -5000)
            m_lCurVolume = -6000;
        SetVolume(m_lCurVolume);
    }
    return m_lCurVolume;
}
...
HRESULT CSound::FillBufferWithSound( LPDIRECTSOUNDBUFFER pDSB, BOOL bRepeatWavIfBufferLarger )
{
    ...
    if( FAILED( hr = RestoreBuffer( pDSB, NULL ) ) ) 
        return MY_DXTRACE_ERR( TEXT("RestoreBuffer"), hr );

    // Lock the buffer down
    if( FAILED( hr = pDSB->Lock( 0, m_dwDSBufferSize, 
                                 &pDSLockedBuffer, &dwDSLockedBufferSize, 
                                 NULL, NULL, 0L ) ) )
        return MY_DXTRACE_ERR( TEXT("Lock"), hr );
    ...
    if( FAILED( hr = m_pWaveFile->Read( (BYTE*) pDSLockedBuffer,
                                        ...
                                        &dwWavDataRead ) ) )           
        return MY_DXTRACE_ERR( TEXT("Read"), hr );
        ...
        {       
            ...
            {  
                ...
                if( FAILED( hr = m_pWaveFile->ResetFile() ) )
                    return MY_DXTRACE_ERR( TEXT("ResetFile"), hr );
...
HRESULT CSound::RestoreBuffer( LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored )
{
    ...
    if( FAILED( hr = pDSB->GetStatus( &dwStatus ) ) )
        return MY_DXTRACE_ERR( TEXT("GetStatus"), hr );
...
//-----------------------------------------------------------------------------
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, DS3DBUFFER * pDS3DBuffer, LONG lVolume, bool b3D )
{
    HRESULT hr;
    BOOL    bRestored;
    ...
    if( pDSB == NULL )
        return MY_DXTRACE_ERR( TEXT("GetFreeBuffer"), E_FAIL );
    ...
    if( bRestored )
    {
        // The buffer was restored, so we need to fill it with RT_NEW data
        if( FAILED( hr = FillBufferWithSound( pDSB, FALSE ) ) )
            return MY_DXTRACE_ERR( TEXT("FillBufferWithSound"), hr );
    ...
    if (b3D)
    {
        ...
        {
            SAFE_RELEASE(m_pDS3DBuffer);
            if (FAILED( hr = pDSB->QueryInterface(IID_IDirectSound3DBuffer,(LPVOID *)(&m_pDS3DBuffer) ) ) )
                return MY_DXTRACE_ERR( TEXT("QueryInterface"), hr );
        }
        ...
        m_pDS3DBuffer->GetAllParameters(&ds3DBuffer);

        ds3DBuffer.flMaxDistance    = pDS3DBuffer->flMaxDistance;
        ds3DBuffer.flMinDistance    = pDS3DBuffer->flMinDistance;
        ds3DBuffer.vPosition        = pDS3DBuffer->vPosition;
        ds3DBuffer.vVelocity        = pDS3DBuffer->vVelocity;
        ds3DBuffer.dwMode           = DS3DMODE_NORMAL;
        */

        if (FAILED( hr = (m_pDS3DBuffer->SetAllParameters(pDS3DBuffer, DS3D_IMMEDIATE) ) ) )
            return MY_DXTRACE_ERR( TEXT("SetAllParameters"), hr );
    }

    const bool bCanControlVolume = true;
    if (bCanControlVolume)
    {
        m_lCurVolume = lVolume;
...
void CSound::Set3dParameters(LPDS3DBUFFER pDS3DBuffer)
{
    if (m_pDS3DBuffer && pDS3DBuffer)
    {
        HRESULT hr;
        /*
        DS3DBUFFER ds3d;
        ds3d.dwSize = sizeof(DS3DBUFFER);
        if ( FAILED( hr = m_pDS3DBuffer->GetAllParameters(&ds3d) ) )
            DXTRACE_MSG( TEXT("GetAllParameters") );
        ds3d.flMaxDistance = pDS3DBuffer->flMaxDistance;
        ds3d.flMinDistance = pDS3DBuffer->flMinDistance;
        ds3d.vPosition = pDS3DBuffer->vPosition;
        ds3d.vVelocity = pDS3DBuffer->vVelocity;
        ds3d.vConeOrientation = pDS3DBuffer->vConeOrientation;
        ds3d.dwInsideConeAngle = pDS3DBuffer->dwInsideConeAngle;
        ds3d.dwOutsideConeAngle = pDS3DBuffer->dwOutsideConeAngle;
        */
        if ( FAILED( hr = m_pDS3DBuffer->SetAllParameters(pDS3DBuffer, DS3D_IMMEDIATE) ) )
            DXTRACE_MSG( TEXT("SetAllParameters") );
    }
}
...
void CStreamingSound::Init( LPDIRECTSOUNDBUFFER pDSBuffer, DWORD dwDSBufferSize, CWaveFile* pWaveFile, DWORD dwNotifySize ) 
{
    CSound::Init(&pDSBuffer, dwDSBufferSize, 1, pWaveFile);
    m_dwLastPlayPos     = 0;
    m_dwPlayProgress    = 0;
...
HRESULT CStreamingSound::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
    ...
    if( FAILED( hr = m_apDSBuffer[0]->GetCurrentPosition( &dwCurrentPlayPos, NULL ) ) )
        return MY_DXTRACE_ERR( TEXT("GetCurrentPosition"), hr );
...
HRESULT CWaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            if( NULL == ( hResData = LoadResource( NULL, hResInfo ) ) )
                return MY_DXTRACE_ERR( TEXT("LoadResource"), E_FAIL );

            if( 0 == ( dwSize = SizeofResource( NULL, hResInfo ) ) ) 
                return MY_DXTRACE_ERR( TEXT("SizeofResource"), E_FAIL );

            if( NULL == ( pvRes = LockResource( hResData ) ) )
                return MY_DXTRACE_ERR( TEXT("LockResource"), E_FAIL );

            m_pResourceBuffer = RT_NEW CHAR[ dwSize ];
    ...
    else
    {
        m_hmmio = mmioOpen( strFileName, NULL, MMIO_ALLOCBUF  | 
                                                  ...
                                                  MMIO_CREATE );
        if( NULL == m_hmmio )
            return MY_DXTRACE_ERR( TEXT("mmioOpen"), E_FAIL );

        if( FAILED( hr = WriteMMIO( pwfx ) ) )
        {
            mmioClose( m_hmmio, 0 );
            return MY_DXTRACE_ERR( TEXT("WriteMMIO"), hr );
        }
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    if( ( 0 != mmioDescend( m_hmmio, &m_ckRiff, NULL, 0 ) ) )
        return MY_DXTRACE_ERR( TEXT("mmioDescend"), E_FAIL );
    ...
    // if there are extra parameters at the end, we'll ignore them
       if( ckIn.cksize < (LONG) sizeof(PCMWAVEFORMAT) )
           return MY_DXTRACE_ERR( TEXT("sizeof(PCMWAVEFORMAT)"), E_FAIL );

    // Read the 'fmt ' chunk into <pcmWaveFormat>.
    if( mmioRead( m_hmmio, (HPSTR) &pcmWaveFormat, 
                  sizeof(pcmWaveFormat)) != sizeof(pcmWaveFormat) )
        return MY_DXTRACE_ERR( TEXT("mmioRead"), E_FAIL );

    // Allocate the waveformatex, but if its not pcm format, read the next
    // word, and thats how many extra bytes to allocate.
    if( pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM )
    {
        m_pwfx = (WAVEFORMATEX*) RT_NEW CHAR[ sizeof(WAVEFORMATEX) ];
        if( NULL == m_pwfx )
            return MY_DXTRACE_ERR( TEXT("m_pwfx"), E_FAIL );
    ...
    else
    {
        ...
        m_pwfx = (WAVEFORMATEX*) RT_NEW CHAR[ sizeof(WAVEFORMATEX) + cbExtraBytes ];
        if( NULL == m_pwfx )
            return MY_DXTRACE_ERR( TEXT("RT_NEW"), E_FAIL );
    ...
    {
        SAFE_DELETE( m_pwfx );
        return MY_DXTRACE_ERR( TEXT("mmioAscend"), E_FAIL );
    }
    ...
    else 
    {
        ...
        if( m_dwFlags == WAVEFILE_READ )
        {
            ...
            if( -1 == mmioSeek( m_hmmio, m_ckRiff.dwDataOffset + sizeof(FOURCC),
                            SEEK_SET ) )
                return MY_DXTRACE_ERR( TEXT("mmioSeek"), E_FAIL );
        ...
        else
        {
            ...
            if( 0 != mmioCreateChunk( m_hmmio, &m_ck, 0 ) ) 
                return MY_DXTRACE_ERR( TEXT("mmioCreateChunk"), E_FAIL );

            if( 0 != mmioGetInfo( m_hmmio, &m_mmioinfoOut, 0 ) )
                return MY_DXTRACE_ERR( TEXT("mmioGetInfo"), E_FAIL );
        }
    }
...
HRESULT CWaveFile::Read( BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead )
{
    ...
    else 
    {
        ...
        for( DWORD cT = 0; cT < cbDataIn; cT++ )
        {
            ...
            if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
            {
                if( 0 != mmioAdvance( m_hmmio, &mmioinfoIn, MMIO_READ ) )
                    return MY_DXTRACE_ERR( TEXT("mmioAdvance"), E_FAIL );

                if( mmioinfoIn.pchNext == mmioinfoIn.pchEndRead )
                    return MY_DXTRACE_ERR( TEXT("mmioinfoIn.pchNext"), E_FAIL );
            }
        ...
        if( 0 != mmioSetInfo( m_hmmio, &mmioinfoIn, 0 ) )
            return MY_DXTRACE_ERR( TEXT("mmioSetInfo"), E_FAIL );
...
// Desc: Support function for reading from a multimedia I/O stream
//       pwfxDest is the WAVEFORMATEX for this RT_NEW wave file.  
//       m_hmmio must be valid before calling.  This function uses it to
...
HRESULT CWaveFile::WriteMMIO( WAVEFORMATEX *pwfxDest )
{
    ...
    if( pwfxDest->wFormatTag == WAVE_FORMAT_PCM )
    {
        if( mmioWrite( m_hmmio, (HPSTR) pwfxDest, 
                       sizeof(PCMWAVEFORMAT)) != sizeof(PCMWAVEFORMAT))
            return MY_DXTRACE_ERR( TEXT("mmioWrite"), E_FAIL );
    }   
    else 
    {
