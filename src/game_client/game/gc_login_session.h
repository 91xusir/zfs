...
public:
	///
	bool			Login(const char* username, const char* password, char code,bool ishall = false, bool bForceLogin = false);

	///
	bool			QueryAccountInfo();

	//人物角色创建
	bool			CreateChar(const char* name, char metier, char bIsS, char bSex, short headModelID, const char* password, short iAnimal, short iImage);
