...
public:
	...
	virtual void OnTimeOut(DWORD dwParam1, DWORD dwParam2, DWORD dwParam3)
	{
		...
		if (pActor) pActor->OnAttackArrive((WORD)dwParam2, false);
		unguard;
	}
	...
	virtual void OnArrive(DWORD dwParam1, DWORD dwParam2, DWORD dwParam3)
	{
		guard;
		GcActor* pActor = FindActor(dwParam1);
		if (pActor) pActor->OnAttackArrive((WORD)dwParam2, true);
		unguard;
	}
};
//////////////////////////////////////////////////////////////////////////
...
GcActor::GcActor() : mBaseActor(this), mItem(((CGameClientFrame*)GetApp())->m_pItemManager, &(this->m_core), this)
{
	guard;
	if (!m_bLoadPRI) LoadPRI();
	m_wAttackID         = 1;
	mpTrump             = NULL;
	m_pRidePet          = NULL;
	m_dwID              = 0;
	...
	if (csv.LoadTableFile(R(RES_TABLE_PRI_PLAYER)))
	{
		for (j=0; j<ACMD_MAX; j++)
		{
			for (i=0; i<ACMD_MAX; i++)
			{
				...
				{
					m_PRIPlayer[j][i] = csv[GetCmdName((ECommands)j)][GetCmdName((ECommands)i)].Int();
				}else
				{
					m_PRIPlayer[j][i] = -1;
				}
	...
	if (csv1.LoadTableFile(R(RES_TABLE_PRI_NETUSER)))
	{
		for (j=0; j<ACMD_MAX; j++)
		{
			for (i=0; i<ACMD_MAX; i++)
			{
				...
				{
					m_PRINetuser[j][i] = csv1[GetCmdName((ECommands)j)][GetCmdName((ECommands)i)].Int();
				}else
				{
					m_PRINetuser[j][i] = -1;
				}
...
DWORD GcActor::ID()
{
	return ((((m_dwID&0xFFFF0000)>>16)&0x0000FFFF) | (((m_dwID&0x0000FFFF)<<16)&0xFFFF0000));
}

void GcActor::SetID(DWORD vID)
{
	m_dwDummyID2 = m_dwDummyID = vID;
	m_dwID = ((((vID&0xFFFF0000)>>16)&0x0000FFFF) | (((vID&0x0000FFFF)<<16)&0xFFFF0000));
}
...
bool GcActor::CreateGraphData(SCreModel* vpModel, short HeadModelID) //创建图形
{
	guard;
	m_pModel = vpModel;
	//m_cModelFlag = cFlag;
...
void GcActor::Run(float fSecond)                                                    //逻辑循环
{
	...
	m_Skill.Run(fSecond*1000);
	mItem.Run(fSecond*1000);

	RunCommand(fSecond);

	mBaseActor.Run(fSecond);
	...
	if ( !m_pNpc && mBaseActor.GetGraph())
		GetGraph()->SetVisible(m_fVisible);

	if (m_bIsDead != m_cIsDeadNet)
	{
		m_dwDeadTime += fSecond * 1000;
		if (m_dwDeadTime > 3000)
		{
			m_dwDeadTime = 0;
...
bool GcActor::RunCommand(float fSecond)
{
	...
	if (m_pCurCmd)
	{
		...
		case ACMD_MOVETO:
			...
			if (!mBaseActor.MoveToPosition(m_pCurCmd->f[0], m_pCurCmd->f[1], m_pCurCmd->fDistance, m_pCurCmd->fTime, fSecond, false))
			{
				m_bDoMoveCmd = false;
				CommandFinished();
			}
		...
		case ACMD_FOLLOW:
			...
			if (pTarget)
			{
				...
				if (!mBaseActor.MoveToPosition(pM16->_30, pM16->_31, m_pCurCmd->fDistance, m_pCurCmd->fTime, fSecond, true))
				{
					CommandFinished();
				}
			}else
		...
		case ACMD_DEATH_ACT:
			if (m_pCurCmd->fRunTime<g_fClientDieTime)
			{
				...
				if ( mBaseActor.m_vPoseID != GcBaseActor::POSE_BODY && mBaseActor.m_vPoseID != GcBaseActor::POSE_DEAD && NpcType() != ENT_SPIRIT )
				{
				...
				if (pGraph && m_bIsDead > 0 && m_eNpcType != ENT_USER)
				{
					...
					{
						mBaseActor.GetGraph()->SetVisible((g_fClientDieTime-m_pCurCmd->fRunTime)/g_fClientDieTime);
					...
					if (m_pCurCmd->dw[1] && this != GetPlayer()) // dw[1]飞
					{
                        ...
                        pGraph->SetMatrix_Row(3, RtgVertex3(_v3->x, _v3->y, _v3->z + g_fClientDieFlyHeight * (fSecond/g_fClientDieTime)));
					}
					if (m_pCurCmd->dw[2]) // dw[2]入地
					{
                        const RtgVertex3* _v3 = pGraph->GetMatrix()->Row(3);
                        pGraph->SetMatrix_Row(3, RtgVertex3(_v3->x, _v3->y, _v3->z + g_fClientDieSinkHeight * (fSecond/g_fClientDieTime)));
					}
				}
			}else
			{
	...
	}else
	{
		if (m_listCmd.size())
		{
			...
			while (m_pCurCmd->cSubType==2)
			{
				SCmd cmd = *m_pCurCmd;
				m_listCmd.pop_front();
				if (cmd.bLoop && m_listCmd.size()==0) // 如果是一个循环命令，并且后面没有命令，就再添加一次
				{
...
//// #endif
	...
	}else if ( mBaseActor.m_vPoseID != GcBaseActor::POSE_DEAD && mBaseActor.m_vPoseID != GcBaseActor::POSE_BODY )
	{
		mBaseActor.PlayPose(GcBaseActor::POSE_BODY, false);
	}
...
// 		}
		...
		if (this == GetPlayer() && g_layerMain->m_formChar->IsVisible() && g_layerMain->m_formChar->GetNowShowType() == CUIForm_Char::EQUIP)
		{
...
void GcActor::RunIdle(float fSecond)
{
	...
	if (m_pNpc && (m_pNpc->Type==7||m_pNpc->Type==8))
	{	// 机关待机
		if ( !mBaseActor.GetGraph()->p()->IsPlayingPose() )
		{
			if (m_bTriggerClose)
...
bool GcActor::SetModel(short Id, short vHeadModelID)
{
	...
	if(Id == 0)
		return false;
	if(m_core.ModelId==Id) return false;
	...
	if (!m_pHUD.IsEmpty())
	{
		if (bDisplay && !m_bHideHUD)
		{
			...
			if(g_layerMain)
			{
				...
				if(g_layerMain->m_eSetting.m_bShowOtherName && (this->m_eNpcType == ENT_USER || this->m_eNpcType == ENT_PET))
				{
					m_pHUD->Show(true);
					m_pHUD->ShowNameSeparate(true);
					showHUDName = true;
				}
				if(g_layerMain->m_eSetting.m_bShowNPCName && (this->NpcType() == ENT_NPC_SERVICE || this->NpcType() == ENT_MONSTER || this->NpcType() == ENT_NPC_COMBATIVE || this->NpcType() == ENT_SPIRIT))
				{
					m_pHUD->Show(true);
					m_pHUD->ShowNameSeparate(true);
					showHUDName = true;
...
void GcActor::RemoveCurCommand()
{
	...
	if (m_pCurCmd->cSubType==1)
	{
		...
		while (m_listCmd.size())
		{
			...
			{
				LOG("RemoveCurCommand: 命令出错了，在读取子命令[cSubType==1]的时候读到了非法的命令.\n");
				break;
			}
		}
		m_pCurCmd = NULL;
...
{
	guard;
	if (GetRideState()==GcActor::ERS_PET) return;
	...
	while (m_listCmd.size())
	{
		...
		if (!bHavePRI)
		{
			if (bCtrl)  iPRI = m_PRIPlayer [vCmd.eCmd][pLast->eCmd];
			else        iPRI = m_PRINetuser[vCmd.eCmd][pLast->eCmd];
		}else
		{
			bHavePRI = false;
		}
		...
		}else if (iPRI==2) // 插入到第一个位置
		{
			...
			bInsert = true;
			break;
		}else if (iPRI==3) // 根据子命令进行排挤，如果这个命令是当前命令就根据当前子命令进行判断，否则直接挤掉
		{
	...
	case ACMD_DEATH:
		...
		if (m_pModel)
		{
			if(m_bIsDead == 4)
			{
				...
				{
				default:
				case MODEL_DEAD_TYPE_TRANS_FLY:  cmd.dw[0] = 1; cmd.dw[1] = 1; cmd.dw[2] = 0; break;
				case MODEL_DEAD_TYPE_TRANS_SINK: cmd.dw[0] = 1; cmd.dw[1] = 0; cmd.dw[2] = 1; break;
				case MODEL_DEAD_TYPE_TRANS:      cmd.dw[0] = 1; cmd.dw[1] = 0; cmd.dw[2] = 0; break;
				case MODEL_DEAD_TYPE_SINK:       cmd.dw[0] = 0; cmd.dw[1] = 0; cmd.dw[2] = 1; break;
				}
				m_listCmd.push_back(cmd);
	...
	case ACMD_MOVETO:
		{
			...
			if (fabs(pPos[0]-cmd.f[0])<10.f && fabs(pPos[1]-cmd.f[1])<10.f && fabs(pPos[1]-cmd.f[1])<10.f)
			{
				break;
			}
...
void GcActor::AttackSomebody(WORD wAttackID, DWORD dwTargetID, int iNetCheck, float fTime)
{
	...
	if (this == GetPlayer()) cmd.fTime = -1.f;
	else cmd.fTime = fTime;
	cmd.eCmd = ACMD_ATTACK_SOMEBODY;
	cmd.bLoop = true;
...
void GcActor::UseIntonate(WORD wAttackID, WORD wSkillID, float fX, float fY, float fZ, int iNetCheck, float fTime)
{
	guard;
	SCmd cmd;
	...
	{
		for(vector <int>::iterator it = pSkill->wManyStrSkillSub.begin() ;it!= pSkill->wManyStrSkillSub.end() ; it++)
		{
	...
	if (cmd.fDistance<0.f) cmd.fDistance = 0.f;
	cmd.f[0] = fX;
	cmd.f[1] = fY;
	cmd.f[2] = fZ;
...
// 对人攻击技能吟唱
void GcActor::UseIntonate(WORD wAttackID, WORD wSkillID, DWORD dwTargetID, int iNetCheck, float fTime)
{
	guard;
	...
	{
		SCmd* pLast = &(m_listCmd.back());
		if (pLast->eCmd==ACMD_INTONATE && pLast->dw[0]==dwTargetID && pLast->wSkillID==wSkillID)
		{
			return; // 上一个命令是相同的命令，就直接返回
		}
	}
...
// 对定点施放技能
void GcActor::UsePositionSkill(WORD wAttackID, WORD wSkillID, float fX, float fY, float fZ, int iNetCheck, float fTime)
{
	guard;
	SCmd cmd;
	SSkill* pSkill = Skill()->FindSkill(wSkillID);
...
// 对某人施放技能
void GcActor::UseTargetSkill(WORD wAttackID, WORD wSkillID, DWORD dwTargetID, int iNetCheck, float fTime)
{
	guard;
	...
	{
		SCmd* pLast = &(m_listCmd.back());
		if (pLast->eCmd==ACMD_SKILL_SOMEBODY && pLast->dw[0]==dwTargetID && pLast->wSkillID==wSkillID)
		{
			return; // 上一个命令是相同的命令，就直接返回
		}
	}
...
void GcActor::DoCommand(SCmd* pCmd)
{
	...
	if (pCmd->cTargetType==SKILL_TARGET_TYPE_ACTOR)
	{
		...
		{
			if ( pActor  && pActor->m_bIsDead != 4 && ( pCmd->eCmd == ACMD_COLLECT && pActor->m_bIsDead == 2 ) ||
				( (pCmd->eCmd == ACMD_PICKITEM ||pCmd->eCmd == ACMD_FOLLOW) && pActor && (pActor->m_bIsDead == 1 || pActor->m_bIsDead == 2)) ||
				pCmd->eCmd == ACMD_PLAY_POSE || pCmd->eCmd == ACMD_DEATH_ACT || pCmd->eCmd == ACMD_PLAYTASKITEM )
			{
	...
	case ACMD_TALKNPC:
		{
			...
			if(pActor)
			{
				...
				float dis = ((pm1->_30-pm2->_30)*(pm1->_30-pm2->_30)+(pm1->_31-pm2->_31)*(pm1->_31-pm2->_31)); 
				fD = sqrt(dis);
			}
			if (fD <= m_pCurCmd->fDistance + 10)
			{
				if (this==GetPlayer())
				{
					if (!pActor->m_pNpc || pActor->m_pNpc->Type == 8)		return;
	...
	case ACMD_PICKITEM:
		{
			...
			if (Distance2(pActor) < (fD*fD) && (pActor->m_bIsDead == 1 || pActor->m_bIsDead == 2 || pActor->m_bIsDead == 3))
			{
			...
			if (pSkill)
			{
				...
				if (this == GetPlayer())
				{
					...
					if (m_pCurCmd->cTargetType==SKILL_TARGET_TYPE_ACTOR)
					{
						...
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( pCmd->wSkillID&0x00FF) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pCmd->wSkillID&0x0FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( pAttack->wAttackID&0x00FF) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pAttack->wAttackID&0x0FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( pCmd->dw[0]&0x000000FF) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pCmd->dw[0]&0x0000FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pCmd->dw[0]&0x00FF0000)>>16) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pCmd->dw[0]&0xFF000000)>>24) ];
					...
					}else if (m_pCurCmd->cTargetType==SKILL_TARGET_TYPE_POSITION)
					{
						...
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw1&0x0000FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw1&0x00FF0000)>>16) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw1&0xFF000000)>>24) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( dw2&0x000000FF) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw2&0x0000FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw2&0x00FF0000)>>16) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw2&0xFF000000)>>24) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( dw3&0x000000FF) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw3&0x0000FF00)>>8) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw3&0x00FF0000)>>16) ];
						crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((dw3&0xFF000000)>>24) ];
						pCmd->dw[0] = dw1;
						pCmd->dw[1] = dw2;
						pCmd->dw[2] = dw3;
						pCmd->wCheck = crcvalue ^ CRC16_XOR_VALUE;
					...
					if (m_pCurCmd->cTargetType==SKILL_TARGET_TYPE_ACTOR)
					{
						GetWorld()->OnNetUploadAttackTarget(pCmd->wSkillID, pAttack->wAttackID, pCmd->dw[0], pCmd, pCmd->iNetCheck);
					}else if (m_pCurCmd->cTargetType==SKILL_TARGET_TYPE_POSITION)
					{
						GetWorld()->OnNetUploadAttackPosition(pCmd->wSkillID, pAttack->wAttackID, pCmd->dw, pCmd, 1);
					}else
					{
						GetWorld()->OnNetUploadAttackSelf(pCmd->wSkillID, pAttack->wAttackID, pCmd, 2);
					}
				}
				else if (this == GetPlayerPet()->GetPetActor())
				{
					...
					{
						LOG("DoCommand:ACMD_ATTACK 响应攻击事件的时候没有找到该事件，该事件可能已经被删除.\n");
					}
	...
	case ACMD_MOVETO:
		...
		fD = fsqrt((m_pCurCmd->f[0]-pM16->_30)*(m_pCurCmd->f[0]-pM16->_30)+(m_pCurCmd->f[1]-pM16->_31)*(m_pCurCmd->f[1]-pM16->_31));
		if ( fD > m_pCurCmd->fDistance)
		{
			...
			if (m_pModel)
			{
				if (m_pCurCmd->fTime<0)
				{
					if (m_eRideState==GcActor::ERS_ACTOR)
					{
						...
						fSpeed = GetWorld()->m_pPet->m_core.MovSpeed.GetValue() / GetWorld()->m_pPet->m_pModel->MoveSpeed;
					}
					else
					{
						// fSpeed = 80.f/m_pModel->MoveSpeed;
						fSpeed = GetWorld()->m_pPlayer->m_core.MovSpeed.GetValue() / GetWorld()->m_pPlayer->m_pModel->MoveSpeed;
					}
				}else
				{
					...
					{
						fSpeed = ((fD-m_pCurCmd->fDistance)/m_pCurCmd->fTime)/m_pModel->MoveSpeed;
					}
				}
			...
			{
				if (m_bTrace || m_eRideState==GcActor::ERS_ACTOR || NpcType() == ENT_PET || NpcType() == ENT_CALLNPC)
				{
	...
	case ACMD_FOLLOW:
		...
		if (fD > m_pCurCmd->fDistance)
		{
			...
			if (m_eRideState==GcActor::ERS_ACTOR || NpcType() == ENT_PET || NpcType() == ENT_CALLNPC)
			{
				mBaseActor.PlayPose(GcBaseActor::POSE_RUN, true, NULL, fSpeed);
			}else
				mBaseActor.PlayPose(GcBaseActor::POSE_WALK, true, NULL, fSpeed);
	...
	case ACMD_ATTACK:
		...
		if (Distance2(pActor) < (fD*fD) && pActor->m_bIsDead == 0)
		{
			...
			if (this==GetPlayer())
			{
				...
				if (m_wAttackID==0) m_wAttackID = 1;
...
#endif
				GetWorld()->OnNetUploadAttackTarget(pCmd->wSkillID, pAttack->wAttackID, pCmd->dw[0], pCmd, pCmd->iNetCheck); // 普通攻击
	...
	case ACMD_USESKILL:
		...
		}else if (m_pCurCmd->cTargetType==SKILL_TARGET_TYPE_POSITION)
		{
			fD = m_pCurCmd->fDistance;
			pM16 = mBaseActor.GetMatrix();
			if (((m_pCurCmd->f[0]-pM16->_30)*(m_pCurCmd->f[0]-pM16->_30)+(m_pCurCmd->f[1]-pM16->_31)*(m_pCurCmd->f[1]-pM16->_31)) > (fD*fD))
			{
				CommandFinished();
				// RemoveCurCommand();
				break;
		...
		if (pSkill)
		{
			...
			{   
				...
				if ((pWeaClass1 && pWeaClass1->bBothHands) || (pWeaClass1 && pWeaClass2 && !ItemIsShield(Item2) && !ItemIsShield(Item1)))
				...
				if (szREffect)
				{
					const char* pPlayEffectLink = pSkill->szREfLink[0] ? pSkill->szREfLink : NULL;					
					mBaseActor.PlayEffect(szREffect, true, pPlayEffectLink, RtgVertex3(0.f, 0.f, 0.f), false,mBaseActor.m_fLastPoseSpeed);
				}
			}
	...
	case ACMD_PLAY_POSE:
		if ( m_bIsDead > 0 )
		{
			if ((GcBaseActor::EPoses)pCmd->dw[0] != GcBaseActor::POSE_DEAD && (GcBaseActor::EPoses)pCmd->dw[0] != GcBaseActor::POSE_RELIVE)
			{
				ClearCommand();
				return;
			}
...
float GcActor::Distance2(GcActor* pActor)
{
	...
	pm2 = pActor->GetMatrix();
	float dis = ((pm1->_30-pm2->_30)*(pm1->_30-pm2->_30)+(pm1->_31-pm2->_31)*(pm1->_31-pm2->_31)); 
	if (dis<0 || pActor->m_pModel==NULL) return 0.f;
	dis = sqrt(dis) - pActor->m_pModel->ModelRadius;
	if(dis < 0) return 0.f;
	return dis * dis;
	unguard;
...
void GcActor::OnAttackEvent(SAttack* pAttack)
{
	...
	if (NpcType() == ENT_PET)
	{
		RtCoreLog().Debug("pet pAttack->wAttackID:%d, m_curAttackId: %d",pAttack->wAttackID,m_curAttackId);
	}
	// test
		...
		if (pAttack->pSkill->bFly)
		{
			...
			if (pTarget)
			{
				...
				if (pInstT)
				{
					...
					pMissile = RT_NEW CBeelineMissile(vPos, TargetVertex3, (char*)pAttack->pSkill->szFlyEffect, &s_ActorMissileListener, pAttack->pSkill->wFlySpeed, pAttack->pSkill->cFlyType==2);
	...
	}else // 普通攻击
	{
		...
		//	}
		//}
		else if (ItemIsWeapon_Wand(item.type) && m_cShapeshiftNet == 0)
		{ // 法杖
			SItemBase *pItemClass = GetWorld()->m_pItemManager->GetItem(item.type);
			if (pItemClass)
			{
	...
	for (; itDamage!=pAttack->listDamage.end(); itDamage++)
	{
		...
		if (pActor)
		{
			...
			if (pAttack->cAnswer<3)
			{
				...
				{
					pActor->mBaseActor.PlayEffect(szHitEffect, true, szHitLink, RtgVertex3(0,0,0));
				}
				else
				{
					if( pActor->IsValid() && (*itDamage).ret != USE_SKILL_MISS )
						pActor->mBaseActor.PlayEffect(szHitEffect, false, NULL, RtgVertex3(0,0,pActor->GetGraph()->p()->GetBoundingBox()->vPos.z));
				}
			...
			{
				bool lasthit = false;
				if (pActor->m_pNpc==NULL || (pActor->m_pNpc->Type!=4 && pActor->m_pNpc->Type!=5))
				{
					if(!m_pCurCmd)
					{
						lasthit = true;

						pActor->ShowDamageNumber((*itDamage).ret,(*itDamage).sDamage,(*itDamage).eleDmg,(*itDamage).criHit,itDamage->eleSpe);
					...
					else 
					{
						if(m_pCurCmd->eCmd == ACMD_ATTACK)
						{
							if(ItemIsWeapon_Hoop(item.type)) //双环的普通攻击
							{
								...
								if(itDamage->criHit)
								{
									if(itDamage->sDamage)
									{
										pActor->ShowHeadNotify(CHAR_BAOJIPUTONG,itDamage->sDamage/2);
									}
								}
								else
								{
									pActor->ShowDamageNumber(itDamage->ret,itDamage->sDamage/2,itDamage->eleDmg,itDamage->criHit,itDamage->eleSpe);								
								}
							}
							else
							{
						...
						{
							// find skill hit info
							SSkill *skill = pAttack->pSkill/*Skill()->FindSkill(m_pCurCmd->wSkillID)*/;
							if(skill)
							{
								int total = 0;
								...
								while(itDamage->hits < m_curAttackEvent && safe++ < 10)
								{
									...
									{
										eleDmg[i] = itDamage->eleDmg[i]*(itDamage->hits+1)/total;
									}
									...
									if(itDamage->criHit)
									{
										if(itDamage->sDamage)
										{
											pActor->ShowHeadNotify(CHAR_BAOJIPUTONG,itDamage->sDamage*(itDamage->hits+1)/total);
										}
										if(itDamage->eleDmg[ELEMENT_WATER])
										{
											pActor->ShowHeadNotify(CHAR_BAOJISHUI,eleDmg[ELEMENT_WATER]);
										}
										if(itDamage->eleDmg[ELEMENT_FIRE])
										{
											pActor->ShowHeadNotify(CHAR_BAOJIHUO,eleDmg[ELEMENT_FIRE]);
										}
										if(itDamage->eleDmg[ELEMENT_POISON])
										{
											pActor->ShowHeadNotify(CHAR_BAOJIDU,eleDmg[ELEMENT_POISON]);
										}
									}
									else
									{
										pActor->ShowDamageNumber(itDamage->ret,itDamage->sDamage*(itDamage->hits+1)/total,eleDmg,itDamage->criHit,itDamage->eleSpe);	
									}
									
									itDamage->hits++;
									// LOG1("hits = %d\n",itDamage->hits);
								}
								if(itDamage->hits == skill->wHits) lasthit = true;
							}
						}
					}
				}
...
bool GcActor::AttackAnswer(unsigned short wAttackID, const char* pPoseName)
{
	...
	for (it=m_listAttack.begin(); it!=m_listAttack.end(); it++)
	{
		...
		if (pAttack->wAttackID==wAttackID)
		{
			if (pAttack->cAnswer==0) pAttack->cAnswer = 1;
			pAttack->pPoseName = pPoseName;
			return true;
		}
	}
...
void GcActor::ShowHeadNotify(char *type, short sDamage, bool spec/* = false*/)
{
	char cDir = 0;
	char szStr[20];
	if(this == GetPlayer()) cDir = DIR_RIGHT;
	else cDir = DIR_LEFT;
	if (sDamage>=0)
...
void GcActor::ShowDamageNumber(int ret,short sDamage,short eleDmg[MAX_ELEMENT_ATTR],
							   bool criHit,char eleSpe)
{
	guard;
	char szStr[20];
	char cDir = 0;
	...
	else if(ret == USE_SKILL_OK)
	{
		...
		{
			if(this == GetPlayer()) cDir = DIR_LEFTUP;
			else cDir = DIR_RIGHTUP;
			rt2_sprintf(szStr, "-%d", sDamage);
			if (criHit)
			{
				GetWorld()->m_dynamicNumber.AddString(CHAR_BAOJIPUTONG, 
					GetHUDPos(),2.f,true,cDir,PIC_CHAR);
			}
			GetWorld()->m_dynamicNumber.AddString(szStr, GetHUDPos(), 0xFF00FF00, 1.f,true,PIC_NORMAL);
		}
		else
		{
			rt2_sprintf(szStr, "+%d", -sDamage);
			GetWorld()->m_dynamicNumber.AddString(szStr, GetHUDPos(), 0xFF00FF00,1.f,true,PIC_NOBAD);
		}
		...
		*/
		...
		if(eleDmg[ELEMENT_WATER] > 0)
		{
			if (criHit)
			{
				GetWorld()->m_dynamicNumber.AddString(CHAR_BAOJISHUI, 
					RtgVertex3(GetHUDPos().x,GetHUDPos().y,GetHUDPos().z + offset), 
					2.f,true,cDir,PIC_CHAR);
			}
			rt2_sprintf(szStr, "-%d", eleDmg[ELEMENT_WATER]);
			GetWorld()->m_dynamicNumber.AddString(szStr, 
				RtgVertex3(GetHUDPos().x,GetHUDPos().y,GetHUDPos().z + offset), 
				0xFF00FFFF,1,true,PIC_WATER);
			offset += 10;
		}
		if(eleDmg[ELEMENT_FIRE] > 0)
		{
			if (criHit)
			{
				GetWorld()->m_dynamicNumber.AddString(CHAR_BAOJIHUO, 
					RtgVertex3(GetHUDPos().x,GetHUDPos().y,GetHUDPos().z + offset), 
					2.f,true,cDir,PIC_CHAR);
			}
		...
		if(eleDmg[ELEMENT_POISON] > 0)
		{
			if (criHit)
			{
				GetWorld()->m_dynamicNumber.AddString(CHAR_BAOJIDU, 
					RtgVertex3(GetHUDPos().x,GetHUDPos().y,GetHUDPos().z + offset), 
					2.f,true,cDir,PIC_CHAR);
			}
...
void GcActor::UpdateFromServerDelta(CG_CmdPacket *cmd)
{
	...
	if (GetWorld()->m_pPlayer == this && GetRideState()!=GcActor::ERS_ACTOR)
	{
		...
		if (m_cHpRate<0) m_cHpRate = 0;
		else if (m_cHpRate>100) m_cHpRate = 100;
	}
	...
	if(GetWorld()->m_pPlayer == this || (GetWorld()->m_pPet == this) )
	{
		if (g_layerMain)
		{
			if (g_layerMain->m_formChar)		g_layerMain->m_formChar->Refresh();
			if (g_layerMain->m_formPetProp)		g_layerMain->m_formPetProp->Update();//tim.yang  神兽系统
			if (g_layerMain->m_formCharSimple)  
			{
				g_layerMain->m_formCharSimple->Update();
				g_layerMain->m_formCharSimple->UpdateTaskTrackList();
			}
			if (g_layerMain->m_formPetSimple)   g_layerMain->m_formPetSimple->Update();
			if (g_layerMain->m_formTrump)		g_layerMain->m_formTrump->Refresh();
...
const char* GcActor::GetCmdName(GcActor::ECommands eCmd)
{
	switch (eCmd)
	{
	case ACMD_NONE               : return "NONE";
	case ACMD_BORN               : return "BORN";
	case ACMD_DIE                : return "DIE";
	case ACMD_TELEPORT           : return "TELEPORT";
	case ACMD_MOVETO             : return "MOVETO";
	case ACMD_FOLLOW             : return "FOLLOW";
	case ACMD_ATTACK             : return "ATTACK";
	case ACMD_USESKILL           : return "USESKILL";
	case ACMD_PICKITEM			 : return "PICKITEM";
	case ACMD_INTONATE           : return "INTONATE";
	case ACMD_COLLECT			 : return "COLLECT";
	case ACMD_PLAYTASKITEM		 : return "PLAYTASKITEM";
	case ACMD_CATCHPET			 : return "CATCHPET";
	case ACMD_UNDER_ATTACK       : return "UNDER_ATTACK";
	case ACMD_PLAY_POSE          : return "PLAY_POSE";
	case ACMD_CHANGE_ACTOR       : return "CHANGE_ACTOR";
	case ACMD_UNEQUIP_ALL        : return "UNEQUIP_ALL";
	case ACMD_EQUIP_ALL          : return "EQUIP_ALL";
	case ACMD_DEATH_ACT          : return "DEATH_ACT";
	case ACMD_DELETE_SELF        : return "DELETE_SELF";
	case ACMD_ATTACK_SOMEBODY    : return "ATTACK_SOMEBODY";
	case ACMD_SKILL_SOMEWHERE    : return "SKILL_SOMEWHERE";
	case ACMD_SKILL_SOMEBODY     : return "SKILL_SOMEBODY";
	case ACMD_DEATH              : return "DEATH";
	case ACMD_START_TRIGGER		 : return "START_TRIGGER";
	case ACMD_TALKNPC			 : return "TALKNPC";
	case ACMD_SHAPESHIFT		 : return "SHAPESHIFT";
	}
	return "Unknowe";
}
...
void GcActor::Serialize(char type,CG_CmdPacket *cmd)
{
	...
	if(GetWorld()->m_pPlayer == this || (GetWorld()->m_pPet == this) )
	{
		if (g_layerMain)
		{
			if (g_layerMain->m_formCharProp)    g_layerMain->m_formCharProp->Update();
			if (g_layerMain->m_formPetProp)		g_layerMain->m_formPetProp->Update();//tim.yang  神兽系统
			if (g_layerMain->m_formCharSimple)  g_layerMain->m_formCharSimple->Update();
			if (g_layerMain->m_formPetSimple)   g_layerMain->m_formPetSimple->Update();
			if (g_layerMain->m_formTrump)   g_layerMain->m_formTrump->Refresh();
		}
		if (GetPlayerPet()->GetPetActor() == this &&
			g_layerMain->m_formPetMain->IsVisible() && 
	...
	if (pName && pName[0])
	{
		...
		if (!m_pHUD.IsEmpty())
		{
			if (m_eNpcType==ENT_USER || m_eNpcType==ENT_PET || NpcType() == ENT_CALLNPC)
			{
				...
				{
					rt2_sprintf(g_strStaticBuffer, "%s[%s]", pName, (m_core.Faction==FACTION_ZHOU?R(G_ZHOU):R(G_SHANG)));
					//sprintf(g_strStaticBuffer, "%s", pName);
				}
...
void GcActor::Ride(GcActor* pPet,bool useUserPos/* = false*/)
{
	guard;
	if (pPet==NULL) return;
	if (IsValid()==NULL || pPet->IsValid()==NULL) return;
	...
	if (!pPet->m_pHUD.IsEmpty()) pPet->m_pHUD->Hide();
...
{
	guard;
	if (m_pRidePet==NULL) return;
	if (GetGraph()->p()==NULL) return;
	...
	if (!pPet->m_pHUD.IsEmpty()) pPet->m_pHUD->Show(true);
...
{
	if (g_layerMain->m_formPickItem->IsVisible()) return;
		...
		if ( type == 2  )
		{
			if (pActor->m_pNpc->Type == 4)
			{
				...
				}else
				{
					...
					{
						rt2_snprintf(str,199,R(MSG_SKILL_BAICAOLOW),pActor->m_pNpc->Name.c_str(),pActor->m_pNpc->Level,pSkill->szName);
			...
			else if (pActor->m_pNpc->Type == 5)
			{
				...
				}else
				{
					...
					if (pSkill->iLevel < pActor->m_pNpc->Level)
					{
						rt2_snprintf(str,199,R(MSG_SKILL_YEJINLOW),pActor->m_pNpc->Name.c_str(),pActor->m_pNpc->Level,pSkill->szName);
						PlayerFailOperate(str);
						return;
					}
				}
			...
			else if (pActor->m_pNpc->Type == 1)
			{
				...
				}else
				{
					...
					{
						rt2_snprintf(str,199,R(MSG_SKILL_BOPILOW),pActor->m_pNpc->Name.c_str(),level,pSkill->szName);
						PlayerFailOperate(str);
						return;
					}
				}
...
{
	// 设置成 分解 的状态
	((CGameClientFrame*)GetApp())->SetMouseCapture(cti_Decompose, GAME_CURSOR_DEITEM, NULL, R(请选择要分解的装备));
}
...
int GcActor::GetDungeonMissionItemNum(DWORD typeId, int difficulty)
{
	...
	{
		if (mItem.m_MissionBag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	else if (ItemIsPet(pItemClass->type))
	{
		if (mItem.m_PetBag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	else
	{
		if (mItem.m_Bag.FindTypeWithParams(typeId, paramIndices, paramParams, ItemColor_All, &count))
			return count;
	}
	return 0;
}

void GcActor::ProcessFixedTaskReward(STaskKey& key, int& goldRew, int& expRew, int& lev, float& percent, int& itemId, int& itemNum, int& itemColor, int& skillID)
{
	if(key.key == "gold")
	{
...
void GcActor::FbTaskState(int taskId, bool& bFinished, bool& bCanFinishNow)
{
	...
	if(task->visDif > info->curDif) return;
...
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.IsAbiDodge();
	else      return m_core.IsAbiDodge();
}

bool GcActor::IsAbiUseSkill()
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.IsAbiUseSkill();
	else      return m_core.IsAbiUseSkill();
}

bool GcActor::IsAbiNormalAttack()
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.IsAbiNormalAttack();
	else      return m_core.IsAbiNormalAttack();
}

int GcActor::GetAbiResist()
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.GetAbiResist();
	else      return m_core.GetAbiResist();
}

int GcActor::GetAbiStateHit()
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.GetAbiStateHit();
	else      return m_core.GetAbiStateHit();
}

int GcActor::IsAbiBeAttack()
{
	GcActor* pPet = GetRidePet();
	if (pPet) return pPet->m_core.IsAbiBeAttack();
	else      return m_core.IsAbiBeAttack();
}
...
void GcActor::DoTrumpSkill(SCmd cmd)
{
	...
	}else if (cmd.cTargetType==SKILL_TARGET_TYPE_POSITION)
	{
		fD = cmd.fDistance;
		pM16 = mBaseActor.GetMatrix();
		if (((cmd.f[0]-pM16->_30)*(cmd.f[0]-pM16->_30)+(cmd.f[1]-pM16->_31)*(cmd.f[1]-pM16->_31)) > (fD*fD))
		{
			return;
		}
	}
	...
	if (pSkill)
	{
		...
		if (bIsLocalCmd) // 如果时间为-1就是本地发出来的命令，本地发出的命令就需要上传到服务器
		{
			...
			{
				if (m_Skill.UseSkill(pSkill->wID, cmd.f)) bCanUseSkill = true;
			}
		}
		if (!bCanUseSkill)
		{
		...
		if (bIsLocalCmd) // 如果时间为-1就是本地发出来的命令，本地发出的命令就需要上传到服务器
		{
			...
			{
				unsigned short crcvalue = CRC16_INIT_VALUE;
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( cmd.wSkillID&0x00FF) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((cmd.wSkillID&0x0FF00)>>8) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( pAttack->wAttackID&0x00FF) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((pAttack->wAttackID&0x0FF00)>>8) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ( cmd.dw[0]&0x000000FF) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((cmd.dw[0]&0x0000FF00)>>8) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((cmd.dw[0]&0x00FF0000)>>16) ];
				crcvalue = ( crcvalue << 8 ) ^ crctableWorld[ ( crcvalue >> 8 ) ^ ((cmd.dw[0]&0xFF000000)>>24) ];
			...
			if (cmd.cTargetType==SKILL_TARGET_TYPE_ACTOR)
			{
				GetWorld()->OnNetUploadAttackTarget(cmd.wSkillID, pAttack->wAttackID, cmd.dw[0], &cmd, cmd.iNetCheck);
			}else if (cmd.cTargetType==SKILL_TARGET_TYPE_POSITION)
			{
				GetWorld()->OnNetUploadAttackPosition(cmd.wSkillID, pAttack->wAttackID, cmd.dw, &cmd, 1);
			}else
			{
...
{
	if (GetGraph() && GetGraph()->p() && GetGraph()->p()->IsPlayingPose() && GetGraph()->p()->GetCurrentPose().Name == "skill_q3")
		return true;
	return false;
}
...
void GcActor::SetPetHUD()
{
	...
	m_pHUD->SetPetFlag(true,szPetAbility,dwPetAbilityColor,szPetFuseLev,dwPetFuseLevColor,szPetMaster,dwPetMasterColor);
}
