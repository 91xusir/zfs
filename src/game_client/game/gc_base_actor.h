...
public:
    ...
    const char* PlayPose(EPoses vPose, bool vLoop=false, SSkill* pSkill=NULL, float fSpeed=1.f);
    ...
    bool MoveToPosition(float fX, float fY, float fDistance, float fNeedTime, float fDiffTime, bool bTarget);
	...
	void PlayEffect(const char *file,bool link,const char *slot,RtgVertex3 offset,bool loop=false,float speed=1.0f,const char* szPosename=NULL);
...
private:
	...
	bool FindPath(int vX,int vY,bool vIgnoreActor=false,bool vIgnoreTarget=false);  //寻找路径
