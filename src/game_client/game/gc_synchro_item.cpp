...
void GcRenderItem::Render(RtgCamera& inCamera, RtgDevice& inDevice, RTGRenderMask eMask)
{
	...
	if (iDistance < 500) // 距离远就不显示道具名字
	{
		...
		if (pItemBase)
		{
			...
			else if (color==ItemColor_Green)		dwColor = 0xff30e512; //0xFF6060E3; 
			else if (color==ItemColor_Blue)			dwColor = 0xff00aaff; 
			else if (color==ItemColor_Golden)		dwColor = 0xffd0c002;
			...
			g_workspace.getFontManager()->DrawString(strShowString, rect, pixel, false,SUiTextAlignment(alignCenter, alignNear));
...
void GcRenderItem::RenderHighLight(RtgCamera& inCamera, RtgDevice& inDevice, RtgVertex3 &color,RtgVertex3 &scale)
{
	guard;
...
void GcSynchroItem::OnDownloadActorSnapshot(DWORD dwOID, DWORD dwTID, BYTE itemColor, BYTE itemBind, float fX, float fY, float fZ, BYTE cDir, BYTE dropStyle)
{
	guard;
...
GcSynchroItem::SGcsItem* GcSynchroItem::NewItem(DWORD dwOID, DWORD dwTID, BYTE itemColor, BYTE itemBind, float fX, float fY, float fZ, BYTE cDir, BYTE dropStyle)
{
	guard;
	...
	if (pNew->degree >= PIx2)		pNew->degree -= PIx2;
...
bool GcSynchroItem::HitTest(RtgVertex3& vOrg, RtgVertex3& vDir, bool bGetNearest, DWORD &outOID, DWORD &outType, DWORD &outColor, DWORD &outBind)
{
	guard;
...
{
	return (GetWorld()->m_pPlayer->GetMatrix()->_30 - pItem->xPosFall) * (GetWorld()->m_pPlayer->GetMatrix()->_30 - pItem->xPosFall)
		+  (GetWorld()->m_pPlayer->GetMatrix()->_31 - pItem->yPosFall) * (GetWorld()->m_pPlayer->GetMatrix()->_31 - pItem->yPosFall);
}
...
bool GcSynchroItem::FindMapItems()						//ac.ma
{
	...
	if(pTarget == NULL) return false; //如果搜索不到指定物品，则return
	if(fDMin > 160000) 
