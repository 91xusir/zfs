...
class GcActor {
   public:
    ...
    struct SAttack {
        ...
        BYTE
            cAnswer;  // 0事件存在但是没有播放动作, 1播放动作但是没有到hit点, 2已经到hit点,等待结束了
    ...
    bool IsTrigger() {
        if (!m_pNpc)
            return false;
        return (m_pNpc->Type == 7 || m_pNpc->Type == 8);
    }
    ...
    // fTime 为使用这个命令的时间
    void UseIntonate(WORD wAttackID, WORD wSkillID, float fX, float fY, float fZ, int iNetCheck,
                     float fTime = -1);
    void UsePositionSkill(WORD wAttackID, WORD wSkillID, float fX, float fY, float fZ,
                          int iNetCheck, float fTime = -1);
    void UseIntonate(WORD wAttackID, WORD wSkillID, DWORD dwTargetID, int iNetCheck,
                     float fTime = -1);
    void UseTargetSkill(WORD wAttackID, WORD wSkillID, DWORD dwTargetID, int iNetCheck,
                        float fTime = -1);
    ...
    //技能部分
    void ShowDamageNumber(int ret, short sDamage, short eleDmg[MAX_ELEMENT_ATTR], bool spec = false,
                          char eleSpe = 0);
    ...
    virtual void OnAreaEnter(RtsSceneBlockRtb::SArea* pArea,
                             RtsSceneBlockRtb*        pBlock);  // 当进入某个区域
    virtual void OnAreaExit(RtsSceneBlockRtb::SArea* pArea,
                            RtsSceneBlockRtb*        pBlock);  // 当离开某个区域
   ...
   public:
    ...
    void ProcessFixedTaskReward(STaskKey& key, int& goldRew, int& expRew, int& lev, float& percent,
                                int& itemId, int& itemNum, int& itemColor, int& skillID);
   ...
   protected:
    ...
    // 使用联合体来存储骑乘宠物或主人
    union {
        GcActor* m_pRidePet;  // 骑乘的宠物
        GcActor* m_pMaster;   // 主人
    };
   ...
   public:
    ...
    void SetInActivity(bool bActi) { bActivity = bActi; }
};
