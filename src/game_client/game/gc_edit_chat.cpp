...
void CEditChat::DealText()
{
	...
	{//文本信息有变化 
		if(mDoskey.IsLock())
		{//如果Doskey已经开启
			if(GetWorld()->m_Chat.GetNowChannel() == PERSONAL_CHANNEL && mName != "")
			{//已经不是私聊标记//关闭doskey
				mDoskey.UnLock();
			}
			//else
...
{
	int num=0;
 	for (std::list<std::string>::iterator iterstr = mTextlist.begin(); iterstr != mTextlist.end(); iterstr++)
 	{
...
void CEditChat::AddHyperLinkText(EChildHyperLinkType eChildType, DWORD itemTag, DWORD itemId, DWORD playerID, const std::string &strName, RtwPixel color)
{
	std::string Name = "[";
	Name += strName;
	Name += "]";
...
void CEditChat::Receive(SChannelAtom* vpChannel,long vFromRegionID,char vFromName[],char vMessage[])
{
	guard;
	/* gao 2010.1.4
	当接收到消息时的处理
	*/
	...
	{	
		...
		GetWorld()->m_Chat.m_worldchatFeedback->Add(vpChannel->mMask,Color,
			vpChannel->mDescription.c_str(),vFromName,vMessage,vFromRegionID,false);
	}else
	{
		std::list<CHATCONTENT*>& chatContent = GetWorld()->m_Chat.GetChatContent();
		CHATCONTENT* tmp = RT_NEW CHATCONTENT(vpChannel->mMask,Color,vFromRegionID,vpChannel->mDescription.c_str(),vFromName,vMessage);
		chatContent.push_back(tmp);
		if(chatContent.size()>100)
		{//当聊天内容超过100条的时候，将第一条从list中取出，并进行删除操作
		...
		for(int i= 0;i< MAX_CHANNEL_CHATBOX;i++)
		{
			...
			if((iMask[i] & vpChannel->mMask) == vpChannel->mMask)
			{
				if(vpChannel->mChannel==PERSONAL_CHANNEL)
				{
					...
					if(GetWorld()->m_pPlayer->m_core.Name!=vFromName)
					{
						GetWorld()->m_Chat.m_chatFeedback[i]->Add(vpChannel->mMask,Color,strFrom.c_str(),vFromName,vMessage,vFromRegionID);
					}else
					{
						Color = 0xffa5a5;
						GetWorld()->m_Chat.m_chatFeedback[i]->Add(vpChannel->mMask,Color,strTo.c_str(),(char*)mName.c_str(),vMessage,vFromRegionID);
					}
				}else
				{
					GetWorld()->m_Chat.m_chatFeedback[i]->Add(vpChannel->mMask, Color, vpChannel->mDescription.c_str(), vFromName, vMessage, vFromRegionID);
				}
			}
		}
	}
