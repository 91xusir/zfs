...
//				}
		...
		case CAESTUS:
			{
				if (pItemClass)
				{
					...
					{
						// 男模型
						m_pMaster->GetGraph()->UnloadSkin(((SArmor*)pItemClass)->UpdateSkins2[itemLevel]);			

					}
					//m_pMaster->GetGraph()->UnloadSkin(((SArmor*)pItemClass)->UpdateSkins[itemLevel]);
					if (((SArmor*)pItemClass)->UpdateEffects[itemLevel][0] != '\0')
					{
						m_pMaster->mBaseActor.GetGraph()->RemoveEffect(((SArmor*)pItemClass)->UpdateEffects[itemLevel]);
					}
	...
	else 
	{
		...
		if ( !pItemClass || !m_pMaster)		return;
...
//					m_pMaster->mBaseActor.StopEffect(pArmorLast->UpdateEffects[itemLevelLast]);
					m_pMaster->mBaseActor.GetGraph()->RemoveEffect(pArmorLast->UpdateEffects[itemLevelLast]);
				}
			}
			...
			{
				// 女模型
				m_pMaster->GetGraph()->LoadSkin(((SArmor*)pItemClass)->UpdateSkins[itemLevel], false);//换上新的skin			
			}else
			{
				// 男模型
				m_pMaster->GetGraph()->LoadSkin(((SArmor*)pItemClass)->UpdateSkins2[itemLevel], false);//换上新的skin			

			}

			if (((SArmor*)pItemClass)->UpdateEffects[itemLevel][0] != '\0')
			{
				m_pMaster->mBaseActor.GetGraph()->ApplyEffect(((SArmor*)pItemClass)->UpdateEffects[itemLevel]);
			}
		}else if ( ItemIsTrump(item) )
		{
			...
			if (ItemID_IsValid(m_nLastItemID[part]))
			{
				...
				if (pWea && pWea->Models1[m_nLastItemID[part].level][0]!='\0')
					m_pMaster->EquipTrump(false, pWea->Models1[m_nLastItemID[part].level], m_nLastItemID[part]);
			}
	...
	if (m_pMaster == GetPlayer() && g_layerMain->m_formChar->IsVisible() && g_layerMain->m_formChar->GetNowShowType() == CUIForm_Char::EQUIP)
	{
		g_layerMain->m_formChar->ReCreate3DViewGraph();
	}

	if (m_pMaster == GetPlayer() && part == TRUMP && g_layerMain->m_formChar->IsVisible() && g_layerMain->m_formChar->GetNowShowType() == CUIForm_Char::TRUMP)
	{
...
bool CItemContainer::_UnEquip(EEquipParts vPart, bool bAddToBag/* = false*/)
{
	...
	if ( !ItemID_IsValid(id) )		return true;
...
void CItemContainer::OnAfterEquip(SItemID& item, EEquipParts vPart)
{
	...
	if (pose == GcBaseActor::POSE_STAND || pose == GcBaseActor::POSE_GUARD || pose == GcBaseActor::POSE_IDLE)
	{
	...
	if (m_pSuit && !m_pLastSuit)
	{
		if (GetWorld())
		{
			if (!m_pSuit->effect.empty() && !m_pMaster->mBaseActor.FindEffect(m_pSuit->effect.c_str()))
				m_pMaster->mBaseActor.PlayEffect(m_pSuit->effect.c_str(),true,NULL,RtgVertex3(0,0,0),true);
		}
	}
...
GcActorGraph* CItemContainer::GetGraph(EEquipParts vPart)
{
	...
	if ( !_IsValidPart(vPart) )		return false;
    ...
    if (ItemIsTrump(itemNew.type))
    {
        if (m_pMaster->mBaseActor.m_pTrump)
        {
            ...
            if (pWeaClass && pWeaClass->Models1[itemNew.level][0])
                m_pMaster->mBaseActor.m_pTrump->ChangeModel(pWeaClass->Models1[itemNew.level], itemNew);
        }
    }
}
...
RtString CItemContainer::GenerateAttribString(SItemBase* pItemClass, SAttribAddtion &attrib, BYTE rate, bool bItemRelated/* = true*/)
{
	guard;

	if (!pItemClass)		return "";

    if (attrib.attrib == ITEM_ATTRIB_APPENDSKILL || attrib.attrib == ITEM_ATTRIB_SKILLONE ||attrib.attrib == ITEM_ATTRIB_SKILLTWO)
	{
	...
	case ITEM_ATTRIB_RATE_POISONDAMAGE:
		if (bItemRelated)			Num = (int)(fRate * (1.0f + pItemClass->requirement[ItemRequirement_Level]/100.0f) * attrib.count);
		else						Num = (int)(fRate * attrib.count);
	...
	case ITEM_ATTRIB_HIT_POWER:
		if (bItemRelated)			Num = rtRound(fRate * attrib.count * pItemClass->requirement[ItemRequirement_Level] / 100.0f);
		else						Num = rtRound(fRate * attrib.count / 100.0f);
	...
	case ITEM_ATTRIB_RATE_ATTACKSPEED: //攻击速度
		if (bItemRelated)			Num = rtRound(fRate * (1.0f + pItemClass->requirement[ItemRequirement_Level]/100.0f) * attrib.count);
		else						Num = rtRound(fRate * attrib.count);
		str += rtFormatNumber(Num);
		str += "%";
		break;
	...
	case ITEM_ATTRIB_SKILLCRITICAL_RATE: //爆击伤害
		if (bItemRelated)			Num = rtRound(fRate * attrib.count);
		else						Num = rtRound(fRate * attrib.count);
		str += rtFormatNumber(Num);
		str += "%";
		break;
...
static void _ItemHint_Requirement(RtString& str, int& outLineCount, SItemID &itemID, SItemBase* pItemClass, GcActor *pPlayer, const RtString& YesColorString, const RtString& NoColorString)
{
	char tmp[256];
	...
	// 属性需求
	int iReqStr=0, iReqDex=0, iReqInt=0, iReqHit=0, iReqCon=0, iReqLevel=0, iMaxLevel=0;
	GetItemAttribRequirement(GetWorld()->m_pItemManager, itemID, iReqStr, iReqDex, iReqInt, iReqHit, iReqCon, iReqLevel,iMaxLevel);
	...
	if (ItemIsArmor(itemID) || ItemIsWeapon(itemID) || ItemIsShipin(itemID))
	{
		...
		if (curWear>0)
		{
			rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %s</FONT></P>", YesColorString, "耐久", strWear);		
		}
		else
		{
	...
	if (iReqLevel != 0)
	{
		if (pPlayer->m_core.Lev >= iReqLevel)			strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d</FONT></P>", strColorTmp, R(G_CRE_REQUIRE_LEV), iReqLevel);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	...
	if (iReqStr != 0)
	{
		if (pPlayer->m_core.Str.GetValue() >= iReqStr)	strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d </FONT></P>", strColorTmp, R(G_CRE_REQUIRE_STR), iReqStr);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	if (iReqInt != 0)
	{
		if (pPlayer->m_core.Int.GetValue() >= iReqInt)	strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d </FONT></P>", strColorTmp, R(G_CRE_REQUIRE_INT), iReqInt);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	if (iReqDex != 0)
	{
		if (pPlayer->m_core.Dex.GetValue() >= iReqDex)	strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d </FONT></P>", strColorTmp, R(G_CRE_REQUIRE_DEX), iReqDex);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	if (iReqHit != 0)
	{
		if (pPlayer->m_core.Hit.GetValue() >= iReqHit)	strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d </FONT></P>", strColorTmp, R(G_CRE_REQUIRE_HIT), iReqHit);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	...
	if (iReqCon != 0)
	{
		if (pPlayer->m_core.Con.GetValue() >= iReqCon)	strColorTmp = YesColorString;
		else											strColorTmp = NoColorString;
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s: %d</FONT></P>", strColorTmp, R(G_CRE_REQUIRE_CON), iReqCon);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	...
	if ( cTmp==0x0F)
	{
		...
		{
			rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s</FONT></P>", strColorTmp, R(MSG_ITEM_REQUIRE_LEV_ALL));
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
	...
	else
	{
		...
		if (pPlayer->m_core.MetierLev < pItemClass->requirement[ItemRequirement_Grade] -1 ||
			(pPlayer->m_core.MetierLev > pItemClass->requirement[ItemRequirement_Grade]-1 && pItemClass->requirement[ItemRequirement_Grade]-1 ==0))
		{
		...
		else 
		{
			...
			{
				if ( (cTmp & 0x01) == 0 )			strColorTmp = NoColorString;
				else								strColorTmp = "#44dd88";
			}
			else if (pPlayer->m_core.Metier == METIER_HUNTER)
			{
				if ( (cTmp & 0x02) == 0 )			strColorTmp = NoColorString;
				else								strColorTmp = "#44dd88";
			}
			else if (pPlayer->m_core.Metier == METIER_TAOIST)
			{
				if ( (cTmp & 0x04) == 0 )			strColorTmp = NoColorString;
				else								strColorTmp = "#44dd88";
			}
			else if (pPlayer->m_core.Metier == METIER_WIZARD)
			{
				if ( (cTmp & 0x08) == 0 )			strColorTmp = NoColorString;
				else								strColorTmp = "#44dd88";
			}
		}
		rt2_snprintf(tmp, 256, "<P><FONT color=\"%s\">%s%s</FONT></P>", strColorTmp, strTmp, R(MSG_ITEM_SPECIAL));
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;	
	}
...
static void _ItemHint_ColorAttribs(RtString& str, int& outLineCount, SItemID &itemID, SItemBase* pItemClass, const RtString& strColor)
{
	if (ItemIsGreen(itemID))
	{
		...
		if (GetWorld()->m_pItemManager->m_pAttribMgr->GetAttrib(itemID.attribID1, arrAttrib))
		{
			for (int i=0; i<(int)arrAttrib.size(); i++)
			{
				...
				str += strColor;
				str += "\">";
				str += CItemContainer::GenerateAttribString(pItemClass, arrAttrib[i], itemID.attribRate1);
				str += "</FONT></P>";
				outLineCount ++;
			}
		}
	...
	else if (ItemIsBlue(itemID))
	{
		...
		if (GetWorld()->m_pItemManager->m_pAttribMgr->GetAttrib(itemID.attribID2, arrAttrib))
		{
			for (int i=0; i<(int)arrAttrib.size(); i++)
			{
				...
				str += strColor;
				str += "\">";
				str += CItemContainer::GenerateAttribString(pItemClass, arrAttrib[i], itemID.attribRate2);
				str += "</FONT></P>";
				outLineCount ++;
			}
		}
...
static void _ItemHint_EnchantAttribs(RtString& str, int& outLineCount, SItemID &itemID, SItemBase* pItemClass, const RtString& strColor)
{
...
static void _ItemHint_Desc(RtString& str, int& outLineCount, SItemID &itemID, SItemBase* pItemClass)
{
	char tmp[1024];
	if ( pItemClass->pDesc )
	{
...
RtString GenerateItemHtmlHint(int &outLineCount, SItemID &itemID, GcActor *pPlayer, CBag *pBag/* = NULL */, int page/* = -1*/, int i/* = -1*/, int j/* = -1*/,  float fPriceRate/* = -1.0f*/, UINT price/* = 0*/, bool bCountAll/* = false*/, float fTaxRate/* = -1.0f*/, int tTimeLimit/*=0*/, int OtherPrice)
{
	guard;
	...
	if ( ItemIsGreen(itemID) )			strColor = GREEN_STRING;
	else if ( ItemIsBlue(itemID) )		strColor = BLUE_STRING;
	else if ( ItemIsGolden(itemID) )	strColor = GOLDEN_STRING;
	else								strColor = WHITE_STRING;
	...
	ItemUpdate_GetExtraAddtion(itemID, GetItemMgr(), fLev_Damage, fLev_Attack, fLev_Dodge, fLev_Armor, fLev_ElementArmor);
	...
	if (itemID.cBind!=IB_NotBind)
	{
		...
		rt2_snprintf(tmp, 1024, "<P align=\"right\"><FONT color=\"#%s\">%s  </FONT></P>", bindColor.c_str(), bindString.c_str());
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
    ...
    if (tLife != 0)
    {
        RtString strLife;
        //struct tm* lTime;
		RtLocateTime lTime(tLife);//tim.yang  此处调用localtime转时间函数时会出现空指针，所以用RtLocateTime
        //lTime = localtime((time_t*)(&tLife));
        strLife.Format("%4d年%d月%d日 %02d:%02d", lTime.iYear, lTime.iMonth, lTime.iDay, lTime.iHour, lTime.iMin);
        if (now >= tLife)
            rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s</FONT></P>", NO_STRING, "使用期限", strLife);
        else 
            rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s</FONT></P>", "#108512", "使用期限", strLife);
        //tmp[255] = 0;
        str += tmp;
        outLineCount ++;
    }
	...
	{
		rt2_snprintf(tmp, 1024, "<P align=\"left\"><FONT color=\"#%s\">%s  </FONT></P>", "afafaf", R(MSG_ITEM_CANNOT_TRADE));
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
    ...
    if (itemID.cLock != 0)
    {
        rt2_snprintf(tmp, 1024, "<P align=\"left\"><FONT color=\"#%s\">%s  </FONT></P>", "afafaf", R(MSG_ITEM_PROTEST_ON_HINT));
        //tmp[255] = 0;
        str += tmp;
        outLineCount ++;
    }
	...
	if (OtherPrice)
	{
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"#dd3399\">%s: %d</FONT></P>", R(MSG_PRICE), OtherPrice);
		str += tmp;
		outLineCount ++;
	}
	else
	{
		if (fPriceRate > 0.0f)
		{
			...
			if (pointBase > 0.0f)
			{
				...
				rt2_snprintf(tmp, 1024, "<P><FONT color=\"#dd3399\">%s: %d</FONT></P>","点数", point);
				//tmp[255] = 0;
			}
			else
			{
				...
				if (fTaxRate > 0.0f)
				{
					...
					rt2_snprintf(tmp, 1024, "<P><FONT color=\"#dd3399\">%s: %d (%s: %d)</FONT></P>", R(MSG_PRICE), price1, R(G_TAX), priceTax);
					//tmp[255] = 0;
				}
				else 
				{
					rt2_snprintf(tmp, 1024, "<P><FONT color=\"#dd3399\">%s: %d</FONT></P>", R(MSG_PRICE), price1);
					//tmp[255] = 0;
				}
		...
		else if (price > 0)
		{
			if((int)price == -99)
			{
				rt2_snprintf(tmp,1024,"<P><FONT color=\"#dd3399\">%s</FONT></P>", R(MSG_TRADE_NOTATSELLNOW));
			}else
			{
				rt2_snprintf(tmp, 1024, "<P><FONT color=\"#dd3399\">%s: %d</FONT></P>", R(MSG_PRICE), price);
			}
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
    ...
    if (tTimeLimit)
    {
        ...
        tTimeLimit %= (60*24);
        rt2_snprintf(tmp+strlen(tmp), 1024-strlen(tmp), "%d:%d", tTimeLimit/60, tTimeLimit%60);
	...
	if (ItemIsWeapon(itemID))
	{
		...
		{
			// 防御
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_ARMOR), pWeapon->nArmor + rtRound(fLev_Armor));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
			// 闪躲
			...
			{
				rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_DODGE), pWeapon->nDodge + rtRound(fLev_Dodge));
				//tmp[255] = 0;
				str += tmp;
				outLineCount ++;
			}
		...
		{
			// 伤害
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d-%d</FONT></P>", R(G_CRE_DAMAGE), pWeapon->nDamageMin + rtRound(fLev_Damage), pWeapon->nDamageMax + rtRound(fLev_Damage));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
			// 命中
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_ATTACK), pWeapon->nAttack + rtRound(fLev_Attack));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			// 攻击速度
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(AttSpeed), (int)(pWeapon->fAttSpeed*ATT_SPEED_SCALE));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
			// 攻击距离
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(AttRange), pWeapon->nAttRange);
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
	...
	else if ( ItemIsTrump(itemID) )
	{
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d-%d</FONT></P>", R(G_CRE_DAMAGE), pTrump->nDamageMin + rtRound(fLev_Damage), pTrump->nDamageMax + rtRound(fLev_Damage));
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_ATTACK), pTrump->nAttack + rtRound(fLev_Attack));
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		// 法宝等级
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d </FONT></P>", "法宝等级", itemID.params[TRUMP_LEV]);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;

		// 修为需求
		if (pPlayer->m_core.Cul >= VigLev)				strColorTmp = YES_STRING;
		else											strColorTmp = NO_STRING;
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s </FONT></P>", strColorTmp, "修为需求", _GetTrumpStageFormLev(itemID.params[TRUMP_LEV]));
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
		...
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: +%d </FONT></P>", GOLDEN_STRING, "主属性", (int)(fInfo * 100.f));
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
		...
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: +%d </FONT></P>", GOLDEN_STRING, "附属性", (int)(fInfo * 100.f));
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
		...
		if (itemID.params[TRUMP_ATTACH_TYPE] != -1)
		{
			...
			char attachType[6][32] = {"火伤","火属性防御","水伤","水属性防御","毒伤","毒属性防御"};
			char attachCorlor[6][32] = {"#dd5511","#dd5511","#00aaff","#00aaff","#30e512","#30e512"};
			// 附加元素类型
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: +%d</FONT></P>",attachCorlor[itemID.params[TRUMP_ATTACH_TYPE]],attachType[itemID.params[TRUMP_ATTACH_TYPE]],itemID.params[TRUMP_ATTACH_POINT]);
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		for (int i=TRUMP_SKILL_1;i<TRUMP_SKILL_MAX;i++)
		{
			if(itemID.params[i] != 0)
			{
				...
				if (pSkill)
				{
					rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ff00ff\">法宝技能: %s（%d级）</FONT></P>",pSkill->szName , pSkill->iLevel);
					//tmp[255] = 0;
					str += tmp;
					outLineCount ++;
				}
	...
	{
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %.2f</FONT></P>", "品级", (float)itemID.params[0] / PET_PARAM_COMPUTE_BEADS);
		//tmp[255] = 0;
		str += tmp;
		outLineCount ++;
	}
	else if ( ItemIsArmor(itemID) )
	{
		...
		// 防御
		rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_ARMOR), pArmor->nArmor + rtRound(fLev_Armor));
        //tmp[255] = 0;
		str += tmp;
		outLineCount ++;
		// 闪躲
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_DODGE), pArmor->nDodge + rtRound(fLev_Dodge));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_ATTACK), pArmor->nAttack + rtRound(fLev_Attack));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(AttSpeed), (int)(pArmor->fAttSpeed*ATT_SPEED_SCALE));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(MoveSpeed), pArmor->nMoveSpeed);
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
...
//							str += "<P><FONT color=\"#00ff00\">";
							...
							str += SUIT_STRING;
							str += "\">";
							str += CItemContainer::GenerateAttribString(pItemBase, arrAttribs[i], SUIT_DEFAULT_ATTRIB_RATE);
							str += "</FONT></P>";
							outLineCount ++;
						}
					}
			...
			else	// 套装说明
			{
				...
				if (pSuit)
				{
					rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s</FONT></P>", SUIT_STRING, pSuit->desc.c_str());
                    //tmp[255] = 0;
					str += tmp;
	...
	else if ( ItemIsShipin(itemID) )
	{
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_FIRE_ARMOR), pShipin->nFireDefence + rtRound(fLev_ElementArmor));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_WATOR_ARMOR), pShipin->nWaterDefence + rtRound(fLev_ElementArmor));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		...
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_POISON_ARMOR), pShipin->nPoisonDefence + rtRound(fLev_ElementArmor));
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		if (pShipin->nMaxHP!=0)
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_MAX_HP), pShipin->nMaxHP);
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
		if (pShipin->nMaxMP!=0)
		{
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %d</FONT></P>", R(G_CRE_MAX_MP), pShipin->nMaxMP);
            //tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
        ...
        if (tLife != 0)
        {
            ...
            struct tm* lTime;
            lTime = localtime((time_t*)(&tLife));
            strLife.Format("%4d年%d月%d日 %02d:%02d", lTime->tm_year+1900, lTime->tm_mon+1, lTime->tm_mday, lTime->tm_hour, lTime->tm_min);
            if (now >= tLife)
                rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s</FONT></P>", NO_STRING, "使用期限", strLife);
            else 
                rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s</FONT></P>", YES_STRING, "使用期限", strLife);
            //tmp[255] = 0;
            str += tmp;
            outLineCount ++;
        }
	...
	else if ( ItemIsScroll(itemID) )
	{
		...
		{
			SItemCompose* pComposeClass = GetWorld()->m_pItemManager->m_pComposeMgr->FindComposing(pItemBase->composeID);
			if (!pComposeClass)			return "";
			SItemBase* pProductClass = GetWorld()->m_pItemManager->GetItemFromIndex(pComposeClass->Product);
			if (!pProductClass)			return "";
			// 制作物品
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %s</FONT></P>", R(制作物品), pProductClass->name);
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
			// 材料
			SItemBase* pMatItemClass = GetWorld()->m_pItemManager->GetItemFromIndex(pComposeClass->Materials[0]);
			if (!pMatItemClass)			return "";
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %s(%d)</FONT></P>", R(材料), pMatItemClass->name, pComposeClass->MaterialNums[0]);
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
			for (int i=1; i<pComposeClass->MaterialTypeCount; i++)
			{
				pMatItemClass = GetWorld()->m_pItemManager->GetItemFromIndex(pComposeClass->Materials[i]);
				if (!pMatItemClass)			return "";
				rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s(%d)</FONT></P>", pMatItemClass->name, pComposeClass->MaterialNums[i]);
				//tmp[255] = 0;
				str += tmp;
				outLineCount ++;
			}
			...
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"%s\">%s: %s(%s%d)</FONT></P>", strColorTmp, R(学习条件), SkillName.c_str(), R(等级), pComposeClass->SkillLevel);
			//tmp[255] = 0;
			str += tmp;
			outLineCount ++;
		}
	...
	else if ( ItemIsOther(itemID) )
	{
		...
		if (ItemIsOther_Dungeon(itemID.type))
		{
			...
			rt2_snprintf(tmp, 1024, "<P><FONT color=\"#ffffff\">%s: %s</FONT></P>", R(DUNGEON_DIFFICULTY), strDifficulty.c_str());
			//tmp[255] = 0;
			str += tmp;
