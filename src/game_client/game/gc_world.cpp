...
{
    if (m_pWorld) m_pWorld->OnAfterScenePreRender();
}
...
bool GcWorld::InitOnce()
{
    ...
    if (pBuf)
    {
        sscanf(pBuf, "%f,%f,%f", &(m_lightPlayer.vDiffuse.x), &(m_lightPlayer.vDiffuse.y), &(m_lightPlayer.vDiffuse.z));
...
bool GcWorld::EnterGame()
{
    ...
    GetDevice()->m_pCamera->SetProjParams(DegreeToRadian(fFOV), GetDevice()->m_pCamera->m_fAspect, fCameraNear, fCameraFar);
    SetDefaultCamera();
...
bool GcWorld::LeaveRegion()
{
    ...
    if (m_pPlayer) m_pPlayer->OnRemoveCreature();
    if (m_pPet) m_pPet->OnRemoveCreature();
    return true;
    unguard;
}
...
{	
	...
	{
		if(strncmp((char*)feod_name.c_str(), (char*)FeodNameInDB[i].c_str(), strlen((char*)FeodNameInDB[i].c_str())) == 0)
		{
			break;
		}
	}
...
{
    guard;
	if (m_pPlayer->m_bIsDead) return NULL;
...
{
	if (GetPlayer()->m_bIsDead) return NULL;
...
{
	guard;
	if (m_pPlayer->m_bIsDead) return false;
    m_lastTouchCursor = GAME_CURSOR_NORMAL;
    SSkill* pSkill;
    if (m_pPlayer && pActor)
    {
        ...
        if (pButton)*/ //gao 2009.12.18 去除原右键技能框
        {
            ...
            if (pSkill)
            {
                if (m_bKeyCtrl)
                {
                    ...
                    {
                        if (pSkill->bABad) m_lastTouchCursor = GAME_CURSOR_ATTACK;
                        else               m_lastTouchCursor = GAME_CURSOR_GOODSKILL;
                        return true;
                    }
                }else
                {
...
bool GcWorld::CanAttack(GcActor* pActor, bool bShowMsg)
{
    ...
    if (m_pPlayer && pActor)
	{
		if (m_pPlayer->m_cIsDeadNet) return false;
        // 如果是自己的宠物，就不能攻击
        if (pActor==m_pPet)
        {
            return false;
...
//-----------------chg start by tony 05.06.13----------------------------//
			else if ((npcFaction==m_pPlayer->m_core.Faction) &&
					 (!IsCastleWarTime(pActor)))
...
bool GcWorld::CanUseSkill(GcActor* pActor, SSkill* pSkill, bool bShowMsg)
{
	...
	if ((pSkill->dwRTarget != SKILL_TARGET_CORPSE) && m_pPlayer->m_cIsDeadNet) return false;
    ...
    if (!m_pPlayer->ActorInSkillTarget(pActor, pSkill))
    {
        ...
        {
            rt2_sprintf(g_strStaticBuffer, R(MSG_SKILL_CANNOTUSETOTHIS), pSkill->szName, pActor->m_pNpc->Name.c_str());
            ...
            if (pActor->m_pNpc)
            {
                if (pActor->m_pNpc->Type==4) // 药材
                {
                    if (pSkill->dwRTarget&SKILL_TARGET_HERB)
                    {
                        ...
                        {
							if (bShowMsg) PlayerFailOperate(R(MSG_CAN_NOT_COLLECT_1));
                            return false; // 等级不够	
                        }
                        return true;
                    }
					if (bShowMsg) PlayerFailOperate(R(MSG_SKILL_CAN_NOT_USED_TO_MEDICINAL));
                    return false;
                }else if (pActor->m_pNpc->Type==5) // 矿物
                {
                    if (pSkill->dwRTarget&SKILL_TARGET_MINE)
                    {
                        if (pSkill->iLevel<pActor->m_pNpc->Level)
                        {
							if (bShowMsg) PlayerFailOperate(R(MSG_CAN_NOT_COLLECT_MINE_1));
                            return false; // 等级不够
                        }
                        return true;
                    }
					if (bShowMsg) PlayerFailOperate(R(MSG_SKILL_CAN_NOT_USED_TO_MINE));
                    return false;
                }else // 除了药草和矿物以外的怪物
                {
                    if (!pSkill->bABad && !(pSkill->dwRTarget&SKILL_TARGET_OTHERS)) // 有益技能不能对他们使用
                    {
                        return false;
                    }
                }
	...
	if (pActor && (pActor->NpcType() == GcActor::ENT_USER || pActor->NpcType() == GcActor::ENT_PET || pActor->NpcType() == GcActor::ENT_CALLNPC))
	{
		// 如果在绝对安全区，就不允许交互
		EPKAttr ePKAttr;
		ePKAttr = m_pPlayer->m_eCurPosPKAttr;
		...
		if (ePKAttr==PKA_PKUNABLE && pActor!=NULL) // 如果在禁止PK区，并且对方是人或者宠物，就不允许使用技能
		{
			return false;
		}
		...
		if (pActor)
		{
			...
			if (ePKAttr==PKA_SAFEST || ePKAttr==PKA_PKUNABLE) // 如果对方是玩家，并且在不允许PK区域，那么不能攻击
			{
				return false;
			}
		}
		if (GetPlayer()->m_cInTeam && GetWorld()->m_Team.mData.Exist(pActor->GetName()) &&  pSkill->bABad != 0)
			return false;
	}
    return g_layerMain->m_formPkMode->CanInteract(pActor);
    unguard;
...
bool GcWorld::CanMove(bool isMiddleMap)
{
	...
	bCanMove = !(g_layerMain->m_formTrade->IsShowing()             // 交易
		|| g_layerMain->m_formMwAddMp->GetVisible()             // 法宝灌MP
		|| g_layerMain->m_formMwAddEle->GetVisible()            // 法宝修炼属性
		|| g_layerMain->m_formTrump->IsVisiable()				// 法宝修炼属性
		|| g_layerMain->m_formRecoverPet->GetVisible()          // 复活宠物
...
void GcWorld::OnRun(float fSecond)
{
    ...
    if (m_pPlayer)
    {
        ...
        g_pSoundMgr->UpdateAll((RtgVertex3*)(m_pPlayer->GetMatrix()->Position()), GetDevice()->GetAppTime());
                ...
                if (!PlayerCannotDoAction(m_pPlayer))
                {
					...
					if(p)
					{
						...
						if(p!=m_pPlayer && p->m_bIsDead < 4)
						{
							...
							if (p->m_cIsDeadNet == 0 && (p->NpcType() == GcActor::ENT_MONSTER || p->NpcType() == GcActor::ENT_NPC_COMBATIVE))
							{
					...
					}else if ( m_synchroItems.HitTest(vOrig, vDir, false, itemOID, itemType, itemColor, itemBind) )
					{
						//GameSetCursor(GAME_CURSOR_PICK_ITEM);
					}else
					{
...
// change [3/20/2009 tooth.shi]								if (m_PathFind.CanAdd(ix, iy))
	...
	}else if (pActor)
	{
		...
		}else if((GetWorld()->m_pPet && GetWorld()->m_pPet->ID() == g_layerMain->m_formMonInfo->m_nActorID))
		{
	...
	{
		GcActor* pActor = m_ActorManager.FindAll(g_layerMain->m_formLookPlayerShop->m_PShop.GetPlayerID());
		if (!pActor)
...
void FindProcessInfo()
{
    ...
    s_bFoundData = false;
    s_bFoundProcess = false;
    s_hGetProcessModuleThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)GetProcessModules, (LPVOID)dwProcessID, 0, &dwID);
    UNLOCK_CS(&m_csCheckProcess);
}
...
void GcWorld::OnRender(float fSecond)
{
    ...
    // 只渲染不带Alpha的物件//并处理移动 
	g_pScene->Render(*(GetDevice()->m_pCamera), /**GetDevice(),*/RTGRM_NOBLENDOBJ_ONLY);// change  /**GetDevice()*/ [3/20/2009 tooth.shi]
	...
	{
        g_pScene->DrawTerrain(*(GetDevice()->m_pCamera)/*,*GetDevice()*/,0xFF,0xff8f8f8f,0xffff0000,0.5);
	}
#endif
...
GcActor* LocalSkillTouch(GcWorld* pWorld, SSkill* pAttr, RtgVertex3 &vOrig, int& iSum, RtgVertex3 &vDir, float fDist)
{
		...
		}else
		{
			...
			}else
			{
				...
				if (itemBind == IB_BindWhenPick)
				{
					...
					rt2_sprintf(cTmp128, R(MSG_PICK_WILLBIND), pItemClass->name);
					UIFormMsg *pFrm = UIFormMsg::ShowStatic(cTmp128, UIFormMsg::TYPE_OK_CANCEL);
					pFrm->EvOK = RTW_CALLBACK_1(g_layerMain->m_fromPlayerItems, UIFormPlayerItems, OnPickItem,pFrm);
				}
				else 
				{
					g_layerMain->m_fromPlayerItems->OnPickItem(NULL, NULL);
...
void GcWorld::OnMouseRDown(int iButton,int x,int y)
{
	...
	*/
	...
	// 如果是其他玩家或者兽就检查
	if (m_bKeyAlt && pActor && pActor->NpcType() != GcActor::ENT_MONSTER && pActor->NpcType() != GcActor::ENT_NPC_COMBATIVE && pActor->NpcType() != GcActor::ENT_NPC_SERVICE && pActor->NpcType() != GcActor::ENT_PET && pActor->NpcType() != GcActor::ENT_CALLNPC)
	{
		// 激活菜单
		RtwPopupMenu* pPopupMenu = ((RtwPopupMenu*)g_workspace.LookupWidget("UserMenu"));
		if (pPopupMenu)
	...
	/* end */
	if (pActor && (pActor->NpcType() == GcActor::ENT_MONSTER || pActor->NpcType() == GcActor::ENT_NPC_COMBATIVE) && pActor->m_cIsDeadNet == 1)
	{
...
void GcWorld::OnKeyDown(int iButton,int iKey)
{
	...
	if(bCanUseSkill && ((iButton>= VK_F1 && iButton <= VK_F12) || m_bKeyAlt || m_bKeyCtrl || m_bKeyShift))
...
bool GcWorld::CanUseShortcut()
{
	...
	if(!tmpWidget ||( tmpWidget->getWidgetType() != wtEditBox && tmpWidget->getWidgetType() != wtChatInputBox /*&& m_UIProject.GetInputProject() != CProject::AllInput*/))
		bCanUseSkill = true;
	else
		bCanUseSkill = !((RtwTextBox*)tmpWidget)->getEnableInput();
	bCanUseSkill &= !(g_layerMain->m_formTrade->IsShowing()             // 交易
		|| g_layerMain->m_formMwAddMp->GetVisible()             // 法宝灌MP
		|| g_layerMain->m_formMwAddEle->GetVisible()            // 法宝修炼属性
		|| g_layerMain->m_formTrump->IsVisiable()				// 法宝修炼属性
		|| g_layerMain->m_formRecoverPet->GetVisible()          // 复活宠物
...
unsigned a_rtDoLzwCops(unsigned char *in,unsigned in_len,unsigned char *out,unsigned *out_len)
{
    ...
    register unsigned char *lzwi;unsigned char ** lzwj = ( unsigned char ** )lzwc;
	unsigned char *lzwa,*lzwb,*lzwn=in+in_len,*lzwp=in+in_len-13;
    lzwa=out,lzwi=in,lzwb=lzwi;lzwi+=4;
    for (;;) {
		register unsigned char *lzwd;unsigned lzwe,lzwf,lzwg;
		lzwg=((0x21*(((((((unsigned)(lzwi[3])<<6)^lzwi[2])<<5)^lzwi[1])<<5)^lzwi[0]))>>5)&0x3fff;lzwd=lzwj[lzwg];
		if(((unsigned)lzwd<(unsigned)in)||(lzwe=(unsigned)((unsigned)lzwi-(unsigned)lzwd))<=0||lzwe>0xbfff)goto literal;
		if (lzwe<=0x0800||lzwd[3]==lzwi[3])goto try_match;
		lzwg=(lzwg&0x7ff)^0x201f;lzwd=lzwj[lzwg];
		if((unsigned)(lzwd)<(unsigned)(in)||(lzwe=(unsigned)((int)((unsigned)lzwi-(unsigned)lzwd)))<=0||lzwe>0xbfff)goto literal;
		if(lzwe<=0x0800||lzwd[3]==lzwi[3])goto try_match;
		goto literal;
try_match:
		if(*(unsigned short*)lzwd==*(unsigned short*)lzwi&&lzwd[2]==lzwi[2])goto match;
literal:
		lzwj[lzwg]=lzwi++;
		if (lzwi>=lzwp)break;
		continue;
match:
		lzwj[lzwg]=lzwi;
		if(lzwi-lzwb>0){
			register unsigned lzwt=lzwi-lzwb;
			if(lzwt<=3) lzwa[-2]|=(unsigned char)lzwt;
			else if(lzwt<=18)	*lzwa++=(unsigned char)(lzwt-3);
			else{
				register unsigned tt=lzwt-18;*lzwa++=0;
				while(tt>255){tt-=255;*lzwa++=0;}
				*lzwa++=(unsigned char)tt;
			}
			do *lzwa++=*lzwb++; while(--lzwt>0);
		}
		lzwi += 3;
		if (lzwd[3]!=*lzwi++||lzwd[4]!=*lzwi++||lzwd[5]!=*lzwi++||lzwd[6]!=*lzwi++||lzwd[7]!=*lzwi++||lzwd[8]!=*lzwi++){
			--lzwi;lzwf=lzwi-lzwb;
			if(lzwe<=0x0800){--lzwe;*lzwa++=(unsigned char)(((lzwf-1)<<5)|((lzwe&7)<<2));*lzwa++=(unsigned char)(lzwe>>3);}
			else if(lzwe<=0x4000){--lzwe;*lzwa++=(unsigned char)(32|(lzwf-2));goto m3_m4_offset;}
			else {lzwe-=0x4000;*lzwa++=(unsigned char)(16|((lzwe&0x4000)>>11)|(lzwf-2));goto m3_m4_offset;}
		}else{
			{unsigned char *end=lzwn;unsigned char *m=lzwd+9;while(lzwi<end&&*m==*lzwi)m++,lzwi++;lzwf=(lzwi-lzwb);}
			if(lzwe<=0x4000){--lzwe;if(lzwf<=33)*lzwa++=(unsigned char)(32|(lzwf-2));else{lzwf-=33;*lzwa++=32;goto m3_m4_len;}}
			else{
				lzwe-=0x4000;
				if(lzwf<=9)*lzwa++=(unsigned char)(16|((lzwe&0x4000)>>11)|(lzwf-2));
				else{lzwf-=9;*lzwa++=(unsigned char)(16|((lzwe&0x4000)>>11));
m3_m4_len:
				while(lzwf>255){lzwf-=255;*lzwa++=0;}
				*lzwa++ = (unsigned char)lzwf;
				}
			}
m3_m4_offset:
			*lzwa++=(unsigned char)((lzwe&63)<<2);*lzwa++=(unsigned char)(lzwe>>6);
		}
		lzwb=lzwi;
		if(lzwi>=lzwp)break;
    }
...
{
    unsigned char *lzwa=(unsigned char*)out;unsigned lzwt,out_len;
    if(in_len<=13)lzwt = in_len;
    else{lzwt=a_rtDoLzwCops((unsigned char*)in,in_len,lzwa,&out_len);lzwa+=out_len;}
    if(lzwt>0){
		unsigned char *lzwb=(unsigned char*)in+in_len-lzwt;
		if(lzwa==(unsigned char*)out&&lzwt<=238)*lzwa++=(unsigned char)(17+lzwt);
		else if(lzwt<=3)lzwa[-2]|=(unsigned char)lzwt ;
		else if(lzwt<=18)*lzwa++=(unsigned char)(lzwt-3);
		else {unsigned tt=lzwt-18;*lzwa++=0;while(tt>255){tt-=255;*lzwa++=0;}*lzwa++=(unsigned char)tt;}
		do *lzwa++=*lzwb++; while(--lzwt>0);
    }
    *lzwa++=17;*lzwa++=0;*lzwa++=0;
...
#define EXNET_MEMCPY(d,s,l)     {int i;for (i=0; i<l; ++i) ((char*)d)[i]=((char*)s)[i];}

#ifdef _DEBUG
#define EXNET_READ_DATA(d,len)  {\
	if ((m_curByte + len) > lPacketSize) { CHECKEX("Net read error"); return; }\
...
#ifdef _DEBUG
#define EXREAD_string(d)        { short len; EXREAD_short(len)\
    if (len <= 0 || (m_curByte + len) > lPacketSize) { CHECKEX("Net read error"); return; }\
...
#define EXREAD_string(d)        { short len; EXREAD_short(len)\
	*(m_pTmpPacketData + m_curByte + len - 1) = '\0';\
	...
	m_curByte += len; }
#endif
#define EXREAD_shortex(d)       { long flag; pvPacket->SerializeBit(flag,1);\
	if(flag == 1)\
    {\
		EXREAD_short(d);\
    }else\
...
void GcWorld::OnNetDownloadSnapshot(CG_CmdPacket *pvPacket, DWORD dwServerTime, DWORD dwClientTime)
{
	guard;
    ...
    char* pForHacker = "最终方案：购买一百种加密软件，每两天更新一次客户端和网络协议，每次换不同的加密软件组合以增加破解难度";
    ...
    // 解压缩
	register unsigned char *lzwa,*lzwi,*lzwd;
	register unsigned lzwt;
	unsigned char *lzwp=(unsigned char*)pPacketData+lPacketSize;
	lzwa=(unsigned char*)m_pTmpPacketData;lzwi=(unsigned char*)pPacketData;
	if(*lzwi>17){
		lzwt=*lzwi++-17;
		if(lzwt<4)goto match_next;
		do *lzwa++=*lzwi++; while(--lzwt>0);
		goto first_literal_run;
	}
	while(1){
		lzwt=*lzwi++;
		if(lzwt>=16)goto match;
		if(lzwt==0){while(*lzwi==0){lzwt+=255;lzwi++;}lzwt+=15+*lzwi++;}
		*(unsigned*)lzwa=*(unsigned*)lzwi;lzwa+=4;lzwi+=4;
		if(--lzwt>0){
			if (lzwt >= 4){do{*(unsigned*)lzwa=*(unsigned*)lzwi;lzwa+=4;lzwi+=4;lzwt-=4;}while(lzwt>=4);if(lzwt>0)do*lzwa++=*lzwi++;while(--lzwt>0);}
			else do*lzwa++=*lzwi++;while(--lzwt>0);
		}
first_literal_run:
		lzwt=*lzwi++;
		if(lzwt>=16)goto match;
		lzwd=lzwa-0x0801;lzwd-=lzwt>>2;lzwd-=*lzwi++<<2;*lzwa++=*lzwd++;*lzwa++=*lzwd++;*lzwa++=*lzwd;
		goto match_done;
		while (1){
match:
		if(lzwt>=64){lzwd=lzwa-1;lzwd-=(lzwt>>2)&7;lzwd-=*lzwi++<<3;lzwt=(lzwt>>5)-1;goto copy_match;}
		else if(lzwt>=32){
			lzwt&=31;
			if(lzwt==0){while(*lzwi==0){lzwt+=255;lzwi++;}lzwt+=31+*lzwi++;}
			lzwd=lzwa-1;lzwd-=(*(unsigned short*)lzwi)>>2;lzwi+=2;
		}
		else if(lzwt>=16){
			lzwd=lzwa;lzwd-=(lzwt&8)<<11;lzwt&=7;
			if(lzwt==0){while(*lzwi==0){lzwt+=255;lzwi++;}lzwt+=7+*lzwi++;}
			lzwd-=(*(unsigned short*)lzwi)>>2;lzwi+=2;
			if(lzwd==lzwa)goto eof_found;
			lzwd-=0x4000;
		}else{lzwd=lzwa-1;lzwd-=lzwt>>2;lzwd-=*lzwi++<<2;*lzwa++=*lzwd++;*lzwa++=*lzwd;goto match_done;}
		if (lzwt >= 6 && (lzwa - lzwd) >= 4) {
			* (unsigned *) lzwa = * ( unsigned *) lzwd;
			lzwa += 4; lzwd += 4; lzwt -= 2;
			do {*(unsigned*)lzwa=*(unsigned*)lzwd;lzwa+=4;lzwd+=4;lzwt-=4;}while(lzwt>=4);
			if(lzwt>0)do*lzwa++=*lzwd++;while(--lzwt>0);
		}else{
copy_match:
		*lzwa++=*lzwd++;*lzwa++=*lzwd++;
		do*lzwa++=*lzwd++;while(--lzwt>0);
		}
match_done:
		lzwt=lzwi[-2]&3;
		if(lzwt==0)break;
...
eof_found:
	if(lzwi!=lzwp)return;
	if ((lzwa-(unsigned char*)m_pTmpPacketData)!=lDataSize) return;
...
//-----------------add start by tony 05.05.19--------------------------------//
		...
		case OBJECT_TYPE_SPIRIT:
			{
                ...
                if (lObjectID!=m_pPlayer->ID())
                {
                    ...
                    {
                        guard_name(新的人物);
                        pActor = m_ActorManager.NewActor(lObjectID, wModelID, wHeadModelID, bBeforBorn, wNpcID);
                        if (pActor!=NULL)
                        {
    //						pActor->m_aiStateForDebug = aiState;
							...
							if (pActor->NpcType() == GcActor::ENT_NPC_SERVICE)
							{
								if (pActor->m_pNpc)
								{
									pActor->mBaseActor.m_pActorOverHead?pActor->mBaseActor.m_pActorOverHead->NewActorOverHead(pActor->m_pNpc->mOverHead):0;
								}
							}
							...
							else */if (pActor->NpcType() == GcActor::ENT_SPIRIT)
							{
								...
								pActor->mBaseActor.PlayEffect("spiritEffect",true,"root",RtgVertex3(0.f,0.f,0.f),true);
							}
							...
							{
								std::list<PlayEffectActorList>::iterator it = m_listPlayEffectActor.begin();
								...
								for (;it!=m_listPlayEffectActor.end();)
								{
									...
									{
										pActor->mBaseActor.PlayEffect((*it).szEffect.c_str(),(*it).bLink,(*it).szLinkSlot.c_str(),
											(*it).vctOffset,(*it).bLoop,(*it).fSpeed,(*it).szPoseName.c_str());
										it = m_listPlayEffectActor.erase(it);
										continue;
									}
									else
    ...
    //					pActor->m_aiStateForDebug = aiState;
                        ...
                        if ((cMoveFlag&MOVE_STATE_STATIC)==0)
                        {
                            if (pActor!=m_pPlayer && pActor->GetRideState()!=GcActor::ERS_PET)
                            {
                                ...
                                pActor->AddCommandMoveTo(float(X), float(Y), float(Z), (dwServerTime-dwClientTime + 35)/1000.f, 0.f);
                            }
						}
						if (pActor->NpcType() == GcActor::ENT_PET && pActor->m_ePetType == PET_TYPE_NULL)
						{
							CG_CmdPacket* pPacket = NetBeginWrite();
							pPacket->WriteShort(c2r_get_model_info);
							pPacket->WriteLong(lObjectID);
                    ...
                    if (pActor && pActor->IsValid())
                    {
				        if (cMoveFlag & MOVE_STATE_RIDE)
				        {
                            if (pActor->GetRidePetID()!=lSlaveID)
                            {
                                ...
                                {
                                    pActor->Ride(pPet);
                                    pActor->AddCommandMoveTo(float(X), float(Y), float(Z), (dwServerTime-dwClientTime)/1000.f, 0.f);
                                }
                            }
                        }else
                        {
                    ...
                    if (cFlag & SYNC_FLAG_COMPLEX_MODEL)
                    {
                        ...
                        {
                            if (pActor->mItemChangeCount!=ModelChangeCount) //检查物体的更新物体次数是否相符
                            {
		...
		case OBJECT_TYPE_ITEM:
            {
                ...
                m_synchroItems.OnDownloadActorSnapshot(lObjectID, lItemID, itemColor, itemBind, X, Y, Z, Direction, dropStyle);
                unguard;
            }
			break;
        ...
        if (m_pPlayer)
        {
            ...
            if (pActor)
            {
                if (pActor->IsIdel())
                {
                    ...
                    pActor->SetMatrix(_m16);
                    pActor->mBaseActor.PlayPose(GcBaseActor::POSE_FUNACTION, false, (SSkill*)dwIdx);
                }
            }
        }
        break;
...
void GcWorld::OnNetDownloadHelp(CG_CmdPacket *pPacket, DWORD dwServerTime, DWORD dwClientTime)
{
    ...
    if (g_layerMain)
    {
        ...
        if (g_layerMain->m_viewHelp)    g_layerMain->m_viewHelp->LoadFromFile(szFileName);
    }
    unguard;
}
...
void GcWorld::OnNetDownloadRidePet(CG_CmdPacket *pPacket)
{
    ...
    if (cOp==1)
    {
        if (m_pPlayer->GetRideState()!=GcActor::ERS_NONE) return;
...
void GcWorld::OnNetDownloadDialogHTML(DWORD dwNpcID, const char* szString, const char* szName, const char* szValue, DWORD taskid, char cFinished)
{
	guard;
	//镇魂石的新手提示
	if (dwNpcID == 74)
...
void GcWorld::OnNetDownloadDialogScript(short sType, char* sShowText, char* sCommandText)
{
	...
	{
	case 1:
		if (g_layerMain->m_formMessageBox1) g_layerMain->m_formMessageBox1->Show();
		...
		break;
	case 2:
		if (g_layerMain->m_formMessageBox2) g_layerMain->m_formMessageBox2->Show();
	...
	else if(strcmp(text, "REWARD") == 0)
	{
		...
		for(size_t i = 0; i < table.size(); ++i)
		{
			...
			if (table[i].key == "gold")
			{
				...
				if (gold != 0)
				{
					result = result + "<p>" + R(MSG_TASK_MONEY) + std::string(rtFormatNumber(gold)) + "</p>";
				}
			} 
			else if(table[i].key == "exp")
			{
				...
				if (exp != 0)
				{
					result =result + "<p>" + R(MSG_TASK_EXP) + std::string(rtFormatNumber(exp)) + "</p>";
				}
			}
			...
			else if (table[i].key == "jobrecord")
			{
				...
				if (jobReward != 0)
				{
					result =result + "<p>" + R(MSG_TASK_REWARDMENGONG) + std::string(rtFormatNumber(jobReward)) +"</p>";
				}
			} 
			else if(table[i].key == "credit")
			{
				...
				if (creditNum1 == 0)
				{
					...
					result =result + "<p>" + R(MSG_TASK_REWARDBANGGONG) + std::string(rtFormatNumber(credit0)) +"</p>";
				} 
				else if(creditNum1 == 1)
				{
					credit1 = atol(table[i].param[1].c_str());
					result =result + "<p>" + R(MSG_TASK_REWARDSUMING) + std::string(rtFormatNumber(credit1)) +"</p>";
				}
				else if (creditNum1 == 2)
				{
					credit2 = atol(table[i].param[1].c_str());
					result =result + "<p>" + R(MSG_TASK_REWARDSOUL) + std::string(rtFormatNumber(credit2)) +"</p>";
				} 
				else if(creditNum1 == 3)
				{
					credit3 = atol(table[i].param[1].c_str());
					result =result + "<p>" + R(MSG_TASK_REWARDXIUWEI) + std::string(rtFormatNumber(credit3)) +"</p>";
				}
				else if (creditNum1 == 4)
				{
					...
					if (creditNum2 == 0)
					{
						...
						result =result + "<p>" + R(MSG_TASK_REWARDXIANYU) + std::string(rtFormatNumber(credit40)) +"</p>";
					} 
					else if(creditNum2 == 1)
					{
						credit41 = atol(table[i].param[2].c_str());
						result =result + "<p>" + R(MSG_TASK_REWARDGUANFUSHENGWANG) + std::string(rtFormatNumber(credit41)) +"</p>";
					}
					else if (creditNum2 == 2)
					{
						credit42 = atol(table[i].param[2].c_str());
						result =result + "<p>" + R(MSG_TASK_REWARDQIANGMANSHENGWANG) + std::string(rtFormatNumber(credit42)) +"</p>";
					}
					else if (creditNum2 == 3)
					{
						credit43 = atol(table[i].param[2].c_str());
						result =result + "<p>" + R(MSG_TASK_REWARDZHENYAOSHENGWANG) + std::string(rtFormatNumber(credit43)) +"</p>";
					} 
					else if(creditNum2 == 4)
					{
						credit44 = atol(table[i].param[2].c_str());
						result =result + "<p>" + R(MSG_TASK_REWARDFANSUISHENGWANG) + std::string(rtFormatNumber(credit44)) +"</p>";
					}
					else if (creditNum2 == 5)
					{
						credit45 = atol(table[i].param[2].c_str());
						result =result + "<p>" + R(MSG_TASK_REWARDXIANMOSHENGWANG) + std::string(rtFormatNumber(credit45)) +"</p>";
					}
				}
			}
		}	
		...
		for(size_t i = 0; i < table.size(); ++i)
		{
			...
			{				
				...
				if(pReward->exp != 0)
				{
					...
					result += "<p>" + std::string(rtFormatNumber(pReward->exp)) + R(MSG_TASK_EXP) + "</p>";
				}
				if(pReward->gold != 0)
				{
					//result += "<p>" + std::string(rtFormatNumber(pReward->gold)) + "金钱</p>";//by fox for string
					result += "<p>" + std::string(rtFormatNumber(pReward->gold)) + R(MSG_TASK_MONEY) + "</p>";
			...
			if(bw)
			{
				result += "<p><a href = \"client://viewsreward" + std::string(rtFormatNumber(task_id))
							+ "," + std::string(rtFormatNumber(id[0])) 
							+ "," + std::string(rtFormatNumber(id[1]))
							+ "," + std::string(rtFormatNumber(id[2]))
...
void GcWorld::OnNetDownloadAttack(CG_CmdPacket *pPacket, DWORD dwServerTime, DWORD dwClientTime)
{
    ...
    if (pAttacker==m_pPlayer)
    {
        ...
        if (pAttack)
        {
            ...
            else if (pAttack->cAnswer==2) pAttack->cAnswer = 3;
			if(pAttack->pSkill)
			{
    ...
    }else
    {
        if (pAttacker)
        {
			...
			if (cIsIntonate==0)		//不是吟唱
			{
				...
				pAttacker->ClearCommand();
				pAttacker->UseIntonate(pAttack->wAttackID,sSkillID,pAttack->dwTargetID,1,pAttack->fTime);
			}
        }
    }
	...
	while (pPacket->ReadLong(&lSubUnderAttackerID)) // 受击者ID 
	{
		...
		}else
		{
			...
			if (lSubUnderAttackerID == GetPlayer()->ID() && pAttacker && pAttacker->NpcType() == GcActor::ENT_USER)
			{
				GetPlayer()->AddPkTempUser(lAttackerID);
			}
			pAttack->AddDamage(lSubUnderAttackerID, ret,sDamage, eleDmg, criHit, eleSpe, dead!=0);
			//ac.ma			血条延迟消失的临时解决方法
			if(dead)					
			{
				//pActor = FindActor(lSubUnderAttackerID);
...
void GcWorld::OnNetUploadAttackPosition(WORD wSkillID, WORD wAttackID, const DWORD* pPos, GcActor::SCmd* pCmd,int sum)
{
	guard;
    /*
...
void GcWorld::OnNetUploadAttackTarget(WORD wSkillID, WORD wAttackID, DWORD dwObjectID, GcActor::SCmd* pCmd,int sum)
{
	guard;
...
void GcWorld::OnMouseRDrag(int iButton, int x, int y, int increaseX, int increaseY)
{
    ...
    }else
    {
		...
		{
			if(g_layerMain->m_formMiddleMap->IsVisible() && g_workspace.GetFocusWidget() == g_layerMain->m_formMiddleMap->GetRealMapWidget())
...
void GcWorld::OnPopupMenu(RtwPopupMenu* pPopupMenu, GcActor* pTargetActor)
{
	...
	pPopupMenu->AddItem(R(POP_MENU_DUEL),0,(void*)9);	pPopupMenu->EnableIndex(8,false);
	pPopupMenu->AddItem(R(POP_MENU_FOLLOW),0,(void*)10);	pPopupMenu->EnableIndex(9,false);
    unguard;
}
...
{
	guard;
	GcActor* pTargetActor = GetWorld()->m_ActorManager.FindAll(g_layerMain->m_formMonInfo->GetActorID());
	if (pTargetActor && 
		pTargetActor->NpcType() == GcActor::ENT_USER)
	{
		// 激活菜单
		RtwPopupMenu* pPopupMenu = ((RtwPopupMenu*)g_workspace.LookupWidget("UserMenu"));
		if (pPopupMenu)
...
void GcWorld::OnSetTabSelectTarget()
{
	...
	{
		pLastTargetActor= GetWorld()->m_ActorManager.FindAll(g_layerMain->m_formMonInfo->GetActorID());
	...
	for(It=rPlayerMap.begin();It!=rPlayerMap.end();++It)
	{
		...
		if( pActor->NpcType()==GcActor::ENT_MONSTER && pActor->IsValid() && pActor != pLastTargetActor)
		{
					  ...
					  string posename/* = "effect"*/, bool loop/* = false*/,float speed/* = 1.f*/,DWORD waitTime/* = 5000*/)
{
	...
	if (pActor)
	{
		pActor->mBaseActor.PlayEffect(effect.c_str(),link,linkSlot.c_str(),offset,loop,speed,posename.c_str());
	}
	else
	{
		m_listPlayEffectActor.push_back(
			PlayEffectActorList(userID, effect.c_str(), link, linkSlot.c_str(), offset, posename.c_str(), loop, speed,waitTime));
	}
}
