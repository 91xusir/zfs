...
void GcRenderActorBase::RenderHighLight(RtgCamera& inCamera, RtgDevice& inDevice, RtgVertex3 &color,RtgVertex3 &scale)
{
...
void GcRenderActorBase::AgentRenderShader()
{
    ...
    default:
    case 1: // 全部都是圈
        m_pGraph->GetGraph()->RenderShadow(*GetDevice(), FALSE, m_pGraph->m_pMaster->GetModel()->ModelRadius);
        break;
    case 2: // 自己和兽动态其他圈
        if (m_pGraph->m_pMaster==GetPlayer() || m_pGraph->m_pMaster==GetWorld()->m_pPet)
        {
            m_pGraph->GetGraph()->RenderShadow(*GetDevice(), TRUE, m_pGraph->m_pMaster->GetModel()->ModelRadius);
        }else
        {
            m_pGraph->GetGraph()->RenderShadow(*GetDevice(), FALSE, m_pGraph->m_pMaster->GetModel()->ModelRadius);
        }
	...
	if(m_pGraph->m_pMaster->ID() == GetWorld()->m_selectedActor)
	{
		...
		if(m_pGraph->m_pMaster->m_bRenderSelect)
			m_pGraph->GetGraph()->RenderSelect(*GetDevice(),color,m_pGraph->m_pMaster->GetModel()->ModelRadius);
	}
}
...
void GcStateOverHead::RenderHighLight(RtgCamera& inCamera, RtgDevice& inDevice, RtgVertex3 &color,RtgVertex3 &scale)
{
    if (m_pRender)
    {
        RtgVertex3 v = m_pActor->GetHUDPos();
...
void GcActorOverHead::RenderHighLight(RtgCamera& inCamera, RtgDevice& inDevice, RtgVertex3 &color,RtgVertex3 &scale)
{
	if (m_pRender)
	{
		RtgVertex3 v = m_pActor->GetHUDPos();
...
mpModel(NULL),
m_Time(0.0),
m_LastTileX(0),m_LastTileY(0),
m_CurrentState(NULL_STATE),
m_RideState(NO_RIDE_STATE),
m_pMaster(vpMaster),
m_eftList(),
mAttackTargetID(0)
{
...
bool GcBaseActor::CreateGraphData(SCreModel* vpModel, short HeadModelID)
{
	...
	if ( m_pMaster && m_pMaster->m_pNpc && (!m_pMaster->m_pNpc->GetTaskId.empty() 
		|| !m_pMaster->m_pNpc->FinishTaskId.empty()) ) 
	{
		if (!m_pStateOverHead)
		{
    ...
    if (mGraph.p())
    {
		...
		else
		{
			if(vpModel->LinkEffect.length() > 0)
				PlayEffect(vpModel->LinkEffect.c_str(),true,vpModel->LinkPoint.c_str(), RtgVertex3(0,0,0), true);
		}
#else
		if(vpModel->LinkEffect.length() > 0)
			PlayEffect(vpModel->LinkEffect.c_str(),true,vpModel->LinkPoint.c_str(), RtgVertex3(0,0,0), true);
...
bool GcBaseActor::SetPosition(float vX,float vY)
{
    ...
    {
        m_pMaster->m_pRidePet->mBaseActor.m_Matrix = m_Matrix;
        m_pMaster->m_pRidePet->mBaseActor.mGraph.SetPosition(m_Matrix._30, m_Matrix._31, m_Matrix._32);
    }else
    {
        mGraph.SetPosition(m_Matrix._30, m_Matrix._31, m_Matrix._32);
    }
...
bool GcBaseActor::HitTest(RtgVertex3& vOrg, RtgVertex3& vDir, float& fOutMinDist, bool bHitTestTouch)
{
    guard;
    ...
    if (mGraph.p())
    {
        ...
        if (pAABB)
        {
            ...
            {
                if (vMax.x<fMinSelect) vMax.x = fMinSelect;
                else if (vMax.x>fMaxSelect) vMax.x=fMaxSelect;
                if (vMax.y<fMinSelect) vMax.y = fMinSelect;
                else if (vMax.y>fMaxSelect) vMax.y=fMaxSelect;
            }
...
void GcBaseActor::Render(RtgDevice& inDevice,RTGRenderMask eMask)
{
		...
		if(bDrawFrame)
		{
			if(m_pMaster->ID() == GetWorld()->m_selectedActor)
			{
				if(eMask == RTGRM_NOBLENDOBJ_ONLY)
					mGraph.p()->RenderFrame(&inDevice,RtgVertex3(1,0.5,0),RtgVertex3(1.03,1.03,1.01));
			}
		}
	}
    ...
    if (m_isDrawPathLine && m_pMaster==GetPlayer() && m_pPathNode)
    {
        ...
        //if (iPathCnt)
        {
            RtgVertex3 vPos, vT1(0.f, -10.f, 5.f), vT2(7.f, 3.f, 5.f), vT3(-7.f, 3.f, 5.f), v1, v2, v3,v4,v5;
...
void GcBaseActor::RenderHighLight(RtgCamera& inCamera, RtgDevice& inDevice, RtgVertex3 &color,RtgVertex3 &scale)
{
	guard;
	if ( !m_bRender )
		return;
...
bool GcBaseActor::MoveToPosition(float fX, float fY, float fDistance, float fNeedTime, float fDiffTime, bool bTarget)
{
    guard;
    if (m_pPathNode==NULL)
    {
    ...
    float fD = sqrt((fX-m_Matrix._30)*(fX-m_Matrix._30)+(fY-m_Matrix._31)*(fY-m_Matrix._31));
    if (fD<=fDistance)
    {
        m_bMoveToPositionNewMove = TRUE;
        return false; // 已经走到了目的地
	}else if (1 == actortype && fD>10000.f && fNeedTime>=0.f)//tim.yang automove  add search path distance of player
    {
#ifdef _DEBUG
        MSG1("距离过远 %.2f\n", fD);
#endif
    ...
    if (fX!=m_fMoveToPositionOldX || fY!=m_fMoveToPositionOldY || m_fMoveToPositionTime!=fNeedTime || m_bMoveToPositionNewMove)
    {
		//if(actortype == 1)
        ...
        if (FindPath(iTileX, iTileY, IgonreActor, bTarget)) //目的地的路径已经找到
        {
            ...
            m_fLastDistance = 1000000.f;
            if (m_pPathNode->Next(iTileX, iTileY,m_pPathFind,IgonreActor, bTarget,actortype)) // 还有下一段
            {
            ...
            fD = sqrt((m_vMoveToPosition.x-m_Matrix._30)*(m_vMoveToPosition.x-m_Matrix._30)+(m_vMoveToPosition.y-m_Matrix._31)*(m_vMoveToPosition.y-m_Matrix._31));
            m_fMoveToPositionSpeedX = m_fMoveToPositionSpeed * (m_vMoveToPosition.x-m_Matrix._30)/fD;
            m_fMoveToPositionSpeedY = m_fMoveToPositionSpeed * (m_vMoveToPosition.y-m_Matrix._31)/fD;
	...
	if (m_fLastMoveSpeed != m_fMoveToPositionSpeed && m_pMaster->m_pCurCmd)
	{
		...
		if (m_pMaster->m_pModel)
		{
			...
			{
				if ((fD-m_pMaster->m_pCurCmd->fDistance)/m_pMaster->m_pCurCmd->fTime>80.f*2.f)
				{
					fSpeed = (80.f*2.f)/m_pMaster->m_pModel->MoveSpeed;
				}else
				{
					fSpeed = ((fD-m_pMaster->m_pCurCmd->fDistance)/m_pMaster->m_pCurCmd->fTime)/m_pMaster->m_pModel->MoveSpeed;
				}
			}
		}
		if (m_pMaster->GetRideState()==GcActor::ERS_ACTOR || m_pMaster->NpcType() == GcActor::ENT_PET)
...
bool GcBaseActor::AddSelfFind(int vX,int vY)
{
	...
	{
		m_blockType = PB_UNBLOCK;
	}else if (m_pMaster->NpcType() == GcActor::ENT_MONSTER || m_pMaster->NpcType() == GcActor::ENT_NPC_COMBATIVE )
	{
		m_blockType = PB_UNBLOCK;
	}else if (m_pMaster->NpcType() == GcActor::ENT_NPC_SERVICE)
	{
		if (m_pMaster->m_pNpc && (m_pMaster->m_pNpc->Type == 8 || m_pMaster->m_pNpc->AIType == "s_guarder"))
		{
			m_blockType = PB_UNBLOCK;
		}else
...
//        if (m_pPathNode=(RtsPathPath*)GetWorld()->GetPathFind()->AddActor(this,vX,vY,true,true))// 注释 [3/16/2009 tooth.shi]
    ...
    ERR2("GcBaseActor::AddSelf (ID=%d, Name=%s)加入场景出错\n", m_pMaster?m_pMaster->ID():0, m_pMaster?m_pMaster->GetName():"Null");
    return false;
    unguard;
}
...
{
    guard;
    if (m_bPutOnPathFinder==0) return false;
...
const char* WhipPoseName[] = 
{
	...
	"hurt_whip",		// 4 持软鞭受击
	"skill",			// 5 默认技能动作
	"waiting_whip",		// 6 持软鞭耍酷动作
	"attack_whip_critical",	// 7 持软鞭致命攻击动作
};

// 持软鞭时武器播放的动作
const char* WhipWeaponPoseName[] = 
{
	"wait",
	"attack",
	"walk",
...
const char* AxePoseName[] = 
{
	...
	"hurt_t",			// 4 双手持斧受击动作
	"skill",			// 5 默认技能动作
	"waiting_t",		// 6 双手持斧耍酷动作
	"attack_t_critical"	// 7 双手持斧致命攻击动作
};
...
const char *GcBaseActor::GetPoseByWeapon(EPoses PoseId, SItemID& item1, SItemID& item2)
{
	...
	{
		case POSE_ATTACK: 
			if(ItemIsWeapon_Pestle(item1.type) || ItemIsWeapon_Axe(item1.type) || ItemIsWeapon_Hoop(item1.type))
			{
				// check heavy hit
				if(rand()%100 > 30)
					ret = pose[1];
			...
			break;
		case POSE_WALK:   ret = pose[2]; break;
		case POSE_STAND:  ret = pose[0]; break;
		case POSE_IDLE:   ret = pose[6]; break;
		case POSE_DEAD:   ret = pose[3]; break;
		case POSE_HURT:   ret = pose[4]; break;
	}
	return ret;
    unguard;
}
...
const char* GcBaseActor::PlayPose(EPoses vPoseID, bool vLoop, SSkill* pSkill, float fSpeed)
{
    ...
    if (mpModel->bComplex)  pPoseName = pDefaultPoseName[vPoseID];
    else                    pPoseName = pDefaultPoseNameSimple[vPoseID];
	...
	if(!pSkill && GetWorld() && GetWorld()->m_pPlayer == m_pMaster && (vPoseID == POSE_ATTACK || vPoseID == POSE_BASH))
	{
		if(tpose)
		{
...
void GcBaseActor::PlayDropItemEffect(const char *file/* = "skill_hit1"*/,const char *link/* = "root" */)
{
	guard;
	if ( m_pItemEffect )
	{
...
void GcBaseActor::PlayEffect(const char *file,bool link,const char *slot,
							 RtgVertex3 offset,bool loop,float speed,const char* szPosename)
{
	...
	for(it=m_eftList.begin(); it!=m_eftList.end();)
	{
		...
		if (effect->m_Name == pFileFull)
		{
			...
			else
			{
				...
				matrix.SetRow(3, RtgVertex3(pActor->GetMatrix()->_30 + offset.x, pActor->GetMatrix()->_31 + offset.y, effect->m_localMat._32));
	...
	{
        ef->SetMatrix_Row(3, RtgVertex3(pActor->GetMatrix()->_30 + offset.x, pActor->GetMatrix()->_31 + offset.y, pActor->GetMatrix()->_32 + offset.z));
	}
}
...
void GcBaseActor::OnPoseEvent(SRT_Pose *pose,SRT_PoseEvent *event)
{
	...
	else if(event->Action == "missile")
	{
		...
		if(   sscanf(event->Param.c_str(),"%s %d %f %f %f %f %f %f %s %f %s %s",buf[0],&UseParentRotation,&OffsetPosition.x,&OffsetPosition.y,&OffsetPosition.z,
			&OffsetRotation.x,&OffsetRotation.y,&OffsetRotation.z,buf[1],&MissileSpeed,buf[2],buf[3])   !=12)
		{
		...
		*/
		...
		if(strcmp(buf[2],"track")==0)
		{
			...
			// missile act file name						//buf[1]
            // missile fly speed							//MissileSpeed
            CTrackMissile*  pMissile= RT_NEW CTrackMissile(Source,mAttackTargetID,buf[1],NULL,MissileSpeed);
        ...
        else if(strcmp(buf[2],"beeline")==0)
        {
            ...
            // missile act file name						//buf[1]
            // missile fly speed							//MissileSpeed
            CBeelineMissile*  pMissile= RT_NEW CBeelineMissile(Source,Target,buf[1],NULL,MissileSpeed);
