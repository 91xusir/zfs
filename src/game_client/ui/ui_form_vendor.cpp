...
void UIFormVendor::ShowItemList(int klass) {
    ...
    stlforeach(std::list<CAuctionItem>, it, *(g_SalesroomGC.GetDealing())) {
        i++;
        if ((klass == 0xff) ||
            ((klass != 0xff) && (GetItemClass((CAuctionItem*)&(*it)) == klass))) {
            ...
            if (pItemClass) {
                ...
                m_pList->SetItemText(
                    id, 2, rtFormatNumber(pItemClass->requirement[ItemRequirement_Level]));
            } else {
            ...
            if (pItemClass) {
                if (ItemID_IsValid((*it).ItemID())) {
                    ...
                    m_pList->SetItemIcon(
                        id, g_workspace.getImageFactory()->createImage(pItemClass->icon));
                }
            }
            id++;
        }
