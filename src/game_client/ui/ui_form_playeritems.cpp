...
UIFormPlayerItems::UIFormPlayerItems() : m_PetLimitTime(USE_PET_BUSY) {
    guard;
    ...
    m_ptrHoverImage =
        g_workspace.getImageFactory()->createImage("ui/x_textures/highlight_button.tga");
...
#define GRID_ITEMSIZE_Y 31
    ...
    RtwImage*  pImagegrids =
        g_workspace.getImageFactory()->createImage("ui/x_textures/gridsitem.tga");
    for (int k = 0; k < ITEM_PLAYER_INVENTORY_PAGE; k++) {
        ...
        for (int i = 0; i < ITEM_PLAYER_INVENTORY_LINE; i++) {
            for (int j = 0; j < ITEM_PLAYER_INVENTORY_COLUMN; j++) {
                ...
                m_lbGridsCommon[k][i][j] =
                    (RtwLabel*)g_workspace.getWidgetFactory()->createWidget(wtLabel);
                ...
                m_btnGridsCommon[k][i][j] =
                    (RtwButton*)g_workspace.getWidgetFactory()->createWidget(wtButton);
                ...
                m_btnGridsCommon[k][i][j]->EvLClick +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnLClick_CommonItem);
                m_btnGridsCommon[k][i][j]->EvLDClick +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnLDClick_CommonItem);
                m_btnGridsCommon[k][i][j]->EvMouseRDown +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnMouseRDown_CommonButton);
                m_btnGridsCommon[k][i][j]->EvHint +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnGetHintText_CommonItem);
                m_btnGridsCommon[k][i][j]->EvDrag +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnDrag_CommonItem);
                m_btnGridsCommon[k][i][j]->EvDragOut +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnDragOut_CommonItem);
                m_btnGridsCommon[k][i][j]->EvDragEnd +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnDragEnd_CommonItem);
                m_btnGridsCommon[k][i][j]->EvMouseMove +=
                    RTW_CALLBACK(this, UIFormPlayerItems, OnMouseMove);
            }
        }
    }
    ...
    for (int i = 0; i < ITEM_PLAYER_INVENTORY_LINE; i++) {
        for (int j = 0; j < ITEM_PLAYER_INVENTORY_COLUMN; j++) {
            ...
            m_btnGridsMission[i][j]->EvHint +=
                RTW_CALLBACK(this, UIFormPlayerItems, OnGetHintText_MissionItem);
            m_btnGridsMission[i][j]->EvRClick +=
                RTW_CALLBACK(this, UIFormPlayerItems, OnRClick_MissionItem);
            m_btnGridsMission[i][j]->EvDrag +=
                RTW_CALLBACK(this, UIFormPlayerItems, OnDrag_MissionItem);
            m_btnGridsMission[i][j]->EvDragEnd +=
                RTW_CALLBACK(this, UIFormPlayerItems, OnDragEnd_MissionItem);
        }
    }
    ...
    for (int j = 0; j < 6; ++j) {
        for (int i = 0; i < 6; ++i) {
            ...
            if (pTempImage->LoadFromFile(strFileName)) {
                pTempImage->SetSourceRect(
                    RtwRect(i * 36 + 2, j * 36 + 1, (i + 1) * 36, (j + 1) * 36 + 1));
        ...
        for (int j = 0; j < ITEM_PLAYER_INVENTORY_COLUMN; j++) {
            ...
            if (ItemID_IsValid(item) && pBag->GetVisible(pageIndex, i, j)) {
                if (m_bInMissionPage)
                    ...
                    m_btnGridsCommon[iSelect][i][j]->ModifyFlags(wfDragOut | wfDragServer | wfHint,
                                                                 0);
                pItemClass = GetWorld()->m_pItemManager->GetItem(item.type);
                if (!pItemClass) {
                    RtCoreLog().Info(
                        "无法找到道具，可能是因为本地数据与服务器不一致，请先升级客户端\n");
                    continue;
                ...
                if (m_bInMissionPage) {
                    ...
                    if (ItemCanPile(item)) {
                        ...
                        m_btnGridsMission[i][j]->SetTextAlignment(
                            SUiTextAlignment(alignFar, alignFar));
                    } else {
                    ...
                    if (ItemCanPile(item)) {
                        ...
                        m_btnGridsCommon[iSelect][i][j]->SetTextAlignment(
                            SUiTextAlignment(alignFar, alignFar));
                    } else {
                    ...
                    if (ItemIsOther_Task(item.type)) {
                        RtwImage* m_ptrHoverTaskImage =
                            LoadSequenceImage("ui/ui_texture/Pic_range_task.dds");
                    ...
                    switch (item.color) {
                        case ItemColor_White:
                            ...
                            break;
                        case ItemColor_Green: {
                            RtwImage* m_ptrHoverGreenImage =
                                LoadSequenceImage("ui/ui_texture/Pic_range_green.dds");
                            if (m_ptrHoverGreenImage) {
                        ...
                        case ItemColor_Blue: {
                            RtwImage* m_ptrHoverBlueImage =
                                LoadSequenceImage("ui/ui_texture/Pic_range_blue.dds");
                            if (m_ptrHoverBlueImage) {
                        ...
                        case ItemColor_Golden: {
                            RtwImage* m_ptrHoverGoldImage =
                                LoadSequenceImage("ui/ui_texture/Pic_range_golden.dds");
                            if (m_ptrHoverGoldImage) {
                ...
                if (m_bInMissionPage) {
                    ...
                    m_btnGridsCommon[iSelect][i][j]->ModifyFlags(0,
                                                                 wfDragOut | wfDragServer | wfHint);
...
void UIFormPlayerItems::Refresh(bool bRefreshRelated /* = false*/) {
    ...
    if (bRefreshRelated) {
        ...
        if (g_layerMain->m_formBank->IsVisible())  // 仓库
            g_layerMain->m_formBank->Show(g_layerMain->m_formBank->m_bStore,
                                          g_layerMain->m_formBank->m_npcID);
    ...
    for (int i = 0; i < MAX_LIMIT_ITEM; i++) {
        for (it = m_vecOperateItemCD[i]->begin(); it != m_vecOperateItemCD[i]->end(); it++) {
            ...
            if ((*it)->m_dwOperateTime != 0) {
                if (!m_pPlayer->mItem.m_Bag._IsValidIndex((*it)->m_iPage, (*it)->m_iI,
                                                          (*it)->m_iJ)) {
                ...
                float fRate = 1.0 - (float)(rtGetMilliseconds() - (*it)->m_dwOperateTime) /
                                        (*it)->m_dwLimitTime;
                if (fRate > 1.0f || fRate < 0.f) {
                    ...
                    if ((*it)->m_iPage != iSelect)
                        continue;
                    // Grid
                    if (m_btnGridsCommon[(*it)->m_iPage][(*it)->m_iI][(*it)->m_iJ]
                            ->GetBackground() != NULL) {
                        m_btnGridsCommon[(*it)->m_iPage][(*it)->m_iI][(*it)->m_iJ]->SetMaskColor(
                            RtwPixel(0xb5000000));
                        m_btnGridsCommon[(*it)->m_iPage][(*it)->m_iI][(*it)->m_iJ]->SetMaskValue(
                            fRate);
                    } else {
                        m_btnGridsCommon[(*it)->m_iPage][(*it)->m_iI][(*it)->m_iJ]->SetMaskValue(
                            0.f);
...
//发送消息到服务器实现(超链接)
//begin
void UIFormPlayerItems::ItemHyperLinkSend(DWORD itemTag, DWORD itemId, DWORD playerID,
                                          const std::string& strName, RtwPixel color) {
    GetWorld()->m_Chat.GetEditChat().AddHyperLinkText(hlinkItem, itemTag, itemId, playerID, strName,
                                                      color);
}
...
void UIFormPlayerItems::OnLClick_CommonItem(RtwWidget* sender, void*) {
    ...
    SItemID item =
        GetWorld()->m_pPlayer->mItem.m_Bag.GetItem(m_pPlayer->mItem.m_Bag.GetPageIndex(), i, j);
    ...
    if (g_layerMain->GetFirstWidget() == g_layerMain->m_formShop->OnGetThisForm() &&
        g_layerMain->m_formShop->m_bIsRepair) {
    ...
    if (((CGameClientFrame*)GetApp())->m_bMouseCapture) {
        ...
        switch (CursorTypeId) {
            case cti_Decompose: {
                ...
                m_iBackup        = i;
                m_jBackup        = j;
                UIFormMsg* pFrm  = UIFormMsg::ShowStatic(R(MSG_ITEM_DECOMPOSE_MAKESURE),
            ...
            case cti_Enchant: {
                ...
                int   EquipmentPosPage = pItemContainer->m_Bag.GetPageIndex(), EquipmentPosI = i,
                    EquipmentPosJ = j;
                ...
                SItemID EquipmentItem =
                    pItemContainer->m_Bag.GetItem(EquipmentPosPage, EquipmentPosI, EquipmentPosJ);
                if (ScrollItem.id == EquipmentItem.id)
                    return;
                SItemBase* pClassEquipment =
                    GetWorld()->m_pItemManager->GetItem(EquipmentItem.type);
                SItemBase* pClassScroll = GetWorld()->m_pItemManager->GetItem(ScrollItem.type);
                CHECK(pClassEquipment && pClassScroll);
                if (!ItemIsWeapon(EquipmentItem) && !ItemIsArmor(EquipmentItem) &&
                    !ItemIsShipin(EquipmentItem)) {
                    PlayerFailOperate(R(MSG_FAIL_CANNOTSTRENGTHEN));
                    return;
                }
                if (pClassScroll->requirement[ItemRequirement_Level] >
                    pClassEquipment->requirement[ItemRequirement_Level]) {
                ...
                UIFormMsg* pFrm =
                    UIFormMsg::ShowStatic(R(MSG_FUZHOU_JIAQIANG), UIFormMsg::TYPE_OK_CANCEL);
...
void UIFormPlayerItems::OnMouseRDown_CommonButton(RtwWidget* sender, void*) {
    ...
    if (g_layerMain->GetFirstWidget() == g_layerMain->m_formItemUpdate->OnGetThisForm()) {
        ...
        if (g_layerMain->m_formItemFuse->OnBagToFuseItem2(sender, &e))
            return;
    } else if (g_layerMain->GetFirstWidget() ==
               g_layerMain->m_formItemDecompound->OnGetThisForm()) {
...
void UIFormPlayerItems::OnDrag_CommonItem(RtwWidget* sender, void*) {
    ...
    if (Is_Widget_Common(WidgetID, i, j)) {
        ...
        if (!GetWorld()->m_bKeyShift) {
            ...
            CItemNetHelper::NetSend_MoveItem(ITEM_CONTAINER_PLAYER_COMMON_BAG, m_iLastTabSelect, i,
                                             j, page, iTo, jTo);
            vector<OPERATEITEMCD*>::iterator it;
            for (int ii = 0; ii < MAX_LIMIT_ITEM; ii++) {
                for (it = m_vecOperateItemCD[ii]->begin(); it != m_vecOperateItemCD[ii]->end();
                     it++) {
                    if (!(*it)) {
                        m_vecOperateItemCD[ii]->clear();
                        break;
                    }
                    if (m_iLastTabSelect == (*it)->m_iPage && i == (*it)->m_iI &&
                        j == (*it)->m_iJ) {
                        // 清除原有的技能CD
                        m_btnGridsCommon[(*it)->m_iPage][(*it)->m_iI][(*it)->m_iJ]->SetMaskValue(0);
    ...
    if (g_layerMain->m_formTrade && g_layerMain->m_formTrade->Is_Widget_Down(WidgetID, i, j)) {
        ...
        CBag*   pBag     = &(m_pPlayer->mItem.m_Bag);
        SItemID itemFrom = pTrade->GetTradeBag(&(m_pPlayer->mItem))
                               ->GetItem(m_pPlayer->mItem.m_Bag.GetPageIndex(), i, j);
        ...
        if (g_layerMain->m_formTrade->m_trade.IsLocked(&(m_pPlayer->mItem))) {
            ...
            g_layerMain->m_formTrade->m_trade.GetTradeOtherBag(&(GetWorld()->m_pPlayer->mItem))
                ->Clear();
            g_layerMain->m_formTrade->m_trade.GetTradeOtherPetBag(&(GetWorld()->m_pPlayer->mItem))
                ->Clear();
            CItemNetHelper::NetSend_c2r_trade_unlock();
        }
        if (g_layerMain->m_formTrade->m_trade.TakeOut(&(m_pPlayer->mItem), i, j,
                                                      pBag->GetPageIndex(), iTo, jTo)) {
            int page1, i1, j1;
...
void UIFormPlayerItems::OnEditNumOK(ui::RtwWidget* vpSender, RtwEventDelegate* vpEvent) {
    ...
    if (lNum) {
        ...
        SItemID item;
        item =
            GetPlayer()->mItem.m_Bag.GetItem(m_byteBuyInfo[0], m_byteBuyInfo[1], m_byteBuyInfo[2]);
        g_layerMain->m_formPlayerShop->OnDeleteItem(item);
        //}
        CItemNetHelper::NetSend_DispartItem(ITEM_CONTAINER_PLAYER_COMMON_BAG, m_byteBuyInfo[0],
                                            m_byteBuyInfo[1], m_byteBuyInfo[2], m_byteBuyInfo[3],
                                            m_byteBuyInfo[4], m_byteBuyInfo[5], lNum);
        vector<OPERATEITEMCD*>::iterator it;
        for (int i = 0; i < MAX_LIMIT_ITEM; i++) {
            for (it = m_vecOperateItemCD[i]->begin(); it != m_vecOperateItemCD[i]->end(); it++) {
                if (!(*it)) {
                ...
                if (m_byteBuyInfo[0] == (*it)->m_iPage && m_byteBuyInfo[1] == (*it)->m_iI &&
                    m_byteBuyInfo[2] == (*it)->m_iJ) {
...
void UIFormPlayerItems::OnDragEnd_CommonItem(RtwWidget* sender, void* unused) {
    ...
    if (!pClient)  // 把图标拖到空白处
    {
        ...
        m_jBackup        = j;
        UIFormMsg* pFrm =
            UIFormMsg::ShowStatic(R(MSG_ITEM_THROW_MAKE_SURE), UIFormMsg::TYPE_OK_CANCEL);
		...
		*/
        ...
        const RtwRect rcAuction =
            LOAD_UI("fmAuction.panOrder.pansource01.btnskill")->GetFrameRect();
        const RtwRect rcDecompound = LOAD_UI("fmseparate.btntbc1")->GetFrameRect();

        const RtwRect rcPetAbilityItem1 =
            g_layerMain->m_formPetAbility->m_pBtnItem1->GetFrameRect();
        const RtwRect rcPetAbilityItem2 =
            g_layerMain->m_formPetAbility->m_pBtnItem2->GetFrameRect();
        const RtwRect rcPetMergeItem = g_layerMain->m_formPetMerge->m_pBtnItem->GetFrameRect();
        const RtwRect rcPetToBeadsItem1 =
            g_layerMain->m_formPetToBeads->m_pBtnItem1->GetFrameRect();
        const RtwRect rcPetToBeadsItem2 =
            g_layerMain->m_formPetToBeads->m_pBtnItem2->GetFrameRect();
        const RtwRect rcPetMergeBeads =
            g_layerMain->m_formPetMergeBeads->m_pBtnItem->GetFrameRect();
        ...
        if (rcHotKey.IsContained(rcClient.left, rcClient.top) ||
            ...
            if (rcHotKey.IsContained(g_workspace.GetMousePosX(), g_workspace.GetMousePosY()) ||
                ...
                for (int i = 0; i < UI_HOT_KEY_COUNT; ++i) {
                    ...
                    if (rcSmallHotKey.IsContained(g_workspace.GetMousePosX(),
            ...
            if (pButton == NULL) {
                ...
                if (ItemCanPile(itemID)) {
                    int p1st, i1st, j1st;
                    m_pPlayer->mItem.m_Bag.FindType(
                        GetWorld()->m_pItemManager->GetIndexFromType(itemID.type), ItemColor_All,
                        &itemCount, &p1st, &i1st, &j1st);
        ...
        } else if (rcTrump.IsContained(rcClient.left, rcClient.top))  // gao 2009.12.30 此处是新添加
        {
            ...
            g_workspace.SetMouseDragWidget(NULL);
        } else if (g_layerMain->m_formPetAbility->IsVisible() &&
                   rcPetAbilityItem1.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
            ...
            g_workspace.SetMouseDragWidget(NULL);
                   ...
                   rcPetAbilityItem2.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
            ...
            g_workspace.SetMouseDragWidget(NULL);
        } else if (g_layerMain->m_formPetMerge->IsVisible() &&
                   rcPetMergeItem.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
            ...
            g_workspace.SetMouseDragWidget(NULL);
        } else if (g_layerMain->m_formPetToBeads->IsVisible() &&
                   rcPetToBeadsItem1.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
            ...
            g_workspace.SetMouseDragWidget(NULL);
                   ...
                   rcPetToBeadsItem2.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
            ...
            g_workspace.SetMouseDragWidget(NULL);
        } else if (g_layerMain->m_formPetMergeBeads->IsVisible() &&
                   rcPetMergeBeads.IsContained(rcClient.left, rcClient.top)) {
            SItemID itemID;
            if (m_iLastTabSelect == -1)
                m_iLastTabSelect = m_pPlayer->mItem.m_Bag.GetPageIndex();
            itemID = m_pPlayer->mItem.m_Bag.GetItem(m_iLastTabSelect, i, j);
    ...
    if (pClient)
        P_LOGINFO(pClient->GetID());
    auto id = pClient->GetID();//lmk 背包其他区域直接释放
...
// 是不是人物的纸娃娃
bool UIFormPlayerItems::Is_Widget_Parts(const string&                    id,
                                        CItemContainerBase::EEquipParts& outPart) {
    guard;
...
//bool UIFormPlayerItems::Is_ItemTagert_Vaild(const string& name) {
//
//}
    // 是不是包裹的格子
bool UIFormPlayerItems::Is_Widget_Common(const string& id, int& outI, int& outJ) {
    ...
    if (!m_pPlayer)                         // 如果玩家对象为空
        m_pPlayer = GetWorld()->m_pPlayer;  // 获取当前世界中的玩家对象
...
void UIFormPlayerItems::OnDrag_Trash(RtwWidget* sender, void*) {
    ...
    if (Is_Widget_Common(WidgetID, i, j)) {
        ...
        m_jBackup        = j;
        UIFormMsg* pFrm =
            UIFormMsg::ShowStatic(R(MSG_ITEM_DELETE_MAKE_SUER), UIFormMsg::TYPE_OK_CANCEL);
        pFrm->EvOK = RTW_CALLBACK_1(this, UIFormPlayerItems, OnDeleteItem, pFrm);
    } else if (Is_Widget_Mission(WidgetID, i, j)) {
        if (m_pPlayer->mItem.m_MissionBag.GetLock(m_pPlayer->mItem.m_MissionBag.GetPageIndex(), i,
                                                  j))
            return;
...
void UIFormPlayerItems::OnDrag_Pass(RtwWidget* sender, void*) {
    ...
    if (Is_Widget_Common(WidgetID, i, j)) {
        ...
        if (item.cLock == 0) {
            CItemNetHelper::NetSend_c2r_player_itemlock(
                item.id, m_pPlayer->mItem.m_Bag.GetPageIndex(), i, j);
        } else {
            m_bBackupMission = false;
            m_pageBackup     = m_iLastTabSelect;
            m_iBackup        = i;
            m_jBackup        = j;
            UIFormTextMsg* pFrm =
                UIFormTextMsg::ShowStatic(R(MSG_ITEM_PROTECT), R(MSG_BANK_PASS_QUERY));
...
void UIFormPlayerItems::OnDrag_MissionItem(RtwWidget* sender, void*) {
    ...
    if (OperateIsLocked())
        return;
    if (m_pPlayer->mItem.m_MissionBag.GetLock(m_pPlayer->mItem.m_MissionBag.GetPageIndex(), iTo,
                                              jTo))
        return;
    ...
    if (Is_Widget_Mission(WidgetID, i, j)) {
        ...
        CBag* pBag = &(m_pPlayer->mItem.m_MissionBag);
        int   page = pBag->GetPageIndex();
        CItemNetHelper::NetSend_MoveItem(ITEM_CONTAINER_PLAYER_MISSION_BAG, page, i, j, page, iTo,
                                         jTo);
        //if ( pBag->MoveItem(i, j, iTo, jTo) )
        //{
...
void UIFormPlayerItems::OnGetHintText_MissionItem(RtwWidget* sender, void*) {
    ...
    RtString str = GenerateItemHtmlHint(nLines, itemID, m_pPlayer, &(m_pPlayer->mItem.m_MissionBag),
                                        page, i, j);
...
void UIFormPlayerItems::OnRClick_MissionItem(RtwWidget* sender, void*) {
    ...
    SItemID item = GetWorld()->m_pPlayer->mItem.m_MissionBag.GetItem(
        GetWorld()->m_pPlayer->mItem.m_MissionBag.GetPageIndex(), i, j);
    if (!ItemID_IsValid(item))
        return;

    CItemNetHelper::NetSend_c2r_player_use_item(
        item.id, ITEM_CONTAINER_PLAYER_MISSION_BAG,
        GetWorld()->m_pPlayer->mItem.m_MissionBag.GetPageIndex(), i, j);
}
...
void UIFormPlayerItems::OnDeleteItem(void*, void*) {
    ...
    P_LOGINFO("正在丢弃: 第" + to_string(m_pageBackup) + "页, 第" + to_string(m_iBackup) +
              "行, 第" + to_string(m_jBackup) + "列的物品");
    ...
    int page = pBag->GetPageIndex();
    if (m_bBackupMission)
        CItemNetHelper::NetSend_DeleteItem(item.id, ITEM_CONTAINER_PLAYER_MISSION_BAG, m_pageBackup,
                                           m_iBackup, m_jBackup);
    else {
        CItemNetHelper::NetSend_DeleteItem(item.id, ITEM_CONTAINER_PLAYER_COMMON_BAG, m_pageBackup,
                                           m_iBackup, m_jBackup);
...
void UIFormPlayerItems::OnLClick_QuickStart(RtwWidget* sender, void*) {
    ...
    if (ItemID) {
        ...
        if (part != CItemContainerBase::MAX_EQUIP_PARTS) {
            SItemID item =
                m_pPlayer->mItem.GetEquipItem((CItemContainerBase::EEquipParts)part, true);
            // 要做一定的检查，看是否满足条件
...
void UIFormPlayerItems::OnGetHintText_QuickStart(RtwWidget* sender, void*) {
    ...
    if (ItemID) {
        int part = CItemContainerBase::EQUIP_DEFAULT_PART + 1;
        for (; part < CItemContainerBase::MAX_EQUIP_PARTS; part++) {
            itemTmp = GetWorld()->m_pPlayer->mItem.GetEquipItem(
                (CItemContainerBase::EEquipParts)part, true);
            if (itemTmp.id == ItemID)
                break;
        }
        if (part != CItemContainerBase::MAX_EQUIP_PARTS) {
            ...
            if (pBag->FindID(ItemID, &page, &i, &j)) {
                ...
                RtString str = GenerateItemHtmlHint(nLines, itemTmp, GetWorld()->m_pPlayer, pBag,
                                                    page, i, j, -1.0f, 0, true);
...
bool UIFormPlayerItems::_RefreshQuickStart(DWORD ItemID, DWORD ItemType) {
    ...
    } else if (ItemIsArmor(ItemType) || ItemIsWeapon(ItemType) || ItemIsShipin(ItemType) ||
               ItemIsTrump(ItemType) || ItemIsFashion(ItemType)) {
        SItemID itemTmp;
        int     part = CItemContainerBase::EQUIP_DEFAULT_PART + 1;
        for (; part < CItemContainerBase::MAX_EQUIP_PARTS; part++) {
            itemTmp = GetWorld()->m_pPlayer->mItem.GetEquipItem(
...
bool UIFormPlayerItems::GetQuickStartItemData(int pageIndex, int gridIndex, DWORD& ItemID,
                                              DWORD& ItemType) {
    guard;
...
void UIFormPlayerItems::OnEquipItem(void*, void*) {
    ...
    if (m_equipItemOID) {
        CItemNetHelper::NetSend_EquipItem(m_equipItemOID, m_equipPart,
                                          ITEM_CONTAINER_PLAYER_COMMON_BAG, m_equipPage,
                                          m_equipGridI, m_equipGridJ);
        LockOperate(true);
    }
...
//add by yz
void UIFormPlayerItems::UseBagItem(int page, int gridI, int gridJ,
                                   const SItemID* pItemID /*= NULL*/) {
    if (NULL != pItemID) {
...
void UIFormPlayerItems::DoUseCommonBagItem(int page, int gridI, int gridJ) {
    ...
    if (!(ItemIsWeapon(item.type) || ItemIsArmor(item.type) || ItemIsShipin(item.type)) ||
        ItemIsTrump(item.type) || ItemIsFashion(item.type) || ItemIsGemMagic(item)) {
    ...
    if (ItemIsWeapon(item.type) || ItemIsArmor(item.type) || ItemIsShipin(item.type) ||
        ...
        if (ItemIsHPBottle(item.type)) {
            ...
            if (m_vecOperateItemCD[0]->size() > 0 &&
                (!(*m_vecOperateItemCD[0]->begin()) ||
                 (*m_vecOperateItemCD[0]->begin())->m_dwOperateTime != 0)) {
                PlayerFailOperate(R(MSG_ITEM_CDNOTFINISH));
                return;
            }
            if (m_pPlayer->GetRideState() == GcActor::ERS_ACTOR) {
                if (m_pPlayer->GetRidePet() && m_pPlayer->GetRidePet()->m_core.GetHp() ==
                                                   m_pPlayer->GetRidePet()->m_core.GetMaxHp())
                    return;
            } else {
            ...
            if (m_vecOperateItemCD[1]->size() > 0 &&
                (!(*m_vecOperateItemCD[1]->begin()) ||
                 (*m_vecOperateItemCD[1]->begin())->m_dwOperateTime != 0)) {
                PlayerFailOperate(R(MSG_ITEM_CDNOTFINISH));
                return;
            }
            if (m_pPlayer->GetRideState() == GcActor::ERS_ACTOR) {
                if (m_pPlayer->GetRidePet() && m_pPlayer->GetRidePet()->m_core.GetMp() ==
                                                   m_pPlayer->GetRidePet()->m_core.GetMaxMp())
                    return;
            } else {
            ...
            if (m_vecOperateItemCD[2]->size() > 0 &&
                (!(*m_vecOperateItemCD[2]->begin()) ||
                 (*m_vecOperateItemCD[2]->begin())->m_dwOperateTime != 0)) {
                PlayerFailOperate(R(MSG_ITEM_CDNOTFINISH));
                return;
            }
            if (m_pPlayer->GetRideState() == GcActor::ERS_ACTOR) {
            ...
            if (m_vecOperateItemCD[3]->size() > 0 &&
                (!(*m_vecOperateItemCD[3]->begin()) ||
                 (*m_vecOperateItemCD[3]->begin())->m_dwOperateTime != 0)) {
                PlayerFailOperate(R(MSG_ITEM_CDNOTFINISH));
                return;
            }
            if (m_pPlayer->GetRideState() == GcActor::ERS_ACTOR) {
                ...
                if (GetPlayerPet()->GetPetActor()->m_core.GetHp() ==
                    GetPlayerPet()->GetPetActor()->m_core.GetMaxHp()) {
            ...
            if (m_vecOperateItemCD[4]->size() > 0 &&
                (!(*m_vecOperateItemCD[4]->begin()) ||
                 (*m_vecOperateItemCD[4]->begin())->m_dwOperateTime != 0)) {
                PlayerFailOperate(R(MSG_ITEM_CDNOTFINISH));
                return;
            }
            if (m_pPlayer->GetRideState() == GcActor::ERS_ACTOR) {
                ...
                if (GetPlayerPet()->GetPetActor()->m_core.GetMp() ==
                    GetPlayerPet()->GetPetActor()->m_core.GetMaxMp()) {
        ...
        CItemNetHelper::NetSend_c2r_player_use_item(item.id, ITEM_CONTAINER_PLAYER_COMMON_BAG, page,
                                                    gridI, gridJ);
        LockOperate(true);
        ...
        if (GetWorld()->IsCastleWarTime("XiQi") &&
            (GetWorld()->m_pPlayer->m_core.Faction == FACTION_ZHOU)) {
            PlayerFailOperate(R(MSG_CASTLETIME_OPERATE_FAIL), true);
            return;
        }
        if (GetWorld()->IsCastleWarTime("ZhaoGe") &&
            (GetWorld()->m_pPlayer->m_core.Faction == FACTION_SHANG)) {
            PlayerFailOperate(R(MSG_CASTLETIME_OPERATE_FAIL), true);
            return;
        }
        ...
        ((CGameClientFrame*)GetApp())
            ->SetMouseCapture(cti_Enchant, GAME_CURSOR_DEITEM, NULL, R(请选择该符咒要增强的装备));
        bEnchant = true;
        ...
        SItemCompose* pComposeClass =
            GetWorld()->m_pItemManager->m_pComposeMgr->FindComposing(pItemClass->composeID);
        if (!pComposeClass)
            return;
        SSkill* pSkill       = NULL;
        WORD    CheckSkillID = 0;
        ...
        // 发送请求
        if (bCanStudy) {
            CItemNetHelper::NetSend_c2r_player_use_item(item.id, ITEM_CONTAINER_PLAYER_COMMON_BAG,
                                                        page, gridI, gridJ);
            LockOperate(true);
        } else {
...
void UIFormPlayerItems::CallPet(void*, void*) {
    ...
    CItemNetHelper::NetSend_c2r_player_use_item(m_pet.id, ITEM_CONTAINER_PLAYER_COMMON_BAG,
                                                m_iPetActivePage, m_iPetActiveI, m_iPetActiveJ);
...
void UIFormPlayerItems::PlayTaskItem(int iTime) {
    if (m_bPlayItem) {
        ...
        g_layerMain->m_formTimeBar->EvTimeOver +=
            RTW_CALLBACK(this, UIFormPlayerItems, PlayTaskItemEnd);
        GetPlayer()->m_bInCallPet = true;
    }
}
...
void UIFormPlayerItems::PlayActivityProgressBar(long lTime) {
    ...
    g_layerMain->m_formTimeBar->EvTimeOver +=
        RTW_CALLBACK(this, UIFormPlayerItems, PlayActivityProgressBarEnd);
}
...
void UIFormPlayerItems::OnLClick_btnSplit(RtwWidget* sender, void*) {
    guard;
    ((CGameClientFrame*)GetApp())
        ->SetMouseCapture(cti_Split, GAME_CURSOR_SPLIT, NULL, R(MSG_ITEM_SELECTTAKEAPARTITEMS));
    unguard;
}

void UIFormPlayerItems::OnSetItemMask(int page, int i, int j, bool isEnable, float values,
                                      DWORD color, DWORD Alpha, bool EnableAlpha) {
    guard;
    if (isEnable) {
...
void UIFormPlayerItems::OnDisableAllItemMask() {
    ...
    for (it = m_vMaskItem.begin(); it != m_vMaskItem.end(); ++it) {
        ...
        m_btnGridsCommon[(*it).ItemPage][(*it).ItemI][(*it).ItemJ]->ModifyFlags(
            wfDragOut | wfDragServer, 0);
    }
    m_vMaskItem.clear();
    unguard;
}
