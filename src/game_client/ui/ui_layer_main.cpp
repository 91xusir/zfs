...
UILayerMain::UILayerMain()
    : m_formSetting(NULL), m_fromPlayerItems(NULL), m_hotKeyGroupNumber(0), strCommand(""),
      bStartActivity(false), strActivityText(""), lActivityTime(0), m_bCountDownStart(false),
      mSelectRewardId(0) {
    guard;
    ...
    LOAD_UI_T(RtwButton, "fmhp.btnPKmode")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_PkMode);
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        m_hotKeyAlias[i]->ModifyFlag(
            wfDragOut | wfHint | wfVisible | wfDClick | wfDragIn | wfGrabMouse, 0);

        rt2_sprintf(hotkey1, "fmshortcut_bar1.lbhotkey%d", i + 1);
        m_hotKeyAlias[i + 10] = LOAD_UI_T(RtwAliasButton, hotkey1);
        m_hotKeyAlias[i + 10]->ModifyFlag(
            wfDragOut | wfHint | wfVisible | wfDClick | wfDragIn | wfGrabMouse, 0);

        rt2_sprintf(hotkey1, "fmshortcut_bar2.lbhotkey%d", i + 1);
        m_hotKeyAlias[i + 20] = LOAD_UI_T(RtwAliasButton, hotkey1);
        m_hotKeyAlias[i + 20]->ModifyFlag(
            wfDragOut | wfHint | wfVisible | wfDClick | wfDragIn | wfGrabMouse, 0);
    }

    //	m_hotKeyButton = LOAD_UI_T(RtwAliasButton, "fmhotkey.lbhotkeybutton");

    LOAD_UI("fmsystem.btnsyschar")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_System_Char);
    LOAD_UI("fmsystem.btnsysitem")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_PlayerItems);
    ...
    LOAD_UI("fmminimap.btnsysmap")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_System_Map);  //gao 2010.1.5 内容取消

    LOAD_UI("fmsystem.btnsystask")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_Attribute_task);

    LOAD_UI("fmsystem.btnsysbattle")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnLClicked_System_Battle);
	...
	*/
    ...
    LOAD_UI("fmsystem.btnsystem")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_System_System);

    LOAD_UI("fmsyssetting.btnsysset")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_SysSetting_SysSet);
    LOAD_UI("fmsyssetting.btnrestart")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_SysSetting_Restart);
    LOAD_UI("fmsyssetting.btnexit")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_SysSetting_Exit);
    LOAD_UI("fmexitconfirm.btnconfirm")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_ExitConfirm_Confirm);
    LOAD_UI("fmexitconfirm.btncancel")->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_ExitConfirm_Cancel);
    ...
    // PopupMenu
    LOAD_UI_T(RtwPopupMenu, "UserMenu")->EvMenuSelect +=
        RTW_CALLBACK(this, UILayerMain, OnMenuUser);
    //LOAD_UI_T(RtwPopupMenu, "UserMenuByname")->EvMenuSelect    += RTW_CALLBACK(this, UILayerMain, OnMenuUserByName);
    LOAD_UI_T(RtwPopupMenu, "MonsterMenu")->EvMenuSelect +=
        RTW_CALLBACK(this, UILayerMain, OnMenuMonster);
    LOAD_UI_T(RtwPopupMenu, "PlayerMenu")->EvMenuSelect +=
        RTW_CALLBACK(this, UILayerMain, OnMenuPlayer);
    ...
    LOAD_UI_T(RtwPopupMenu, "SystemMenu")->EvMenuSelect +=
        RTW_CALLBACK(this, UILayerMain, OnMenuSystem);
    //人物的界面
    ...
    m_btnMainCancelFinished->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_CancelFinishMainTask);
    //新手指引
    //气泡
    ...
    m_btnAutoBattle->EvLClick +=
        RTW_CALLBACK(this, UILayerMain, OnClicked_AutoBattle);  //for  auto    ac.ma
    ...
    g_workspace.SetSysEditName(
        "fmdialg.fmtxtdialog.txtdialog");  // add by fancui for system edit @2005-04-27
...
void UILayerMain::OnHtmlTask_HtmlHelpView(RtwWidget* sender, void* pParam) {
    ...
    const char*        szHref;
    RtwTextBox::SItem* pItem =
        ((RtwHtmlView*)sender)->PickItem(SPoint(pEvent->mouse.x, pEvent->mouse.y));
    if (!pItem)
        return;
    szHref = (const char*)(pItem->hyperLink.Text.c_str() + 2);
    if (pEvent->type == weMouseClick && pItem->hyperLink.Type == hlinkClient) {
        if (strncmp(szHref, "clientmoveto", 12) == 0 ||
            strncmp(szHref, "clientmovetonpc", 15) == 0) {
...
void UILayerMain::ClientMoveto(std::string szHref) {
    ...
    } else if (strstr(szHref.c_str(), "scene4"))  // 检查 szHref 是否包含 "scene4" 字符串
    {
        // 去掉 "clientmovetoscene4" 及其后面的空格
        temp = temp.substr(strlen("clientmovetoscene4") + 1);

        std::string sceneName1 = "";                      // 第一个场景名称
        std::string sceneName2 = "";                      // 第二个场景名称
        int         x1         = -99999;                  // X 坐标初始值，表示无效坐标
        int         y1         = -99999;                  // Y 坐标初始值，表示无效坐标
        int         npcID2     = 0;                       // NPC ID 初始值
        int         start      = 0;                       // 用于记录解析开始的位置
        int         end        = temp.find(step, start);  // 找到第一个分隔符的位置

        if (std::string::npos != end)
            sceneName1 = temp.substr(start, end);  // 提取第一个场景名称

        start = end + 1;                 // 更新解析开始位置
        end   = temp.find(step, start);  // 找到第二个分隔符的位置

        if (std::string::npos != end)
            x1 = atoi(temp.substr(start, end).c_str());  // 提取 X 坐标并转换为整数

        start = end + 1;                 // 更新解析开始位置
        end   = temp.find(step, start);  // 找到第三个分隔符的位置

        if (std::string::npos != end)
            y1 = atoi(temp.substr(start, end).c_str());  // 提取 Y 坐标并转换为整数

        start = end + 1;                 // 更新解析开始位置
        end   = temp.find(step, start);  // 找到第四个分隔符的位置

        if (std::string::npos != end)
            sceneName2 = temp.substr(start, end);  // 提取第二个场景名称

        start  = end + 1;                           // 更新解析开始位置
        npcID2 = atoi(temp.substr(start).c_str());  // 提取 NPC ID 并转换为整数

        if (GetWorld()->GetSceneFileName() ==
            sceneName1)  // 检查当前场景文件名是否与第一个场景名称相同
        {
            if (-99999 != y1 && -99999 != x1)  // 检查坐标是否有效
            {
                GetWorld()->m_pPlayer->GotoThere(x1, y1);  // 将玩家传送到指定坐标
            }
        } else {
            SNpc* pNpc = g_TableNpc.FindNpc(npcID2);  // 查找指定 ID 的 NPC
            if (pNpc) {
                GetWorld()->m_pPlayer->GotoNpcFront(pNpc);  // 将玩家传送到 NPC 前面
            }
        }
    }
...
void UILayerMain::OnHtmlAction_HtmlHelpView(RtwWidget* sender, void* pParam) {
    ...
    if (pEvent->type == weMouseClick && pItem->hyperLink.Type == hlinkRServer) {
        ...
        if (szHref && szHref[0]) {
            if (strncmp(szHref, "Script_ShowRet", 14) == 0) {
                ...
                if (g_layerMain->m_formHelp->IsVisible() &&
                    g_layerMain->m_formMiddleMap->GetFocusParentWidget(sender) ==
                        g_layerMain->m_formHelp) {
        ...
        if (g_layerMain->m_formHelp->IsVisible() &&
            g_layerMain->m_formMiddleMap->GetFocusParentWidget(sender) == g_layerMain->m_formHelp) {
            g_layerMain->m_formHelp->Hide();
        }
        if (strncmp(szHref, "showItemInfo", 12) == 0) {
            ...
            g_layerMain->m_formBankPass->Show(GetWorld()->m_dwInteractiveNPCID,
                                              UIFormBankPass::sAddPass);
        } else if (strncmp(szHref, "bankreleasepass", 15) == 0) {
            g_layerMain->m_formBankPass->Show(GetWorld()->m_dwInteractiveNPCID,
                                              UIFormBankPass::sRemovePass);
        } else if (strncmp(szHref, "bankchangepass", 14) == 0) {
            g_layerMain->m_formBankPass->Show(GetWorld()->m_dwInteractiveNPCID,
                                              UIFormBankPass::sChangePass);
            ...
            int x = (winRect.right - winRect.left - rect1.getWidth() - rect2.getWidth()) * 0.5 +
                    winRect.left;
        ...
        //------------------add end   by tony 05.06.06-------------------------------//
            ...
            UIFormTextMsg* pFrm          = UIFormTextMsg::ShowStatic(R(MSG_PRENSYS_REMOVE),
                                                                     R(MSG_PRENSYS_INPUT_PRENTICE_NAME));
            ...
            UIFormTextMsg* pFrm =
                UIFormTextMsg::ShowStatic(R(MSG_PRENSYS_REMOVE), R(MSG_PRENSYS_INPUT_MASTER_NAME));
            g_GcPrenticeMgr.m_pFrmRemove = pFrm;
        ...
        //-添加跑环挂起功能
            ...
            for (it = task->m_taskList.begin(); it != task->m_taskList.end(); it++) {
                ...
                RtString msg;
                int      leftTime = desc->finishTimeLimit -
                               (GetWorld()->m_NetSynachro.GetServerSec() - it->BeginTime);
                if (leftTime < 0)
                    leftTime = 0;
                msg.Format(desc->dynamicText[0].key.c_str(), task->m_currCircle, g_tcTotalCircle,
                           leftTime / 3600 / 10, (leftTime / 3600) % 10,
        ...
        } else if (strncmp(szHref, "clientmoveto", 12) == 0 ||
                   strncmp(szHref, "clientmovetonpc", 15) == 0)  //tim.yang  自动
        {
            ClientMoveto(szHref);
		...
		*/
        ...
        GetWorld()->OnNetDownloadDialogHTML((DWORD)m_viewHelp->GetUserData(), filename, "", "", 1,
                                            0);
    }
    unguard;
}
...
void UILayerMain::LoadShortcut(CG_CmdPacket* pPacket) {
    ...
    if (!file) {
        ...
        if (NULL == pPacket || !isHaveData) {
            ...
            m_hotKeyAlias[0]->GetButton()->EvRClick +=
                RTW_CALLBACK(this, UIFormSkill, OnLClickCommonButton);

            rt2_sprintf(loadStr, "%d;%d", 1, 113);
            button = OnLoadSkillShortcut(loadStr);
            m_hotKeyAlias[9]->SetButton(button);
            m_hotKeyAlias[9]->GetButton()->EvRClick +=
                RTW_CALLBACK(this, UIFormSkill, OnLClickCommonButton);

            rt2_sprintf(loadStr, "%d;%d", 1, 112);
            button = OnLoadSkillShortcut(loadStr);
            m_hotKeyAlias[8]->SetButton(button);
            m_hotKeyAlias[8]->GetButton()->EvRClick +=
                RTW_CALLBACK(this, UIFormSkill, OnLClickCommonButton);

            rt2_sprintf(loadStr, "%d;%d", 1, 107);
            button = OnLoadSkillShortcut(loadStr);
            m_hotKeyAlias[7]->SetButton(button);
            m_hotKeyAlias[7]->GetButton()->EvRClick +=
                RTW_CALLBACK(this, UIFormSkill, OnLClickCommonButton);
...
RtwButton* UILayerMain::OnLoadSkillShortcut(const char* str) {
    ...
    if (version == 1) {
        ...
        if (pAttr == NULL)
            return NULL;
        GcSkill* pSkill =
            (GcSkill*)GetWorld()->m_pPlayer->m_Skill.FindActiveSkillBySubID(pAttr->wSubID);
        if (pSkill == NULL)
            return NULL;
        ...
        if (pButton == NULL) {
            ...
            pButton->SetBackgroundImage(
                g_workspace.getImageFactory()->createImage(pSkill->m_pAttr->szIcon));
...
RtwButton* UILayerMain::OnLoadItemShortcut(const char* str) {
    ...
    if (version == 1) {
        ...
        if (ItemIsMissionItem(type)) {
            ...
            if (ItemCanPile(type)) {
                int p1st, i1st, j1st;
                if (!GetWorld()->m_pPlayer->mItem.m_Bag.FindType(
                        GetWorld()->m_pItemManager->GetIndexFromType(type), ItemColor_All,
        ...
        pButton->SetUserData(pDword);
        pButton->EvClose +=
            RTW_CALLBACK(m_fromPlayerItems, UIFormPlayerItems, DeleteQuickStartUserData);
        //pButton->EvLClick		+= RTW_CALLBACK(m_fromPlayerItems, UIFormPlayerItems, OnLClick_QuickStart);
        pButton->EvRClick +=
            RTW_CALLBACK(m_fromPlayerItems, UIFormPlayerItems, OnLClick_QuickStart);
        pButton->EvHint +=
            RTW_CALLBACK(m_fromPlayerItems, UIFormPlayerItems, OnGetHintText_QuickStart);
...
void UILayerMain::AddRemindIcon(UIRemindType type, const char* hint /* = ""*/,
...
void UILayerMain::OnClicked_AutoHp30(RtwWidget* sender, RtwEventDelegate* e)  //ac.ma
{
    ...
    if (m_btnAutoHp30->GetChecked()) {
        ...
        } else if (m_btnAutoHp60->GetChecked()) {
...
void UILayerMain::OnClicked_AutoHp60(RtwWidget* sender, RtwEventDelegate* e)  //ac.ma
{
    ...
    if (m_btnAutoHp60->GetChecked()) {
        ...
        } else if (m_btnAutoHp90->GetChecked()) {
            GetWorld()->m_pPlayer->SetAutoHp90(false);
            m_btnAutoHp90->SetChecked(false);
        }
    } else {
...
void UILayerMain::BuildMainTaskList() {
    ...
    for (itGroup = taskChapterId.begin(); itGroup != taskChapterId.end(); itGroup++) {
        ...
        for (itLine = taskMainLine.begin(); itLine != taskMainLine.end(); itLine++) {
            if ((*itLine).nGroupID == (*itGroup)) {
                //职业判断
                if ((*itLine).nJob) {
                    if ((*itLine).nJob != GetWorld()->m_pPlayer->m_core.Metier)
                        continue;
                }
                m_taskTree->InsertTreeItem(taskGroup->strGroupName.c_str(),
                                           (*itLine).strDataName.c_str(), "", 0, -1, 0);
    ...
    if (section) {
        ...
        if (pItem) {
            ...
            RtwTree::Item* pItemGroup =
                m_taskTree->FindTreeItemByText(taskGroup->strGroupName.c_str());
        ...
        if (pActor) {
            ...
            switch (iData) {
                ...
                case 2:  // 交易
                    ...
                    break;
                case 3:  // 队伍
                    GetWorld()->m_Team.OnCommand(
                        pActor, ((RtwPopupMenu*)sender)->GetItemUserString(iSelectItem));
                    break;
                case 4:  // 好友
                    GetWorld()->m_Friend.OnCommand(
                        pActor, ((RtwPopupMenu*)sender)->GetItemUserString(iSelectItem));
                    break;
                case 5:  // 工会
                    g_layerMain->m_formUnion->OnCommand(
                        pActor);  //, ((RtwPopupMenu*)sender)->GetItemUserString(iSelectItem));
                    break;
                case 6:  //复制玩家的名字
...
void UILayerMain::CountDownTimer(DWORD dwMillisecond) {
    ...
    static const RtwPixel CountDownColor[] = {0xffffc125, /*倒计时颜色(两个切换)*/
                                              0xffff7f24};

    static const int nCountDownTimeList[7] = {//分钟:		5, 10, 15, 20, 20, 25, 25
    ...
    if (bStart)  //倒计时可用
    {
        ...
        if (nPos == 0) {
            ...
            rt2_sprintf(szBuf, "继续在线%d分钟,有惊喜礼包!",
                        (nCountDownTimeList[nPos] - nCountDownTimeList[nPos - 1]) / 60);
        }
...
void UILayerMain::OnResHotKeyState(int iButton) {
    ...
    for (size_t i = 0; i < UI_HOT_KEY_COUNT; ++i) {
        if (m_hotKeySet[i] == now || m_hotKeySet[i] == now1 || m_hotKeySet[i] == now2 ||
            m_hotKeySet[i] == now3) {
