...
bool CGameClientFrame::isDrawMouseXY = false;

void CGameClientFrame::ChangeDrawMouseXY() {
    isDrawMouseXY = !isDrawMouseXY;
}
...
void GameScreenSnapShot() {
    ...
    if (bFirst) {
        ...
        rt2_snprintf(szFilename, 56, "capture\\%04d%02d%02d_", ptm->tm_year + 1900, ptm->tm_mon + 1,
                     ptm->tm_mday);
...
void CGameClientFrame::OnEndRender() {
    ...
    if (isDrawMouseXY) {
        ...
        TCHAR cTmp[64];
        _stprintf_s(cTmp, _T("MouseX: %d  MouseY: %d"), mouseX, mouseY);
...

    {
        ...
        if (m_bShowStatAudio) {
            rt2_sprintf(szOutputString,
                        ...
                        "Memory:       %d\n"
                        "Enable:       %d",
                        g_pSoundMgr->GetUseAudioCnt(), g_pSoundMgr->GetUseAudioMem(),
...
void CGameClientFrame::OnRender() {
    ...
    if (g_WorldMapRenderer
            .GetRenderFlag()) {  // 如果世界地图渲染器的渲染标志为真，则调用其 Render 方法进行渲染
...
bool CGameClientFrame::OnConsoleCommand(const char* szCommand) {
    ...
    if (stricmp(token, "stat") == 0) {
        ...
        m_pConsole->AddConsoleString(
            "  Command List: [scene], [char], [client], [allon], [alloff]");
        return true;
        ...
        if (token) {
            if (stricmp(token, "scene") == 0) {
                ...
                CRT_ActorInstance::m_bGlobalRenderShadow =
                    !CRT_ActorInstance::m_bGlobalRenderShadow;
                return true;
        ...
        m_pConsole->AddConsoleString(
            "  Command List: [scene/water/char/shadow/ui/boundbox/charboundbox]");
        return true;
        ...
        if (token) {
            if (stricmp(token, "volume") == 0) {
                token = strtok(NULL, seps);
                if (token)
                    g_pBackMusic->SetVolume(atof(token));
                else
                    m_pConsole->AddConsoleString("  Command Format: > music volume number");
                return true;
            } else if (stricmp(token, "load") == 0) {
                token = strtok(NULL, seps);
                if (token)
                    g_pBackMusic->Play(token);
                else
                    m_pConsole->AddConsoleString("  Command Format: > music load filename");
                return true;
            } else if (stricmp(token, "quiet") == 0) {
                token = strtok(NULL, seps);
                if (token) {
                    if (stricmp(token, "true") == 0) {
                        g_pBackMusic->Quiet(true);
                        return true;
                    } else if (stricmp(token, "false") == 0) {
                        g_pBackMusic->Quiet(false);
                        return true;
                    }
                }
        ...
        if (GetWorld()->m_pPlayer) {
            ...
            rt2_sprintf(szPosition, "  Position: (%.2f, %.2f, %.2f) [%d, %d], Dir: [%d]", pM16->_30,
                        pM16->_31, pM16->_32, iPx, iPy, rtgEncodeUprightDirection(*pM16));
        ...
        */
        ...
        if (token) {
            if (stricmp(token, "actor") == 0) {
                ...
                stlforeach(std::list<SSkillEffect*>, itEffect,
                           GetPlayer()->m_Skill.m_listSkillEffect) {
        ...
        m_pConsole->AddConsoleString(
            "  Command List: [stat], [render], [py], [exit], [music], [fullscreen], [window] ");
        m_pConsole->AddConsoleString(
            "    [texture], [pos], [actor], [block], [device], [object], [effect], [print]");
