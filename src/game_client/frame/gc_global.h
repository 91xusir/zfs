...
extern G_MEMDEF(s_szRegionHost, 80)
extern G_MEMDEF(s_szStringTemp, STRING_LENGTH)
...
#define CheckCallStack(xx, yy, nn)  if (s_listCallStack.back()!=(xx)) {char* p=(char*)(yy);for (int i=0; i<nn; i++) *p++ = 0;}
...
void PlayerFailOperate(const char* szOutputString, bool vIgnoreRepeated=false,const char* szSoundFile=NULL, int iLevel=0); // iLevel 0普通级别, 10顶级(会弹出Dialog)
...
void ShowHelperMessage(const char* szSimpleMsg, const char* szDetailedMsg); // 显示提示信息, 简单信息, 详细信息
