...
NetSynchroPosition::NetSynchroPosition()
{
    ...
    m_sMaxPileCount = 0;    // 在开始运行前需要积累的数据包个数，积累的时候m_bPileSnapshot=0，积累完成则=1
...
void NetSynchroPosition::DownloadNetSnapshot(CG_CmdPacket *pPacket,bool event)
{
	...
	if(!event)
	{
		...
		if(!pPacket->ReadLong((long*)&m_dwServerSec)) return;
	}

	long curServerTime;
	if(!pPacket->ReadLong(&curServerTime)) return;
    ...
    }else
    {
        ...
        {
            // 如果误差超过9秒，就认为是使用加速器
            long lDiffTime = (long)(rtGetMilliseconds()-m_dwFirstClientTime)-(long)(curServerTime-m_dwFirstServerTime);
            if (lDiffTime>9000)
            {
                ...
                s_iCntNetTimeout ++;
#ifdef _DEBUG
                float fRate = ((float)(rtGetMilliseconds()-m_dwFirstClientTime))/((float)(curServerTime-m_dwFirstServerTime));
                ERR4("Net Accelerated, Time=%dms Rate=%.3f, Count=%d/%d\n", lDiffTime, fRate, s_iCntNetTimeout, s_iMaxNetTimeout);
#endif
            }
            m_dwFirstServerTime = 0;
    ...
    if (m_SnapshotCmdList.size() > m_sLimitPileCount)
    {
		...
		if (m_SnapshotCmdList.size() >= 3)
        {
            ERR2("%d: 缓冲的数据太多了(%d个包)，客户端发生严重错误，或者太慢。\n",
				(int)time(NULL),(int)m_SnapshotCmdList.size());
        }
    }
}
...
void NetSynchroPosition::Render(DWORD dwDifMillisecond)
{
	...
	*/
    ...
    {
        LOG4("网络: STOP Client time (%d). %u, [%u, %u].\n", m_dwClientTime-m_dwLastServerTime, m_dwClientTime, m_dwPrevServerTime, m_dwLastServerTime);
    ...
    float fProgress;
    if (m_dwLastServerTime == m_dwPrevServerTime) fProgress = 1.f;
    else fProgress = ((m_dwClientTime-m_dwPrevServerTime)/(m_dwLastServerTime-m_dwPrevServerTime));
