...
GWDatabase::GWDatabase()
    : m_mysql(NULL), m_shutdown(false), m_connectTime(0), m_dwDeleteLevel(30) {}
...
bool GWDatabase::SaveUserData(GWUser* data, bool sync, int param, GWDBCallback callback) {
    ...
    rt2_snprintf(
        ...
        //",UpdateSCDTime=%d"
        " WHERE ID=%d LIMIT 1",
        data->scene.c_str(), data->attr.hp, data->attr.mp, data->attr.level, data->attr.exp,
        data->attr.ap, data->attr.sp, data->attr.x, data->attr.y, data->attr.hp, data->attr.mp,
        data->attr.exp, data->attr.strength, data->attr.constitution, data->attr.dexterity,
        data->attr.intellect, data->attr.hit, data->strSkills.c_str(), data->items.c_str(),
        data->money, data->attr.actorID, data->attr.faction, data->attr.pk, data->shortcut.c_str(),
        data->configData.c_str(), data->task.c_str(), data->attr.credit, data->onlineTime,
        data->attr.metierLevel, data->attr.spClearCnt, data->attr.Soul, data->attr.Vigour,
        data->attr.VigourPoint, data->attr.Sex, data->attr.Contribute, data->attr.Point,
        data->attr.Record, data->attr.UnionRecord, data->attr.JobRecord, data->attr.BornTag,
        data->attr.RelivePostion, data->dungeonTID, data->dungeonUID, data->dungeonLastQuit,
        data->fbTask.c_str(), (int)data->IsInWarfield, data->lWarfieldLastQuit,
        strAttribAssert.c_str(), data->dwWithdrawMPGift,
        data->extraData.data == NULL
            ? ""
            : EscapeString((char*)data->extraData.data, data->extraData.size).c_str(),
        cTeamFree, data->attr.Cul, data->attr.CulExp, data->attr.PlayerOnlineTime,
        data->attr.exploit, data->attr.GradeLev, data->attr.HeadImageID,
        //data->attr.EnP,
        data->LineSYSGroupID, data->LineSYSLineID, data->popularity.c_str(), data->petItemID,
        data->isFirstLogin, data->isDead, data->strEffects.c_str(), data->ChangeModelID,
        //data->UpdateSCDTime,
        data->userID);
    ...
    if (!data->giftsWithdrawed.empty()) {
        for (int i = 0; i < (int)data->giftsWithdrawed.size(); i++) {
            ...
            if (pLc)
                ip = pLc->info.ip;
            g_server.LogUserEvent(PLT_WithdrawGift, data->userID, ip.c_str(),
                                  data->giftsWithdrawed[i].type,
                                  data->giftsWithdrawed[i].attribs.c_str(), 0, "", 0, "");
        }
    ...
    //	if (data->bankModified)
    {
        rt2_sprintf(
            cmd.command,
            "UPDATE T_bank SET Items='%s', BankMoney=%d , Pets='%s' WHERE UserID='%d' LIMIT 1",
            data->bank.c_str(), data->bankMoney, data->bankPet.c_str(), data->userID);
    ...
    rt2_snprintf(cmd.command, GW_MAX_DB_COMMAND_LENGTH,
                 "UPDATE T_pet SET Pets='%s' WHERE UserID='%d' LIMIT 1", data->pet.c_str(),
                 data->userID);
...
bool GWDatabase::ExecCommandEx(const char* command, MYSQL_RES** res, int* iAffectedRows,
                               bool bLogErr) {
...
bool GWDatabase::QueryActorList(GWLoginClient& account) {
    ...
    rt2_snprintf(cmd, GW_MAX_DB_COMMAND_LENGTH,
                 "SELECT "
                 "ID,"
                 "Name,"
                 "X,"
...
GWCreateUserResult GWDatabase::CreateUserData(GWUser* data) {
    ...
    rt2_sprintf(cmd, "SELECT COUNT(*) FROM user WHERE AccountID = '%d' and deleted=0",
                data->accountId);
    ...
    rt2_sprintf(
        cmd,
        "INSERT INTO user"
        " (UniqueName, Name, ActorID, AccountID, Account, Scene, X, Y, HP, MP, EXP, AP, SP, Level, "
        "Skills, Items, Gold, Metier,"
        "MetierLevel, HeadModelID, LastDelUnionTime, CreateDate, LastRemoveMPTime, Password, "
        "LineSysGroupID, LineSysLineID,HeadImageID,c_BornTag,isFirstLogin,IsDead,GradeLev) "
        " VALUES('%s', '%s', '%d', '%d', '%s', '%s', %d, %d, %d, %d, %d, %d,%d, %d, '%s', '%s', "
        "'%d', '%d', '%d', '%d', '%s', NOW(), '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d', "
        "'%d') ",
        uniqueName, safeName, data->attr.actorID, data->accountId, data->accountName.c_str(),
        data->scene.c_str(), data->attr.x, data->attr.y, data->attr.hp, data->attr.mp,
        data->attr.exp, data->attr.ap,
        data->attr.sp = 1,  //Tianh  人物初始给1SP
        data->attr.level, data->strSkills.c_str(), data->items.c_str(), data->money,
        data->attr.metier, data->attr.metierLevel, data->headModelID, "0000-00-00 00:00:00",
        "0000-00-00 00:00:00", data->bankPass.c_str(), data->LineSYSGroupID, data->LineSYSLineID,
        data->attr.HeadImageID, data->attr.BornTag,
        data->isFirstLogin = 0,  //0是未登陆
        data->isDead = 0, data->ChangeModelID = 0, data->attr.GradeLev = 1);
    ...
    rt2_sprintf(cmd, "SELECT Items,BankMoney,Pets,Password FROM T_bank WHERE UserID = '%d'",
                data->userID);
    if (!ExecCommand(cmd, &res, true))
        return gws_create_user_error;
    ...
    if (count == 0) {
        ...
        data->bankMoney = 0;
        rt2_sprintf(cmd,
                    "INSERT INTO T_bank(UserID,AccountID,Account,Items,BankMoney,Pets,Password) "
                    "VALUES('%d',%d,'%s','%s',%d,'%s','%s')",
                    data->userID, data->accountId, data->name.c_str(), data->bank.c_str(), 0,
                    data->bankPet.c_str(), "");
    ...
    rt2_snprintf(cmd, GW_MAX_DB_COMMAND_LENGTH, "SELECT Pets FROM T_pet WHERE UserID = '%d'",
                 data->userID);
    if (!ExecCommand(cmd, &res, true))
        return gws_create_user_error;
    count = mysql_num_rows(res);
    if (count == 0) {
        ...
        rt2_sprintf(cmd,
                    "INSERT INTO T_pet(UserID,AccountID,Name,Pets) VALUES('%d','%d','%s','%s')",
...
GWCreateUserResult GWDatabase::CreateUser(DWORD accountID, const string& name, int actorID,
                                          short headModelID, int faction, std::string password,
                                          GWUser* outputData, int Sex, short headImageID,
                                          short iAnimal) {
    if (!IsValidName(name))
...
GWUser* GWDatabase::LoadUserData(DWORD accountID, int userID) {
	...
	*/
    ...
    try {
        ...
        rt2_sprintf(cmd.command,
                    "SELECT Items,BankMoney,Pets,Password FROM T_bank WHERE UserID='%d'",
                    data->userID);
        if (!ExecCommand(cmd.command, &res))
            return NULL;
        ...
        if (count == 0) {
            ...
            rt2_sprintf(
                ...
                "INSERT INTO T_bank(UserID,AccountID,Account,Items,BankMoney,Pets,Password) "
                "VALUES('%d','%d','%s','%s',%d,'%s','%s')",
                data->userID, accountID, data->name.c_str(), data->bank.c_str(), 0,
                data->bankPet.c_str(), "");
        ...
        rt2_snprintf(cmd.command, GW_MAX_DB_COMMAND_LENGTH,
                     "SELECT Pets FROM T_pet WHERE UserID = '%d'", data->userID);
        ...
        if (count == 0) {
            mysql_free_result(res);
            if (data->pet.empty()) {
                CBag bagEmpty(g_ItemMgr.GetBankInitLineCount(), BANK_GRID_COLUMN_COUNT, 1,
                              &g_ItemMgr);
                data->pet = bagEmpty.SaveToString(&g_ItemMgr);
            }
            rt2_sprintf(cmd.command,
        ...
        if (!strAttribAssert.empty()) {
            ...
            if (MD5String != strAttribAssert) {
                ...
                if (strcmp(cTmpVer1, cTmpVer2) == 0) {
                    // Log to LogServer
                    char cTmp1024[1024];
                    rt2_sprintf(cTmp1024, "GW=%s,UserName=%s,UserID=%d",
                                g_cfgGW.game.szGameworldName.c_str(), data->name.c_str(),
                                data->userID);
...
bool GWDatabase::DeleteUser(int userID) {
    ...
    // 更新删除信息
    rt2_sprintf(cmd.command, "UPDATE `user` SET `deleted`=1, `DeleteDate`=NOW() WHERE `ID`='%d'",
                userID);
    if (!ExecCommand(cmd.command, NULL))
        return false;
...
GWActor* GWDatabase::LookupActor(int id) {
    stlforeach(GWActorTable, actor, m_actors) if (actor->attr.actorID == id) return &*actor;
...
void GWDatabase::EscapeString(string& result, const string& str) {
    ...
    result.resize(
        mysql_real_escape_string(m_mysql, &result[0], str.c_str(), (unsigned long)str.length()));
}
...
bool GWDatabase::Connect() {
    ...
    if (!mysql_real_connect(m_mysql, g_cfgGW.connectDatabase.ip.c_str(),
                            g_cfgGW.connectDatabase.username.c_str(),
                            g_cfgGW.connectDatabase.password.c_str(), NULL,
                            g_cfgGW.connectDatabase.port, NULL, 0)) {
        LOG3("Can't connect to database server %s on port %d\n    MSG= %s\n",
             g_cfgGW.connectDatabase.ip.c_str(), g_cfgGW.connectDatabase.port,
             mysql_error(m_mysql));
    ...
    if (mysql_select_db(m_mysql, g_cfgGW.connectDatabase.database.c_str()) < 0) {
        LOG2("Can't select database %s\n    MSG= %s\n", g_cfgGW.connectDatabase.database.c_str(),
             mysql_error(m_mysql));
