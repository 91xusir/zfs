...
GW_BEGIN_CMD(cmd_lgs2gws_user_login_ret) {
    ...
    if (ret == LOGIN_RET_SUCCESS) {
        ...
        if (!(info.privileges & EUP_LoginGM) &&
            ((g_server.GetCurrentUserCnt() >= g_cfgGW.game.maxUser) ||
             (GetMaxRegionUser() >= g_cfgGW.game.maxRegionUser))) {
            ret = LOGIN_RET_FAILED_SERVER_FULL;
        }
        //PZH
        bool bPower = ulc->IsPowerLogin();
        if (bPower) {
            int nOld = CLogicExt::GetCLogicExt().DeleteOldLogin(info.accountId, ulc, lPoint,
                                                                cPointType, cVIP);
            if (0 == nOld) {
                return eOkay;
            }
...
GW_BEGIN_CMD(cmd_lgs2gws_add_gift) {
    ...
    if (g_GiftMgr.AddGift(lUserID, sLevel, sType, szGiftString, (DWORD)time(NULL), tLimitTime)) {
        ...
        string StrItemID = "";
        g_server.m_mail.AddMail(lUserID, 0, R(MSG_GIFT_RECEIVE_ITEM), g_strStaticBuffer, true,
                                StrItemID, 0);
    }
    return eOkay;
}
...
GW_BEGIN_CMD(cmd_lgs2gws_add_point_ret) {
    ...
    if (!packet->ReadByte(&cSucceed))
        return eInvalidData;
    if (!packet->ReadString(&cResult))
        return eInvalidData;
    if (!packet->ReadShort(&sPoint))
        return eInvalidData;
...
GW_BEGIN_CMD(cmd_lgs2gws_deduct_ret) {
    ...
    if (!packet->ReadLong(&lUserID))
        return eInvalidData;
    if (!packet->ReadLong(&lDeductOrderID))
        return eInvalidData;
    if (!packet->ReadString(&cResult))
        return eInvalidData;
    if (!packet->ReadShort(&sPoint))
        return eInvalidData;
    if (!packet->ReadShort(&sCurPoint))
        return eInvalidData;
...
GW_BEGIN_CMD(cmd_lgs2gws_prededuct_ret) {
    ...
    if (!packet->ReadString(&cResult))
        return eInvalidData;
    if (!packet->ReadLong(&event))
        return eInvalidData;
    if (!packet->ReadLong(&lAuctionId))
        return eInvalidData;
...
void GWLoginSession::InitChannel() {
    ...
    m_channel.RegisterCommand(lgs2gws_add_account_time_ret, cmd_lgs2gws_add_account_time_ret,
                              "");  //tim.yang  VIP卡
}
...
void GWLoginSession::Process() {
    ...
    switch (GetState()) {
        default: {
            ...
            // 如果当前时间与上次连接时间的间隔大于等于 GW_CONNECT_LOGIN_INTERVAL
            if (now - m_lastConnectTime >= GW_CONNECT_LOGIN_INTERVAL) {
                Close();
                // 记录连接到登录服务器的通知
                NOTICE("Connect to login server. IP=%s Port=%d, BindIP=%s BindPort=%d\n",
                       (char*)g_cfgGW.connectLogin.ip.c_str(), g_cfgGW.connectLogin.port,
                       (char*)g_cfgGW.connectLogin.bindIP.c_str(), g_cfgGW.connectLogin.bindPort);
...
bool GWLoginSession::LoginAccount(int seed, const char* account, const char* pwaccount,
                                  const char* pubwinKey, const char* glompwd, const char* password,
                                  const char* ip /*PZH*/, bool bPowerLogin /**/) {
    //PZH 移到前面一点处理
...
extern G_MEMDEF(g_szExeMD5, 40) extern G_MEMDEF(g_szDataMD5, 40)
...
long GWLoginSession::CheckSubServerVersion(const char* szServerName, const char* szExeMD5,
