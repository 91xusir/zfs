  ...
  for (int i = 0; i < attcnt; i++)
    {
	  ...
	  Element *e=0;
	    ...
	    //The element may be referenced in an imported schemaparser
...
#ifdef LOGGING
	    logFile_<<typeNs<<":"<<name<<" ->  element reference("<<type_id<<")"<<std::endl;
#endif
	  }
        }
...
SchemaParser::parseSimpleType()
{
  ...
  do
	...
	std::string members = xParser_->getAttributeValue("", "membersTypes");
	size_t s = 0;
	while(s < members.length()){
	  while(members[s]==' ')s++;
	  std::string type = members.substr(s,members.find(' ',s)-s);
	  basetype_id = getTypeId(Qname(type));
...
SchemaParser::parseImport()
{
  ...
  // FV Sat Dec 22 15:33:09 CET 2007 I added the absolute path and the file:/ use-cases in the the test
...
//store a reference to an imported element reference
  int  SchemaParser::addExternalElement(const std::string & name , const std::string & nspace, int localTypeId)
{
    Element e(name,nspace,localTypeId);
  lElems_.push_back(e);
  return lElems_.size()-1;
}
...
const Element*
SchemaParser::getElement(const Qname & element)const
{
...
const XSDType *
SchemaParser::getType(const Qname & type ) 
{
  int id;
  Qname t=type;
  ...
  if (baseType) {
	...
	array = array.substr(0,array.length()-2);
      ...
      Schema::Type t = (Schema::Type)getTypeId(q,true);
          Element e("*",tnsUri_,t,0,UNBOUNDED);
      if (ct->getContents() == 0){
	ContentModel * cm = new ContentModel(Schema::Sequence);
	ct->setContents(cm);
      }
