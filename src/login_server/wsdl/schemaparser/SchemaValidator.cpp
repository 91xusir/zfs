  ...
  case Schema::Sequence:
      ...
      do
	{
	  ...
	  if(xpp->getEventType() != XmlPullParser::END_TAG){
	    ...
	    if(ci->second==ContentModel::Particle &&   //if we validated a particle
	       xpp->getName()!=ci->first.e->getName() && //if the next particle is different
	       ci->first.e->getName()!="*") {
	      
	      
	      ci++; // move to the next in the list of contents
	    }

	    //TODO multiple occurrences of a content model like sequence
	    //cant be validated as yet
	  }else {
			     ...
			     ContentModel::ContentsIterator & found)
{
  for (ContentModel::ContentsIterator ci=start;
    ...
    if(ci->second==ContentModel::Particle){
#ifdef LOGGING   
       std::cout<<"Looking for "<<ci->first.e->getName()<<" in   "<<name<<std::endl;
#endif
