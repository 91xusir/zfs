...
class W3Client {
   ...
   private:
    typedef struct __HTTP_ARG {
        ...
        __HTTP_ARG(const char* szn, const char* szv, bool b = false) : name(szn), value(szv) {
            if (b)
                f = true;
            else
                f = false;
        ...
        __HTTP_ARG(const char* szn, const int v) : name(szn), f(false) {
            char t[15] = "\0";
            sprintf_s(t, "%d", v);
            value = t;
        }

        __HTTP_ARG(const char* szn, const long v) : name(szn), f(false) {
            char t[15] = "\0";
            sprintf_s(t, "%d", v);
            value = t;
        }

        __HTTP_ARG(const char* szn, const float v) : name(szn), f(false) {
            char t[15] = "\0";
            sprintf_s(t, "%f", v);
            value = t;
        }

        __HTTP_ARG(const char* szn, const double v) : name(szn), f(false) {
            char t[15] = "\0";
            sprintf_s(t, "%f", v);
            value = t;
        }
        ...
        const char* type() {
            ...
            if (RegOpenKeyEx(HKEY_CLASSES_ROOT, szword, 0, KEY_QUERY_VALUE, &hKEY) ==
                ERROR_SUCCESS) {
                if (RegQueryValueEx(hKEY, TEXT("Content Type"), NULL, NULL, (LPBYTE)szt,
                                    (unsigned long*)&nlen) != ERROR_SUCCESS)
        ...
        unsigned long dump2(unsigned char* buf, unsigned long s,
                            const char* boundry = __DEFAULT_BOUNDRY_TAG) {
            unsigned long l = 0;
            if (f) {  // file
                /*
		...
		*/
                ...
                if (hFile && hFile != INVALID_HANDLE_VALUE) {
                    ...
                    while (::ReadFile(hFile, pbuf + nt, 1024, &nread, NULL) && nread > 0 &&
                           nt <= ns)
                        nt += nread;
    ...
    typedef struct __HTTP_COOKIE {
        ...
        __HTTP_COOKIE(const char* szn, const int v) : name(szn) {
            char t[15] = "\0";
            sprintf(t, "%d", v);
            value = t;
        }

        __HTTP_COOKIE(const char* szn, const long v) : name(szn) {
            char t[15] = "\0";
            sprintf(t, "%d", v);
            value = t;
        }

        __HTTP_COOKIE(const char* szn, const float v) : name(szn) {
            char t[15] = "\0";
            sprintf(t, "%f", v);
            value = t;
        }

        __HTTP_COOKIE(const char* szn, const double v) : name(szn) {
            char t[15] = "\0";
            sprintf(t, "%f", v);
            value = t;
        }
   ...
   public:
    W3Client() {
        _hOpen       = NULL;
        _hConnection = NULL, _hRequest = NULL;
    }

    virtual ~W3Client() { InitializePostArguments(); }

   public:
    bool Connect(const char* szaddress, const char* szuser = NULL, const char* szpassword = NULL,
                 const char* szagent = __W3_DEFAULT_AGENT);
    virtual bool Connect(const char* szaddress, long nport, const char* szuser = NULL,
    ...
    void AddPostArgument(const char* buf, unsigned long len) {
        postBuf_    = buf;
        postBufLen_ = len;
    }
    ...
    unsigned long PutFile(const char* szuri, unsigned char* buf, unsigned long len,
                          bool ascii = false);
    unsigned long GetFile(const char* szuri, unsigned char* buf, unsigned long len,
                          bool ascii = false);
   ...
   protected:
    ...
    unsigned long AllocMultiPartsFormData(unsigned char*& buf,
                                          const char*     szboundry = __DEFAULT_BOUNDRY_TAG);
