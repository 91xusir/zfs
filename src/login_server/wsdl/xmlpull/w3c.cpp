...
void __w3cexcept(const char* szaddress, long nport, W3Client::w3t t, const char* szmsg) {
    ...
    DWORD  dwRet = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE,
                                 GetModuleHandle("wininet.dll"), err,
                                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                                 reinterpret_cast<LPTSTR>(&lpMsgBuffer), 0, NULL);
...
void __w3cexcept(const char* szaddress, long nport, W3Client::w3t t, const char* szuri,
                 const char* szmsg) {
#ifdef _DEBUG
    string sztmp;
    sztmp += "[ ";
...
bool W3Client::Connect(const char* szaddress, const char* szuser /*=NULL*/,
                       const char* szpassword /*=NULL*/,
                       const char* szagent /*=__W3_DEFAULT_AGENT*/) {

    char szp[__DEFAULT_BUF_SIZE] = "\0", szus[__DEFAULT_BUF_SIZE] = "\0",
         szpw[__DEFAULT_BUF_SIZE]         = "\0";
    ...
    return Connect(sza, port, ((szuser || (!szuser && strlen(szus) == 0)) ? szuser : szus),
                   ((szpassword || (!szpassword && strlen(szpw) == 0)) ? szpassword : szpw), wt,
                   szagent);
}

bool W3Client::Connect(const char* szaddress, long nport, const char* szuser /*=NULL*/,
                       const char* szpassword /*=NULL*/, w3t t /*=w3http*/,
                       const char* szagent /*=__W3_DEFAULT_AGENT*/) {
        ...
        switch (t) {
            case w3ftp:
                _hConnection = ::InternetConnect(
                    _hOpen, szaddress, (nport ? nport : INTERNET_DEFAULT_FTP_PORT), szuser,
                    szpassword, INTERNET_SERVICE_FTP, INTERNET_FLAG_PASSIVE, NULL);
                break;
            case w3http:
                _hConnection = ::InternetConnect(
                    _hOpen, szaddress, (nport ? nport : INTERNET_DEFAULT_HTTP_PORT), szuser,
                    szpassword, INTERNET_SERVICE_HTTP, 0, NULL);
                break;
            case w3https:
                _hConnection = ::InternetConnect(
                    _hOpen, szaddress, (nport ? nport : INTERNET_DEFAULT_HTTPS_PORT), szuser,
                    szpassword, INTERNET_SERVICE_HTTP, 0, NULL);
                break;
        }
...
bool W3Client::RequestGet(const char* szuri, const char* szref /*=NULL*/) {
    ...
    // REPLACE HEADER
    if (!::HttpAddRequestHeaders(_hRequest, __HTTP_ACCEPT, strlen(__HTTP_ACCEPT),
    ...
    if (_listcookies.size() > 0) {
        ...
        if (!((_t == w3http && _nport == INTERNET_DEFAULT_HTTP_PORT) ||
        ...
        for (list<HTTP_COOKIE*>::iterator it = _listcookies.begin(); it != _listcookies.end();
             it++) {
...
bool W3Client::RequestPost(const char* szuri, const char* szref /*=NULL*/) {
    ...
    try {
        ...
        if (!::HttpSendRequest(
                _hRequest,                      // handle by returned HttpOpenRequest
                NULL,                           // additional HTTP header
...
bool W3Client::RequestPost2(const char* szuri, const char* szref /*=NULL*/) {
    ...
    if (!::HttpAddRequestHeaders(_hRequest, szContentType, strlen(szContentType),
...
unsigned long W3Client::QueryCookie(unsigned char* buf, unsigned long len,
                                    unsigned long idx /*=0*/) {
...
unsigned long W3Client::QueryContentLength() {
    ...
    if (!::HttpQueryInfo(_hRequest, HTTP_QUERY_CONTENT_LENGTH, szt,
                         reinterpret_cast<unsigned long*>(&nread), NULL)) {
        __w3cexcept(_szaddress.c_str(), _nport, _t, _szuri.c_str(),
...
const char* W3Client::QueryContentType() {
    ...
    unsigned long nread                   = __DEFAULT_BUF_SIZE;
    memset(szt, 0x00, __DEFAULT_BUF_SIZE);
    if (!::HttpQueryInfo(_hRequest, HTTP_QUERY_CONTENT_TYPE, szt,
                         reinterpret_cast<unsigned long*>(&nread), NULL)) {
...
unsigned int W3Client::QueryResult() {
    ...
    unsigned long nread                   = __DEFAULT_BUF_SIZE;
    memset(szt, 0x00, __DEFAULT_BUF_SIZE);
    if (!::HttpQueryInfo(_hRequest, HTTP_QUERY_STATUS_CODE, szt,
                         reinterpret_cast<unsigned long*>(&nread), NULL)) {
...
bool W3Client::GetFile(const char* szuri, const char* szfile, bool ascii /*=false*/) {
    ...
    try {
        ...
        _hRequest = ::FtpOpenFile(
            _hConnection, szuri, GENERIC_READ,
            (ascii ? INTERNET_FLAG_TRANSFER_ASCII : INTERNET_FLAG_TRANSFER_BINARY), NULL);

        if (!_hRequest || _hRequest == INVALID_HANDLE_VALUE)
            throw "request failed...";

        HANDLE f = ::CreateFile(szfile, GENERIC_WRITE, FILE_SHARE_WRITE, 0, CREATE_ALWAYS,
...
unsigned long W3Client::GetFile(const char* szuri, unsigned char* buf, unsigned long len,
                                bool ascii /*=false*/) {
...
bool W3Client::PutFile(const char* szuri, const char* szfile, bool ascii /*=false*/) {
    ...
    try {
        ...
        _hRequest = ::FtpOpenFile(
            _hConnection, szuri, GENERIC_WRITE,
            (ascii ? INTERNET_FLAG_TRANSFER_ASCII : INTERNET_FLAG_TRANSFER_BINARY), NULL);

        if (!_hRequest || _hRequest == INVALID_HANDLE_VALUE)
            throw "request failed...";

        HANDLE f = ::CreateFile(szfile, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
                                FILE_ATTRIBUTE_NORMAL, 0);
...
unsigned long W3Client::PutFile(const char* szuri, unsigned char* buf, unsigned long len,
                                bool ascii) {
...
void __w3curlparse(const char* szurl, char* szprotocol, char* szuser, char* szpassword,
