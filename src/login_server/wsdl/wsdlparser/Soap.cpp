...
const std::string Soap::soapEncUri = "http://schemas.xmlsoap.org/soap/encoding/";
const std::string Soap::soapEnvUri = "http://schemas.xmlsoap.org/soap/envelope/";
const std::string Soap::soapBindingUri ="http://schemas.xmlsoap.org/wsdl/soap/";

Soap::Soap(const std::string & schemaPath)
  :sNamespace(soapBindingUri),
   startId(0),
   mySchemaParser(0),
   mySchemaValidator(0),
   wParser_(0),
...
int  Soap::processOp(int parent, TypeContainer * t)
{
  TypeContainer * temp = 0;
...
{
  if (elemId - startId >= idCounter)            //invalid elem Id
    return;
