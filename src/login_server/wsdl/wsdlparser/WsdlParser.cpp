...
//  addExtensibilityHandler (soap_);
  ...
  sParser = new SchemaParser(soap_->getEncodingSchema(),
			     Soap::soapEncUri,ostr,schemaPath_);
  sParser->parseSchemaTag();
  schemaParser_.push_back(sParser);
}
...
WsdlParser::~WsdlParser()
{
  ...
  oslink::directory dir(".");
  while (dir) {
    std::string fname = dir.next();
    if (fname.find(".wp-tmp") != std::string::npos)
      {
#ifdef WIN32
      ...
      for (size_t j = 0; j < wsdlExtensions_.size(); j++)
	    ...
	    xtmpSchemaParser->addImport(schemaParser_[0]);
            if (xtmpSchemaParser->parseSchemaTag())
              ...
              error("Error parsing extensibility schema for " +
                    wsdlExtensions_[j].we->getNamespace());
          }
    }
...
WsdlParser::parseOperation(PortType * p)
{
  ...
  if (state_ == INPUT)
    {
      op->setMessage(pgetMessage
                     (Qname(xParser_->getAttributeValue("", "message"))),
                     Input);
      peek();
      if (state_ == OUTPUT)
        {
          op->setMessage(pgetMessage
                         (Qname
                          (xParser_->getAttributeValue("", "message"))),
                         Output);
          peek();
        }
      while (state_ == FAULT)
  ...
  try
    {
      while (state_ == SCHEMA)
        {
	      ...
	      sParser->setUri(uri_);
          sParser->addImport(schemaParser_[0]);//soap encoding schema
          sParser->addImport(schemaParser_[1]);//wsdl schema for wsdl:arrayType
      ...
      for (size_t i = 2; i < schemaParser_.size(); i++)
        {
       	  
          for (size_t j = 2; j < schemaParser_.size(); j++) {
 ...
 if (name.getNamespace() == Schema::SchemaUri)
  ...
  for (size_t i = 0; i < schemaParser_.size(); i++) {
	...
	if( schemaParser_[i]->getNamespace() == type.getNamespace() ||
		schemaParser_[i]->isImported(type.getNamespace())) {
			
		//check for definitions
			
    if ((isType && schemaParser_[i]->getTypeId(name) != Schema::XSD_INVALID) ||
...
WsdlParser::error(string s,int level)
{
  if(level==0){
    ...
    WsdlException we("Wsdl Parser Exception : " + s);
    if(xParser_){
