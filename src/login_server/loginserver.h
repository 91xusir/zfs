...
public:
	...
	bool LoadGameWorldList(const char *file);

    // --------------- Game world -------------------
    //	void ShutDownGws(int idx);
    //	void ReportSysInfo();
    //	void ReportGameWorldInfo();
    //	bool TryAddUser(SListCmd *cmd,SUserInfo *user);
    ...
    SUserInfo *TryAddUser(SListCmd *cmd, char cPointType, long lPoint, char cVIP); // cPointType 1点数, 2时间, cVIP 0不是VIP, 1是VIP
    ...
    SUserInfo* OnChargePass(long lSn, char cPointType, long lPoint, char cVIP); // cPointType 1点数, 2时间, cVIP 0不是VIP, 1是VIP
    ...
    // --------------- Gws list -------------------
    void SendGWServerList(GSClientSessionManager* pClient, int iCount = 0);		// If count == 0, send all servers
    void RegisterGws(long lIdx, const char* szName, const char* szIP,
        short sPort, long lGwID, SAllowIP& allowIP);
    void UnregisterGws(long lIdx);

    // --------------- Account -------------------
    void CreateAccount(long lOrderID, const char* szUsername, const char* szPassword,
    ...
    void LockAccountByID(CGameWorldStub* pStub, long lResultAccountID, long lAccountID,
        short sLocked, long tLockBegin, long tLockEnd);
    void SetAccountPriv(CGameWorldStub* pStub, long lResultAccountID, const char* szUsername, long lPriv);
    void SetAccountPrivByID(CGameWorldStub* pStub, long lResultAccountID, long lAccountID, long lPriv);
    void SetAccountPassword(CGameWorldStub* pStub, long lResultAccountID,
        const char* szUsername, const char* szPassword, long lLoginOrderID=0);
    void SetAccountPasswordByID(CGameWorldStub* pStub, long lResultAccountID,
    ...
    // --------------- Point operation -------------------
	void AddPointRet(short sServer, long lAccountID, long lUserID, short sPoint,char* cResult,char cSucceed);
    ...
    void OnDeduct(long lSn, short sServer, long lAccountID, long lUserID,
        short sPoint, short sCurPoint, char* cResult, long lDeductOrderID);
    void OnPreDeduct(short sServer, long lAccountID, long lUserID,
    ...
    void OnChargeCard(long lSn, short sServer, long lUserID,
        const char* szCardID, char cSucceeded, const char* szMsg);
	void GiftErrorRet(short sServer, long lUserID, char* szCardID, char* msg, long lUserParam,char szSucceed = 0);
    void AddGift(long lSnCard, const char* szUsername,
    ...
    void CardCanTrade(long lSalesmanAccountID, const char* szSalesmanAccountName,
        short sServer, long lCharID, char cCardType, const char* szCardID,
        const char* szCardPwd, long lUserParam, short sTime);
    void OnCardCanTradeResult(long lSn, short sServer, long lUserID,
        const char* szCardID, char cSucceeded, const char* szMsg, long lCardGroupType);
    ...
    void LogChargeCard(long lSn, SCmdCardChargeLog::ECCOP eOP, char cCardType,
        long lAccountID, short sServer, long lUserID, const char* szCardID);
...
public:
	...
	// HASH_STR(string,SUserInfo*) m_userMap;
    // HASH_STR(string,int)        m_userLogin; // 此变量用来防止用户多次login, 造成数据库不必要的负担
