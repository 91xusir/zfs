...
bool Node::movetoprevious_child(Node* node)
{
    xmlNode* child = impl_->children;
    if(!child)
        return false;
...
bool Node::movetonext_child(Node* node)
{
    xmlNode* child = impl_->children;
    if(!child)
        return false;

    xmlNode* _next = child->next;

    while (_next && child)
    {
        if (child == node->cobj())
        {
            xmlAddNextSibling(_next, child);
