...
public:
    ...
    std::string getWidgetNameFromElement(const xmlpp::Element* pElement);
private:
    ...
    bool _TranslateFromRtXmlToLibXml(xmlpp::Element* pLibXmlNode, const RtsXmlDoc::NodePtr* pRtXmlNode, xmlpp::Element* pLibXmlParentNode);
    bool _TranslateFromLibXmlToRtXml(RtsXmlDoc::NodePtr* pRtXmlNode, xmlpp::Element* pLibXmlNode);

    bool _Paste(RtwTree::Item* pTreeItemTo, RtwTree::Item* pTreeItemFrom);

    std::string getDefaultName(RtwWidget* pWidget);
public:
    const xmlpp::Document* getXmlDocument() const {/*LOG("");*/ return m_pXmlDoc; }
private:
    xmlpp::Document*    m_pXmlDoc;
    xmlpp::DomParser*   m_pXmlDomParser;
